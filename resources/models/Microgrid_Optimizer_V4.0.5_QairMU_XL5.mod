/*********************************************
 * Microgrid Optimisation Model v4.0.1
 * Author: j.leduc
...
//		- costs from imports from the main-grid expressed in ï¿½/mWh instead of ï¿½/kWh) so mutliplied by 1000 (see electricityPrice param)
...
tuple t_operation_steps_link {
 	...
 	string afrr_energy_step;	// aFRR voluntary step ID generated by Everest
 	string gas_day_ahead_step;
 	string ppa_step;
 	string asset_step_day_local_time;
 	string asset_step_hour_local_time;
 	string asset_step_minute_local_time;	
...
tuple t_assets {
 	...
 	float initial_gas_energy;				// gas already consumed during the current gas step (kWh)
...
/* SITE_STEPS */
tuple t_site_steps {
	key string site_id;				// ID of site(s) within the microgrid operation
	key string step_id;				// Step ID generated by Everest
	//float network_usage_cost;		// Currency per kWh
	//float elec_import_price;		// Currency per kWh
	//float elec_export_price;		// Currency per kWh
	float availability_in;			// Site availability to withdraw from the grid
	float availability_out;			// Site availability to inject to the grid
}
{t_site_steps} SITE_STEPS = ...; // variable holding SITE_STEPS data
...
// indexing set of day_ahead steps
{string} isDA_STEPS = {osl.day_ahead_step | osl in OPERATION_STEPS_LINK };
// indexing set of PPA steps
{string} isPPA_STEPS = {osl.ppa_step | osl in OPERATION_STEPS_LINK };
...
{string} isAFRR_VOLUNTARY_STEPS = {osl.afrr_energy_step | osl in OPERATION_STEPS_LINK };
...
{string} isAFRR_VOLUNTARY_STEPS_POS = {osl.afrr_energy_step | osl in OPERATION_STEPS_LINK : osl.afrr_energy_step !="-1.0" &&  osl.afrr_energy_step !="-1"};
...
{string} isDA_STEPS_POS = {osl.day_ahead_step | osl in OPERATION_STEPS_LINK : osl.day_ahead_step !="-1.0" &&  osl.day_ahead_step !="-1"};
// indexing set of day_ahead steps (hourly) with only positive values
{string} isPPA_STEPS_POS = {osl.ppa_step | osl in OPERATION_STEPS_LINK : osl.ppa_step !="-1.0" &&  osl.ppa_step !="-1"};
...
{string} isREAL_HOURS = {osl.asset_step_hour_local_time | osl in OPERATION_STEPS_LINK};
// All assets
// -------------------------------------------
...
{string} isASSETS = {a.asset_id | a in ASSETS : a.asset_id not in isDUMMY_ASSETS};
...
{string} isE_ASSETS = {a.asset_id | a in E_ASSETS : a.asset_id not in isDUMMY_ASSETS};
...
{string} isH_ASSETS = {a.asset_id | a in H_ASSETS : a.asset_id not in isDUMMY_ASSETS};
...
{string} isFLEX_LOADS = {a.asset_id | a in ASSETS: a.type == "FLEX_LOAD" && a.asset_id not in isDUMMY_ASSETS};
...
{string} isINTER_GENS = {a.asset_id | a in ASSETS: a.type == "INTERMITTENT" && a.asset_id not in isDUMMY_ASSETS};
...
{string} isDISP_GENS = {a.asset_id | a in ASSETS: a.type == "GENERATOR" && a.asset_id not in isDUMMY_ASSETS};
...
// -------------------------------------------
// indexing set of energy storage assets excluding aFRR unvailability capacity for ZEE
{string} isSTORAGES = {a.asset_id | a in ASSETS: a.type == "STORAGE" && a.asset_id not in isDUMMY_ASSETS} diff {"ZEE_capa_PORTAL_Indispos_aFRR_EDR1", "ZEE_capa_PORTAL_Indispos_aFRR_Vert"};
...
{string} isNF_LOADS = {a.asset_id | a in ASSETS: a.type == "LOAD" && a.asset_id != "E_boiler_1000kW" && a.asset_id != "E_boiler_1200kW" && a.asset_id not in isDUMMY_ASSETS};
...
// -------------------------------------------
{string} isSITES = {a.asset_id | a in ASSETS: a.type == "SITE" && a.asset_id not in isDUMMY_ASSETS};
// indexing sets of assets by types of control
// -------------------------------------------
// indexing set of elec assets controled by power targets
{string} isPC_E_ASSETS = {a.asset_id | a in E_ASSETS: a.control == "POWER" && a.asset_id not in isDUMMY_ASSETS};
// indexing set of elec assets controled by temperature targets
{string} isTC_E_ASSETS = {a.asset_id | a in E_ASSETS: a.control == "TEMPERATURE" && a.asset_id not in isDUMMY_ASSETS};
...
{string} isCONGESTIONS = {c.congestion_id | c in CONGESTIONS : c.congestion_id != "dummy_congestion"};
...
{string} isFCR_ASSETS = (microgridName == "MICROGRID VALOREM LIMOUX") ? isE_ASSETS inter {"VALOREM_Limoux_BESS"} :
						...
						{};
{string} isAFRR_ASSETS = (microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B", "MICROGRID ZEE Vert"}) ? isE_STORAGES : {};
...
{string} isAFRR_UP_ASSETS = (microgridName == "MICROGRID MOPABLOEM" ) ? isE_ASSETS inter {"BESS_MOPABLOEM","CHP_1600kW"}:
						    (microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B", "MICROGRID ZEE Vert"}) ? isE_STORAGES :
 						    {};
...
{string} siteIdByAsset[isASSETS] = [a.asset_id:{a.site}| a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
int isaFRRVoluntaryStepNumberSpecified = card({o.param_val | o in OPERATION: o.param_id == "afrr_voluntary_step_number" || o.param_id == "afrr_energy_step_number"});
// Number of aFRR voluntary steps (NOT USED, to be deleted?)
int aFRRVoluntaryStepNumber = (isaFRRVoluntaryStepNumberSpecified > 0 ? intValue(first({o.param_val | o in OPERATION: o.param_id == "afrr_voluntary_step_number" || o.param_id == "afrr_energy_step_number"})) : 0);
// flag indicating if the afrrVoluntaryStepDuration is present in the input
int isaFRRVoluntaryStepDurationSpecified = card({o.param_val | o in OPERATION: o.param_id == "afrr_voluntary_step_duration" || o.param_id == "afrr_energy_step_duration"});
// duration (expressed in minutes) of affr voluntary step
// MUST be expressed as a whole number of minutes (int) b/c is used in the defintion of a range
int afrrVoluntaryStepDuration = (isaFRRVoluntaryStepDurationSpecified > 0 ? intValue(first({o.param_val | o in OPERATION: o.param_id == "afrr_voluntary_step_duration" || o.param_id == "afrr_energy_step_duration"})) : 0);
...
// duration (expressed in minutes) of ppa step
// MUST be expressed as a whole number of minutes (int) b/c is used in the defintion of a range
int ppaStepDuration = intValue(first({o.param_val | o in OPERATION: o.param_id == "ppa_step_duration"}));
...
// duration (expressed in hours) of ppa step
float ppaStepDurationInHours = ppaStepDuration / 60.0;
...
string assetStepDAStep[isDECISION_STEPS] = [t.asset_step : t.day_ahead_step | t in OPERATION_STEPS_LINK];
// effective duration (expressed in minutes) of each day ahead step
// (accounts for incomplete steps at beginning and end of optimisation window)
float effDaStepDurationInHours[da in isDA_STEPS] = assetStepDurationInHours * sum(t in isDECISION_STEPS: assetStepDAStep[t] == da) 1;
...
string daStepFCRStep[isDA_STEPS] = [h.day_ahead_step : h.fcr_step | h in OPERATION_STEPS_LINK];
// afrr voluntary index in asset_step reference frame
string assetStepAfrrVoluntaryStep[isDECISION_STEPS] = [t.asset_step : t.afrr_energy_step | t in OPERATION_STEPS_LINK];
// afrr capacity index in afrr_voluntary_step reference frame
string voluntaryAfrrStepCapacityStep[isAFRR_VOLUNTARY_STEPS] = [afrr.afrr_energy_step : afrr.afrr_capacity_step | afrr in OPERATION_STEPS_LINK];
// imbalance index in afrr_voluntary_step reference frame
string voluntaryAfrrStepImbalanceStep[isAFRR_VOLUNTARY_STEPS] = [afrr.afrr_energy_step : afrr.imbalance_step | afrr in OPERATION_STEPS_LINK];
...
// ppa index according to asset step
string assetStepPPAStep[isDECISION_STEPS] = [t.asset_step : t.ppa_step | t in OPERATION_STEPS_LINK];
// real hour according to hourly step
//int ppaStepRealHourStep[isPPA_STEPS] = [t.ppa_step : t.real_hour | t in OPERATION_STEPS_LINK];
int ppaStepRealHourStep[isPPA_STEPS];
//real minute according to da step
string ppaStepRealMinuteStep[isPPA_STEPS] = [ppa.ppa_step : ppa.asset_step_minute_local_time | ppa in OPERATION_STEPS_LINK];;
// real hour according to imbalance step
//int imbStepRealHour[isIMBALANCE_STEPS] = [t.imbalance_step : t.real_hour | t in OPERATION_STEPS_LINK];
int imbStepRealHour[isIMBALANCE_STEPS];
...
//set including day-ahead steps such as the real hour is 1am
{string} is1am = {ppa | ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] == 1};
{int} is1amIndexes = {intValue(i) | i in is1am};
{string} is1amOclock = {da| da in is1am : (intValue(da)+3) in is1amIndexes || (intValue(da)-1) not in is1amIndexes};
//set including day-ahead steps such as the real hour is 4am
{string} is4am = {ppa | ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] == 4};
{int} is4amIndexes = {intValue(i) | i in is4am};
{string} is4amOclock = {da| da in is4am : (intValue(da)+3) in is4amIndexes || (intValue(da)-1) not in is4amIndexes};
//set including day-ahead steps such as the real hour is 5am
{string} is5am = {ppa | ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] == 5};
{int} is5amIndexes = {intValue(i) | i in is5am};
{string} is5amOclock = {da| da in is5am : (intValue(da)+3) in is5amIndexes || (intValue(da)-1) not in is5amIndexes};
//set including day-ahead steps such as the real hour is 10am
{string} is10am = {ppa | ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] == 10};
{int} is10amIndexes = {intValue(i) | i in is10am};
{string} is10amOclock = {da | da in is10am : intValue(da)==1 && intValue(da)+3 in is10amIndexes}
						union {da | da in is10am : (intValue(da)-1 > 0 && intValue(da)-1 not in is10amIndexes)};
//set including day-ahead steps such as the real hour is 10pm
{string} is10pm = {ppa | ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] == 22};
{int} is10pmIndexes = {intValue(i) | i in is10pm};
{string} is10pmOclock = {da | da in is10pm : intValue(da)==1 && intValue(da)+3 in is10pmIndexes}
						union {da | da in is10pm : (intValue(da)-1 > 0 && intValue(da)-1 not in is10pmIndexes)};
//set including day-ahead steps such as the real hour is midnight
{string} isMidnight = {ppa | ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] == 0};
{int} isMidnightIndexes = {intValue(i) | i in isMidnight};
{string} isMidnightOclock = {da | da in isMidnight : intValue(da)==1 && intValue(da)+3 in isMidnightIndexes}
						union {da | da in isMidnight : (intValue(da)-1 > 0 && intValue(da)-1 not in isMidnightIndexes)};
// optim step indexes
...
float assetStartupCost[isASSETS] = [a.asset_id : a.startup_cost | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
//(a in ASSETS)
float activePowerLossPct[isASSETS] = [a.asset_id : a.active_power_loss | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
//(a in ASSETS)
float activePowerSurgePct[isASSETS] = [a.asset_id : a.active_power_surge | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
//(a in ASSETS)
float reactivePowerLossPct[isASSETS] = [a.asset_id : a.reactive_power_loss | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
//(a in ASSETS)
float reactivePowerSurgePct[isASSETS] = [a.asset_id : a.reactive_power_surge | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// Maximum import or export nominal power at the level of each site
float siteNomMaxImport[isSITES] = [a.asset_id : a.max_power | a in ASSETS : a.type == "SITE"];
float siteNomMaxExport[isSITES] = [a.asset_id : -a.min_power | a in ASSETS : a.type == "SITE"];
...
float nomPowerMax[isASSETS] = [a.asset_id : a.nominal_max_power | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float powerMax[isASSETS] = [a.asset_id : ((a.max_power == -1.0 || a.max_power == -1) ? a.nominal_max_power : a.max_power) | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// If mapped, assets real-time minimum power (dynamic data from telemetry), nominal power otherwise (static).
float powerMin[isASSETS] = [a.asset_id : ((a.min_power == -1.0 || a.min_power == -1) ? a.nominal_min_power : a.min_power) | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// assets nominal minimum power (static data from configuration)
float nomPowerMin[isASSETS] = [a.asset_id : a.nominal_min_power | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// assets nominal number of blocks (static data from configuration)
//Normally default value is 1, but it might be -1.
int nomBlockNbr[isASSETS] = [a.asset_id : (a.total_block_number >= 0 ? a.total_block_number: 1) | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// If mapped, measured number of healthy blocks for asset a. If not mapped, it should be nominal number, or -1.
int healthyBlockNbr[isASSETS] = [a.asset_id : (a.healthy_block_number>= 0 ? a.healthy_block_number : 1)  | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// asset's non-linear cost model ID
string assetVarCostModelId[isASSETS] = [a.asset_id : a.var_cost_model | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// asset's non-linear efficiency model id
string assetVarEffModelId[isASSETS] = [a.asset_id : a.var_efficiency_model | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float initialPower[isASSETS] = [a.asset_id : -a.initial_power | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// active power floor under which asset is conseidered to be off (in kW)
float onStateTolerance[isASSETS] = [a.asset_id : a.power_tolerance | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
int  minTimeOn[isASSETS] = [a.asset_id : a.min_time_on | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
int maxTimeOn[isASSETS] = [a.asset_id : a.max_time_on | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
int minRecoveryTime[isASSETS] = [a.asset_id : a.min_recovery_period | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
int initialTimeOn[isASSETS] = [a.asset_id : a.initial_time_on | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
int initialTimeOff[isASSETS] = [a.asset_id : a.initial_time_off | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float assetScheduledAvailability[isASSETS][isDECISION_STEPS] = [as.asset_id : [as.step_id : maxl(0,as.availability)] | as in ASSET_STEPS];
// Site scheduled availability to withdraw from the grid of site s over decision step t
float siteScheduledAvailabilityIn[isSITES][isDECISION_STEPS] = [s.site_id : [s.step_id : maxl(0,s.availability_in)] | s in SITE_STEPS];
// Site scheduled availability to inject to the grid of site s over decision step t
float siteScheduledAvailabilityOut[isSITES][isDECISION_STEPS] = [s.site_id : [s.step_id : maxl(0,s.availability_out)] | s in SITE_STEPS];
...
// No unit, in between 0 and 1
float availGlobalForPmax[a in isASSETS][t in isDECISION_STEPS] = (availMeasuredTotForPmax[a] >= assetScheduledAvailability[a][first(isDECISION_STEPS)] - avail_tolerance ?
		maxl(assetScheduledAvailability[a][t], 0) // The measured availability was scheduled, we now only read the scheduled availability
		: minl(availMeasuredTotForPmax[a], maxl(assetScheduledAvailability[a][t], 0))); // The measured availability was not scheduled, we now have to be carefull, and take the min
// Global availability for Pmin, for each time step t, t in isDECISION_STEPS
// No unit, in between 0 and 1
float availGlobalForPmin[a in isASSETS][t in isDECISION_STEPS] = (availMeasuredTotForPmin[a] >= assetScheduledAvailability[a][first(isDECISION_STEPS)] - avail_tolerance ?
		maxl(assetScheduledAvailability[a][t],0) // The measured availability was scheduled, we now only read the scheduled availability
		: minl(availMeasuredTotForPmin[a], maxl(assetScheduledAvailability[a][t],0))); // The measured availability was not scheduled, we now have to be carefull, and take the min
...
// int heatStorAvail[s in isH_STORAGES][t in isDECISION_STEPS] = (assetScheduledAvailability[s][t] == 1.0 ? 1 : 0 );
// flag indicates whether asset a is partially or fully unavailable
int dispGenAvail[d in isDISP_GENS][t in isDECISION_STEPS] = (assetScheduledAvailability[d][t] != 0.0 ? 1 : 0 );
//int flexLoadAvail[f in isFLEX_LOADS][t in isDECISION_STEPS] = (assetScheduledAvailability[f][t] == 1.0 ? 1 : 0 );
// int convAvail[c in isCONVS][t in isDECISION_STEPS] = (assetScheduledAvailability[c][t] == 1.0 ? 1 : 0 );
// flag indicating whether the pool is available (equals 1) or not (equals 0) for aFRR (only specified for MICROGRID ZEE EDR1 for now) and equals 1 for all other sites
float afrrPoolAvail[t in isDECISION_STEPS] = 
  	(microgridName == "MICROGRID ZEE EDR1") ? assetScheduledAvailability["ZEE_capa_PORTAL_Indispos_aFRR_EDR1"][t] 
  	:(microgridName == "MICROGRID ZEE Vert") ? assetScheduledAvailability["ZEE_capa_PORTAL_Indispos_aFRR_Vert"][t] 
    : 1;
...
//float NFLoadAvail[n in isNF_LOADS][t in isDECISION_STEPS] = (assetScheduledAvailability[n][t] == -1.0 ? 0.0 : 100.0 * assetScheduledAvailability[n][t]);
///float interGenAvail[i in isINTER_GENS][t in isDECISION_STEPS] = (assetScheduledAvailability[i][t] == -1.0 ? 0.0 : 100.0 * assetScheduledAvailability[i][t]);
...
// Maximum import and export power for site s over decision step t taking into account possible site unavailabilities
float siteMaxAvailImportPower[s in isSITES][t in isDECISION_STEPS] = siteNomMaxImport[s] * siteScheduledAvailabilityIn[s][t];
float siteMaxAvailExportPower[s in isSITES][t in isDECISION_STEPS] = siteNomMaxExport[s] * siteScheduledAvailabilityOut[s][t];									
// If mapped, assets real-time maximum energy (dynamic data from telemetry), nominal value otherwise.
float energyMax[isASSETS] = [a.asset_id : a.max_energy | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// assets nominal maximum energy (static data from configuration)
float nomEnergyMax[isASSETS] = [a.asset_id : a.nominal_max_energy | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
// No unit, in between 0 and 1
float availGlobalForEmax[a in isASSETS][t in isDECISION_STEPS] = (availMeasuredTotForEmax[a] >= assetScheduledAvailability[a][first(isDECISION_STEPS)] - avail_tolerance ?
		maxl(assetScheduledAvailability[a][t], 0)
		: minl(availMeasuredTotForEmax[a], maxl(assetScheduledAvailability[a][t],0)));
...
float initialSOC[isASSETS] = [a.asset_id : a.initial_SOC | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// asset final SOC in %
float finalSOCLowerBound[isASSETS] = [a.asset_id : 0.0 | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float SOCMax[isASSETS] = [a.asset_id : a.max_SOC | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// assets minimum SOC as a %
float SOCMin[isASSETS] = [a.asset_id : a.min_SOC | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float dailyMaxCycles[isASSETS] = [a.asset_id : a.daily_maximum_number_of_cycles | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
string siteID[isASSETS] = [a.asset_id : a.site | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// asset's current injection potential (expressed in A)
float currentInjectionPotential[isASSETS] = [a.asset_id : a.injection_current_potential | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float assetVariableCost[isASSETS] = [a.asset_id : a.variable_cost | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
// Early aging cost caused by cycling for FCR (expressed in currency unit/kW engaged/4h)(s in isE_STORAGES)
// Hard-coded for now
float cyclingVariableCostFCR[s in isE_STORAGES] = (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID GDS Leforest B"?
													0.00309
													: 0) ; 
...
float assetCurtComp[isASSETS] = [a.asset_id : a.compensation_cost | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
string assetCurtEstimationMethod[isASSETS] = [a.asset_id : a.compensation_model | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
// average power forced (expressed in kW) for microgid asset a over decision step t ((a, t) in ASSET_STEPS)
float forcedPower[isASSETS][isDECISION_STEPS] = [as.asset_id : [as.step_id : as.forced_power] | as in ASSET_STEPS];
// average power forced (expressed in kW) for dispatchable Generator d over decision step t
float dispGenForcedPower[d in isDISP_GENS][t in isDECISION_STEPS] = -forcedPower[d][t];
// average power forced (expressed in kW) for asset stoage s over decision step t (positive values means charge, negative values means discharge)
float storageForcedPower[s in isSTORAGES][t in isDECISION_STEPS] = -forcedPower[s][t];
// average power forced (expressed in kW) for InterGen Asset i over decision step t (negative values means generation, default value is -1 for no forced power)
// (i in INTER_GENS, t in DECISION_STEPS)
float interGenForcedPower[i in isINTER_GENS][t in isDECISION_STEPS] = -forcedPower[i][t];
...
// In case of forced power, forced power replaces the forecast
// (i in INTER_GENS, t in DECISION_STEPS)
float interGenActivePowerForecast[i in isINTER_E_GENS][t in isDECISION_STEPS] = forcedPower[i][t] != -1 ? maxl(minl(interGenMaxAvailActivePower[i][t], interGenForcedPower[i][t]), interGenMinAvailActivePower[i][t])
																										: maxl(minl(interGenMaxAvailActivePower[i][t], -availGlobalForPmin[i][t] * powerPrediction[i][t]), interGenMinAvailActivePower[i][t]);
...
// maximum average power generation forecast (expressed in kW) for intermittent generation asset i over decision step t, below PMaxNomPDL of the related site
// (i in INTER_GENS, t in DECISION_STEPS)
float interGenPowerForecastBelowPDL[i in isINTER_E_GENS][t in isDECISION_STEPS] = minl(interGenActivePowerForecast[i][t],-nomPowerMin[first(siteIdByAsset[i])]);
// maximum average power generation forecast (expressed in kW) for intermittent generation asset i over decision step t, over PMaxNomPDL of the related site
// (i in INTER_GENS, t in DECISION_STEPS)
float interGenPowerForecastOverPDL[i in isINTER_E_GENS][t in isDECISION_STEPS] = maxl(0, interGenActivePowerForecast[i][t] - (-nomPowerMin[first(siteIdByAsset[i])]));
...
// (da in isDA_STEPS)
float daEngagement[isDA_STEPS] = [m.step_index : m.engagement | m in MARKET_ENGAGEMENTS : m.type == "DAY_AHEAD" && m.step_index != "0"];
...
// (da in isDA_STEPS)
int isDAStepCleared[isDA_STEPS] = [m.step_index : m.is_step_cleared | m in MARKET_ENGAGEMENTS : m.type == "DAY_AHEAD" && m.step_index != "0"];
// ppa market engagement (expressed in kW)
// (ppa in isPPA_STEPS)
float ppaEngagement[isPPA_STEPS] = [m.step_index : m.engagement | m in MARKET_ENGAGEMENTS : m.type == "PPA" && m.step_index != "0"];
// ppa market engagements on the ppa_step before the first ppa_step of the optimization window (expressed in kW)
float previousPPAEngagement = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "PPA" && m.step_index == "0"});
// ppa market engagement status (1 = cleared / 0 = not cleared)
// (ppa in isPPA_STEPS)
int isPPAStepCleared[isPPA_STEPS] = [m.step_index : m.is_step_cleared | m in MARKET_ENGAGEMENTS : m.type == "PPA" && m.step_index != "0"];
...
float daElecPriceRaw[isDA_STEPS] = [p.step_index : p.price | p in MARKET_PRICE_STEPS : p.type == "DAY_AHEAD"];
...
float sellOfferPrice = (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID VALECO Baignollais") ? 1.0 / 1000.0	// Valorem pays EP 0.4 �/MWh so we decided that it was not profitable for them to generate for less than 1 �/MWh
                        ...
                        : (microgridName in {"MICROGRID ZEE Vert"}) ? -0.01  / 1000.0 //0.00 / 1000.0
        :  0.0;
        
// Price at which we want to buy electricity (expressed in currency unit / kWh)       
float buyOfferPrice[h in isDA_STEPS] = //(microgridName in {"MICROGRID ZEE Vert"}) ? 0.15 // ZEE Vert buying price set to 0.15 �/kWh
											(daElecPriceRaw[h]>= 0
												? 1.2 * daElecPriceRaw[h] + 3/1000 
												: 0.8 * daElecPriceRaw[h] + 3/1000);
...
float daElecPrice[da in isDA_STEPS] = 
		(microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2" ? 
		 0.08 // Hard cod for Qair
		 :
		   (isDAStepCleared[da] == 0													// for non cleared steps...
		...
		&& (card(isE_STORAGES) == 0	|| max(s in isE_STORAGES) min(t in isDECISION_STEPS : assetStepDAStep[t] == da) availGlobalForPmax[s][t] == 0)	// and if there's no storage units, available at least...
			// we then floor forecast DA prices at sellOfferPrice to avoid curtailing PV production in case actual DA prices are > 0 
			? maxl(daPriceMinValue, sellOfferPrice, daElecPriceRaw[da]) 
			...
			: (daElecPriceRaw[da] >= 0 ? maxl(daPriceMinValue, daElecPriceRaw[da]) : minl(-daPriceMinValue, daElecPriceRaw[da]))
			)
		);
// long term agragated electricity market engagements (expressed in kW)
float long_term_engagement[isDA_STEPS] = [m.step_index : m.engagement | m in MARKET_ENGAGEMENTS : m.type == "ELECTRICITY_LONG_TERM_AGREGATED_BIDS" && m.step_index != "0"];
...
float previsousLTEngagement = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "ELECTRICITY_LONG_TERM_AGREGATED_BIDS" && m.step_index == "0"});
...
float previsousMFRRactivatedPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3" && m.step_index == "0"});
...
//PPA prices ppaTariff
float ppaTariff = 0.08;
// Hard-code for Qair
// Set of ppa steps when engagement is the same as step before
{string} isPPAEngagementStepSameAsBefore= {};
/////////////////////////////////////////
...
float maxPrice = maxl(
	...
	max(da in isDA_STEPS_POS) abs(daElecPrice[da]));
float maxCost = maxl(
...
float penaltyBaseForImb = maxl(maxPrice, maxCost, 0.001);
			...
			:(microgridName == "MICROGRID MOPABLOEM" ?				// if MOPABLOEM
				 (mFRRactivatedPower_imb[imb] < 0	? 			// if upward mFRR activation
				    ...
				    :(daElecPrice[imbStepHourlyStep[imb]] >= 0 ?
				    	...
				    	: daElecPrice[imbStepHourlyStep[imb]] / 1.5 + 120/1000))
				:(microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2" ? 				// If Qair /!\ electricityPrice is not indexed on imb step but rather on decision step (but imb=t for Qair)
					 (ppaStepRealHourStep[imb] < 9 ? // We penalize more imbalance during night because it is not suppose to go to imbalance unless there are unavailabilities or other unexpected things
			    			1.2 * ppaTariff
			    			: 0.8 * ppaTariff)
				: (daElecPrice[imbStepHourlyStep[imb]] >= 0	? 	// if not MOPABLOEM
				    	(optContext in {"MARKET_STRAT_FCR", "MARKET_STRAT_DA","MARKET_STRAT_AFRR"} ? daElecPrice[imbStepHourlyStep[imb]] * 2 + 50/1000: daElecPrice[imbStepHourlyStep[imb]] * 1.2)
				    	:(optContext in {"MARKET_STRAT_FCR", "MARKET_STRAT_DA","MARKET_STRAT_AFRR"} ? daElecPrice[imbStepHourlyStep[imb]] / 2 + 50/1000: daElecPrice[imbStepHourlyStep[imb]] / 1.2 + 50/1000))
			)
	));
		...
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
				? (mFRRactivatedPower_imb[imb] > 0					// if downward mFFR activation
				    ? -penaltyBaseForImb * 100	// We penalize positive imbalance, lower though than PV curtailment penalty
			        :(daElecPrice[imbStepHourlyStep[imb]] >= 0
				    	? daElecPrice[imbStepHourlyStep[imb]] / 1.5 - 120/1000
				    	: daElecPrice[imbStepHourlyStep[imb]] * 1.5 - 120/1000))
					:(microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2" ? 					// If Qair  and not Mopabloem/!\ electricityPrice is not indexed on imb step but rather on decision step (but imb=t for Qair)
			    		(ppaStepRealHourStep[imb] < 9 ? // We penalize more imbalance during night because it is not suppose to go to imbalance unless there are unavailabilities or other unexpected things
			    			-1.2 * ppaTariff
			    			:-0.8 * ppaTariff)
			    : (mFRRactivatedPower_imb[imb] > 0					// if not MOPABLOEM but downward mFFR activation
				    	? -penaltyBaseForImb * 100	// We put an arbitrary price, less favourable than day-ahead elec spot price
...
// base penalty cost
float maxImbalancAbsValue = card(isIMBALANCE_STEPS_POS) > 0  ?
								maxl(
								max(imb in isIMBALANCE_STEPS_POS) abs(positive_imb_price[imb]) * 1.2,
								max(imb in isIMBALANCE_STEPS_POS) abs(negative_imb_price[imb]) * 1.2)
								: 0.0;
float penaltyBase = maxl(
	penaltyBaseForImb,
	maxImbalancAbsValue
);
float catMultFactor = 10.0;
// cat 5 Violation variable penality costs (without unit)
// we should start from cat5_1Pen since cat5_1Pen must be higher than the max cost and max price. So for this minSocTargetStoragePenaltyCost must be lower than the minimum startup cost
///////////////////////////////////////
...
float unauthorizedInterGenCurtPenaltyCost =  cat5_1Pen * penaltyBase;
// penalty cost (expressed in currency unit per kwh) applied if ctStorageTargetSoc constraint cannot be satisfied
float minSocTargetStoragePenaltyCost = cat5_1Pen * penaltyBase;
...
float socMinViolationPenaltyCost = cat5_1Pen * penaltyBase;
// cat 6 Violation variable penality costs (without unit)
///////////////////////////////////////
// the storArtificialPenalityCost should be lower than the min variable cost and the min diffÃ©rence between segCost (to avoid using a generator instead of  battery when both are available)
...
float cat6Pen = minl(cat5_1Pen / catMultFactor, 1);
...
float gridTargetChangePenaltyCost = (2/1000);
...
// when calculaiting the cat4Pen we add the catMultFactor to ensure that the cat5_1Pen * penality base is strictly higher than the maxGenCost
...
float cat4Pen = maxl(catMultFactor*cat5_1Pen,(catMultFactor * maxGenCost) / penaltyBase);
...
float minACEnergyForAFRRDeficitCost = cat3Pen * penaltyBase;
...
/*********************************************/ 
float assetChargeEfficiency[isASSETS] = [a.asset_id : a.storage_charging_efficiency | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
float assetDischargeEfficiency[isASSETS] = [a.asset_id : a.storage_discharging_efficiency | a in ASSETS : a.asset_id not in isDUMMY_ASSETS];
...
float availInverterNbr[s in isE_STORAGES][t in isDECISION_STEPS] = (
	assetScheduledAvailability[s][first(isDECISION_STEPS)] > 0.0
		? minl(nomBlockNbr[s], healthyBlockNbr[s] / assetScheduledAvailability[s][first(isDECISION_STEPS)] * assetScheduledAvailability[s][t])
		: nomBlockNbr[s] * assetScheduledAvailability[s][t]
	);
...
// minimum nominal AC-side charge rateÅ“s (expressed in kW) for elec storage asset s (s in E_STORAGES)
...
// penality under cat5_1 must be higher than the cat5_2
float cat5_2Pen = cat5_1Pen / 1.2;
// penalty cost for non-respect of forced active power
float forcedPowerPenaltyCost = cat5_2Pen * penaltyBase;
////////////////////////////
// FCR
////////////////////////////
...
// Define max available power to do FCR, per asset, taking into consideration availability
// availability can be included into max/min power. 
float fcrAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] =
    (first(siteIdByAsset[a]) == "dummy_site") ?  // specific case for "dummy_site" because maxOuput["dummy_site"] and maxInput["dummy_site"] do not exist
        (
            (a in (isINTER_E_GENS union isDISP_E_GENS union isE_STORAGES)) ?
                ((assetScheduledAvailability[a][t] >= 0.85 && -nomPowerMin[a] >= fcrCertfiedPower[a]) ?
                    fcrCertfiedPower[a] : 0)
            :
                ((assetScheduledAvailability[a][t] >= 0.85 && nomPowerMax[a] >= fcrCertfiedPower[a]) ?
                    fcrCertfiedPower[a] : 0)
        )
    :
        minl(
            ...
            siteMaxAvailExportPower[first(siteIdByAsset[a])][t], siteMaxAvailImportPower[first(siteIdByAsset[a])][t]
        );


// Does not take into consideration if limited energy assets have required energy min/max (including PDL availability)
float fcrAvailPower[t in isDECISION_STEPS] = sum(a in isFCR_ASSETS) fcrAssetAvailPower[a][t];	 
...
float fcrReqPower[fcr in isFCR_STEPS_POS] =
    maxl(
        0.0,
        minl(
            fcrEngPower[fcr],
            min(t in isDECISION_STEPS : assetStepFCRStep[t] == fcr) (fcrAvailPower[t])
        )
    );	
...
// 1 means we donï¿½t take any risk and block all. (a in ASSETS)
...
//fCFD fo GDS
//////
//A reference tariff in currency unit per kWh
float t_ref = microgridName == "MICROGRID ZEE EDR1"
				? 50/1000
				:60/1000;
...
float inCFDDaPrice[h in isDA_STEPS] = (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_AFRR" ? t_ref : daElecPrice[h]);
float inCFDDaPriceRaw[h in isDA_STEPS] = (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_AFRR" ? t_ref : daElecPriceRaw[h]);
...
int isP0Submitted = (microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B", "MICROGRID ZEE Vert"}  && card ({m.step_index | m in MARKET_ENGAGEMENTS : m.type == "P0"}) > 0 ?
							1
							:0);
...
float afrrUpCertfiedPower[a in isASSETS] =
							(a == "BESS_MOPABLOEM") ? 1000
							:(a == "CHP_1600kW") ? 1600
							:(a == "ZEE_Mennetou_PDL1_BESS") ? 413
							:(a == "ZEE_Mennetou_PDL2_BESS") ? 396
							:(a == "ZEE_Mennetou_PDL3_BESS") ? 388
							:(a == "ZEE_Gievres_PDL1_BESS") ? 1006
							:(a == "ZEE_Gievres_PDL2_BESS") ? 1006
							:(a == "ZEE_Gievres_PDL3_BESS") ? 991
							:(a == "ZEE_Saint_Sauveur_BESS") ? 813
							:(microgridName ==  "MICROGRID GDS Leforest B" && a in isAFRR_UP_ASSETS) ? 1000 
							:(a == "ZEE_Vert_PDL1_BESS") ? 2000
							:(a == "ZEE_Vert_PDL2_BESS") ? 1950
							:(a == "ZEE_Vert_PDL3_BESS") ? 1550
							:(a == "ZEE_Vert_PDL4_BESS") ? 800
							:(a == "ZEE_Vert_PDL5_BESS") ? 1100
							:0
							;
...
float afrrDwnCertfiedPower[a in isASSETS] =
							...
							:(a == "ZEE_Gievres_PDL3_BESS") ? 991
							:(a == "ZEE_Saint_Sauveur_BESS") ? 813
							:(microgridName ==  "MICROGRID GDS Leforest B" && a in isAFRR_DOWN_ASSETS) ? 1000
							:(a == "ZEE_Vert_PDL1_BESS") ? 2000
							:(a == "ZEE_Vert_PDL2_BESS") ? 1950
							:(a == "ZEE_Vert_PDL3_BESS") ? 1550
							:(a == "ZEE_Vert_PDL4_BESS") ? 800
							:(a == "ZEE_Vert_PDL5_BESS") ? 1100 
							: 0;
// Bandwith max taken by an aFRR_STORAGE s when participating aFRR.It includes both partiicpation to the engagement, and P0 (usually excluded). It is mostly to impact curtaiment target for ZEE since we don't have the hand on the PV.
// Otherwise only participation to aFRR will be enough to consider.
float afrrMaxAssetBandwith[s in isAFRR_UP_STORAGES] = 
							 (s == "ZEE_Mennetou_PDL1_BESS") ? 521
							:(s == "ZEE_Mennetou_PDL2_BESS") ? 509
							:(s == "ZEE_Mennetou_PDL3_BESS") ? 517
							:(s == "ZEE_Gievres_PDL1_BESS") ? 1167
							:(s == "ZEE_Gievres_PDL2_BESS") ? 1167
							:(s == "ZEE_Gievres_PDL3_BESS") ? 1167
							:(s == "ZEE_Saint_Sauveur_BESS") ? 1025
							:(microgridName ==  "MICROGRID GDS Leforest B" && s in isAFRR_UP_ASSETS) ? 1000
							:(s == "ZEE_Vert_PDL1_BESS") ? 2000
							:(s == "ZEE_Vert_PDL2_BESS") ? 1950
							:(s == "ZEE_Vert_PDL3_BESS") ? 1550
							:(s == "ZEE_Vert_PDL4_BESS") ? 800
							:(s == "ZEE_Vert_PDL5_BESS") ? 1100  
							: 0;
float poolaFRRCertifiedPower = 
							(microgridName == "MICROGRID ZEE EDR1") ? 4000 
							:(microgridName ==  "MICROGRID GDS Leforest B") ? 1000
							:(microgridName ==  "MICROGRID ZEE Vert") ? 7000
							:0;
...
float afrrUpAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] = 
    (first(siteIdByAsset[a]) == "dummy_site") ?  // specific case for "dummy_site" because maxOuput["dummy_site"] does not exist
        (
            (a in (isINTER_E_GENS union isDISP_E_GENS union isE_STORAGES)) ?
                (-nomPowerMin[a] >= afrrUpCertfiedPower[a] ?
                    assetScheduledAvailability[a][t] * afrrUpCertfiedPower[a] : 0)
            :
                ( nomPowerMax[a] >= afrrUpCertfiedPower[a] ?
                    assetScheduledAvailability[a][t] * afrrUpCertfiedPower[a] : 0)
        )
    :
        minl(
          (  
            ...
            siteMaxAvailExportPower[first(siteIdByAsset[a])][t]
	);
...
float afrrDwnAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] = 
    (first(siteIdByAsset[a]) == "dummy_site") ?  // specific case for "dummy_site" because maxOuput["dummy_site"] does not exist
        (
            (a in (isINTER_E_GENS union isDISP_E_GENS union isE_STORAGES)) ?
                ((-nomPowerMin[a] >= afrrDwnCertfiedPower[a]) ?
                    assetScheduledAvailability[a][t] * afrrDwnCertfiedPower[a] : 0)
            :
                (( nomPowerMax[a] >= afrrDwnCertfiedPower[a]) ?
                    assetScheduledAvailability[a][t] * afrrDwnCertfiedPower[a] : 0)
        )
    :
        minl(
            ...
            siteMaxAvailImportPower[first(siteIdByAsset[a])][t]
        );
...
float aFRRUpVoluntaryEngPower[isAFRR_VOLUNTARY_STEPS] = [m.step_index : (m.engagement >= 0 ? m.engagement : 0) | m in MARKET_ENGAGEMENTS : (m.type == "AFRR_R2_VOLUNTARY_UP" || m.type == "AFRR_R2_ENERGY_UP")  && m.step_index != "0"];
...
float previousAFRRUpVoluntaryEngPower = isP0Submitted == 1 ? first({m.engagement | m in MARKET_ENGAGEMENTS : (m.type == "AFRR_R2_VOLUNTARY_UP" || m.type == "AFRR_R2_ENERGY_UP") && m.step_index == "0"}):0;
...
float aFRRDwnVoluntaryEngPower[isAFRR_VOLUNTARY_STEPS] = [m.step_index : (m.engagement >= 0 ? m.engagement : 0) | m in MARKET_ENGAGEMENTS : (m.type == "AFRR_R2_VOLUNTARY_DOWN" || m.type == "AFRR_R2_ENERGY_UP") && m.step_index != "0"];
...
float previousAFRRDwnVoluntaryEngPower = isP0Submitted == 1 ? first({m.engagement | m in MARKET_ENGAGEMENTS : (m.type == "AFRR_R2_VOLUNTARY_UP" || m.type == "AFRR_R2_ENERGY_UP") && m.step_index == "0"}): 0;
...
// Hard-coded for Qair Storsun
// Tolerance dead band within there is no imbalance, upward, in percent
float toleranceUpPercentImb = 10;
// Tolerance dead band within there is no imbalance, downward, in percent
float toleranceDwnPercentImb = 10;
// Max import from auxiliaries
float maxImportFromAux = sum(n in isFLEX_E_LOADS) flexLoadMaxNomPower[n];
// Minimum plateau position
float minPlateauValue = 0.1 * max(t in isDECISION_STEPS) siteNomMaxExport[first(isSITES)];
// Coefficient of proportionality of auxiliaries consumption in function of storage consumption (no unit)
float auxStorConsoCoeff = microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2" ?
								0.0582848 // =2500/60050
								: 0;
// Auxiliaries base consumption (in kW)
float auxStorConsoBase = microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2" ?
								80
								: 0;
...
execute CPX_PARAM {
	...
	if (operationID == "1335" || operationID == "1530" || operationID == "1664" || operationID == "1793" || operationID == "1795")
	...
	if (operationID == "1800")
		microgridName = "MICROGRID ZEE Vert";
	if (operationID == "9999")
		microgridName = "MICROGRID Qair STORSUN1";
	if (operationID == "1694" || operationID == "1799")
		microgridName = "MICROGRID Qair STORSUN2";
...
//		}
	...
	// HARD-CODED
 // hourString to get the hour from input
	hourString = optimisationIntervalStartTime.substring(11,13);

	// Define hourStart to be sure it also works for hours that start with 0 (no more issues with 09)
	if (hourString.charAt(0)=='0'){
	  hourStart = parseInt(hourString.substring(1,2));
	  }
	  else {
	    hourStart = parseInt(hourString);
	    }  
    ...
    for (var ppa in isPPA_STEPS) {
      for (var t in isDECISION_STEPS) {
		    if (ppa == assetStepPPAStep[t]) {
		       ppaStepRealHourStep[ppa] = assetStepRealHour[t];
          }	
		}
	}
	
	// Define isPPAEngagementStepSameAsBefore
	for (var ppa in isPPA_STEPS){
		var h = ppaStepRealHourStep[ppa]
		
		if (((2<=h && h<=3) || (6<=h && h<= 8) || (11<=h && h<=20) || (23<= h) ) ){
			isPPAEngagementStepSameAsBefore.add(parseFloat(ppa))		
		}
		else if(((h == 1 || h == 5 || h == 10 || h == 22) && ppaStepRealMinuteStep[ppa]!= "0")){
			isPPAEngagementStepSameAsBefore.add(parseFloat(ppa))			
		}
	}	
...
// average power generation target (expressed in kW) for intermittent generation asset i over decision step t, over PmaxNomPDL
// (i in INTER_E_GENS, t in DECISION_STEPS)
dvar float+ InterGenActivePowerOverPDL[isINTER_E_GENS][isDECISION_STEPS];
// estimation of curtailed power (expressed in kW) below PmaxNom PDL for intermittent generation asset i over step t
dvar float+ InterGenCurtBelowPDL[isINTER_E_GENS][isDECISION_STEPS];
// average power generation target (expressed in kW) for intermittent generation asset i over decision step t, below PmaxNomPDL
// (i in INTER_E_GENS, t in DECISION_STEPS)
dexpr float InterGenActivePowerBelowPDL[i in isINTER_E_GENS][t in isDECISION_STEPS] = interGenPowerForecastBelowPDL[i][t] - InterGenCurtBelowPDL[i][t];
// flag indicating whether intermittent generation asset i is producing over PmaxNomPDL over step t
// (i in INTER_GENS, t in DECISION_STEPS)
dvar boolean IsInterGenOverPDL[isINTER_E_GENS][isDECISION_STEPS];
...
// forced power excess violation for interGen i over step t
// positive values means excess, negative values deficit
// (i in isINTER_E_GENS, t in DECISION_STEPS)
dexpr float InterGenForcedPowerViolation[i in isINTER_E_GENS][t in isDECISION_STEPS] =  (interGenForcedPower[i][t] != 1 ? InterGenActivePower[i][t] - interGenForcedPower[i][t] : 0);
// forced power deficit violation for interGen i over step t
// (i in isINTER_E_GENS, t in DECISION_STEPS)
// dvar float+ InterGenForcedPowerViolationDeficit[isINTER_E_GENS][isDECISION_STEPS];
// cost of the non-respect of the forced power for interGen i over step t
// No longer costs
// (i in isINTER_E_GENS, t in DECISION_STEPS)
dexpr float InterGenForcedPowerViolationCost[i in isINTER_E_GENS][t in isDECISION_STEPS] = 0;//(InterGenForcedPowerViolationExcess[i][t] + InterGenForcedPowerViolationDeficit[i][t]) * forcedPowerPenaltyCost * assetStepDurationInHours;
// Storage assets
/////////////////
...
// flag indicating if storage asset sï¿½s injection target for decision step t corresponds to a charge or a discharge
...
// forced power Excess violation for storage asset s over decision step t
// (s in isSTORAGES, t in DECISION_STEPS)
dvar float+ StorageAssetChargeForcedPowerViolationExcess[isSTORAGES][isDECISION_STEPS];
// forced power Deficit violation for storage asset s over decision step t
// (s in isSTORAGES, t in DECISION_STEPS)
dvar float+ StorageAssetChargeForcedPowerViolationDeficit[isSTORAGES][isDECISION_STEPS];
// forced power violation for storage asset s over decision step t during the discharging phase
// (s in isSTORAGES, t in DECISION_STEPS)
dvar float StorageAssetDischargeForcedPowerViolation[isSTORAGES][isDECISION_STEPS];
// cost of the non-respect of the forced power for asset storage s over step t
// (s in isSTORAGES, t in DECISION_STEPS)
dexpr float StorageAssetForcedPowerViolationCost[s in isSTORAGES][t in isDECISION_STEPS] = (StorageAssetChargeForcedPowerViolationDeficit[s][t] + StorageAssetChargeForcedPowerViolationExcess[s][t]) * forcedPowerPenaltyCost * assetStepDurationInHours;
// Connection to grid
/////////////////////
...
//We define an average Power Target: the average in between NetElecImportTarget of t and t-1(given NetElecImportTarget is the target for the end of step)
//initial power is in production convetion
dexpr float AvgNetElecImportTarget[t in isDECISION_STEPS] = 
                                t == first(isDECISION_STEPS) ?
                                   (NetElecImportTarget[t] + (-initialPower[first(isSITES)])) / 2
                                  : (NetElecImportTarget[t] + NetElecImportTarget[prev(isDECISION_STEPS,t)]) / 2;
// difference of net ElecImportTarget of t and t-1
dexpr float DeltaNetElecImportTarget[t in isDECISION_STEPS] =
								t == first(isDECISION_STEPS) ?
                                   (NetElecImportTarget[t] - (-initialPower[first(isSITES)]))
                                  : (NetElecImportTarget[t] - NetElecImportTarget[prev(isDECISION_STEPS,t)]) ; 
//Absolute difference of NetElecImportTarget of t and t-1
dvar float+ AbsDeltaNetElecImportTarget[isDECISION_STEPS];
// Change cost of grid Traget
dexpr float GridTargetChangeStepCost[t in isDECISION_STEPS] = 
	(microgridName == "MICROGRID Qair STORSUN2"  ?
		AbsDeltaNetElecImportTarget[t] * gridTargetChangePenaltyCost * assetStepDurationInHours
 		:0);
...
// (hr in isDA_STEPS)
dvar float DAOffer[isDA_STEPS];
// Offer/Nomination made on the ppa market for ppa step expressed in kW (> 0 means purchase / < 0 means sale)
// (ppa in isPPA_STEPS)
dvar float PPAOffer[isPPA_STEPS];
...
dvar float InCFDDaPosition[isDA_STEPS];
...
dvar float NotInCFDDaPosition[isDA_STEPS];
// Position taken on the day-ahead market fro DA step da expressed in kW (> 0 means purchase / < 0 means sale)
// If da has already been cleared then position is fixed, otherwise it is up to optimisation
// (da in isHOURLY_STEPS)
dexpr float DaPosition[da in isDA_STEPS] = 
				(isDAStepCleared[da] == 1 ?
				   daEngagement[da]
	               : DAOffer[da]); // For uncleared steps, it is a 'free' decision variable

// Position taken, expressed in kW (> 0 means purchase / < 0 means sale)
// If da has already been cleared then position is fixed, otherwise it is up to optimisation
// (ppa in isPPA_STEPS)
dexpr float PPAPosition[ppa in isPPA_STEPS] = 
			(isPPAStepCleared[ppa] == 1 ?
				  (microgridName in {"MICROGRID Qair STORSUN1", "MICROGRID Qair STORSUN2"} ?
	                  ppaEngagement[ppa] *
	                    (ppaStepRealHourStep[ppa] in {0,9} ? //Ramp up ?
	                      (ppaStepRealMinuteStep[ppa] == "0" ? (1/8)
	                      	:(ppaStepRealMinuteStep[ppa] == "15" ? (1/4 + 1/8)
	                      		:(ppaStepRealMinuteStep[ppa] == "30" ? (2/4 + 1/8)
	                      			: (3/4 + 1/8)
	                      			)
	                      		)
	                      	)
		                  :(ppaStepRealHourStep[ppa] in {4,21} ? //Ramp down ?
		                      (ppaStepRealMinuteStep[ppa] == "0" ? (3/4 + 1/8)
		                      	:(ppaStepRealMinuteStep[ppa] == "15" ? (2/4 + 1/8)
		                      		:(ppaStepRealMinuteStep[ppa] == "30" ? (1/4 + 1/8)
		                      			: (1/8)
		                      			)
		                      		)
		                      	) 
		                 		: 1 //Other cleared hours, we directly take the engagement
		                 	)
		                 )
		                 : ppaEngagement[ppa]) //not Qair
	                 : PPAOffer[ppa]); // For uncleared steps, it is a 'free' decision variable
// Hard code for Qair
//Define ramp limitations for each time step
// it is the engagement of the night plateau at night or day plateau in between 9am and 10pm. Otherwise it is quite null
dexpr float StepRampLimitation[t in isDECISION_STEPS] = 
      //Fetch in the future
      // night ramp, engagement is positive (take from the grid)
      (assetStepRealHour[t] < 3 ? 
        (ord(isPPA_STEPS, t) + 4 < card(isPPA_STEPS) ?
          PPAPosition[item(isPPA_STEPS,ord(isPPA_STEPS,assetStepPPAStep[t])+4)] / 4
          : siteNomMaxExport[first(isSITES)] / 4
          )
          //morning ramp, engagement is negative
        :(9 <= assetStepRealHour[t] < 20) ? 
	        (ord(isPPA_STEPS, t) + 4 < card(isPPA_STEPS) ?
	          -PPAPosition [item(isPPA_STEPS,ord(isPPA_STEPS,assetStepPPAStep[t])+4)] / 4
	          : siteNomMaxExport[first(isSITES)] / 4
	          )
	        //Fetch in the past
	        // night ramp, engagement is positive (take from the grid)
	        :(assetStepRealHour[t] < 5 ?
		          (ord(isPPA_STEPS, t) - 4 >= 0 ?
		            PPAPosition [item(isPPA_STEPS,ord(isPPA_STEPS,assetStepPPAStep[t])-4)] / 4
		            : siteNomMaxExport[first(isSITES)] / 4
		          )
		          //evening ramp, engagement is negative
		          :(20 <= assetStepRealHour[t] < 22 ?
			          (ord(isPPA_STEPS, t) - 4 >= 0 ?
			            -PPAPosition [item(isPPA_STEPS,ord(isPPA_STEPS,assetStepPPAStep[t])-4)] / 4
			            : siteNomMaxExport[first(isSITES)] / 4
			          )
				      // Other hours, just auxiliaries variation
				      : 1000//nfElecLoadMaxNomPower[first(isNF_E_LOADS)]/4
        ))); 
/*FCR*/
...
dexpr float CyclingStepContribution[s in isE_STORAGES][t in isDECISION_STEPS] = (nomEnergyMax[s] > 0.0 ? (StorDCPowerCharge[s][t] + StorDCPowerDischarge[s][t]) * assetStepDurationInHours / (2 * nomEnergyMax[s]) : 0.0); 
...
// forced power excess violation for generator d over step t
// (d in DISP_E_GENS, t in DECISION_STEPS)
dvar float+ DispGenForcedPowerViolationExcess[isDISP_E_GENS][isDECISION_STEPS];
// forced power deficit violation cost for generator d over step t
// (d in DISP_E_GENS, t in DECISION_STEPS)
dvar float+ DispGenForcedPowerViolationDeficit[isDISP_E_GENS][isDECISION_STEPS];
// cost of the non-respect of the forced power for dispGen d over step t
// (d in DISP_E_GENS, t in DECISION_STEPS)
dexpr float DispGenForcedPowerViolationCost[d in isDISP_E_GENS][t in isDECISION_STEPS] = (DispGenForcedPowerViolationExcess[d][t] + DispGenForcedPowerViolationDeficit[d][t]) * forcedPowerPenaltyCost * assetStepDurationInHours;
...
/////////////////////////////////////
// asset step cost of energy drawn for normal sale / purchase operations from the maingrid
// For Equans : those costs are not applied at microgrid level but only for meter 8
dexpr float TradeNetworkStepCosts[t in isDECISION_STEPS] = assetStepDurationInHours *
	(microgridName == "Equans BIC microgrid" ?
	Meter8Import[t]
	: ImportTarget[t]
	) * networkDrawingTax[t];
...
// For Equans : those costs are not applied at microgrid level but only for meter 8
dexpr float TotalTradeNetworkCosts = sum(t in isDECISION_STEPS) TradeNetworkStepCosts[t];
// Cost of energy drawn for normal sale / purchase operations from the maingrid over steps that are not already cleared
dexpr float OptimisedTradeNetworkCosts = assetStepDurationInHours * sum(t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0) ImportTarget[t] * networkDrawingTax[t];
...
dexpr float OptimisedFCRNetworkCosts =
	  1.0 / (pow(storefficiencyfcr / 100.0, 2) * pow(inverterefficiencyfcrout / 100.0, 2))
	* sum (s in isE_STORAGES, t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0) networkDrawingTax[t] * FCRUnitarianStepACEnergyOut * FCRPower[s][assetStepFCRStep[t]];
	// net costs from injecting/drawing electricity into/from maingrid over decision step t
// positive values = net cost / negative values = net revenue
dexpr float ElectricityStepNetCosts[t in isDECISION_STEPS] = electricityPrice[t] * NetElecImportTarget[t] * assetStepDurationInHours;
// total net costs from injecting/drawing electricity into/from maingrid over optimisation window
// positive values = net cost / negative values = net revenue
dexpr float ElectricityTotalNetCosts = sum(t in isDECISION_STEPS) ElectricityStepNetCosts[t];
// net costs from injecting/drawing electricity into/from maingrid over decision steps that are not already cleared
// positive values = net cost / negative values = net revenue
dexpr float OptimisedElecNetCosts = sum(t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0) electricityPrice[t] * NetElecImportTarget[t] * assetStepDurationInHours;
// asset step variable costs for generation from generators with non-linear variable costs
dexpr float GenNonLinVarStepCosts[t in isDECISION_STEPS] = sum(g in isNL_COST_E_GENS) GenNonLinVarCost[g][t] * assetStepDurationInHours;
// total variable costs for generation from generators with non-linear variable costs over optimisation window
dexpr float TotalGenNonLinVarCosts = sum(t in isDECISION_STEPS) GenNonLinVarStepCosts[t];
// starting costs from dispatchable generators over decision step t
dexpr float DispGenStartupStepCosts[t in isDECISION_STEPS] = sum(d in isSTART_COST_E_GENS) DispGenStartup[d][t] * dispGenStartupCost[d];
// total starting costs from dispatchable generators over optimisation window
dexpr float DispGenTotalStartupCosts = sum(t in isDECISION_STEPS) DispGenStartupStepCosts[t];
// curtailment costs from intermittent generation over decision step t
dexpr float InterGenCurtStepCosts[t in isDECISION_STEPS] = sum(i in isCURT_COMP_E_GENS) interGenCurtComp[i] * InterGenCurtEstimation[i][t] * assetStepDurationInHours;
...
// late discharge cost over decision step t
dexpr float StorChargeDischargeStepCost[t in isDECISION_STEPS] = (
		...
		? sum(s in isE_STORAGES)
 		...
 		: (
 			microgridName == "MICROGRID TPL Tongatapu"																	// for Tonga, we want to encourage early discharge sor Matatoa BESS only (see storArtificialPenalityCost param)
 				? sum(s in isE_STORAGES inter {"Tongatapu_Matatoa_BESS"})
 					storArtificialPenalityCost  [t] * StorACPowerDischarge[s][t] * assetStepDurationInHours
 				: sum(s in isE_STORAGES)															// for IDP and Mare, we want to encourage early discharge (see storArtificialPenalityCost param)
 					storArtificialPenalityCost[t] * StorACPowerDischarge[s][t] * assetStepDurationInHours
 		)
 	);
 // asset step late discharge cost over optimisation window// total penalty for storage power changes between decision steps
dexpr float StorChargeDischargeCost = sum(t in isDECISION_STEPS) StorChargeDischargeStepCost[t];
dexpr float StorPowerChangeStepCost[t in isDECISION_STEPS] =
    sum (s in isE_STORAGES) storPowerChangePenalty * (StorACActivePowerInc[s][t] + StorACActivePowerDec[s][t]) * assetStepDurationInHours;
// total penalty for storage power changes between decision steps
dexpr float StorPowerChangeTotalCost = sum (t in isDECISION_STEPS) StorPowerChangeStepCost[t];
...
// FCR eng deficit over decision step t
// only used in post-processing
dexpr float FCRPowerEngDeficitStepCost[t in isDECISION_STEPS] = fcrStepDuration not in {-1, 0} ? FCRPowerEngDeficitPenaltyCost * fcrStepDurationInHours * FCRPowerEngDeficit[assetStepFCRStep[t]] * (assetStepDuration/fcrStepDuration) : 0;
...
// CFD
//////
// asset step Cost of energy to be sold on the market Spot, for asset not not subject to the CFDf
// used for Post processing
dexpr float NotInCFDDaStepTradCost[t in isDECISION_STEPS] = daStepDuration not in {-1, 0} ? (NotInCFDDaPosition[assetStepDAStep[t]] * daElecPrice[assetStepDAStep[t]] * effDaStepDurationInHours[assetStepDAStep[t]])*(assetStepDuration/daStepDuration)
																							: 0;
// asset step revenues of energy to be sold on the market Spot, in the case of a contract for differnce without M0 arbitratio, with a fixed tref price.
// used for Post processing
dexpr float InCFDDaStepTradRevenues[t in isDECISION_STEPS] = daStepDuration not in {-1, 0} ? (-InCFDDaPosition[assetStepDAStep[t]] * inCFDDaPrice[assetStepDAStep[t]] * effDaStepDurationInHours[assetStepDAStep[t]])*(assetStepDuration/daStepDuration)
																							: 0;
// Total Cost of energy to be sold on the market Spot, for asset not not subject to the CFDf
dexpr float NotInCFDDaTotalTradeCost = sum(da in isDA_STEPS_POS) NotInCFDDaPosition[da] * daElecPrice[da] * effDaStepDurationInHours[da];
dexpr float NotInCFDDaTotalTradeCostRaw = sum(da in isDA_STEPS_POS) NotInCFDDaPosition[da] * daElecPriceRaw[da] * effDaStepDurationInHours[da];
...
dexpr float InCFDDaTotalTradeRevenues = sum(da in isDA_STEPS_POS) -InCFDDaPosition[da] * inCFDDaPrice[da] * effDaStepDurationInHours[da];
dexpr float InCFDDaTotalTradeRevenuesRaw = sum(da in isDA_STEPS_POS) -InCFDDaPosition[da] * inCFDDaPriceRaw[da] * effDaStepDurationInHours[da];
...
dexpr float DayAheadTotalTradeCost = (
	microgridName != "MICROGRID Qair STORSUN1" && microgridName != "MICROGRID Qair STORSUN2"
	? NotInCFDDaTotalTradeCost - InCFDDaTotalTradeRevenues
	:0
	);
dexpr float DayAheadTotalTradeCostRaw = (
	microgridName != "MICROGRID Qair STORSUN1" && microgridName != "MICROGRID Qair STORSUN2"
	? NotInCFDDaTotalTradeCostRaw - InCFDDaTotalTradeRevenuesRaw
	:0
	);

// ppa step cost
dexpr float PPAStepTradeCost[ppa in isDECISION_STEPS] = AvgNetElecImportTarget[ppa] * ppaTariff * assetStepDurationInHours;
//Total ppa trade cost over ppa step
dexpr float PPATotalTradeCost = sum(ppa in isDECISION_STEPS) PPAStepTradeCost[ppa];
// not in CFD asset step imbalance cost
// Positive values mean it is a cost, negative that it is a revenue
// Only used for post processing
dexpr float NotInCFDImbalanceStepCost[t in isDECISION_STEPS] = imbStepDuration not in {-1, 0} ?(NotInCFDNegativeImbalancePower_imb[assetStepImbalanceStep[t]] * negative_imb_price[assetStepImbalanceStep[t]]  - NotInCFDPositiveImbalancePower_imb[assetStepImbalanceStep[t]] * positive_imb_price[assetStepImbalanceStep[t]])
																						* effImbStepDurationInHours[assetStepImbalanceStep[t]] * (assetStepDuration/imbStepDuration)
																						: 0;
...
// CFD imbalance cost over asset step t
// Positive values mean it is a cost, negative that it is a revenue
// Only used for post processing
dexpr float InCFDImbalanceStepCost[t in isDECISION_STEPS] = imbStepDuration not in {-1, 0} ? (InCFDNegativeImbalancePower_imb[assetStepImbalanceStep[t]] * inCFDNegImbPrice[assetStepImbalanceStep[t]] - InCFDPositiveImbalancePower_imb[assetStepImbalanceStep[t]] * inCFDPosImbPrice[assetStepImbalanceStep[t]]) 
																						* effImbStepDurationInHours[assetStepImbalanceStep[t]] * (assetStepDuration/imbStepDuration)
																						: 0;
...
dexpr float OptimisedDayAheadTradeCost =
    sum(da in isDA_STEPS_POS: isDAStepCleared[da] == 0) (NotInCFDDaPosition[da] * daElecPriceRaw[da]  + InCFDDaPosition[da] * inCFDDaPrice[da]) * effImbStepDurationInHours[da];
// Estimation of cost of energy purchased for during FCR over decision step t (SOC management, we ignore regulation b/c symetrical with sales)
...
dexpr float DayAheadFCRStepCost[t in isDECISION_STEPS] =
	  (1.0 / (pow(storefficiencyfcr / 100.0, 2) * pow(inverterefficiencyfcrout / 100.0, 2)) - 1.0)
	* sum (s in isE_STORAGES) daElecPriceRaw[assetStepDAStep[t]] * FCRUnitarianStepACEnergyOut * FCRPower[s][assetStepFCRStep[t]];
...
// (day ahead price applied to estimated AC energy in - estimated AC energy out = estimated AC energy out x (1 / (inverter efficiency x battery efficiency x battery efficiency x inverter efficiency) - 1)
dexpr float TotalDayAheadFCRCost = sum (t in isDECISION_STEPS) DayAheadFCRStepCost[t];
...
dexpr float OptimisedDayAheadFCRCost =
	  (1.0 / (pow(storefficiencyfcr / 100.0, 2) * pow(inverterefficiencyfcrout / 100.0, 2)) - 1.0)
	* sum (s in isE_STORAGES, t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0) daElecPriceRaw[assetStepDAStep[t]] * FCRUnitarianStepACEnergyOut * FCRPower[s][assetStepFCRStep[t]];
///////
// PAA VALECO
///////
...
///////
// Aggregator fees that may be implemented for some sites, over decision step t
// For Equans : it is applied for both imports and exports on meter 8, and also on BIC_PV1 production
dexpr float AggregatorStepCost[t in isDECISION_STEPS] = 
	(microgridName == "Equans BIC microgrid" ?
...
// For Equans : it is applied for both imports and exports on meter 8, and also on BIC_PV1 production
dexpr float AggregatorCost = sum(t in isDECISION_STEPS) AggregatorStepCost[t];
// Linear costs
// Linear costs
// variable costs for generation from generation assets with linear variable costs over optimiation window
dexpr float GenLinearVarStepCosts[t in isDECISION_STEPS] =
	  sum(i in isINTER_E_GENS inter isLIN_COST_E_GENS) InterGenActivePower[i][t] * genVariableCost[i] * assetStepDurationInHours
	+ sum(d in isDISP_E_GENS inter isLIN_COST_E_GENS) DispGenEffActivePower[d][t] * genVariableCost[d] * assetStepDurationInHours
    + sum(s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s]) * nomEnergyMax[s]
    + (fcrStepDuration not in {-1, 0} ? sum(s in isE_STORAGES inter isLIN_COST_E_GENS) (cyclingVariableCostFCR[s] * FCRPower[s][assetStepFCRStep[t]]) * (assetStepDuration/fcrStepDuration) : 0);
// Linear costs
// total variable costs for generation from generation assets with linear variable costs over optimiation window
dexpr float GenTotalLinearVarCosts = sum(t in isDECISION_STEPS) GenLinearVarStepCosts[t];
...
dexpr float OptimisedGenLinearVarCosts =
	  sum(t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0, i in isINTER_E_GENS inter isLIN_COST_E_GENS) InterGenActivePower[i][t] * genVariableCost[i] * assetStepDurationInHours
	+ sum(t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0, d in isDISP_E_GENS inter isLIN_COST_E_GENS) DispGenEffActivePower[d][t] * genVariableCost[d] * assetStepDurationInHours
    + sum(t in isDECISION_STEPS: isDAStepCleared[assetStepDAStep[t]] == 0, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s]) * nomEnergyMax[s] * assetStepDurationInHours
    + sum(da in isDA_STEPS_POS: isDAStepCleared[da] == 0 && card(isFCR_STEPS_POS) > 0 , s in isE_STORAGES inter isLIN_COST_E_GENS) (cyclingVariableCostFCR[s] * FCRPower[s][daStepFCRStep[da]]*(daStepDuration/fcrStepDuration));

// asset step costs, other than electricity, for conv assets
dexpr float ConvLinearStepCosts[t in isDECISION_STEPS] =
	sum(c in isEIN_CONVS) assetVariableCost[c] * ConvActivePowerIn[c][t] * assetStepDurationInHours;
// Total costs, other than electricity, for conv assets
dexpr float ConvTotalLinearCosts = sum(t in isDECISION_STEPS) ConvLinearStepCosts[t];

dexpr float ObjStepCost[t in isDECISION_STEPS] =
			electricityPrice[t] * NetElecImportTarget[t] * assetStepDurationInHours
			+ NotInCFDDaStepTradCost[t] - InCFDDaStepTradRevenues[t]
			+ NotInCFDImbalanceStepCost[t]  + InCFDImbalanceStepCost[t]
			+ TradeNetworkStepCosts[t]
			+ TotalFCRNetworkCostsByStep[t]
			+ DayAheadFCRStepCost[t]
			+ GenNonLinVarStepCosts[t]
			+ DispGenStartupStepCosts[t]
			+ GenLinearVarStepCosts[t]
			+ ConvLinearStepCosts[t]
			+ InterGenCurtStepCosts[t]
			+ AggregatorStepCost[t]
			+ StorChargeDischargeStepCost[t]
			+ StorPowerChangeStepCost[t]
			+ FCRPowerEngDeficitStepCost[t]
			+ PPAStepTradeCost[t]
			+ (t == "1" ? sum(d in isDISP_E_GENS) (DispGenInitialPowerUpViolationCost[d] + DispGenInitialPowerDwnViolationCost[d]) : 0)
			+ (afrrVoluntaryStepDuration != -1 &&  afrrVoluntaryStepDuration != 0 ? (AFRRUpPowerEngDeficitCost[assetStepAfrrVoluntaryStep[t]] + AFRRDwnPowerEngDeficitCost[assetStepAfrrVoluntaryStep[t]]) * (assetStepDuration/afrrVoluntaryStepDuration) : 0)
			+ (afrrVoluntaryStepDuration != -1 &&  afrrVoluntaryStepDuration != 0 ? (sum(s_afrr in isAFRR_UP_STORAGES) MinACEnergyForAFRRUpCost[s_afrr][assetStepAfrrVoluntaryStep[t]] + sum (s_afrr in isAFRR_DWN_STORAGES)MinACEnergyForAFRRDwnCost[s_afrr][assetStepAfrrVoluntaryStep[t]]) * (assetStepDuration/afrrVoluntaryStepDuration) : 0)
			+ PowerDeficitCost[t] + PowerExcessCost[t]
			+ HeatDeficitCost[t] + HeatExcessCost[t]
			+ UnauthorizedInterGenCurtCost[t]
			+ sum(s in isE_STORAGES) (ElecEnerStorStrictMinDeficitCost[s][t] + ElecEnerStorMaxExcessCost[s][t] + ElecEnerStorMinDeficitCost[s][t])
			+ sum(s in isH_STORAGES) (HeatStorEnerMinExcessCost[s][t] + HeatStorEnerMinDeficitCost[s][t])
			+ sum(s in isE_STORAGES) StorageAssetForcedPowerViolationCost[s][t]
			+ sum(s in isE_STORAGES) minSocTargetStorageDeficitCost[s][t]
			+ sum(s in isE_STORAGES) StorageDailyMaxNumCyclExcessCost[s][t]
			+ sum(i in isSITES) (SiteMaxInputViolationCost[i][t] + SiteMaxOutputViolationCost[i][t])
			+ sum(c in isCONGESTIONS) (CongestionUpperLimViolationCost[c][t] + CongestionLowerLimViolationCost[c][t])
			+ (t == "1" ? sum(d in isDISP_E_GENS, i in 0..(genInitialStepsOnMax[d]-1)) DispGenMaxStepOnInitialExcessCost[d][i] : 0)
			+ sum(d in isDISP_E_GENS) DispGenMaxStepOnExcessCost[d][t]
			+ (t == "1" ? sum(d in isDISP_E_GENS) DispGenMinStepOnInitialDeficitCost[d] : 0)
			+ sum(d in isDISP_E_GENS) DispGenMinStepOnDeficitCost[d][t]
			+ (t == "1" ? sum(d in isDISP_E_GENS) DispGenMinStepOffInitialDeficitCost[d] : 0)
			+ sum(d in isDISP_E_GENS) DispGenMinStepOffDeficitCost[d][t]
			+ sum(d in isDISP_E_GENS) DispGenMinActivePowerDeficitCost[d][t]
			+ sum(d in isDISP_E_GENS) DispGenForcedPowerViolationCost[d][t]
			+ sum(d in isDISP_E_GENS) DispGenActivePowerRaiseReserveDeficitCost[d][t]
			+ sum(i in isINTER_E_GENS) InterGenActivePowerRaiseReserveDeficitCost[i][t]
			+ sum(i in isINTER_E_GENS) InterGenForcedPowerViolationCost[i][t]
			+ sum(s in isE_STORAGES) StorActivePowerRaiseReserveDeficitCost[s][t]
			+ sum(n in isNF_E_LOADS) NFLoadActivePowerRaiseReserveDeficitCost[n][t]
			+ sum(i in isINTER_E_GENS) InterGenActivePowerLowerReserveDeficitCost[i][t]
			+ sum(s in isE_STORAGES) StorActivePowerLowerReserveDeficitCost[s][t]
			+ sum(f in isFLEX_E_LOADS) FlexLoadActivePowerLowerReserveDeficitCost[f][t]
			+ sum(n in isNF_E_LOADS) NFLoadActivePowerLowerReserveDeficitCost[n][t]
			+ sum(d in isDISP_E_GENS) DispGenReactivePowerRaiseReserveDeficit[d][t]
			+ sum(i in isINTER_E_GENS) InterGenReactivePowerRaiseReserveDeficitCost[i][t]
			+ sum(s in isE_STORAGES) StorReactivePowerRaiseReserveDeficitCost[s][t]
			+ sum(n in isNF_E_LOADS) NFLoadReactivePowerRaiseReserveDeficitCost[n][t]
			+ sum(i in isINTER_E_GENS) InterGenReactivePowerLowerReserveDeficitCost[i][t]
			+ SpinningRaiseReserveDeficitCost[t] + SpinningLowerReserveDeficitCost[t]
			+ sum(s in isE_STORAGES) StorReactivePowerLowerReserveDeficitCost[s][t]
			+ sum(f in isFLEX_E_LOADS) FlexLoadReactivePowerLowerReserveDeficitCost[f][t]
			+ sum(n in isNF_E_LOADS) NFLoadReactivePowerLowerReserveDeficitCost[n][t]
			+ DefaultCurrentRequirementDeficitCost[t];
...
minimize
  ...
  + sum(s in isE_STORAGES, t in isDECISION_STEPS) StorageAssetForcedPowerViolationCost[s][t]
  ...
  + sum(d in isDISP_E_GENS, t in isDECISION_STEPS) DispGenForcedPowerViolationCost[d][t]
  ...
  + sum(i in isINTER_E_GENS, t in isDECISION_STEPS) InterGenForcedPowerViolationCost[i][t]
  ...
  + sum(t in isDECISION_STEPS) (GridTargetChangeStepCost[t]);
  
/*********************************************************************
 * Constraints
 *********************************************************************/
  subject to {
//    ctTest: -NetElecImportTarget["25"] <= -13220;
  
//	forall (t in isDECISION_STEPS: 48<=ord(isDECISION_STEPS, t)<=91)
// 	ctTest6: -NetElecImportTarget[t]==30000;
// 		forall (t in isDECISION_STEPS: 144<=ord(isDECISION_STEPS, t)<=187)
// 	ctTest7: -NetElecImportTarget[t]==30000;
// 	forall (t in isDECISION_STEPS: 93<=ord(isDECISION_STEPS, t)<=96)
// 	ctTest: -NetElecImportTarget[t]==15000;

  
//	forall (t in isDECISION_STEPS : 11 <= assetStepRealHour[t] <= 14)
//		ctTest2: DaPosition[assetStepDAStep[t]]==-30000;
...
// Pool market
// Pool engagement on imbalance step. Long term and day-ahead engagement at an hourly step gives the same engagement at the imbalance step.
...
if (microgridName not in {"MICROGRID Qair STORSUN1", "MICROGRID Qair STORSUN2"}) {
...
if (microgridName != "MICROGRID Qair STORSUN1" && microgridName != "MICROGRID Qair STORSUN2") {
	forall (imb in isIMBALANCE_STEPS_POS)
		ctInCFDStepImbPower:
...
// (> 0 means purchase / < 0 means sale)
	forall (da in isDA_STEPS_POS)
		ctDaPosition:
			DaPosition[da] == InCFDDaPosition[da] + NotInCFDDaPosition[da];
...
//Define AbsDeltaNetElecImportTarget
	forall(t in isDECISION_STEPS)
	  ctAbsNetTarget1:
	  	AbsDeltaNetElecImportTarget[t] >= DeltaNetElecImportTarget[t];
	forall(t in isDECISION_STEPS)
	  ctAbsNetTarget2:
	  	AbsDeltaNetElecImportTarget[t] >= -DeltaNetElecImportTarget[t];
...
// Electricity generation from a disp generation asset should respect the power input by the portal customer.
	forall(d in isDISP_E_GENS, t in isDECISION_STEPS: dispGenForcedPower[d][t] != 1)
	  ctDispGenForcedPower:
	  	DispGenActivePower[d][t] == dispGenForcedPower[d][t] + DispGenForcedPowerViolationExcess[d][t] - DispGenForcedPowerViolationDeficit[d][t];	
...
// HARD-CODED
	...
	// For limited energy assets, we have to add a condition : forcing BESS participation in capacity
		forall (s in isSTORAGES inter isAFRR_UP_ASSETS, t in isDECISION_STEPS)
		  ctForceBESSaFRRUp:
		  	(aFRRUpReqPower[assetStepAfrrVoluntaryStep[t]] > 0 ?
		  		AFRRCapacityPowerUp[s][assetStepAfrrVoluntaryStep[t]] >= 1000 * (assetScheduledAvailability[s][t] >= 100 ? 1 : 0)
	...
	// For steps cleared, CHP can be on, only if the engagement is over a treshold, otherwise, it has to be off
     ...
     forall (d in isDISP_E_GENS inter {"CHP_1600kW"}, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) < card(isDECISION_STEPS) - 1 && isDAStepCleared[assetStepDAStep[t]]==1) {
	   ctNoCogeChange10:
	     ((dispGenAvail[d][t] == 1) && (mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) &&
	     (daEngagement[assetStepDAStep[t]] + long_term_engagement[assetStepDAStep[t]] <= -600))
	     ||
	     ...
	     (daEngagement[assetStepDAStep[nextc(isDECISION_STEPS, t, 1)]] + long_term_engagement[assetStepDAStep[nextc(isDECISION_STEPS, t, 1)]] <= -600)) ?
	         IsGenOn[d][t] >= 0
	        :IsGenOn[d][t] <= 0;	}
...
// Specific constraints for Vert
	if (microgridName == "MICROGRID ZEE Vert") {
		// In the case of Vert, PV generation is splitted in between PV produced below PMaxNomPDL and above PMaxNomPDL
		forall (i in isINTER_GENS, t in isDECISION_STEPS)
			ctPVProductionSplitted: InterGenActivePower[i][t] == InterGenActivePowerBelowPDL[i][t] + InterGenActivePowerOverPDL[i][t];
		
		// InterGenActivePowerOverPDL is only used if InterGenActivePowerBelowPDL is equal to PMaxNomPDL
		forall (i in isINTER_GENS, t in isDECISION_STEPS)
			ctIsInterGenOverPDLDef1: InterGenActivePowerOverPDL[i][t] <= interGenPowerForecastOverPDL[i][t] * IsInterGenOverPDL[i][t];
		
		forall (i in isINTER_GENS, t in isDECISION_STEPS)
			ctIsInterGenOverPDLDef2: InterGenActivePowerBelowPDL[i][t] >= -nomPowerMin[first(siteIdByAsset[i])] * IsInterGenOverPDL[i][t];
	}
...
// Specific constraints for VALECO
	if (card(isPPAOrOA_ASSETS) > 0) {
		...
		forall (i in isINTER_E_GENS, t in isDECISION_STEPS)
			ctPotentialNetInterGenPPA: InterGensPPANetActivePower[t] >= minl(sum(i in isINTER_E_GENS) interGenActivePowerForecast[i][t] - sum(f in isNF_E_LOADS) NFLoadActivePower[f][t], siteMaxAvailExportPower[first(siteIdByAsset[i])][t]);
	}
// Constraints for HPP in CFD or spot : for DA bid, we don't want to curtail PV based on price forecast, beacuse price is not certain, then we force to be at least equal to forecast or avail congestion - flex engagements
	if (microgridName in {"MICROGRID GDS Leforest B", "MICROGRID VALOREM LIMOUX", "MICROGRID ZEE EDR1", "MICROGRID ZEE Vert"} && optContext == "MARKET_STRAT_DA") {
		forall (si in isSITES , t in isDECISION_STEPS : isDAStepCleared[assetStepDAStep[t]] == 0)  
			ctNoCurtailHPPDAbid: 
				sum(i in isINTER_E_GENS : siteID[i] == si) InterGenActivePower[i][t] >= 
										maxl(0, 
											minl(sum(i in isINTER_E_GENS : siteID[i] == si) interGenActivePowerForecast[i][t] - sum(nf in isNF_E_LOADS : siteID[nf] == si) NFLoadActivePower[nf][t], 
												siteMaxAvailExportPower[si][t] - sum(fcr_a in isFCR_ASSETS: si == siteID[fcr_a]) FCRPower[fcr_a][assetStepFCRStep[t]] - sum(afrr_a in isAFRR_ASSETS: si == siteID[afrr_a]) afrrMaxAssetBandwith[afrr_a] * IsAFRRUp[afrr_a][assetStepAfrrVoluntaryStep[t]])
											);
	}
...
if (microgridName in {"MICROGRID GDS Leforest A", "MICROGRID GDS Leforest B"}) {
		forall (i in isINTER_E_GENS, t in isDECISION_STEPS : isDAStepCleared[assetStepDAStep[t]]==1 && interGenForcedPower[i][t] == 1) // no forced power
		  ctNoInterGenGDS: InterGenActivePower[i][t] <= (daElecPrice[assetStepDAStep[t]] >= 0.0 ? -nomPowerMin[i] : 0.0);
 	}
// NEXHOS	  
if (microgridName in {"MICROGRID NEXHOS Montbert", "MICROGRID NEXHOS Colisee"}) {
		forall (i in isINTER_E_GENS, t in isDECISION_STEPS : isDAStepCleared[assetStepDAStep[t]]==1 && interGenForcedPower[i][t] == 1) // no forced power
		  ctNoInterGenNexhos: InterGenActivePower[i][t] <= (daElecPrice[assetStepDAStep[t]] >= 0.0 ? -nomPowerMin[i] : 0.0);
	}
// ZEE GiÃ¨vre and Mennetou : all PV is in CFD
if (microgridName in {"MICROGRID ZEE EDR1"}) {
		forall (i in isINTER_E_GENS, t in isDECISION_STEPS : isDAStepCleared[assetStepDAStep[t]]==1 && interGenForcedPower[i][t] == 1) // no forced power
		  ctNoInterGenZEE1: InterGenActivePower[i][t] <= (daElecPrice[assetStepDAStep[t]] >= 0.0 ? -nomPowerMin[i] : 0.0);
	}
// ZEE Vert : curtail only if net position is equal to 0
if (microgridName in {"MICROGRID ZEE Vert"}) {
		forall (i in isINTER_E_GENS , t in isDECISION_STEPS : isDAStepCleared[assetStepDAStep[t]]==1 && interGenForcedPower[i][t] == 1) // no forced power
		  ctNoInterGenZEEVert1: InterGenActivePower[i][t] <= 
		  (abs(daEngagement[assetStepDAStep[t]] + long_term_engagement[assetStepDAStep[t]]) <epsilon || abs(availGlobalForPmin[i][t])<epsilon ?  0.0 : -nomPowerMin[i]);
	}
...
// Hard-coded for Qair
if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2") {
	// PPA engagement must be constant on certain steps
	// PPA steps when engagements are same as before 
	// first step always cleared, no need to deal with
 	forall (ppa in isPPA_STEPS : ( ppa != first(isPPA_STEPS) && isPPAStepCleared[ppa] != 1 && ppa in isPPAEngagementStepSameAsBefore))
		ctPPAEngagementConstant:
		-PPAPosition[ppa] == -PPAPosition[prev(isPPA_STEPS, ppa)];

	//Uncleared steps : ramps
	// Ramp up from 0:00am to 1:00am and 9:00am to 10:00am, must be linear
	forall (ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] in {0,9} && isPPAStepCleared[ppa] != 1)
	  // If the ramp is right at the end of the optimization window, the engagement of the day before is taken as a reference
	  ctPPAEngagementRampUp0amdAnd9am:
	  	 -PPAPosition[ppa] == -PPAPosition[item(isPPA_STEPS, (ord(isPPA_STEPS, ppa) + 4 < card(isPPA_STEPS) ?// If 09:00am->09:45am is in the optimization window
	  														ord(isPPA_STEPS, ppa) + 4							// 09:00am->09:45am the same day
											  				: ord(isPPA_STEPS, ppa) - 20 * 4 ))					// 01:00pm->01:45pm the day before
										] * 
                                            (ppaStepRealMinuteStep[ppa] == "0" ? (1/8)
                                           	:(ppaStepRealMinuteStep[ppa] == "15" ? (1/4 + 1/8)
                                           		:(ppaStepRealMinuteStep[ppa] == "30" ? (2/4 + 1/8)
	...
	// Engagement ramp down from 4:00am to 5:00am to 9:00pm to 10:00pm, must be linear
	// Only useful for the optim of 7:45am, afterwards during the day we want to keep the value
    forall (ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] in {4,21} && isPPAStepCleared[ppa] != 1)
      ctPPARampDownEngagement5amAnd9pm:
       -PPAPosition[ppa] == -PPAPosition[item(isPPA_STEPS, (ord(isPPA_STEPS, ppa) - 4 >= 0 ?
                                                         ord(isPPA_STEPS, ppa) -4
	  													: ord(isPPA_STEPS, ppa) + 20 * 4 ))					// 01:00pm->01:45pm the day before
										] * 
                                            (ppaStepRealMinuteStep[ppa] == "0" ? (3/4 + 1/8)
                                           :ppaStepRealMinuteStep[ppa] == "15" ? (2/4 + 1/8)
	...
	// Position must be larger or equal to minimum plateau value. If already cleared, not relevant since it is an input parameter.
		forall (ppa in isPPA_STEPS : ppaStepRealHourStep[ppa] >= 10 && ppaStepRealHourStep[ppa] <= 20 && isPPAStepCleared[ppa] != 1) 	
		  ctPPAEngagementMin:	
		  	-PPAPosition[ppa] >= minPlateauValue;
		  	
	// Position cannot be larger than max grid capacity
		forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 10 && assetStepRealHour[t] <= 20 && isPPAStepCleared[assetStepPPAStep[t]] != 1) 	
		  ctPPAEngagementMax:	
		  	-PPAPosition[assetStepPPAStep[t]] <= siteNomMaxExport[first(isSITES)] ;
	
	//PPA position must be in between max import from aux and zero outside from the previous hours
		forall (ppa in isPPA_STEPS : (ppaStepRealHourStep[ppa] >= 5 && ppaStepRealHourStep[ppa] <= 8) || (ppaStepRealHourStep[ppa] >= 22))	
		  ctPPAEngagement0:	
		  	-maxImportFromAux <= -PPAPosition[ppa] <= 0 ;
		  	
	// At any time, ramps must be respected
	//initial Power is in production convention
	// Ramp up
	//1st step
	    ctRampUpInit: -NetElecImportTarget[first(isDECISION_STEPS)]
		  	 - (initialPower[first(isSITES)]) <= 
				StepRampLimitation[first(isDECISION_STEPS)];
	//Other steps
	  forall (t in isDECISION_STEPS : t != first(isDECISION_STEPS))
	    ctRampUp: -NetElecImportTarget[t]
		  	 - (-NetElecImportTarget[prev(isDECISION_STEPS,t)]) <= 
				StepRampLimitation[t];
	// Ramp down
	//1st step
	    ctRampDwnInit: initialPower[first(isSITES)]
		  	 - (-NetElecImportTarget[first(isDECISION_STEPS)]) <= 
				StepRampLimitation[first(isDECISION_STEPS)];
	//Other steps
	  forall (t in isDECISION_STEPS : t != first(isDECISION_STEPS))
	    ctRampDwn: -NetElecImportTarget[prev(isDECISION_STEPS,t)]
		  	 - (-NetElecImportTarget[t]) <= 
				StepRampLimitation[t];
			
	// BESS aux consumption
	forall (f in isFLEX_E_LOADS, t in isDECISION_STEPS)
 	  ctParametricConsumption:
 	  	FlexLoadActivePower[f][t] == auxStorConsoCoeff * (StorACPowerDischarge[first(isE_STORAGES)][t] + StorACPowerCharge[first(isE_STORAGES)][t]) + auxStorConsoBase;

	//Physical constraints
	
	//Follow profile
	// Import: For all day long, except from midnight to 5am, import is limited to auxiliairies
	forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 5)	
	  ctPPAImport:	
		NetElecImportTarget[t] <= FlexLoadActivePower[first(isFLEX_E_LOADS)][t];

	// No right to export to the grid except in between 9am and 10pm
	forall (t in isDECISION_STEPS: assetStepRealHour[t] < 9 || assetStepRealHour[t] >= 22)
	  ctPPAExport:
	  	-NetElecImportTarget[t] <= 0;
                                    
	// Engagements linked to average physical exchanged, no tolerance: tolerance onlu during plateau
	// It should be enough for the site to follow engagements also during ramps
	forall (t in isDECISION_STEPS : assetStepRealHour[t]<=9 || assetStepRealHour[t]>=21)
	  ctPPAImbalanceNoTolerance:  	 	  	
	  	-PPAPosition[assetStepPPAStep[t]] + ImbalancePower_imb[assetStepImbalanceStep[t]] == -AvgNetElecImportTarget[t];

	// During the plateau, some tolerance on imbalance is allowed
	forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 10 && assetStepRealHour[t] <= 20)
	  ctPPAImbalanceToleranceDwn:  	 	  	
	  	(1-toleranceDwnPercentImb/100)*(-PPAPosition[assetStepPPAStep[t]]) - NegativeImbalancePower_imb[assetStepImbalanceStep[t]] <= -AvgNetElecImportTarget[t];
	
	forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 10 && assetStepRealHour[t] <= 20)	
	  ctPPAImbalanceToleranceUp:	
	  	-AvgNetElecImportTarget[t] 
	  	<= (1+toleranceUpPercentImb/100)*-PPAPosition[assetStepPPAStep[t]] + PositiveImbalancePower_imb[assetStepImbalanceStep[t]];
	
	// No imbalance greater than auxiliaries consumption active power out of allowed periods
	forall (t in isDECISION_STEPS : (assetStepRealHour[t] >= 5 && assetStepRealHour[t] <= 8) || (assetStepRealHour[t] >= 22)) 
	  ctPPAImbalance0: 	
	  	PositiveImbalancePower_imb[assetStepImbalanceStep[t]] + NegativeImbalancePower_imb[assetStepImbalanceStep[t]] <= FlexLoadActivePower[first(isFLEX_E_LOADS)][t];
...
if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2"){
	 forall (f in isFLEX_E_LOADS, t in isDECISION_STEPS)
 	  ctQairFlexLoadConstraints: ModulationTarget[f][t] >= 0.0;
} else {
 	forall (f in isFLEX_E_LOADS, t in isDECISION_STEPS)
...
//{t_empty} STEP_COSTS_HEADER = {<"step_id", "imbalance_Elec_up_price", "imbalance_Elec_down_price", "imbalance_Elec_up_power", "imbalance_Elec_down_power", "imbalance_gas_up", "imbalance_gas_down", "electricity_cost", "gas_cost", "transport_taxes", "objective_step_cost", "", "", "", "", "", "", "", "", ""> | i in 1..1};
///* OPERATION DATA */
...
{t_operation_output} OPERATION_OUTPUT = {<"operation_id", operationID>	 									// variable holding OPERATION output data
										...
										, <"ppa_total_trade_cost", strPPATotalTradeCost>					// PPATotalTradeCost
										};
...
// ppa nomination : recompute position calculated in the optim to fit expected position nomination behaviour
float ppaNomination[isPPA_STEPS];
...
tuple t_market_bids_output {
 ...
 // We round 
float aFRRUpReqPower_h_round[afrr_c in isAFRR_CAPACITY_STEPS_POS] = 1000 * floor(min (afrr_v in isAFRR_VOLUNTARY_STEPS_POS : voluntaryAfrrStepCapacityStep[afrr_v]==afrr_c)aFRRUpReqPower[afrr_v]/1000);
float aFRRDwnReqPower_h_round[afrr_c in isAFRR_CAPACITY_STEPS_POS] = 1000 * floor (min (afrr_v in isAFRR_VOLUNTARY_STEPS_POS : voluntaryAfrrStepCapacityStep[afrr_v]==afrr_c)aFRRDwnReqPower[afrr_v]/1000);
float avgPossibleStoredDCEnergyAvailForAFRRCase2;
float p0RequiredCase2;
float P0_RB[isAFRR_CAPACITY_STEPS_POS]; // P0_RB is the P0 position by rule based for fall-back computing
...
////													-minl(sum(si in isSITES : si != "ZEE_Saint_Sauveur")(min(t in isDECISION_STEPS : assetStepDAStep[t] == h) maxOutput[si][t]), 
////													sum(i in isINTER_E_GENS, t in isDECISION_STEPS : assetStepDAStep[t] == h) minl(interGenActivePowerForecast[i][t], maxl(0,maxOutput[si][t] - sum(s in isE_STORAGES :  first(siteIdByAsset[s]) == si) afrrMaxAssetBandwith[s]))/4)
//													-sum(si in isSITES : si != "ZEE_Saint_Sauveur") (
//													    sum(i in isINTER_E_GENS : first(siteIdByAsset[i]) == si, t in isDECISION_STEPS : assetStepDAStep[t] == h)
//													        minl(
//													            interGenActivePowerForecast[i][t],
//													            maxl(0, maxOutput[si][t] - sum(s in isE_STORAGES : first(siteIdByAsset[s]) == si) afrrMaxAssetBandwith[s])
//													        ) / 4
//													    )
//													:DaPosition[h])
													DaPosition[h]  , (DaPosition[h] <= 0.0 ? sellOfferPrice : buyOfferPrice[h] )>| h in isDA_STEPS_POS} union
											(optContext != "OPERATIONS" ?
											{<da, "DAY_AHEAD_CFD", (InCFDDaPosition[da] <= 0 ? "SELL" : "BUY"), daStepDuration, InCFDDaPosition[da], (InCFDDaPosition[da] <= 0 ? sellOfferPrice : daElecPrice[da])>| da in isDA_STEPS_POS}:
											{}
											)
											union ({<ppa, "PPA_POSITION", (PPAPosition[ppa] <= 0 ? "SELL" : "BUY"), ppaStepDuration, PPAPosition[ppa], 0.0> | ppa in isPPA_STEPS_POS}) 
											union (optContext == "MARKET_STRAT_AFRR" ?
											 {<afrr_c, "AFRR_CAPACITY_UP", "SELL", afrrCapacityStepDuration, aFRRUpReqPower_h_round[afrr_c], 0.0> | afrr_c in isAFRR_CAPACITY_STEPS_POS}
											union {<afrr_c, "AFRR_CAPACITY_DOWN", "SELL", afrrCapacityStepDuration, aFRRDwnReqPower_h_round[afrr_c], 0.0> | afrr_c in isAFRR_CAPACITY_STEPS_POS}
											: {}) 
...
int nMarketBids = card(isDA_STEPS_POS) 
                + (optContext != "OPERATIONS" ? card(isDA_STEPS_POS) : 0) 
                + card(isPPA_STEPS_POS)
...
{t_asset_step_cost} ASSET_STEPS_COST = {<a, s, t, startupCostPaidForAsset[a][s][t], assestVarCost[a][s][t], assetCurtailmentCost[a][s][t]>| a in isASSETS diff isSITES, s in siteIdByAsset[a]: s!="dummy_site", t in isDECISION_STEPS}; // variable holding ASSET_STEPS_COST output data
...
string stepCostsExcelRange = "'STEP_COSTS'!A2:K";
...
transportTaxes[t], ObjStepCost[t]>| a in isASSETS diff isSITES, t in isDECISION_STEPS}; // variable holding ASSET_STEPS_COST output data
...
execute {
	...
	strInCFDDaTotalTradCost =  -InCFDDaTotalTradeRevenues;
	strNotInCFDDaTotalTradCost = NotInCFDDaTotalTradeCost;
	...
	strPPATotalTradeCost = PPATotalTradeCost;
 	...
 	for (var t in isDECISION_STEPS) {
		...
		electricityCost[t] = (electricityPrice[t] * NetElecImportTarget[t] + DaPosition[assetStepDAStep[t]] * daElecPrice[assetStepDAStep[t]])*assetStepDurationInHours;
		...
		else
		{
 		...
 		for	(var i in isINTER_E_GENS) {
 				...
 				if (microgridName == "MICROGRID GDS Leforest A" || microgridName == "MICROGRID GDS Leforest B"){
					assetCurtailmentTarget[i][t] = -2.0;
					if (daElecPriceRaw[assetStepDAStep[t]] < 0.0 || Math.abs(availGlobalForPmin[i][t])<epsilon){
						//assetPowerTarget[i][t] = 1.0;								// old version :  this is to indicate to the PMS that PV generation must be curtailed to 0;
						assetCurtailmentTarget[i][t] = 0.0;									// new convention
					}
					if (interGenForcedPower[i][t] != 1){								// If a forced power has been implemented
					 	assetCurtailmentTarget[i][t] = -3.0;							// this is to indicate to the PMS that PV generation must follow the power target as mush as possible
						assetPowerTarget[i][t] = -InterGenActivePower[i][t];			// respects Everest's sign convention (-ve values = power generation)
					}
				}
 				if (microgridName == "MICROGRID NEXHOS Montbert" || microgridName == "MICROGRID NEXHOS Colisee" || microgridName == "MICROGRID ZEE EDR1") {
				...
				if (interGenForcedPower[i][t] != 1){								// If a forced power has been implemented
     			...
     			if (microgridName == "MICROGRID ZEE Vert") {
 					assetCurtailmentTarget[i][t] = -2.0;								// this is to indicate to the PMS that PV generation should not be curtailed 					
 					if (Math.abs(DaPosition[assetStepDAStep[t]] + long_term_engagement[assetStepDAStep[t]]) <epsilon || Math.abs(availGlobalForPmin[i][t])<epsilon){
 						assetCurtailmentTarget[i][t] = 0.0;								// this is to indicate to the PMS that PV generation must be curtailed to 0
     			}
				if (interGenForcedPower[i][t] != 1){								// If a forced power has been implemented
					 	assetCurtailmentTarget[i][t] = -3.0;							// this is to indicate to the PMS that PV generation must follow the power target as mush as possible
 				...
 				if (microgridName == "Equans BIC microgrid"
 					&& (daElecPriceRaw[assetStepDAStep[t]] < aggregation_fee || interGenForcedPower[i][t]==0)
 					&& i == "BIC_PV1")
 					assetPowerTarget[i][t] = 1.0;
				if (microgridName == "Equans BIC microgrid"
 					&& (daElecPriceRaw[assetStepHourlyStep[t]] < - networkDrawingTax[t] + aggregation_fee || interGenForcedPower[i][t]==0)
  		        ...
  		        if (microgridName == "MICROGRID VALOREM LIMOUX") {
 					assetCurtailmentTarget[i][t] = -2.0;								// this is to indicate to the PMS that PV generation should not be curtailed
 					if (daElecPriceRaw[assetStepDAStep[t]] < -networkDrawingTax[t] + sellOfferPrice
 						&& InterGenActivePower[i][t] <= 0.0)
 						assetCurtailmentTarget[i][t] = 0.0;							// this is to indicate to the PMS that PV generation must be curtailed to 0
 					if (daElecPriceRaw[assetStepDAStep[t]] >= -networkDrawingTax[t] + sellOfferPrice
 						&& daElecPriceRaw[assetStepDAStep[t]] < sellOfferPrice
					...
					if (interGenForcedPower[i][t] != 1){								// If a forced power has been implemented, stronger than the rest
						 	assetCurtailmentTarget[i][t] = -3.0;						// this is to indicate to the PMS that PV generation must follow the power target as mush as possible
							assetPowerTarget[i][t] = -InterGenActivePower[i][t];		// respects Everest's sign convention (-ve values = power generation)
						} 
     			}
...
for(var s in isSITES)
 		...
 		for	(var s in isE_STORAGES)
 		{
 			...
 			if (isDAStepCleared[assetStepDAStep[t]] == 0) {
...
//  		} 			
	...
	// forced Power violation
	var anyForcedPowerViolation = false;
	...
	for (var s in isE_STORAGES)
		for (var t in isDECISION_STEPS) {
			...
			var excessOrDeficit;
 			if(StorageAssetChargeForcedPowerViolationDeficit[s][t] > epsilon) {
				excessOrDeficit = storageForcedPower[s][t] > 0 ? "deficit" : "excess";
			    if(!anyForcedPowerViolation) {
			      	strDispatchStatusString += ", some forced power violation ";
			    	anyForcedPowerExcess = true;
			    }
				VIOLATIONS_OUTPUT.add("elec_storage_forced_active_power_" + excessOrDeficit, s, t, StorageAssetChargeForcedPowerViolationDeficit[s][t], StorageAssetForcedPowerViolationCost[s][t], 100);
			   }
			
			   if(StorageAssetChargeForcedPowerViolationExcess[s][t] > epsilon){
				  excessOrDeficit = storageForcedPower[s][t] > 0 ? "excess" : "deficit";
			      if(!anyForcedPowerViolation) {
			      	strDispatchStatusString += ", some forced power violation ";
			    	anyForcedPowerExcess = true;
			    }
    				VIOLATIONS_OUTPUT.add("elec_storage_forced_active_power_" + excessOrDeficit, s, t, StorageAssetChargeForcedPowerViolationExcess[s][t], StorageAssetForcedPowerViolationCost[s][t], 100);
			}
		}
	...
	//Check if any dispatchable gen forced active power constraint violation
	for (var d in isDISP_E_GENS)
		for (var t in isDECISION_STEPS) {
		if(DispGenForcedPowerViolationExcess[d][t] > epsilon){
				if (!anyForcedPowerViolation) {
					strDispatchStatusString += ", some minimum power ";
					anyForcedPowerViolation = true;
 			}
 			VIOLATIONS_OUTPUT.add("disp_gen_forced_active_power_excess", d, t, DispGenForcedPowerViolationExcess[d][t], DispGenForcedPowerViolationCost[d][t], 100);
			}

		if(DispGenForcedPowerViolationDeficit[d][t] > epsilon){
				if (!anyForcedPowerViolation) {
					strDispatchStatusString += ", some minimum power ";
					anyForcedPowerViolation = true;
 			}
 			VIOLATIONS_OUTPUT.add("disp_gen_forced_active_power_deficit", d, t, DispGenForcedPowerViolationDeficit[d][t], DispGenForcedPowerViolationCost[d][t], 100);
			}
		}
	//Check if any dispatchable gen initial active power constraint up violation
	anyDeficit = false;
	for (var d in isDISP_E_GENS)
			if(DispGenInitialPowerUpViolation[d] > epsilon){
				if (!anyDeficit) {
					strDispatchStatusString += ", some initial power up violation";
					anyDeficit = true;
 			}
 			VIOLATIONS_OUTPUT.add("disp_gen_initial_power_up_violation", d, t, DispGenInitialPowerUpViolation[d], DispGenInitialPowerUpViolationCost[d], 100);
			}
			if(DispGenInitialPowerDwnViolation[d] > epsilon){
			  if (!anyDeficit) {
					strDispatchStatusString += ", some initial power Down violation";
					anyDeficit = true;
 			}
 			VIOLATIONS_OUTPUT.add("disp_gen_initial_power_down_violation", d, t, DispGenInitialPowerDwnViolation[d], DispGenInitialPowerDwnViolationCost[d], 100);
		}
	...
	//Check if any interGen gen forced active power constraint violation
	if(InterGenForcedPowerViolation[i][t] > epsilon){
		if (!anyForcedPowerViolation) {
			strDispatchStatusString += ", some forced power excess ";
			anyForcedPowerViolation = true;
		}
	VIOLATIONS_OUTPUT.add("inter_gen_forced_active_power_excess", i, t, InterGenForcedPowerViolation[i][t], Opl.intValue(""), 100);
	}	

	if(InterGenForcedPowerViolation[i][t] < - epsilon){
		if (!anyForcedPowerViolation) {
			strDispatchStatusString += ", some forced power deficit";
			anyForcedPowerViolation = true;
		}
	VIOLATIONS_OUTPUT.add("inter_gen_forced_active_power_deficit", i, t, InterGenForcedPowerViolation[i][t], Opl.intValue(""), 100);
			}
 		}		
	...
	// Qair hard-coded post treatment, to correct PPAPosition and set it 'end of target'
	if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2"){
		for (var ppa in isPPA_STEPS){	
	
                    if (ppaStepRealHourStep[ppa] == 0 || ppaStepRealHourStep[ppa] == 9){ //Ramp up ?
                        ppaNomination[ppa] = PPAPosition[Opl.item(isPPA_STEPS, (Opl.ord(isPPA_STEPS, ppa) + 4 < Opl.card(isPPA_STEPS) ?// If 09:00am->09:45am is in the optimization window
	  														Opl.ord(isPPA_STEPS, ppa) + 4							// 09:00am->09:45am the same day
											  				: Opl.ord(isPPA_STEPS, ppa) - 20 * 4 ))					// 01:00pm->01:45pm the day before
										] * 
                          (ppaStepRealMinuteStep[ppa] == "0" ? (1/4)
                          	:(ppaStepRealMinuteStep[ppa] == "15" ? (2/4)
                          		:(ppaStepRealMinuteStep[ppa] == "30" ? (3/4)
                          :(4/4))))
                          }
                   else if (ppaStepRealHourStep[ppa] == 4 || ppaStepRealHourStep[ppa] == 21){ //Ramp down ?
                      ppaNomination[ppa] = PPAPosition[Opl.item(isPPA_STEPS, (Opl.ord(isPPA_STEPS, ppa) - 4 >= 0 ?
                                                             Opl.ord(isPPA_STEPS, ppa) -4
    	  													: Opl.ord(isPPA_STEPS, ppa) + 20 * 4 ))					// 01:00pm->01:45pm the day before
    										] *
                          (ppaStepRealMinuteStep[ppa] == "0" ? (3/4)
                          	:(ppaStepRealMinuteStep[ppa] == "15" ? (2/4)
                          		:(ppaStepRealMinuteStep[ppa] == "30" ? (1/4)
                          			:(0/4))));
                        }
                   else{
                   	ppaNomination[ppa] = PPAPosition[ppa]; //Other hours, we directly take the engagement                  
                   }
	...
	writeln(StepRampLimitation);
	writeln(AvgNetElecImportTarget);
