/*********************************************
* Microgrid Optimisation Model v3.11
 * Author: j.leduc
...
int NFElecLoadChangedForecast[n in isNF_E_LOADS][t in isDECISION_STEPS] = (NFElecLoadForecast[n][t] != powerPrediction[n][t]*NFLoadAvail[n][t] / 100.0  ? 1 : 0);
...
int flexLoadChangedForecast[f in isFLEX_E_LOADS][t in isDECISION_STEPS] = (flexLoadForecast[f][t] != flexLoadAvail[f][t]*powerPrediction[f][t] ? 1 : 0);
	...
	(
		microgridName == "MICROGRID ENERCAL Ile des Pins" || microgridName == "MICROGRID ENERCAL Mare"
			...
			: (
				...
				? 0.01 * (optimisationStepNumber - stepIndex[t]) / optimisationStepNumber
				: (
					microgridName == "MICROGRID TPL Tongatapu"
...
// Hard-coded for now
float cyclingVariableCostFCR[s in isE_STORAGES] = (microgridName == "MICROGRID VALOREM LIMOUXX" ? // For Valorem Limoux, we've got a price per MW engaged per fcr time slot: we divide per 1000 and 16 to go back to �/kWh per asset Step, and divide again by energy per asset Step per kW FCR
													((3.09/1000)*(assetStepDuration/fcrStepDuration))/(nomEnergyMax[s]*((100 / storefficiencyfcr) + 1) * (FCRUnitarianStepACEnergyOut * 100 / inverterefficiencyfcrout) / (2 * nomEnergyMax[s])) 	
													:0) ; 
//////
//fCFD fo GDS
//////
...
dexpr float NFLoadActivePower[n in isNF_E_LOADS][t in isDECISION_STEPS] = maxl(minl(maxNFElecLoad[n][t], NFElecLoadForecast[n][t]), minNFElecLoad[n][t]);
...
dexpr float GenTotalLinearVarCosts =
    ...
    + sum(t in isDECISION_STEPS, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s] + StorStepFCRCycle[s][t] * cyclingVariableCostFCR[s]) * nomEnergyMax[s] ;
...
dexpr float OptimisedGenLinearVarCosts =
    ...
    + sum(t in isDECISION_STEPS: isDAStepCleared[assetStepHourlyStep[t]] == 0, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s] + StorStepFCRCycle[s][t] * cyclingVariableCostFCR[s]) * nomEnergyMax[s];
 ...
 *********************************************************************/
  subject to {
	/*forall (fcr in isFCR_STEPS, s in isSTORAGES)
		ctTest1: FCRPower[s][fcr] == 1000;*/
//	forall (t in isDECISION_STEPS: 46 <= ord(isDECISION_STEPS, t) <=68)
//		ctTest3: StorACActivePower["MORB_ENERGIES_Flex_batterie"][t] == 0;
//	ctTest2: StorACActivePower["VALOREM_Limoux_BESS"]["2"] <= 0.0;
...
//	forall (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= 3)
//		ctTest1: StorACPowerCharge["VALOREM_Limoux_BESS"][t] >= 1000;
	//forall (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) >= 5)
		//ctTest2: StorACPowerDischarge["VALOREM_Limoux_BESS"][t] >= 902.5;
...
forall (s in isE_STORAGES, t in isDECISION_STEPS: 46 <= ord(isDECISION_STEPS, t) <=68)
		ctTest: StorStoredDCEnergy[s][t] == SOCMax[s]*nomPowerMax[s];
...
// We will have to deal with energy availability, but for now we cannot multiply by availability, otherwise we will loose the information
	forall (s in isE_STORAGES, t in isDECISION_STEPS: t != first(isDECISION_STEPS))
	  ctStorageStoredEnergy: StorStoredDCEnergy[s][t] == StorStoredDCEnergy[s][prev(isDECISION_STEPS, t)] * scheduledAvailability[s][t] + StorStepDCEnergyIn[s][t];
...
{t_asset_steps_output} ASSET_STEPS_OUTPUT = {<a, t, assetPowerTarget[a][t], assetEnergyType[a][t], assetStorageTarget[a][t] , assetTempTarget[a][t], assetCurtailmentTarget[a][t], assetSOCTarget[a][t]> | a in isASSETS union {"MAINGRID"}, t in isDECISION_STEPS}; // variable holding ASSET x STEPS output data
	...
	for (var s in isH_STORAGES)
		for (var t in isDECISION_STEPS) {
			...
			if (HeatSOCminDeficit[s][t] > epsilon) {
				...
				VIOLATIONS_OUTPUT.add("minimum_heat_SOC_deficit", s, t, HeatSOCminDeficit[s][t], HeatSOCminDeficitCost[s][t], 1);
			}
			// max SOC excess
			if (HeatSOCmaxExcess[s][t] > epsilon) {
				...
				VIOLATIONS_OUTPUT.add("maximum_heat_SOC_excess", s, t, HeatSOCmaxExcess[s][t], HeatSOCmaxExcessCost[s][t], 1);
			}
		}
