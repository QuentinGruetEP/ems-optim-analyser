/***************************************************
 * Microgrid Optimisation Model v2.9 w. TPL Tongatapu
 * Author: n.bergevin
...
// hard-coded parameters in this model for Enercal's Ile des Pins and Marï¿½ microgrids and TPS's Tongatapu microgrid
//		- SOC strict minimum for any storage unit set to 5% (see strictMinSOC param)

// hard-coded parameters in this model for Enercal's Ile des Pins and Marï¿½ microgrids
...
// hard-coded parameters in this model for Enercal's Marï¿½ microgrid
...
//			-> impacts computation of linear and constant terms of linear approximation of assets min and max reactive powers see (aQmax and bQmax param)
//			-> impacts computation of current injection potential for each storage unit (see storCurrentInjection param)
// 		- artificial penality to encourage early battery discharges set to 0.02 * step_index[t] / step_number (see storArtificialPenality param and StorChargeDischargeCost var)
...
float NFLoadSpinLowerReserveReq[isNF_LOADS] = [(microgridName == "MICROGRID TPL Tongatapu" ? 40.0 : 0.0)];
...
float storArtificialPenality[t in isDECISION_STEPS] =
	(
		microgridName == "MICROGRID ENERCAL Ile des Pins" || microgridName == "MICROGRID ENERCAL Mare" 
			? stepIndex[t] / optimisationStepNumber * 10.0	// for IDP and Marï¿½, we want to encourage early discharge (see StorChargeDischargeCost var)
			: (
				...
				: (
					microgridName == "MICROGRID TPL Tongatapu"
					? stepIndex[t] / optimisationStepNumber / 50.0	// for Tongatapu we want to encourage early discharge (see StorChargeDischargeCost var)
					: 0.0
				  )
			   )
	); 
...
// average power generation target excess (expressed in kWh) over intermittent production asset iï¿½s potential forecast for decision step t
// (i in INTER_PRODS, t in DECISION_STEPS)
...
dexpr float StorChargeDischargeCost =
 	...
 	* (
 		microgridName == "MICROGRID MORBIHAN ENERGIES FlexMobIle" || microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid"	// for FlexMob'Ile and Kergrid, we want to encourage late charge (see storArtificialPenality param)
 			...
 			: StorACPowerDischarge[s][t]												// for IDP and Marï¿½, we want to encourage early discharge (see storArtificialPenality param) 
 	  )
 ...
 *********************************************************************/
  subject to {
//	forall (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= 27)
//	  ctTest: sum(g in isTHERMAL_GENS) IsGenOn[g][t] <= 0; 
...
// Raise reserve should cover 30% of total non-flexibile load for each decision step
	forall (t in isDECISION_STEPS)
	  ctSpinningRaiseReserveReq:
		...
		>= sum (n in isNF_LOADS) NFLoadSpinRaiseReserveReq[n] / 100 * NFLoadActivePower[n][t];
...
// Lower reserve should cover 30% of total non-flexibile load for each decision step
	forall (t in isDECISION_STEPS)
	  ctSpinningLowerReserveReq:
		...
		>= sum (n in isNF_LOADS) NFLoadSpinLowerReserveReq[n] / 100 * NFLoadActivePower[n][t];
