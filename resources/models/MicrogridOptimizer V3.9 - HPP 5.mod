/*********************************************
* Microgrid Optimisation Model v3.9
 * Author: n.bergevin
...
//		- intermittent generation is curtailed down to zero when day-ahead elec prices are < suppl fee - network fee (see ctNoInterGenBIC constraint)
//		- intermittent generators' active power targets set to -1kW in ASSET_STEPS_OUTPUT table when day-ahead elec prices are < suppl fee - network fee (see assetPowerTarget output param for intermittent generators)

// hard-coded parameters in this model for NEXHOS Montbert et Colisee microgrids
...
//		- DA sell offer price set to 0.0 / 1000.0 currency unit / kWh (see sellOfferPrice param)
//		- intermittent generator's active power curtailment targets set to
//			0kW in ASSET_STEPS_OUTPUT table when day-ahead elec prices are strict. negative (see assetCurtailmentTarget output param for intermittent generators)
//			-2kW in ASSET_STEPS_OUTPUT table when day-ahead elec prices are zero or positive (see assetCurtailmentTarget output param for intermittent generators)
...
// HARD-CODED
float networkDrawingTax[t in isDECISION_STEPS] = (microgridName == "Microgrid Srisangtham Microgrid") ? 3.0 :
												 (microgridName == "VidoFleur Scheduled Assets") ? 0.045 :
...
int nomBlockNbr[isASSETS] = [a.asset_id : a.total_block_number | a in ASSETS];
// If mapped, measured number of healthy blocks for asset a. If not mapped, it should be nominal number, or -1.
int healthyBlockNbr[isASSETS] = [a.asset_id : a.healthy_block_number  | a in ASSETS];
...
// No unit, in between 0 and 1, except for default value which is -1.0
float scheduledAvailability[isASSETS][isDECISION_STEPS] = [as.asset_id : [as.step_id : as.availability] | as in ASSET_STEPS];
...
float maxInput[i in isSITES][t in isDECISION_STEPS] =
	(microgridName == "MICROGRID VALOREM LIMOUX" && card(isASSETS inter {"VALOREM_Limoux_PDL_in"}) > 0) ? maxNFElecLoad["VALOREM_Limoux_PDL_in"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_in"}) > 0) ? maxNFElecLoad["GENERALE_DU_SOLAIRE_Leforest_B_PDL_in"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in"}) > 0) ? maxNFElecLoad["GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in"][t] :
	powerMax[i];
float maxOutput[i in isSITES][t in isDECISION_STEPS] =
	(microgridName == "MICROGRID VALOREM LIMOUX" && card(isASSETS inter {"VALOREM_Limoux_PDL_out"}) > 0) ? maxNFElecLoad["VALOREM_Limoux_PDL_out"][t] :
	(microgridName == "MICROGRID GDS Leforest A" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_A_PDL_out"}) > 0) ? maxNFElecLoad["GENERALE_DU_SOLAIRE_Leforest_A_PDL_out"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_out"}) > 0) ? maxNFElecLoad["GENERALE_DU_SOLAIRE_Leforest_B_PDL_out"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"}) > 0) ? maxNFElecLoad["GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"][t] :
	(microgridName == "MICROGRID NEXHOS Montbert" && card(isASSETS inter {"NEXHOS_MONTBERT_PDL_out"}) > 0) ? maxNFElecLoad["NEXHOS_MONTBERT_PDL_out"][t] :
	(microgridName == "MICROGRID NEXHOS Colisee" && card(isASSETS inter {"NEXHOS_COLISEE_PDL_out"}) > 0) ? maxNFElecLoad["NEXHOS_COLISEE_PDL_out"][t] :
	-powerMin[i];
...
int NFElecLoadChangedForecast[n in isNF_E_LOADS][t in isDECISION_STEPS] = (NFElecLoadForecast[n][t] != powerPrediction[n][t] ? 1 : 0);
...
float flexLoadForecast[f in isFLEX_E_LOADS][t in isDECISION_STEPS] = maxl(minl(maxFlexLoad[f], powerPrediction[f][t]), minFlexLoad[f]);
// A flag that signals the changing on the flexLoad forecasts values
int flexLoadChangedForecast[f in isFLEX_E_LOADS][t in isDECISION_STEPS] = (flexLoadForecast[f][t] != powerPrediction[f][t] ? 1 : 0);
...
int interGenChangedForecast[i in isINTER_E_GENS][t in isDECISION_STEPS] = (interGenActivePowerForecast[i][t] != -powerPrediction[i][t] ? 1 : 0);
...
float sellOfferPrice = 	  (microgridName == "MICROGRID VALOREM LIMOUX") ? 1.0 / 1000.0	// Valorem pays EP 0.4 �/MWh so we decided that it was not profitable for them to generate for less than 1 �/MWh
...
float storElecChargeCstEff[s in isE_STORAGES] = (microgridName == "MICROGRID MORBIHAN ENERGIES FlexMobIle"  ? -1 : assetChargeEfficiency[s]*100);
float storElecDischCstEff[s in isE_STORAGES] = (microgridName == "MICROGRID MORBIHAN ENERGIES FlexMobIle"  ? -1 : assetDischargeEfficiency[s]*100);
...
float storeOpMaxPower[o in isSTORE_OPERATION_TYPES] [s in isE_STORAGES] = (operationType[o] == "charge" ? nomPowerMax[s] : -nomPowerMin[s]);
// charge/discharge efficiency, as coefficient, according to the operation type : charge / discharge
float storeOpEff[o in isSTORE_OPERATION_TYPES] [s in isE_STORAGES] = (operationType[o] == "charge" ?  (storElecChargeCstEff[s] > 0.0 ? 100/storElecChargeCstEff[s] : 1) : storElecDischCstEff[s]/100);
...
microgridName == "MICROGRID MORBIHAN ENERGIES FlexMobIle"  // all factors and min/max power are DC side : we must keep the same logic as Everest. : in fact on Everest side we send all storage input data on the DC side
    ?
    {   
	<s, 1,"charge", 0, 2.011, 1/0.3129, 0.0> | s in isE_STORAGES } union 
    {<s, 2,"charge",2.011, nomPowerMax[s], 1/0.9482, 4.0838/0.9482> | s in isE_STORAGES} union 
	{<s, 1,"discharge", 0, 11.11, 1/1.4811867, 0.0> | s in isE_STORAGES} union 
	{<s, 2,"discharge", 11.11, -nomPowerMin[s], 1/1.0864, -2.9609/1.0864> | s in isE_STORAGES }
	:

    {<s, 1, o.op_id, 0, storeOpMaxPower[o.op_id][s], storeOpEff[o.op_id][s] ,0.0> | s in isE_STORAGES, o in STORE_OPERATION_TYPES}

);

{string} isSTOR_CHARGE_EFF_MODELS = {ve.model_id | ve in BATTERY_EFFICIENCY_MODELS: ve.type == "charge"};
// indexing set of non-linear storage discharge efficiency model
{string} isSTOR_DISCHARGE_EFF_MODELS = {ve.model_id | ve in BATTERY_EFFICIENCY_MODELS: ve.type == "discharge"};
// power segment index for each non-linear battery charge efficiency model (used in declarations of non-linear model efficiency  variables and constraints)
int chargeEffModelSegIndx[isSTOR_CHARGE_EFF_MODELS] = [ve.model_id : ve.power_interval_nbr | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="charge"];
// number of power segments (per model) for each non-linear stor charge efficiency model (used in declarations of non-linear model efficiency  variables and constraints)
int chargeStorSegNbr[isSTOR_CHARGE_EFF_MODELS] = [em: maxl(0, max(ve in BATTERY_EFFICIENCY_MODELS : ve.model_id == em ) chargeEffModelSegIndx[em])|  em  in isSTOR_CHARGE_EFF_MODELS];
// set containing the number of segments per model during the phase charge
int maxChargeSegNbr = max (em in isSTOR_CHARGE_EFF_MODELS) chargeStorSegNbr[em];
// upper limit (in kW) of power segment ve in efficiency model em
float chargeEffSegUpLim[isSTOR_CHARGE_EFF_MODELS][1..maxChargeSegNbr] = [ve.model_id : [ve.power_interval_nbr : ve.upper_limit] | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="charge"];
//  linear function slope associated with segment charge ve in efficiency model em
float storChargeSegSlope[isSTOR_CHARGE_EFF_MODELS][1..maxChargeSegNbr] = [ve.model_id : [ve.power_interval_nbr : ve.slope] | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="charge"];
//  linear function ordinate associated with segment charge ve in efficiency model em
float storChargeSegOrdinate[isSTOR_CHARGE_EFF_MODELS][1..maxChargeSegNbr] = [ve.model_id : [ve.power_interval_nbr :  ve.ordinate ] | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="charge"];
// power segment index for each non-linear battery discharge efficiency model (used in declarations of non-linear model efficiency  variables and constraints)
int dischEffModelSegIndx[isSTOR_DISCHARGE_EFF_MODELS] = [ve.model_id : ve.power_interval_nbr | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="discharge"];
// number of power segments for each non-linear stor dicharge efficiency model (used in declarations of non-linear model efficiency  variables and constraints)
int dischStorSegNbr[isSTOR_DISCHARGE_EFF_MODELS] = [em: maxl(0, max(ve in BATTERY_EFFICIENCY_MODELS : ve.model_id == em ) dischEffModelSegIndx[em])|  em  in isSTOR_DISCHARGE_EFF_MODELS];
// set containing the number of segments per model during the phase discharge
int maxDischSegNbr = max(em in isSTOR_DISCHARGE_EFF_MODELS) dischStorSegNbr[em];
// upper limit (in kW) of power segment ve in efficiency model em
float dischEffSegUpLim[isSTOR_DISCHARGE_EFF_MODELS][1..maxDischSegNbr] = [ve.model_id : [ve.power_interval_nbr : ve.upper_limit] | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="discharge" ];
// linear function slope associated with segment discharge ve in efficiency model em
float storDischSegSlope[isSTOR_DISCHARGE_EFF_MODELS][1..maxDischSegNbr] = [ve.model_id : [ve.power_interval_nbr :  ve.slope ] | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="discharge"];
//  linear function ordinate associated with segment discharge ve in efficiency model em
float storDischSegOrdinate[isSTOR_DISCHARGE_EFF_MODELS][1..maxChargeSegNbr] = [ve.model_id : [ve.power_interval_nbr :  ve.ordinate ] | ve in BATTERY_EFFICIENCY_MODELS : ve.type =="discharge"];
// Storage assets variable efficiency model id
string chargeVarEffModelId[s in isE_STORAGES] = (assetVarCostModelId[s] == "-1" || assetVarCostModelId[s] == "-1.0" ? s : assetVarCostModelId[s]);
// Storage assets variable efficiency model id
string dischVarEffModelId[s in isE_STORAGES] = (assetVarCostModelId[s] == "-1" || assetVarCostModelId[s] == "-1.0" ? s : assetVarCostModelId[s]);
...
float storMaxAvailDCActivePowerDischarge[s in isE_STORAGES][t in isDECISION_STEPS] = - availGlobalForPmax[s][t] * nomPowerMin[s];
...
float storCurrentInjection[s in isE_STORAGES][t in isDECISION_STEPS] = availGlobalForPmax[s][t] * currentInjectionPotential[s];
...
float negative_imb_price[imb in isIMBALANCE_STEPS_POS] =
			(microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
				...
				: (daElecPrice[imbStepHourlyStep[imb]] >= 0		// if not MOPABLOEM 
				    	? (optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_FCR" ? daElecPrice[imbStepHourlyStep[imb]] * 10 : daElecPrice[imbStepHourlyStep[imb]] * 1.2)
				    	: (optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_FCR" ? daElecPrice[imbStepHourlyStep[imb]] / 10 : daElecPrice[imbStepHourlyStep[imb]] / 1.2))
			);
...
// Hard coded, only for market-related microgrids, otherwise the price is equal to zero -> no impact in the objective function
float posImbalancePenalty = -5000.0 / 1000.0;
			 ...
			 (microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
			    ...
			    : (mFRRactivatedPower_imb[imb] > 0					// if not MOPABLOEM but downward mFFR activation
				    	...
				    	: (daElecPrice[imbStepHourlyStep[imb]] >= 0	// if no downward mFFR activation
				    		? (optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_FCR"
...
float fcrReqPower[fcr in isFCR_STEPS] = maxl(0.0, fcrEngPower[fcr]
	...
	- sum(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && availableFCRACActivePowerDisharge[t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]
	- sum(a in isASSETS inter {"VALOREM_Limoux_PDL_in"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoad[a][t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]
	- sum(a in isASSETS inter {"VALOREM_Limoux_PDL_out"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoad[a][t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]
	- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_in"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoad[a][t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]
	- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_out"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoad[a][t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]
	- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoad[a][t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]
	- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoad[a][t] < 0.9 * fcrEngPower[fcr])
		fcrEngPower[fcr]);
...
// HARD-CODED
	for (var s in isE_STORAGES){
		...
		// Values for GDS Leforest B's BESS
		if (microgridName == "MICROGRID GDS Leforest B" && (s == "GENERALE_DU_SOLAIRE_Leforest_B_BESS" || s == "GENERALE_DU_SOLAIRE_Leforest_B_BESS_1MW")) {
			nomPowerMax[s] = 2500.0;		
			nomPowerMin[s] = -2500.0;		
			nomEnergyMax[s] = 2610.0;		
		}
...
// flag indicating if non-linear variable charge efficiency model segment se is used for asset storage s over decision step t 
...
// flag indicating if non-linear variable discharge efficiency model segment se is used for asset storage s over decision step t 
// 1 means it is used, 0 that it is not
// (isE_STORAGES, s in VAR_Eff_SEGS, t in DECISION_STEPS)
dvar boolean StorACSegDischFlag[isE_STORAGES][1..maxDischSegNbr][isDECISION_STEPS];
// the average charge power (expressed in kw) of the segment se, of the non-linear variable efficiency model, used for storing asset s during decision step t
...
// the average discharge power (expressed in kw) of the segment se, of the non-linear variable efficiency model, used for storing asset s during decision step t
...
dexpr float ImbalanceTotalCost =
    sum (imb in isIMBALANCE_STEPS_POS) (NegativeImbalancePower_imb[imb] * negative_imb_price[imb] - PositiveImbalancePower_imb[imb] * positive_imb_price[imb]) * effImbStepDurationInHours[imb];
// Imbalance cost over decision steps that are not already cleared
// Positive values mean it is indeed a cost, negative that it is a revenue
dexpr float ImbalanceOptimisedCost =
    sum (imb in isIMBALANCE_STEPS_POS: isDAStepCleared[imbStepHourlyStep[imb]] == 0) (NegativeImbalancePower_imb[imb] * negative_imb_price[imb] - PositiveImbalancePower_imb[imb] * positive_imb_price[imb]) * effImbStepDurationInHours[imb];
// Total cost of energy explicitly traded on DA market over optimisation window
// Positive values mean it is indeed a cost, negative that it is a revenue
dexpr float DayAheadTotalTradeCostRaw = sum(h in isHOURLY_STEPS_POS) (DaPosition[h] * daElecPriceRaw[h] * effDaStepDurationInHours[h]);
dexpr float DayAheadTotalTradeCost = sum(h in isHOURLY_STEPS_POS) (DaPosition[h] * daElecPrice[h] * effDaStepDurationInHours[h]);
...
dexpr float DayAheadOptimisedTradeCost =
    sum(h in isHOURLY_STEPS_POS: isDAStepCleared[h] == 0) (DaPosition[h] * daElecPriceRaw[h] * effDaStepDurationInHours[h]);
...
// For Equans : it is applied for both imports and exports on meter 8, and also on BIC_PV2 production
dexpr float AggregatorCost = sum(t in isDECISION_STEPS)
	(microgridName == "Equans BIC microgrid" ?
	Meter8Import[t] + Meter8Export[t] + InterGenActivePower["BIC_PV2"][t]
...
minimize
  ...
  + dispGenInitialPowerViolPenalty * sum(d in isDISP_E_GENS) (DispGenInitialPowerUpViolation[d] + DispGenInitialPowerDwnViolation[d]) * assetStepDurationInHours
  + powerImbalancePenaltyCost * sum(t in isDECISION_STEPS) (PowerDeficit[t] + PowerExcess[t]) * assetStepDurationInHours
  + heatImbalancePenaltyCost * sum(t in isDECISION_STEPS) (HeatDeficit[t] + HeatExcess[t]) * assetStepDurationInHours
  + unauthorizedInterGenCurtPenaltyCost * sum(t in isDECISION_STEPS) UnauthorizedInterGenCurt[t] * assetStepDurationInHours
  + socStrictMinMaxViolationPenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (storMaxAvailDCEnergy[s][t] * (SOCstrictMinDeficit[s][t] + SOCmaxExcess[s][t]) / 100.0)
  + socStrictMinMaxViolationPenaltyCost * sum(s in isH_STORAGES, t in isDECISION_STEPS) (storMaxHeatCharge[s] * (HeatSOCminDeficit[s][t] + HeatSOCmaxExcess[s][t]) / 100.0)
  + minSocTargetStoragePenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (storMaxAvailDCEnergy[s][t] * (minSocTargetStorageDeficit[s][t]) / 100.0)
  + storageDailyMaxCyclPenaltyCost * sum (s in isE_STORAGES, t in isDECISION_STEPS) (StorageDailyMaxNumCyclExcess[s][t] * nomEnergyMax[s])
  + socMinViolationPenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (storMaxAvailDCEnergy[s][t] * SOCminDeficit[s][t] / 100.0)
  + siteInOutViolationPenaltyCost * sum(i in isSITES, t in isDECISION_STEPS) (SiteMaxInputViolation[i][t] + SiteMaxOutputViolation[i][t]) * assetStepDurationInHours
  + siteInOutViolationPenaltyCost * sum(c in isCONGESTIONS, t in isDECISION_STEPS) (CongestionUpperLimViolation[c][t] + CongestionLowerLimViolation[c][t]) * assetStepDurationInHours
  + genMinMaxStepOnOffPenaltyCost * sum(d in isDISP_E_GENS, i in 0..(genInitialStepsOnMax[d]-1)) DispGenMaxStepOnInitialExcess[d][i]* assetStepDurationInHours * maxDispGenActivePower[d]
  + genMinMaxStepOnOffPenaltyCost * sum(d in isDISP_E_GENS, t in isDECISION_STEPS) DispGenMaxStepOnExcess[d][t] * assetStepDurationInHours * maxDispGenActivePower[d]
  + genMinMaxStepOnOffPenaltyCost * sum(d in isDISP_E_GENS) DispGenMinStepOnInitialDeficit[d] * assetStepDurationInHours * maxDispGenActivePower[d]
  + genMinMaxStepOnOffPenaltyCost * sum(d in isDISP_E_GENS, t in isDECISION_STEPS) DispGenMinStepOnDeficit[d][t] * assetStepDurationInHours * maxDispGenActivePower[d]
  + genMinMaxStepOnOffPenaltyCost * sum(d in isDISP_E_GENS) DispGenMinStepOffInitialDeficit[d] * assetStepDurationInHours * maxDispGenActivePower[d]
  + genMinMaxStepOnOffPenaltyCost * sum(d in isDISP_E_GENS, t in isDECISION_STEPS) DispGenMinStepOffDeficit[d][t] * assetStepDurationInHours * maxDispGenActivePower[d]
  + minDispGenActivePowerDeficitPenalty * sum(d in isDISP_E_GENS, t in isDECISION_STEPS) DispGenMinActivePowerDeficit[d][t] * assetStepDurationInHours
  + activePowerRaiseReserveDeficitPenaltyCost * sum(d in isDISP_E_GENS, t in isDECISION_STEPS) (DispGenActivePowerRaiseReserveDeficit[d][t] * assetStepDurationInHours)
  + activePowerRaiseReserveDeficitPenaltyCost * sum(i in isINTER_E_GENS, t in isDECISION_STEPS) (InterGenActivePowerRaiseReserveDeficit[i][t] * assetStepDurationInHours)
  + activePowerRaiseReserveDeficitPenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (StorActivePowerRaiseReserveDeficit[s][t] * assetStepDurationInHours)
  + activePowerRaiseReserveDeficitPenaltyCost * sum(n in isNF_E_LOADS, t in isDECISION_STEPS) (NFLoadActivePowerRaiseReserveDeficit[n][t] * assetStepDurationInHours)
  + activePowerLowerReserveDeficitPenaltyCost * sum(i in isINTER_E_GENS, t in isDECISION_STEPS) (InterGenActivePowerLowerReserveDeficit[i][t] * assetStepDurationInHours)
  + activePowerLowerReserveDeficitPenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (StorActivePowerLowerReserveDeficit[s][t] * assetStepDurationInHours)
  + activePowerLowerReserveDeficitPenaltyCost * sum(f in isFLEX_E_LOADS, t in isDECISION_STEPS) (FlexLoadActivePowerLowerReserveDeficit[f][t] * assetStepDurationInHours)
  + activePowerLowerReserveDeficitPenaltyCost * sum(n in isNF_E_LOADS, t in isDECISION_STEPS) (NFLoadActivePowerLowerReserveDeficit[n][t] * assetStepDurationInHours)
  + reactivePowerRaiseReserveDeficitPenaltyCost * sum(d in isDISP_E_GENS, t in isDECISION_STEPS) (DispGenReactivePowerRaiseReserveDeficit[d][t] * assetStepDurationInHours)
  + reactivePowerRaiseReserveDeficitPenaltyCost * sum(i in isINTER_E_GENS, t in isDECISION_STEPS) (InterGenReactivePowerRaiseReserveDeficit[i][t] * assetStepDurationInHours)
  + reactivePowerRaiseReserveDeficitPenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (StorReactivePowerRaiseReserveDeficit[s][t] * assetStepDurationInHours)
  + reactivePowerRaiseReserveDeficitPenaltyCost * sum(n in isNF_E_LOADS, t in isDECISION_STEPS) (NFLoadReactivePowerRaiseReserveDeficit[n][t] * assetStepDurationInHours)
  + reactivePowerLowerReserveDeficitPenaltyCost * sum(i in isINTER_E_GENS, t in isDECISION_STEPS) (InterGenReactivePowerLowerReserveDeficit[i][t] * assetStepDurationInHours)
  + spinningReserveDeficitPenaltyCost * sum(t in isDECISION_STEPS) (SpinningRaiseReserveDeficit[t] + SpinningLowerReserveDeficit[t]) * assetStepDurationInHours
  + reactivePowerLowerReserveDeficitPenaltyCost * sum(s in isE_STORAGES, t in isDECISION_STEPS) (StorReactivePowerLowerReserveDeficit[s][t] * assetStepDurationInHours)
  + reactivePowerLowerReserveDeficitPenaltyCost * sum(f in isFLEX_E_LOADS, t in isDECISION_STEPS) (FlexLoadReactivePowerLowerReserveDeficit[f][t] * assetStepDurationInHours)
  + reactivePowerLowerReserveDeficitPenaltyCost * sum(n in isNF_E_LOADS, t in isDECISION_STEPS) (NFLoadReactivePowerLowerReserveDeficit[n][t] * assetStepDurationInHours)
  + defaultCurrentReqDeficitPenaltyCost * sum(t in isDECISION_STEPS) (DefaultCurrentRequirementDeficit[t]);
 ...
 *********************************************************************/
  subject to {
//	forall (t in {"1", "2"})
//	  ctTest1: PositiveImbalancePower_imb[t] <= 0.0;
//	forall (t in {"1", "2", "3"})
//	  ctTest1: NetElecImportTarget[t] == 0.0;
//	forall (s in isSTORAGES, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) > 3)
//		ctTest1: StorACActivePower[s][t] == 800;
//	ctTest2: InterGenPowerCurtailment["GENERALE_DU_SOLAIRE_Leforest_B_PV"]["2"] <= 0.0;
...
// FCR engagement must be equal to the sum of assets' FCRPower
	forall (fcr in isFCR_STEPS)
	  ctFCRPoolEngagement :
	  	fcrReqPower[fcr] == sum(a in isASSETS) FCRPower[a][fcr];
     ...
     forall (d in isDISP_E_GENS inter {"CHP_1600kW"}) {
	    ctNoCogeChange20:
	      ...
	      daEngagement[assetStepHourlyStep[last(isDECISION_STEPS)]] + long_term_engagement[assetStepHourlyStep[last(isDECISION_STEPS)]] <= -600 ?
				...
				IsGenOn[d][last(isDECISION_STEPS)] >= 1
	        	: IsGenOn[d][last(isDECISION_STEPS)] <= 1 ;    }
     ...
     forall (d in isDISP_E_GENS inter {"CHP_1600kW"}) {
	    ctNoCogeChange21:
	      ...
	      daEngagement[assetStepHourlyStep[last(isDECISION_STEPS)]] + long_term_engagement[assetStepHourlyStep[last(isDECISION_STEPS)]] > -600 ?
				...
				IsGenOn[d][last(isDECISION_STEPS)] <= 0
	        	: IsGenOn[d][last(isDECISION_STEPS)] <= 1 ;    }
...
if (lastMinuteCurtOption == 1 && card(isINTER_E_GENS) > 0 && card(isE_STORAGES) > 0 ) { 
		forall (t in isDECISION_STEPS)
	  		ctAuthorizedCurt1 : sum(s in isE_STORAGES : storMaxAvailACActivePowerCharge[s][t] > 0.0)(nomEnergyMax[s] * storElecMaxSOC[s] / 100 - StorStoredDCEnergy[s][t]) 
	  			<= card(isE_STORAGES) * max(s in isE_STORAGES) storMaxNomDCEnergy[s] * (1 - AreAllStoragesFull[t]);
		forall (t in isDECISION_STEPS)
...
if (microgridName in {"MICROGRID NEXHOS Montbert", "MICROGRID NEXHOS Colisee"}) {
 ...
 // EQUANS 
if (microgridName in {"Equans BIC microgrid"}) {
		forall (i in isINTER_E_GENS inter {"BIC_PV1"}, t in isDECISION_STEPS)
		  ctNoInterGenBIC: InterGenActivePower[i][t] <= (daElecPrice[assetStepHourlyStep[t]] >= -3 * electricityPrice[t] - networkDrawingTax[t] ? -nomPowerMin[i] : 0.0);
 	}	  
...
// For one specific charging station on Kergrid microgrid, power consumption by a storage asset in charge is limited by its minimum charge rate
	if (microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid") {
		forall (s in isE_STORAGES inter {"MORB_ENERGIES_Kergrid_V1G_C1", "MORB_ENERGIES_Kergrid_V1G_C2"}, t in isDECISION_STEPS)
		  ctStorageChargeMin: StorACPowerCharge[s][t] >= minl(9.0, storMaxAvailACActivePowerCharge[s][t]) * IsCharging[s][t];
  }

// Power injection by a storage asset in discharge is limited by its maximum discharge rate
	forall (s in isE_STORAGES, t in isDECISION_STEPS)
	  ctStorageDischargeMax: StorACPowerDischarge[s][t] <= storMaxAvailACActivePowerDischarge[s][t] * (1 - IsCharging[s][t]);
...
// average power generation (expressed in kW) from asset storage s with non-linear efficiency over decision step t is the sum of segment power over all segments
...
/*// Energy stored, taking into consideration availability, on 1st step
	forall (s in isE_STORAGES)
	  ctStorageStoredEnergyAvail1: StorStoredDCEnergyAvail[s][first(isDECISION_STEPS)] == storInitialElecCharge[s] * availGlobalForEmax[s][first(isDECISION_STEPS)] + StorStepDCEnergyIn[s][first(isDECISION_STEPS)];
// Energy stored, taking into consideration availability, on other steps
	forall (s in isE_STORAGES, t in isDECISION_STEPS: t != first(isDECISION_STEPS))
	  ctStorageStoredEnergyAvail: StorStoredDCEnergyAvail[s][t] == StorStoredDCEnergy[s][prev(isDECISION_STEPS, t)] * availGlobalForEmax[s][t] + StorStepDCEnergyIn[s][first(isDECISION_STEPS)];*/
...
// Heat injection by a storage asset in discharge is limited by its maximum discharge rate
	forall (s in isH_STORAGES, t in isDECISION_STEPS: heatStorAvail[s][t] == 1)
	  ctHeatStorageDischargeMax: StorHeatDischarge[s][t] <= maxStorHeatDischarge[s] * (1 - IsCharging[s][t]);
...
// Elec power consumed by an energy converter is limited by the maximum and the minimum power consumption possible for that asset
	forall (c in isEIN_CONVS, t in isDECISION_STEPS)
	  ctConvMinMaxElecPowerIn:
	  	(convAvail[c][t] == 1 ? minConvActivePowerIn[c] : 0.0) <= ConvActivePowerIn[c][t] <= (convAvail[c][t] == 1 ? maxConvActivePowerIn[c] : 0.0);
// Converters consuming something else than elec are not supported
...
{t_empty} ASSET_STEPS_HEADER = {<"asset_id", "step_id", "power_target", "energy_target", "temperature_target", "curtailment_target","target_soc", "", "", "", "", "", "", "", "", "", "", "", "", ""> | i in 1..1};
...
string strStorPowerChangeTotalCost;
...
{t_operation_output} OPERATION_OUTPUT = {<"operation_id", operationID>	 									// variable holding OPERATION output data
										...
										, <"storage_use_penalties", strStorPowerChangeTotalCost>			// StorPowerChangeTotalCost
...
{t_asset_steps_output} ASSET_STEPS_OUTPUT = {<a, t, assetPowerTarget[a][t], assetStorageTarget[a][t] , assetTempTarget[a][t], assetCurtailmentTarget[a][t], assetSOCTarget[a][t]> | a in isASSETS union {"MAINGRID"}, t in isDECISION_STEPS}; // variable holding ASSET x STEPS output data
string assetStepsExcelRange = "'ASSET_STEPS_OUTPUT'!A2:G";
...
{t_market_bids_output} MARKET_BIDS_OUTPUT = {<h, "DAY_AHEAD", (DaPosition[h] <= 0 ? "SELL" : "BUY"), daStepDuration, DaPosition[h], (DaPosition[h] <= 0.0 ? sellOfferPrice : daElecPriceRaw[h])>| h in isHOURLY_STEPS_POS}
											union {<fcr, "FCR", "SELL", fcrStepDuration, FCRPower[fcr_a][fcr], 0>| fcr in isFCR_STEPS_POS, fcr_a in isFCR_ASSETS} ; // variable holding ENERGY_BIDS output data
string marketBidsExcelRange = "'MARKET_BIDS_OUTPUT'!A2:F";
int nMarketBids = card(isHOURLY_STEPS_POS) + card(isFCR_STEPS_POS)*card(isFCR_ASSETS);
...
execute {
	...
	strStorPowerChangeTotalCost = StorPowerChangeTotalCost;
...
// 				assetPowerTarget[i][t] = -InterGenActivePowerEnd[i][t];				// respects Everest's sign convention (-ve values = power generation)
 				...
 				if (microgridName == "Equans BIC microgrid"
 					&& daElecPriceRaw[assetStepHourlyStep[t]] < -3.0 * electricityPrice[t] - networkDrawingTax[t]
 					&& i == "BIC_PV1")
 					assetPowerTarget[i][t] = 1.0;									// this is to indicate to the PMS that PV generation must be curtailed to 0
...
// 			assetPowerTarget[d][t] = -DispGenActivePowerEnd[d][t];				// respects Everest's sign convention (-ve values = power generation)
 		...
 		for	(var n in isNF_E_LOADS)
 		{
			...
			if ((microgridName == "MICROGRID MORBIHAN ENERGIES FlexMobIle" || microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid")
				&& storMaxDCEnergy[Opl.first(isE_STORAGES)] > 0.0)
 				assetTempTarget[n][t] = 100 * StorStoredDCEnergy[Opl.first(isE_STORAGES)][t] / storMaxDCEnergy[Opl.first(isE_STORAGES)];
		}
		...
		// elec soc targets for elec storage units
 		...
 		{
			assetSOCTarget[h][t] = HeatStorSocTarget[s][t];
  		}
