...
{string} isMICROGRID_HPP_NOT_OAPPA = {"MICROGRID VALOREM LIMOUX", "MICROGRID GDS Leforest B"};
		...
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
				...
				: (daElecPrice[imbStepHourlyStep[imb]] >= 0		// if not MOPABLOEM
				    	? (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" ? daElecPrice[imbStepHourlyStep[imb]] * 2 + 50/1000: daElecPrice[imbStepHourlyStep[imb]] * 1.2)
				    	: (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" ? daElecPrice[imbStepHourlyStep[imb]] / 2 + 50/1000: daElecPrice[imbStepHourlyStep[imb]] / 1.2))
			)
		);
...
//		ctTest : StorACActivePower[first(isE_STORAGES)][t] == 0;
				...
				sum(i in isINTER_E_GENS : siteID[i] == si) InterGenActivePower[i][t] >= 
										maxl(0, 
											minl(sum(i in isINTER_E_GENS : siteID[i] == si) interGenActivePowerForecast[i][t] - sum(nf in isNF_E_LOADS : siteID[nf] == si) NFLoadActivePower[nf][t], 
												maxOutput[si][t] - sum(fcr_a in isFCR_ASSETS: si == siteID[fcr_a]) FCRPower[fcr_a][assetStepFCRStep[t]])
											);
	}
...
{t_market_bids_output} MARKET_BIDS_OUTPUT = {<h, "DAY_AHEAD", (DaPosition[h] <= 0 ? "SELL" : "BUY"), daStepDuration, DaPosition[h]
															, (DaPosition[h] <= (1.1*sum(nf in isNF_LOADS, t in isDECISION_STEPS : assetStepHourlyStep[t] == h) NFElecLoadForecast[nf][t]*assetStepDurationInHours) ? sellOfferPrice : 
