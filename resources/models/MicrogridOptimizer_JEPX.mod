...
tuple t_assets {
	...
	float max_power;						// in kW: max consumption for 'CONSUMPTION', -min generation for 'GENERATION', max charge rate for 'MIXED', max input for 'SITE'
 	float nominal_max_power;                // in kW: nominal max consumption for 'CONSUMPTION', -min generation for 'GENERATION', -max charge rate for 'MIXED', and max intput for 'SITE'. Static from configuration
...
tuple t_market_engagements {
	...
	//float step_duration;			// DAY_AHEAD TYPE will be sampled at : day_ahead_step / TRANSPORT will be sampled at : asset_stepmber of the market
...
// indexing set of all dummy assets (incl. sites) that could be generated by Everest
{string} isDUMMY_ASSETS = {"dummy_congestion_asset","dummy_genset","dummy_site"};
...
ASSETS diff {a | a in ASSETS: a.asset_id in {"JEPX_KOMEKURAYAMA_Yamanashi_H2_Storage",  "JEPX_KOMEKURAYAMA_Yamanashi_H2_Load",  "JEPX_SUNTORY_Yamanashi_H2_Load"}} // Hydrogen storage assets are not E_ASSETS  
);
{string} isE_ASSETS = {a.asset_id | a in E_ASSETS};
// indexing set of dispatchable assets (in theory isDisp_Load union isDISP_GEN)
// Assets that have minTimeOn, minTimeOff and maxTimeOn configured
//{string} isDISP_ASSETS = {a.asset_id | a in ASSETS: a.min_time_on != -1.0 || a.max_time_on != -1.0 || a.min_recovery_period != -1.0 };
{string} isDISP_ASSETS = {a.asset_id | a in ASSETS: a.type in {"FLEX_LOAD", "GENERATOR"}} diff {"dummy_genset"};  
// indexing set of dispatchable elec assets
{string} isDISP_E_ASSETS = isDISP_ASSETS inter isE_ASSETS;
// indexing set of loads that have a parametric consumption
{string} isPARAM_LOADS = {"JEPX_KOMEKURAYAMA_Yamanashi_Aux", "JEPX_SUNTORY_Yamanashi_Aux"} inter isASSETS;
// Set of assets that consume or generate heat
// HARD-CODED
...
// Set of assets that consume, generate or storage hydrogen
// HARD-CODED
//{t_assets} H2_ASSETS = {a | a in ASSETS: a.energie_in == "HYDROGEN" || a.energies_out == "HYDROGEN" || a.energies_out == "ELEC+HYDROGEN"}
{t_assets} H2_ASSETS = (
	microgridName == "MICROGRID JEPX Komekurayama"
	? ASSETS inter {a | a in ASSETS:
		a.asset_id == "JEPX_KOMEKURAYAMA_Yamanashi_H2_Storage" ||
		a.asset_id == "JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser" ||
		a.asset_id == "JEPX_KOMEKURAYAMA_Yamanashi_H2_Load"
	}
	: microgridName == "MICROGRID JEPX Suntory"
		? ASSETS inter {a | a in ASSETS:
		a.asset_id == "JEPX_SUNTORY_Yamanashi_Electrolyser" ||
		a.asset_id == "JEPX_SUNTORY_Yamanashi_H2_Load"
		}
		: {}
	);
...
// idexing set of hydrogen assets
{string} isH2_ASSETS = {a.asset_id | a in H2_ASSETS};
// indexing set of assets that consume hydrogen
{string} isH2IN_ASSETS = isH2_ASSETS inter {"JEPX_KOMEKURAYAMA_Yamanashi_H2_Load", "JEPX_SUNTORY_Yamanashi_H2_Load"};
// indexing set of assets that generate hydrogen
{string} isH2OUT_ASSETS = isH2_ASSETS inter {"JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser","JEPX_SUNTORY_Yamanashi_Electrolyser"};
// -------------------------------------------
// indexing set of dispatchable load units
// {string} isDISP_LOADS = {a.asset_id | a in ASSETS: a.type == "DISP_LOADS"};
{string} isDISP_LOADS = {a.asset_id | a in ASSETS: a.type == "FLEX_LOAD"} inter isDISP_ASSETS;
// indexing set of dispatchable elec load units
{string} isDISP_E_LOADS = isDISP_LOADS inter isE_ASSETS;
// indexing set of dispatchable heat load units
{string} isDISP_H_LOADS = isDISP_LOADS inter isH_ASSETS;
// indexing set of dispatchable elec and heat load units
{string} isDISP_EH_LOADS = isDISP_E_LOADS inter isDISP_H_LOADS;
// indexing of felx load units
// {string} isFLEX_LOADS = {a.asset_id | a in ASSETS: a.type == "FLEX_LOAD"};
// to keep Flex Load integration tests, flex loads will be modeled as hard-coded FLEX_LOAD inter isASSETS. 
// The set will be non-empty when the FLEX_LOAD is sent to the assets table.
{string} isDUMMY_FLEX_LOADS = {"FLEX_LOAD"};
// indexing of Hydrogen consumption Flex Load
{string} isFLEX_H2IN_LOADS = isDISP_LOADS inter isH2IN_ASSETS;
// Flexible loads (in this model we only model energy or energies consumed by these assets)
{string} isFLEX_LOADS = (isDUMMY_FLEX_LOADS inter isDISP_LOADS) union isFLEX_H2IN_LOADS;
// indexing of is elec felx load units
...
// indexing set of non-flexible hydrogen loads
{string} isNF_H2IN_LOADS = isNF_LOADS inter isH2_ASSETS;
// indexing set of asset loads that consume elec
{string} isEIN_LOADS = isFLEX_E_LOADS union isDISP_E_LOADS union isNF_E_LOADS;
// -------------------------------------------
...
// inter isDISP_ASSETS to exclued dummy generator
{string} isDISP_GENS = {a.asset_id | a in ASSETS: a.type == "GENERATOR"} inter isDISP_ASSETS;
...
// indexing set of energy (En: all energy types En in {elec, heat, hydrogen, etc ...}) storage assets
// {string}isEn_STORAGES = {a.asset_id | a in ASSETS: a.type == "STORAGE" &&  a.energies_in == a.energies_out && a.energies_out == "En"};
...
// indexing set of Hydrogen storage assets
{string} isH2_STORAGES = isSTORAGES inter isH2_ASSETS;
// indexing set of no elec storage assets (tous les assets de stockage de l'énergie sauf l'énergie elec)
{string} isNotE_STORAGES = isSTORAGES diff isE_STORAGES;
...
// set representing the storage operation directions
...
//{string} isCONVS = {a.asset_id | a in ASSETS: a.energies_in != "-1.0" && a.energies_out != a.energies_in };
{string} isCONVS = (microgridName == "MICROGRID MOPABLOEM" ? isASSETS inter {"E_boiler_1000kW", "E_boiler_1200kW"} : 
					microgridName == "MICROGRID JEPX Komekurayama" ? isASSETS inter {"JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"} : 
					microgridName == "MICROGRID JEPX Suntory" ? isASSETS inter {"JEPX_SUNTORY_Yamanashi_Electrolyser"} : {}) ;
// indexing set of assets converting elec into some ossther type of energy
// HARD-CODED
...
// indexing set of assets converting some type of energy into Hydrogen
// HARD-CODED
//{string} isH2OUT_CONVS = isCONVS inter {a.asset_id | a in ASSETS: a.energies_out == "HYDROGEN"};
{string} isH2OUT_CONVS = (microgridName == "MICROGRID JEPX Komekurayama" ? isASSETS inter {"JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"} : 
						 microgridName == "MICROGRID JEPX Suntory" ? isASSETS inter {"JEPX_SUNTORY_Yamanashi_Electrolyser"} : {});
// indexing set of assets converting elec into hydrogen 
{string} isE_H2_CONVS = isEIN_CONVS inter isH2OUT_CONVS;
...
// indexing set of dispatchable elec load units controled by power targets
{string} isPC_DISP_E_LOADS = isDISP_E_LOADS inter isPC_E_ASSETS;
// indexing set of dispatchable elec load units controled by temperature targets
{string} isTC_DISP_E_LOADS = isFLEX_E_LOADS inter isTC_E_ASSETS;
// Other inexing sets
// -------------------------------------------
...
{string} isLIN_COST_E_GENS = {a.asset_id | a in E_ASSETS: a.variable_cost > 0.0};
...
// indexing set of assets subjected to piecewise linear efficiency
{string} isCONV_NL_EFF_ASSETS = {a.asset_id | a in ASSETS: a.asset_id in isCONVS && a.var_efficiency_model != "-1.0" && a.var_efficiency_model != "-1"};
		
// indexing set of elec intermittent generation assets with curtailment compensation costs
{string} isCURT_COMP_E_GENS = {a.asset_id | a in E_ASSETS: a.compensation_cost > 0.0};
// indexing set of asset with startup costs
// isSTART_COST_ASSETS should be included in isDISP_ASSETS. Assets that have start_up_cost are by definition disp assets
{string} isSTART_COST_ASSETS = {a.asset_id | a in ASSETS: a.startup_cost > 0.0} inter isDISP_ASSETS;
// indexing set of dispatchable generation assets with startup costs
{string} isSTART_COST_DISPGENS = isSTART_COST_ASSETS inter isDISP_GENS;
// indexing set of dispatchable elec generation assets with startup costs
{string} isSTART_COST_DISP_E_GENS = isSTART_COST_ASSETS inter isDISP_E_GENS;
// indexing set of dispatchable load assets with startup costs
{string} isSTART_COST_DISPLOADS = isSTART_COST_ASSETS inter isDISP_LOADS;
// indexing set of dispatchable elec load assets with startup costs
{string} isSTART_COST_DISP_E_LOADS = isSTART_COST_DISPLOADS inter isDISP_E_LOADS;
// indexing set of dispatchable heat load assets with startup costs
// not used 
{string} isSTART_COST_H_DISPLOADS = isSTART_COST_DISPLOADS inter isDISP_H_LOADS;
...
{string} isFCR_ASSETS = (microgridName == "MICROGRID VALOREM LIMOUX") ? isE_ASSETS inter {"VALOREM_Limoux_BESS"} :
						(microgridName == "MICROGRID GDS Leforest B") ? isE_ASSETS inter ({"GENERALE_DU_SOLAIRE_Leforest_B_BESS"} union {"GENERALE_DU_SOLAIRE_Leforest_B_BESS_1MW"}) :
						(microgridName == "MICROGRID VALECO Baignollais") ? isE_ASSETS inter {"VALECO_Baignollais_BESS_bis"} :
						(microgridName == "MICROGRID JEPX Komekurayama") ? isH2_ASSETS inter {"JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"} :
						(microgridName == "MICROGRID JEPX Suntory") ? isH2_ASSETS inter {"JEPX_SUNTORY_Yamanashi_Electrolyser"} :
						{};
// indexing set of site contains assets participating in FCR
// we model it as NF_Load to be able to model partial dispo on the EVEREST side
{string} isFCR_SITES = isASSETS inter {"VALOREM_Limoux_PDL_in", "VALOREM_Limoux_PDL_out", "GENERALE_DU_SOLAIRE_Leforest_B_PDL_in", 
						"GENERALE_DU_SOLAIRE_Leforest_B_PDL_out", "GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in", "GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"};
...
{string} isCFD_ASSETS = (
	microgridName == "MICROGRID GDS Leforest A" || microgridName == "MICROGRID GDS Leforest B"
...
// efficiency (no unit) associated with power segment s in cost model cm
float nlEffSegSlope[isVAR_COST_MODELS][1..maxSegNbr] = [s.model_id : [s.power_interval_nbr : s.efficiency_slope] | s in VARIABLE_COST_MODELS];
...
// startup costs for dispatchable generators d (expressed in currency unit) (d in isSTART_COST_DISP_E_GENS)
float dispGenStartupCost[d in isSTART_COST_DISP_E_GENS] = assetStartupCost[d];
// startup costs for dispatchable loads d (expressed in currency unit) (d in isSTART_COST_DISP_E_LOADS)
float dispLoadStartupCost[d in isSTART_COST_DISP_E_LOADS] = assetStartupCost[d];
...
float initialAssetPower[a in isASSETS] = abs(initialPower[a]);
// active power floor under which asset is considered to be off (in kW)
float powerTolerance[isASSETS] = [a.asset_id : a.power_tolerance | a in ASSETS];
float onStateTolerance[d in isDISP_ASSETS] = (d == "JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser" || d == "JEPX_SUNTORY_Yamanashi_Electrolyser" ? maxl(powerTolerance[d], 200) : maxl(powerTolerance[d],1));
// dispatchable asset initial state
// 0 means it was off, 1 it was on
int dispAssetInitialState[d in isDISP_ASSETS] = initialAssetPower[d] <= onStateTolerance[d] ? 0 : 1;
// dispatchable asset initial power (expressed in kW)
// If the initial power is negative for dispatchable asset, it mays mean that it is due to its auxiliaries' consumption. Then we set dispAssetInitialPower to zero, otherwise it will cause issues.
float dispAssetInitialPower[d in isDISP_ASSETS] = initialAssetPower[d] <= onStateTolerance[d] ? 0 :initialAssetPower[d];
...
// minimum number of steps each dispatchable asset can continuously generate for
int dispAssetMinStepsOn[d in isDISP_ASSETS] = ftoi(ceil(minTimeOn[d] / assetStepDuration));
...
int dispAssetMaxStepsOn[d in isDISP_ASSETS] = ftoi(floor(maxTimeOn[d] / assetStepDuration));
...
// minimum recovery time (expressed in number os steps) between two continuous generation from each dispatchable asset
int dispMinRecoverySteps[d in isDISP_ASSETS] = ftoi(ceil(minRecoveryTime[d] / assetStepDuration));
...
// number of time steps each dispatchable assets has been continuously on before the beginning of decision step 1
int dispInitialStepsOnMax[d in isDISP_ASSETS] = ftoi(ceil(initialTimeOn[d] / assetStepDuration));
int maxDispInitialStepsOnMax = maxl(1, max(d in isDISP_ASSETS) dispInitialStepsOnMax[d]);
range rgInitialStepOffset = 0..(maxDispInitialStepsOnMax-1);
int dispInitialStepsOnMin[d in isDISP_ASSETS] = ftoi(floor(initialTimeOn[d] / assetStepDuration));
...
int dispInitialStepsOff[d in isDISP_ASSETS] = ftoi(floor(initialTimeOff[d] / assetStepDuration));
...
float availMeasuredForBlockNbr[a in isASSETS] = (nomBlockNbr[a] != 0 ? 
	    (healthyBlockNbr[a] == -1 ? 1				// It was not mapped, we don't measure the availability here
...
int dispGenAvail[d in isDISP_GENS][t in isDECISION_STEPS] = (scheduledAvailability[d][t] == 1.0 ? 1 : 0 );
int dispAssetTotUnavail[d in isDISP_ASSETS][t in isDECISION_STEPS] = (scheduledAvailability[d][t] < 0.1 ? 1 : 0);
// Partial availability
// availability = % of availability for asset a over decision step t
float assestPartialAvail[a in isASSETS][t in isDECISION_STEPS] = (scheduledAvailability[a][t] == -1.0 ? 0.0 : 100.0 * scheduledAvailability[a][t]);
float storPartialAvail[s in isSTORAGES][t in isDECISION_STEPS] = assestPartialAvail[s][t];
float NFLoadPartialAvail[n in isNF_LOADS][t in isDECISION_STEPS] = assestPartialAvail[n][t];
float interGenPartialAvail[i in isINTER_GENS][t in isDECISION_STEPS] = assestPartialAvail[i][t];
float dispLoadPartialAvail[d in isDISP_LOADS][t in isDECISION_STEPS] = assestPartialAvail[d][t];
float flexPartialLoadAvail[f in isFLEX_LOADS][t in isDECISION_STEPS] = assestPartialAvail[f][t];
float convPartialAvail[c in isCONVS][t in isDECISION_STEPS] = assestPartialAvail[c][t];
...
float maxInterGenActivePower[i in isINTER_E_GENS][t in isDECISION_STEPS] = -powerMin[i] * interGenPartialAvail[i][t] / 100.0;
float minInterGenActivePower[i in isINTER_E_GENS][t in isDECISION_STEPS] = -powerMax[i] * interGenPartialAvail[i][t] / 100.0;
// maximum and minimum nominal power generation (expressed in kW) physically possible for dispatchable generation asset d (d in DISP_GENS)
float maxNomDispGenPower[d in isDISP_GENS] = -nomPowerMin[d];
float minNomDispGenPower[d in isDISP_GENS] = -nomPowerMax[d];
// maximum and minimum power generation (expressed in kW) physically possible for dispatchable generation asset d (d in DISP_GENS)
float maxDispGenPower[d in isDISP_GENS] = -powerMin[d];
float minDispGenPower[d in isDISP_GENS] = -powerMax[d];
// maximum and minimum Avail power generation (expressed in kW) for dispatchable assets unit d (d in DISP_GENS)
float maxAvailDispGenPower[d in isDISP_GENS][t in isDECISION_STEPS] = -availGlobalForPmin[d][t] * nomPowerMin[d];
float minAvailDispGenPower[d in isDISP_GENS][t in isDECISION_STEPS] = -availGlobalForPmax[d][t] * nomPowerMax[d];
// maximum and minimum nominal power consumption (expressed in kW) for dispatchable load unit d (d in DISP_LOADS)
float maxNomDispLoadPower[d in isDISP_LOADS] = nomPowerMax[d];
float minNomDispLoadPower[d in isDISP_LOADS] = nomPowerMin[d];
// maximum and minimum power consumption (expressed in kW) for dispatchable load unit d (d in DISP_LOADS)
float maxDispLoadPower[d in isDISP_LOADS] = powerMax[d];
float minDispLoadPower[d in isDISP_LOADS] = powerMin[d];
// maximum and minimum Avail power consumption (expressed in kW) for dispatchable assets unit d (d in isDISP_LOADS)
float maxAvailDispLoadPower[d in isDISP_LOADS][t in isDECISION_STEPS] = availGlobalForPmax[d][t] * nomPowerMax[d];
float minAvailDispLoadPower[d in isDISP_LOADS][t in isDECISION_STEPS] = availGlobalForPmin[d][t] * nomPowerMin[d];
// maximum and minimum nominal power consumption/generation (expressed in kW) for dispatchable load unit d (d in isDISP_ASSETS)
float maxDispAssetNomPower[d in isDISP_ASSETS] = (d in isDISP_GENS ? maxNomDispGenPower[d] : maxNomDispLoadPower[d]);
float minDispAssetNomPower[d in isDISP_ASSETS] = (d in isDISP_GENS ? minNomDispGenPower[d] : minNomDispLoadPower[d]);
// maximum and minimum power consumption/generation (expressed in kW) for dispatchable assets unit d (d in DISP_ASSETS)
float maxDispAssetPower[d in isDISP_ASSETS] = (d in isDISP_GENS ? maxDispGenPower[d] : maxDispLoadPower[d]);
float minDispAssetPower[d in isDISP_ASSETS] = (d in isDISP_GENS ? minDispGenPower[d] : minDispLoadPower[d]);
// maximum and minimum Avail power consumption/generation (expressed in kW) for dispatchable assets unit d (d in DISP_ASSETS)
float maxAvailDispAssetPower[d in isDISP_ASSETS][t in isDECISION_STEPS] = (d in isDISP_GENS) ? maxAvailDispGenPower[d][t] : maxAvailDispLoadPower[d][t];
float minAvailDispAssetPower[d in isDISP_ASSETS][t in isDECISION_STEPS] = (d in isDISP_GENS) ? minAvailDispGenPower[d][t] : minAvailDispLoadPower[d][t];
// maximum and minimum active power generation (expressed in kW) physically possible for dispatchable generation asset d (d in DISP_E_GENS)
float maxDispGenActivePower[d in isDISP_E_GENS] = maxDispGenPower[d];
float minDispGenActivePower[d in isDISP_E_GENS] = minDispGenPower[d];
// maximum and minimum active power consumption (expressed in kW) for dispatchable load unit d (d in isDISP_E_LOADS)
float maxDispLoadActivePower[d in isDISP_E_LOADS] = maxDispLoadPower[d];
float minDispLoadActivePower[d in isDISP_E_LOADS] = minDispLoadPower[d];
// maximum and minimum power consumption/generation (expressed in kW) for dispatchable asset unit a (f in isDISP_E_ASSETS)
float maxDispAssetActivePower[a in isDISP_E_ASSETS] = (a in isDISP_E_GENS ? maxDispGenActivePower[a] : maxDispLoadActivePower[a]);
float minDispAssetActivePower[a in isDISP_E_ASSETS] = (a in isDISP_E_GENS ? minDispGenActivePower[a] : minDispLoadActivePower[a]);
// minimum active power generation or consumption (expressed in kW) physically possible for dispatchable asset d (d in DISP_E_ASSETS)
float physMinDispAssetActivePower[d in isDISP_E_ASSETS] = minl(5.0, minDispAssetActivePower[d]);
// maximum and minimum heat consumption (expressed in kW) for non-flexible heat load n (n in NF_H_LOADS)
float maxNFHeatLoad[n in isNF_H_LOADS] = powerMax[n];
float minNFHeatLoad[n in isNF_H_LOADS] = powerMin[n];
// maximum and minimum hydorgen consumption (expressed in kW) for non-flexible hyrogen load n (n in NF_H2_LOADS)f
float maxNFH2Load[n in isNF_H2IN_LOADS] = powerMax[n];
float minNFH2Load[n in isNF_H2IN_LOADS] = powerMin[n];
// maximum and minimum power consumption (expressed in kW) for flex load unit f (f in FLEX_LOADS)
float maxFlexLoadPower[f in isFLEX_LOADS] = powerMax[f];
float minFlexLoadPower[f in isFLEX_LOADS] = powerMin[f];
// maximum and minimum elec power consumption (expressed in kW) for flex elec load unit f (f in FLEX_E_LOADS)
float maxFlexElecLoadPower[f in isFLEX_E_LOADS] = maxFlexLoadPower[f];
float minFlexElecLoadPower[f in isFLEX_E_LOADS] = minFlexLoadPower[f];
...
float maxNFElecLoadPower[n in isNF_E_LOADS][t in isDECISION_STEPS] = powerMax[n] * NFLoadPartialAvail[n][t] / 100.0;
...
// accounts for partial availability of asset n over decisions step t
float minNFElecLoadPower[n in isNF_E_LOADS][t in isDECISION_STEPS] = powerMin[n] * NFLoadPartialAvail[n][t] / 100.0;
...
float maxInput[i in isSITES][t in isDECISION_STEPS] =
	(microgridName == "MICROGRID VALOREM LIMOUX" && card(isASSETS inter {"VALOREM_Limoux_PDL_in"}) > 0) ? maxNFElecLoadPower["VALOREM_Limoux_PDL_in"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_in"}) > 0) ? maxNFElecLoadPower["GENERALE_DU_SOLAIRE_Leforest_B_PDL_in"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in"}) > 0) ? maxNFElecLoadPower["GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in"][t] :
	powerMax[i];
float maxOutput[i in isSITES][t in isDECISION_STEPS] =
	(microgridName == "MICROGRID VALOREM LIMOUX" && card(isASSETS inter {"VALOREM_Limoux_PDL_out"}) > 0) ? maxNFElecLoadPower["VALOREM_Limoux_PDL_out"][t] :
	(microgridName == "MICROGRID GDS Leforest A" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_A_PDL_out"}) > 0) ? maxNFElecLoadPower["GENERALE_DU_SOLAIRE_Leforest_A_PDL_out"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_out"}) > 0) ? maxNFElecLoadPower["GENERALE_DU_SOLAIRE_Leforest_B_PDL_out"][t] :
	(microgridName == "MICROGRID GDS Leforest B" && card(isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"}) > 0) ? maxNFElecLoadPower["GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"][t] :
	(microgridName == "MICROGRID NEXHOS Montbert" && card(isASSETS inter {"NEXHOS_MONTBERT_PDL_out"}) > 0) ? maxNFElecLoadPower["NEXHOS_MONTBERT_PDL_out"][t] :
	(microgridName == "MICROGRID NEXHOS Colisee" && card(isASSETS inter {"NEXHOS_COLISEE_PDL_out"}) > 0) ? maxNFElecLoadPower["NEXHOS_COLISEE_PDL_out"][t] :
	-powerMin[i];
...
float storMaxDCEnergy[s in isE_STORAGES][t in isDECISION_STEPS] = (
	storPartialAvail[s][first(isDECISION_STEPS)] > 0.0
		? minl(nomEnergyMax[s], energyMax[s] / storPartialAvail[s][first(isDECISION_STEPS)] * storPartialAvail[s][t])
		: nomEnergyMax[s] * storPartialAvail[s][t] /100.0
	);
// maximum Not_elec_energy charge (expressed in kWh) possible for Not_elec_energy storage asset s (s in NotE_STORAGES)
float storNotElecMaxEnergyCharge[s in isNotE_STORAGES] = energyMax[s];
// avail maximum Not_elec_energy charge (expressed in kWh) possible for Not_elec_energy storage asset s (s in NotE_STORAGES, t in DECISION_STEPS) (only palnified avail are taken into account)
float availStorNotElecMaxCharge[s in isNotE_STORAGES][t in isDECISION_STEPS] = storNotElecMaxEnergyCharge[s] * storPartialAvail[s][t]/ 100.0;
// maximum Hydrogen charge (expressed in kWh) possible for hydrogen storage asset s (s in H2_STORAGES)
float storMaxH2Charge[s in isH2_STORAGES] = energyMax[s];
...
float storInitialElecCharge[s in isE_STORAGES] = initialSOC[s] / 100.0 * storMaxNomDCEnergy[s];
// not_elec_energy (expressed in kWh) initially stored (that is, stored at the beginning of decision step 1) in not_elec_energy storage asset s (s in NotE_STORAGES)
float storInitNotElecEnergyCharge[s in isNotE_STORAGES] = initialSOC[s] / 100.0 * storNotElecMaxEnergyCharge[s];
...
// maximum state of charge (expressed as a % of asset's maximum not_elec_energy storage capacity)
// allowed in not_elec_energy storage asset s over decision step t (s in NotE_STORAGES, t in DECISION_STEPS)
float storNotElecMaxSOC[s in isNotE_STORAGES] = SOCMax[s];
// minimum state of charge (expressed as a % of asset's maximum not_elec_energy energy storage capacity)
// allowed in not_elec_energy storage asset s over decision step t (s in NotE_STORAGES, t in DECISION_STEPS)
float storNotElecMinSOC[s in isNotE_STORAGES] = SOCMin[s];
// maximum state of charge (expressed as a % of asset's maximum hydrogen storage capacity)
// allowed in hydrogen storage asset s over decision step t (s in H2_STORAGES, t in DECISION_STEPS)
float storH2MaxSOC[s in isH2_STORAGES] = SOCMax[s];
// minimum state of charge (expressed as a % of asset's maximum hydrogen energy storage capacity)
// allowed in hydrogen storage asset s over decision step t (s in H2_STORAGES, t in DECISION_STEPS)
float storH2MinSOC[s in isH2_STORAGES] = SOCMin[s];
...
// Early aging cost caused by cycling for FCR (expressed in currency unit/kWh)(s in isE_STORAGES)
// Hard-coded for now
float cyclingVariableCostFCR[s in isE_STORAGES] = 0.003 ; 
...
float interGenCurtComp[i in isCURT_COMP_E_GENS inter isINTER_E_GENS] = assetCurtComp[i];
...
// dispatchable load unit's linear factor of max reactive power approximation as a linear function of active power
float aDispLoadQmax[isDISP_E_LOADS];
// dispatchable load unit's constant term of max reactive power approximation as a linear function of active power
float bDispLoadQmax[isDISP_E_LOADS];
// dispatchable load unit's linear factor of min reactive power approximation as a linear function of active power
float aDispLoadQmin[isDISP_E_LOADS];
// dispatchable load unit's constant term of min reactive power approximation as a linear function of active power
float bDispLoadQmin[isDISP_E_LOADS];
...
float assetEnergyConvEfficiency[isASSETS] = [a.asset_id : 1 | a in ASSETS]; // [a.asset_id : a.conv_efficiency | a in ASSETS];
//elec-to-heat conversion efficiency ratio
float convElecToHeatEff[ceh in isE_H_CONVS] = assetEnergyConvEfficiency[ceh] * 0.998;
// elec-to-hydrogen conversion efficiency ratio kwh/kwh
float convElecToH2Eff[cehy in isE_H2_CONVS] = assetEnergyConvEfficiency[cehy] * 0.750;
...
float NFElecLoadForecast[n in isNF_E_LOADS][t in isDECISION_STEPS] = maxl(minl(maxNFElecLoadPower[n][t], NFLoadPartialAvail[n][t] / 100.0 * powerPrediction[n][t]), minNFElecLoadPower[n][t]);
// A flag that signals the changing on the NFElecLoad forecasts values
int NFElecLoadChangedForecast[n in isNF_E_LOADS][t in isDECISION_STEPS] = (NFElecLoadForecast[n][t] != powerPrediction[n][t]*NFLoadPartialAvail[n][t] / 100.0  ? 1 : 0);
...
int NFLoadHeatChangedForecast[n in isNF_H_LOADS][t in isDECISION_STEPS] = (NFHeatLoadForecast[n][t] != powerPrediction[n][t] ? 1 : 0);
// average hydrogen consumption forecast (expressed in kW) for non-flexible hydrogen load n over decision step t
// (n in NF_H2_LOADS, t in DECISION_STEPS)
float NFH2LoadForecast[n in isNF_H2IN_LOADS][t in isDECISION_STEPS] = maxl(minl(maxNFH2Load[n], powerPrediction[n][t]), minNFH2Load[n]);
// A flag that signals the changing on the NFH2Load forecasts values
int NFLoadH2ChangedForecast[n in isNF_H2IN_LOADS][t in isDECISION_STEPS] = (NFH2LoadForecast[n][t] != powerPrediction[n][t] ? 1 : 0);
// nominal average power consumption (expressed in kW) forecast for dispatchable load unit f over decision step t
// (f in FLEX_LOADS, t in DECISION_STEPS)
float flexLoadForecast[f in isFLEX_LOADS][t in isDECISION_STEPS] = maxl(minl(maxFlexLoadPower[f], powerPrediction[f][t]), minFlexLoadPower[f]) * flexPartialLoadAvail[f][t] / 100.0;
// A flag that signals the changing on the flexLoad forecasts values
int flexLoadChangedForecast[f in isFLEX_LOADS][t in isDECISION_STEPS] = (flexLoadForecast[f][t] != (flexPartialLoadAvail[f][t] / 100.0) * powerPrediction[f][t] ? 1 : 0);
...
float interGenActivePowerForecast[i in isINTER_E_GENS][t in isDECISION_STEPS] = maxl(minl(maxInterGenActivePower[i][t], -interGenPartialAvail[i][t] / 100.0 * powerPrediction[i][t]), minInterGenActivePower[i][t]);
// A flag that signals the changing on the InterGen forecasts values
int interGenChangedForecast[i in isINTER_E_GENS][t in isDECISION_STEPS] = (interGenActivePowerForecast[i][t] != -powerPrediction[i][t]*interGenPartialAvail[i][t]/100.0   ? 1 : 0);
...
// left hand side coefficient of power consumption from dispatchable load unit f in network congestion constraint c
// (d in isDISP_LOADS, c in CONGESTION)
float dispElecLoadFactor[c in isCONGESTIONS][d in isDISP_E_LOADS] = assetCongestionFactor[c][d];
...
// left hand side coefficient of fcr asset unit n particpating to fcr in network congestion constraint c
// (n in NF_LOADS, c in CONGESTION)
...
// optimisation's context (OPERATIONS, MARKET_STRAT_DA or MARKET_STRAT_FCR)
...
float sellOfferPrice = 	  (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID VALECO Baignollais") ? 1.0 / 1000.0	// Valorem pays EP 0.4 �/MWh so we decided that it was not profitable for them to generate for less than 1 �/MWh
						: (microgridName in {"MICROGRID GDS Leforest A", "MICROGRID GDS Leforest B"}) ? 0.0 / 1000.0	// GDS is on a contract for difference agreement so it is profitable for them to generate as long as prices are >= 0 �/MWh
						: (microgridName in {"MICROGRID NEXHOS Montbert", "MICROGRID NEXHOS Colisee"}) ? 0.0 / 1000.0	// NEXHOS is on a contract for difference agreement so it is profitable for them to generate as long as prices are >= 0 �/MWh
						:  0.0;
...
float daElecPrice[h in isHOURLY_STEPS] = (
	...
	&& (optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_FCR")	// in the optim context of market strat...
		...
		microgridName == "MICROGRID ENERCAL Ile des Pins" || microgridName == "MICROGRID ENERCAL Mare"
			...
			: (
				...
				: (
					...
					: (
						microgridName == "MICROGRID JEPX Komekurayama"
						? 0.0 //1 * stepIndex[t] / optimisationStepNumber // we encourage early charge
						:0.0
					   )
				  )
			   )
...
float storPowerChangePenalty = (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID GDS Leforest B" ? 0.0001 : 0.0);
...
// hypothesis : In calcul of the maxNlVarCost, the generator is on, on average for dispAssetMaxStepsOn / 2.
// The max seg cost of all asset that have a non linear cost
// The max Lin cost of all asset that have a lin cost
// We should take into account this costs when calculating the dispMinMaxStepOnOffPenaltyCost (dispMinMaxStepOnOffPenaltyCost should be
...
float maxStartNlVarCost = maxl(
						  max(g in isNL_COST_E_GENS inter isSTART_COST_DISP_E_GENS: assetStepDurationInHours > 0.0 && maxDispGenActivePower[g] > 0.0, s in 1..maxSegNbr : s < varCostModelSegNumber[genVarCostModelId[g]])
                          ((assetStartupCost[g] / (assetStepDurationInHours * maxDispGenActivePower[g])) + (varCostSegCost[genVarCostModelId[g]][s] * dispAssetMaxStepsOn[g]) / 2.0 )
						  ,0.0);

float maxStartLinVarCost = maxl(
						   max(g in isLIN_COST_E_GENS inter isSTART_COST_DISP_E_GENS:  assetStepDurationInHours > 0.0 && maxDispGenActivePower[g] > 0.0)
                          ((assetStartupCost[g] / (assetStepDurationInHours * maxDispGenActivePower[g])) + (genVariableCost[g]  * dispAssetMaxStepsOn[g] / 2.0 ))
						  ,0.0);
...
float dispMinMaxStepOnOffPenaltyCost = cat4Pen * penaltyBase;
...
float cat2_1Pen = minl(maxl((card(isDISP_E_GENS) + card(isE_STORAGES) + card(isNotE_STORAGES )), 2)*cat2_2Pen, subCatPhyMultPenalty*cat2_2Pen);
...
// penalty cost for non-respect of the minimum and maximum physical SOC for the storage asset (Elec + Heat + Hydrogen)
...
// average charge / discharge efficiencies (expressed as a %) of not_elec_charge storage asset s (s in NotE_STORAGES)
//float storHeatChargeEfficiency[s in isH_STORAGES] = assetChargeEfficiency[s] * 100; // to be deleted
//float storHeatDischargeEfficiency[s in isH_STORAGES] = (assetDischargeEfficiency[s] == -1 ? storHeatChargeEfficiency[s] : assetDischargeEfficiency[s] * 100); // to be deleted
float storNotElecChargeCstEff[s in isNotE_STORAGES] = assetChargeEfficiency[s] * 100; 
float storNotElecDischCstEff[s in isNotE_STORAGES] = (assetDischargeEfficiency[s] == -1 ? storNotElecChargeCstEff[s] : assetDischargeEfficiency[s] * 100);
// maximum power charge / discharge rates (expressed in kW) for not-elec_energy storage asset s (s in NotE_STORAGES)
float maxStorNotElecPowerCharge[s in isNotE_STORAGES] = (storNotElecChargeCstEff[s] > 0.0 ? powerMax[s] * (100.0 / storNotElecChargeCstEff[s]) : 0.0);
float maxStorNotElecPowerDischarge[s in isNotE_STORAGES] = -powerMin[s] * (storNotElecDischCstEff[s] / 100.0);
...
{string} isSTOR_VAR_MODEL = {ve.model_id | ve in VARIABLE_COST_MODELS: ve.direction == "CHARGE" || ve.direction == "DISCHARGE"} inter {a.var_cost_model | a in ASSETS: a.type == "STORAGE"};
...
// minimum nominal AC-side charge rates (expressed in kW) for elec storage asset s (s in E_STORAGES)
float storMinNomACActivePowerCharge[s in isE_STORAGES][t in isDECISION_STEPS] = storMinNomDCActivePowerCharge[s] * storChargeSegSlope[chargeVarEffModelId[s]][chargeStorSegNbr[chargeVarEffModelId[s]]] + storChargeSegOrdinate[chargeVarEffModelId[s]][chargeStorSegNbr[chargeVarEffModelId[s]]];
...
// accounts for partial availability of asset s over decisions step t
float availInverterNbr[s in isE_STORAGES][t in isDECISION_STEPS] = (scheduledAvailability[s][first(isDECISION_STEPS)] > 0.0
...
float negative_imb_price[imb in isIMBALANCE_STEPS_POS] =
		(microgridName == "Equans BIC microgrid" || microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
				? (mFRRactivatedPower_imb[imb] < 0				// if upward mFRR activation
					? penaltyBase * 100
				    : (daElecPrice[imbStepHourlyStep[imb]] >= 0	// if no upward mFRR activation
				...
				: (daElecPrice[imbStepHourlyStep[imb]] >= 0		// if not MOPABLOEM
				    	? (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" ? daElecPrice[imbStepHourlyStep[imb]] * 10 : daElecPrice[imbStepHourlyStep[imb]] * 1.2)
				    	: (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" ? daElecPrice[imbStepHourlyStep[imb]] / 10 : daElecPrice[imbStepHourlyStep[imb]] / 1.2))
			)
		);
		...
		(microgridName == "Equans BIC microgrid" || microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
				? (mFRRactivatedPower_imb[imb] > 0					// if downward mFFR activation
			        ...
			        : -5)											// if no downward mFFR activation
			    : (mFRRactivatedPower_imb[imb] > 0					// if not MOPABLOEM but downward mFFR activation
				    	...
				    	: (daElecPrice[imbStepHourlyStep[imb]] >= 0	// if no downward mFFR activation
				    		? (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA"
				    			? (daElecPriceRaw[imbStepHourlyStep[imb]] >= sellOfferPrice	// are DA prices >= offer prices?
				    				? daElecPrice[imbStepHourlyStep[imb]] / 10
...
float fcrCertfiedPower[a in isASSETS] =	(microgridName ==  "MICROGRID VALOREM LIMOUX" && a in isFCR_ASSETS) ? 1000 :
										(microgridName ==  "MICROGRID GDS Leforest B" && a in isFCR_ASSETS) ? 2000 :
										(microgridName == "MICROGRID JEPX Komekurayama" && a in isFCR_ASSETS) ? 700 :
										(microgridName == "MICROGRID JEPX Suntory" && a in isFCR_ASSETS) ? 3000 :
										0;
// The minimum power an asset can engage in FCR, expressed in %.
// take into account availableFCRACActivePowerCharge when calculating thresholdMinFCRPower
// For Komekuryama, since there are 3 lectrolyser, having only one electrolyser is enough to give the service
float thresholdMinFCRPower[fcr_a in isFCR_ASSETS] = (microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory") ?
										0.3
										:0.9;
// avail FCRPower that can be engaged into FCR for asset a, expressed in kW (a in ASSETS)
// we take the minimum available power certified during the n steps t of a step fcr
// we need to take Pmin for Gen, since it represent the maximum power 
float fcrAvailCertfiedPower[fcr_a in isFCR_ASSETS][fcr in isFCR_STEPS_POS] = ( fcr_a in isDISP_E_LOADS ? 
				min(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr) availGlobalForPmax[fcr_a][t] < thresholdMinFCRPower[fcr_a] ? 0 
					: min(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr) fcrCertfiedPower[fcr_a] * availGlobalForPmax[fcr_a][t]
				: min(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr) minl(availGlobalForPmin[fcr_a][t], availGlobalForPmin[fcr_a][t]) < thresholdMinFCRPower[fcr_a] ? 0 
					: min(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr) fcrCertfiedPower[fcr_a] * availGlobalForPmin[fcr_a][t]
				);	
/*
// max AC power charge available from FCR assets
float availableFCRACActivePowerCharge[t in isDECISION_STEPS] = sum(s in isE_STORAGES inter isFCR_ASSETS) storMaxAvailACActivePowerDischarge[s][t];
// max AC power discharge available from FCR assets
float availableFCRACActivePowerDicharge[t in isDECISION_STEPS] = sum(s in isE_STORAGES inter isFCR_ASSETS) storMaxAvailACActivePowerDischarge[s][t];
*/
// Minimum Avail power for assets participating in FCR
// for batteries, the control band is symmetrical with respect to a control power (target) of 0
float fcrAseetsAvailMinPower[a in isFCR_ASSETS][t in isDECISION_STEPS] = (a in isSTORAGES ? - storMaxAvailACActivePowerDischarge[a][t] : minAvailDispAssetPower[a][t]);
// Maximum Avail power for assets participating in FCR
float fcrAseetsAvailMaxPower[a in isFCR_ASSETS][t in isDECISION_STEPS] = (a in isSTORAGES ? storMaxAvailACActivePowerCharge[a][t] :  maxAvailDispAssetPower[a][t]);
...
float fcrReqPower[fcr in isFCR_STEPS_POS] = minl(
		fcrEngPower[fcr], sum(fcr_a in isFCR_ASSETS) fcrAvailCertfiedPower[fcr_a][fcr]
		) - sum(s in isFCR_SITES, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[s][t] < 0.9 * fcrEngPower[fcr])
		- fcrEngPower[fcr];
/*if (card(isFCR_ASSETS inter isE_STORAGES) > 0) {
fcrReqPower[fcr in isFCR_STEPS_POS] = maxl(0.0, fcrEngPower[fcr]
		- sum(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && availableFCRACActivePowerCharge[t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && availableFCRACActivePowerDicharge[t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(a in isASSETS inter {"VALOREM_Limoux_PDL_in"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[a][t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(a in isASSETS inter {"VALOREM_Limoux_PDL_out"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[a][t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_in"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[a][t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_PDL_out"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[a][t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_in"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[a][t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]
		- sum(a in isASSETS inter {"GENERALE_DU_SOLAIRE_Leforest_B_1MW_PDL_out"}, t in isDECISION_STEPS: assetStepFCRStep[t] == fcr && maxNFElecLoadPower[a][t] < 0.9 * fcrEngPower[fcr])
			fcrEngPower[fcr]);
}*/
...
// flag indicating if there is fcr engagement on decision step t.
float isFcrPeriod[t in isDECISION_STEPS] = (fcrEngPower[assetStepFCRStep[t]] > 0 ? 1 : 0);
//////
//fCFD fo GDS
//////
...
float inCFDDaPrice[h in isHOURLY_STEPS] = (optContext == "MARKET_STRAT_FCR" ? t_ref : daElecPrice[h]);
float inCFDDaPriceRaw[h in isHOURLY_STEPS] = (optContext == "MARKET_STRAT_FCR" ? t_ref : daElecPriceRaw[h]);
// CFD positive imbalance price
float inCFDPosImbPrice[imb in isIMBALANCE_STEPS_POS] = (optContext == "MARKET_STRAT_FCR" ? t_ref / 10 : positive_imb_price[imb]);
// CFD in tariff negative imbalance price
float inCFDNegImbPrice[imb in isIMBALANCE_STEPS_POS] = (optContext == "MARKET_STRAT_FCR" ? t_ref * 10 : negative_imb_price[imb]);
// HVAC hardcoded data
//For Mopabloem
{string} isTARGET_LEVELS = {"LOW", "NOMINAL", "HIGH"};
float dummyTargetLevelTemps[isTARGET_LEVELS] = [0.0, 25.0, 50.0];
float targetLevelTemps[isDISP_E_LOADS][isTARGET_LEVELS] = [f : [l : dummyTargetLevelTemps[l]] | f in isDISP_E_LOADS, l in isTARGET_LEVELS];
// For Equans
// in currency unit / kWh
float aggregation_fee = 19.20 / 1000;
//////
// Hydrogen variable price
/////
// Coefficient of proportionality of auxiliaries consumption in function of converter consumption
float auxConvConsoCoeff = microgridName == "MICROGRID JEPX Komekurayama" ?
								0.06
								: microgridName == "MICROGRID JEPX Suntory" ?
								0.05
								: 0;
// Auxiliaries base consumption (in kW)
float auxConvConsoBase = microgridName == "MICROGRID JEPX Komekurayama" ?
								40
								: microgridName == "MICROGRID JEPX Suntory" ?
								300
								: 0;
// H2 selling price. Based on electricity threshold determined by client, and auxiliaries + electrolyser efficiencies
float h2Price = microgridName == "MICROGRID JEPX Komekurayama" ?
								assetCurtComp["JEPX_KOMEKURAYAMA_Yamanashi_H2_Load"]
								: microgridName == "MICROGRID JEPX Suntory" ?
								assetCurtComp["JEPX_SUNTORY_Yamanashi_H2_Load"]
								: 0;
...
execute CPX_PARAM {
	...
	if (operationID == "1335" || operationID == "1336")
	...
	if (operationID == "1595" || operationID == "1596")
		microgridName = "MICROGRID JEPX Komekurayama";
	if (operationID == "1476")
		microgridName = "MICROGRID JEPX Suntory";
...
// (s in E_STORAGES union isNotE_STORAGES, t in DECISION_STEPS)
// 1 means charge, 0 means discharge or nothing
dvar boolean IsCharging[isE_STORAGES union isNotE_STORAGES][isDECISION_STEPS];
// flag indicating if storage asset s�s injection target for decision step t corresponds to a charge or a discharge
// (s in E_STORAGES union isNotE_STORAGES, t in DECISION_STEPS)
// 1 means discharge, 0 means charge or nothing
dvar boolean IsDischarging[isE_STORAGES union isNotE_STORAGES][isDECISION_STEPS];
...
// flag indicating if all storage assets s that have a strictly positive storMaxAvailACActivePowerCharge, are full on decision step t
// (t in DECISION_STEPS)
...
dexpr float NFLoadActivePower[n in isNF_E_LOADS][t in isDECISION_STEPS] = maxl(minl(maxNFElecLoadPower[n][t], NFElecLoadForecast[n][t]), minNFElecLoadPower[n][t]);
...
// Dispatchable Loads
//////////////////////
// average power consumption target (expressed in kW) for dispatchable load unit d over decision step t
// (d in isDISP_LOADS, t in DECISION_STEPS)
dvar float+ DispLoadPower[isDISP_LOADS][isDECISION_STEPS];
// average power active consumption target (expressed in kW) for dispatchable load unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dvar float+ DispLoadActivePower[d in isDISP_E_LOADS][t in isDECISION_STEPS] = DispLoadPower[d][t];
// active power raise reserve (expressed in kW) from dispatchable load unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dexpr float DispLoadActiveRaiseReserve[d in isDISP_E_LOADS][t in isDECISION_STEPS] = (dispLoadPartialAvail[d][t] / 100.0 == 1 ? DispLoadActivePower[d][t] - minDispLoadPower[d] : 0.0);
// active power lower reserve (expressed in kW) from dispatchable load unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dexpr float DispLoadActiveLowerReserve[d in isDISP_E_LOADS][t in isDECISION_STEPS] = (dispLoadPartialAvail[d][t] / 100.0 == 1 ? maxDispLoadPower[d] - DispLoadActivePower[d][t] : 0.0);
// max reactive power consumption (expressed in kVAR) for dispatchable load unit d over decision step t 
// (d in isDISP_E_LOADS,t in DECISION_STEPS)
dexpr float DispLoadMaxReactivePower[d in isDISP_E_LOADS][t in isDECISION_STEPS] = (
	(maxDispLoadPower[d] > 0.0 && dispLoadPartialAvail[d][t] / 100.0 == 1)
		? aDispLoadQmax[d] * DispLoadActivePower[d][t] + bDispLoadQmax[d]
		: 0.0
	);
// min reactive power consumption (expressed in kVAR) for dispatchable load unit d over decision step t 
// (d in isDISP_E_LOADS,t in DECISION_STEPS)
dexpr float DispLoadMinReactivePower[d in isDISP_E_LOADS][t in isDECISION_STEPS] = (
	(minDispLoadPower[d] > 0.0 && dispLoadPartialAvail[d][t] / 100.0 == 1)
		? aDispLoadQmin[d] * DispLoadActivePower[d][t] + bDispLoadQmin[d]
		: 0.0
	);
// reactive power consumption (expressed in kVAR) for dispatchable load unit d over decision step t 
// (d in isDISP_E_LOADS,t in DECISION_STEPS)
dvar float+ DispLoadReactivePower[isDISP_E_LOADS][isDECISION_STEPS];
// reactive power raise reserve (expressed in kVAR) from dispatchable load unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dexpr float DispLoadReactiveRaiseReserve[d in isDISP_E_LOADS][t in isDECISION_STEPS] = DispLoadReactivePower[d][t] - DispLoadMinReactivePower[d][t];
// reactive power lower reserve (expressed in kW) from dispatchable load unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dexpr float DispLoadReactiveLowerReserve[d in isDISP_E_LOADS][t in isDECISION_STEPS] = DispLoadMaxReactivePower[d][t] - DispLoadReactivePower[d][t];
// spinning raise reserve (expressed in kW) from dispatchable elc load unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dvar float+ DispLoadSpinRaiseReserve[isDISP_E_LOADS][isDECISION_STEPS];
// spinning lower reserve (expressed in kW) from dispatchable elec unit d over decision step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dvar float+ DispLoadSpinLowerReserve[isDISP_E_LOADS][isDECISION_STEPS];
///////////////////////////////
// dispatchable elec generators
///////////////////////////////
//flag indicating if a dispatchable asset is on at decision step t
// 1 means it is on, 0 that it is off
// (a in DISP_Asset, t in DECISION_STEPS)
dvar boolean IsDispAssetOn[isDISP_ASSETS][isDECISION_STEPS];
...
// flag indicating if piecewise linear efficency model segment s is used for asset a over decision step t 
// 1 means it is used, 0 that it is not
// (a in isCONV_NL_EFF_ASSETS, s in VAR_COST_SEGS, t in DECISION_STEPS)
dvar boolean NLEffSegFlag[isCONV_NL_EFF_ASSETS][s in 1..maxSegNbr][isDECISION_STEPS];
// part (power in kw) of non-linear variable cost model segment s is used for generator g over decision step t
// (a in isCONV_NL_EFF_ASSETS, s in VAR_COST_SEGS, t in DECISION_STEPS)
dvar float+ NLEffSegPower[isCONV_NL_EFF_ASSETS][s in 1..maxSegNbr][isDECISION_STEPS];
...
// indicator giving evolution of each dispatchable asset a's status between decision step t and the previous one. 
// -1 means d is shut down at t, 0 means d stays on or stays off, and +1 means d is started
// (a in isDISP_ASSETS ,t in DECISION_STEPS)
dvar int DispAssetOnEvol[isDISP_ASSETS][isDECISION_STEPS];
// flag indicating if asset energy generator a is started up at decision step t
// 1 means it is started up, 0 that it is not
dvar boolean DispAssetStartup[isDISP_ASSETS][isDECISION_STEPS];
// active power raise reserve (expressed in kW) from dispatchable generator d over decision step t
// (d in DISP_GENS, t in DECISION_STEPS)
dexpr float DispGenActiveRaiseReserve[d in isDISP_E_GENS][t in isDECISION_STEPS] = IsDispAssetOn[d][t] * maxDispGenActivePower[d] - DispGenActivePower[d][t];
// max reactive power generation (expressed in kVAR) for dispatchable generator d over decision step t 
// (d in DISP_GENS,t in DECISION_STEPS)
dexpr float DispGenMaxReactivePower[d in isDISP_E_GENS][t in isDECISION_STEPS] = aDispGenQmax[d] * DispGenActivePower[d][t] + IsDispAssetOn[d][t] * bDispGenQmax[d];
...
// Disp Assets
//////////////////////
// average elec generation/consumption (expressed in kW) for dispatchable assset d over decision step t
dexpr float DispAssetActivePower[d in isDISP_E_ASSETS][t in isDECISION_STEPS] = (d in isDISP_E_GENS ? DispGenActivePower[d][t] : DispLoadActivePower[d][t]); 
//////////////////////
// FLEX Loads
//////////////////////
// average power modulation target (expressed in kW) for flex load unit f away from its nominal consumption forecast
// over decision step t (f in FLEX_LOADS, t in DECISION_STEPS)
...
//////////////////////
// LOAD = FLEX_LOAD union DISP_LOAD union NF_LOAD
// currently FLEX_LOAD are included in DISP_LOAD
/////////////////////
// target elec power consumption of all elec Loads
dexpr float LoadActivePower[a in isEIN_LOADS][t in isDECISION_STEPS] = (a in isNF_E_LOADS ? NFLoadActivePower[a][t] : DispLoadActivePower[a][t]);
// After separating FLEX_LOAD and DISP_LOAD, you should instead go through 
//dexpr float LoadActivePower[a in isEIN_LOADS][t in isDECISION_STEPS] = (a in isNF_E_LOADS ? NFLoadActivePower[a][t] : (a in isDISP_E_LOADS ? DispLoadActivePower[a][t] : FlexLoadActivePower[a][t]));
// Energy (Heat & Hydrigen) storage assets
//////////////////////
// average (Not_elec_energy: heat & hydrogen) power injection target (expressed in kW) into the microgrid for storage asset s over decision step t
// (s in isNotE_STORAGES, t in DECISION_STEPS)
dvar float+ StorNotElecPowerDischarge[isNotE_STORAGES][isDECISION_STEPS];
// average (Not_elec_energy: heat & hydrogen) extraction target (expressed in kW) from the microgrid for storage asset s over decision step t
// (s in isNotE_STORAGES, t in DECISION_STEPS)
dvar float+ StorNotElecPowerCharge[isNotE_STORAGES][isDECISION_STEPS];
// incremental Not_elec_energy charge / discharge target (expressed in kWh) for storage asset s over decision step t
// (s in isNotE_STORAGES, t in DECISION_STEPS)
// positive values mean energy inputs into the asset, negative values mean energy output from the asset
dvar float StorStepNotElecPowerIn[isNotE_STORAGES][isDECISION_STEPS];
// Not_elec_energy energy charge target (expressed in kWh) for storage asset s at the end of decision step t
// (s in isNotE_STORAGES, t in DECISION_STEPS)
dvar float+ StorStoredEnergy[isNotE_STORAGES][isDECISION_STEPS];
// average Not_elec_energy power target (expressed in kW) for storage asset s over decision step t
// (s in isNotE_STORAGES, t in DECISION_STEPS)
// positive values = power injections (discharges) and negative values = power extractions (charges)
dexpr float StorNotElecOutInPower[s in isNotE_STORAGES][t in isDECISION_STEPS] = StorNotElecPowerDischarge[s][t] - StorNotElecPowerCharge[s][t];
// Soc target (expressed as a % of asset's maximum Not_elec_energy storage capacity) for Not_elec_energy asset storage s over decision step t
// (a in isNotE_STORAGES, t in DECISION_STEPS)
dexpr float NotElecEnergyStorSocTarget[h in isNotE_STORAGES][t in isDECISION_STEPS] = 100 * (availStorNotElecMaxCharge[h][t] != 0 ? StorStoredEnergy[h][t] / availStorNotElecMaxCharge[h][t] : 0);
// Hydrogen energy, FI: to be generalized with other stor energy asset (almost same constraints for all stor asset)
// dvar float+ StorEnergyCharge[isENERGY_STOR][isSTORAGES][isDECISION_STEPS]
// tuple store_Energy_types {
//    key string op_id;
// }
// set representing the storage operation directions
// {store_Energy_types} STORE_ENERGY_TYPES = {<"HEAT">, <"HYDROGEN">, <"Elec">};
// indexing of Storage Energy types
// {string} isSTORE_ENERGY_TYPES = {o.op_id | o in STORE_OPERATION_TYPES};
////////////////////
// Energy converters
////////////////////
// average elec power consumption (expressed in kW) for energy converter c over decision step t (auxiliaries consumptions)
// (c in EIN_CONVS, t in DECISION_STEPS)
dexpr float ConvActivePowerIn[c in isEIN_CONVS][t in isDECISION_STEPS] = LoadActivePower[c][t];
// average power generation (expressed in kW) for energy converter c over decision step t
// (c in HOUT_CONVS union isE_H2_CONVS, t in DECISION_STEPS)
dvar float+ ConvPowerOut[isHOUT_CONVS union isE_H2_CONVS][t in isDECISION_STEPS];
// average power generation (expressed in kW) for energy converter c over decision step t
// (c in HOUT_CONVS, t in DECISION_STEPS)
dexpr float ConvHeatOut[h in isHOUT_CONVS][t in isDECISION_STEPS] = ConvPowerOut[h][t];
// average power generation (expressed in kW) for energy converter c over decision step t
// (c in isH2_E_CONVS, t in DECISION_STEPS) 
dexpr float ConvH2Out[h2 in isE_H2_CONVS][t in isDECISION_STEPS] = ConvPowerOut[h2][t];
/* Market*/
////////////////////////////////
...
// average hydrogen heat deficit cost over decision step t (t in DECISION_STEPS)
...
// average hydrogen Excess cost over decision step t (t in DECISION_STEPS)
...
// SOC min deficit expressed as a percentage (s in NotE_STORAGES, t in DECISION_STEPS)
dvar float+ NotElecSOCminDeficit[isNotE_STORAGES][isDECISION_STEPS];
// SOC min deficit cost (s in NotE_STORAGES, t in DECISION_STEPS)
dexpr float NotElecSOCminDeficitCost[s in isNotE_STORAGES][t in isDECISION_STEPS] = socStrictMinMaxViolationPenaltyCost * availStorNotElecMaxCharge[s][t] * NotElecSOCminDeficit[s][t]/100.0;
// SOC max excess expressed as a percentage (s in NotE_STORAGES, t in DECISION_STEPS)
dvar float+ NotElecSOCmaxExcess[isNotE_STORAGES][isDECISION_STEPS];
// SOC max excess cost (s in NotE_STORAGES, t in DECISION_STEPS)
dexpr float NotElecSOCmaxExcessCost[s in isNotE_STORAGES][t in isDECISION_STEPS] = socStrictMinMaxViolationPenaltyCost * availStorNotElecMaxCharge[s][t] * NotElecSOCmaxExcess[s][t]/100.0;
// SOC min deficit expressed as a percentage (s in H2_STORAGES, t in DECISION_STEPS)
dvar float+ H2SOCminDeficit[isH2_STORAGES][isDECISION_STEPS];
// SOC min deficit cost (s in H2_STORAGES, t in DECISION_STEPS)
dexpr float H2SOCminDeficitCost[s in isH2_STORAGES][t in isDECISION_STEPS] = socStrictMinMaxViolationPenaltyCost * storMaxH2Charge[s] * H2SOCminDeficit[s][t]/100.0;
// SOC max excess expressed as a percentage (s in H2_STORAGES, t in DECISION_STEPS)
dvar float+ H2SOCmaxExcess[isH2_STORAGES][isDECISION_STEPS];
// SOC max excess cost (s in H2_STORAGES, t in DECISION_STEPS)
dexpr float H2SOCmaxExcessCost[s in isH2_STORAGES][t in isDECISION_STEPS] = socStrictMinMaxViolationPenaltyCost * storMaxH2Charge[s] * H2SOCmaxExcess[s][t]/100.0;
...
// (d in DISP_GENS)
dvar float+ DispAssetInitialPowerUpViolation[isDISP_E_ASSETS];
dvar float+ DispAssetInitialPowerDwnViolation[isDISP_E_ASSETS];
dexpr float DispAssetInitialPowerUpViolationCost[d in isDISP_E_ASSETS] = dispGenInitialPowerViolPenalty * DispAssetInitialPowerUpViolation[d] * assetStepDurationInHours;
dexpr float DispAssetInitialPowerDwnViolationCost[d in isDISP_E_ASSETS] = dispGenInitialPowerViolPenalty * DispAssetInitialPowerDwnViolation[d] * assetStepDurationInHours;
// economic min active power violation for dispatchable assset d over step t
// (d in isDISP_E_ASSETS,t in DECISION_STEPS)
dvar float+ DispAssetMinActivePowerDeficit[isDISP_E_ASSETS][isDECISION_STEPS];
// cost of the violation of the economic min active power for generator d over step t
// (d in DISP_GENS,t in DECISION_STEPS)
dexpr float DispAssetMinActivePowerDeficitCost[d in isDISP_E_ASSETS][t in isDECISION_STEPS] = minDispGenActivePowerDeficitPenalty * assetStepDurationInHours * DispAssetMinActivePowerDeficit[d][t];
...
// active power lower reserve deficit to cover sudden drop in consumption from dispatchable load asset d over step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dvar float+ DispLoadActivePowerLowerReserveDeficit[isDISP_E_LOADS][isDECISION_STEPS];
// cost of the active power lower reserve deficit to cover sudden drop in consumption from dispatchable load asset d over step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dexpr float DispLoadActivePowerLowerReserveDeficitCost[d in isDISP_E_LOADS][t in isDECISION_STEPS] = activePowerLowerReserveDeficitPenaltyCost * assetStepDurationInHours * DispLoadActivePowerLowerReserveDeficit[d][t];
...
// reactive power lower reserve deficit to cover sudden drop in consumption from dispatchable load asset d over step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dvar float+ DispLoadReactivePowerLowerReserveDeficit[isDISP_E_LOADS][isDECISION_STEPS];
// cost of the reactive power lower reserve deficit to cover sudden drop in consumption from dispatchable load asset d over step t
// (d in isDISP_E_LOADS, t in DECISION_STEPS)
dexpr float DispLoadReactivePowerLowerReserveDeficitCost[d in isDISP_E_LOADS][t in isDECISION_STEPS] = reactivePowerLowerReserveDeficitPenaltyCost * assetStepDurationInHours * DispLoadReactivePowerLowerReserveDeficit[d][t];
...
// violation of maximum time allowed to be continuously on by dispatchable asset d over step t
// (d in isDISP_ASSETS, t in DECISION_STEPS)  
dvar float+ DispAssetMaxStepOnInitialExcess[isDISP_ASSETS][i in 0..(maxDispInitialStepsOnMax-1)];
dvar float+ DispAssetMaxStepOnExcess[isDISP_ASSETS][isDECISION_STEPS];
//MAxStepOnInitialExcess and MaxStepOnExcess violation cost
// (d in isDISP_ASSETS, t in DECISION_STEPS)
dexpr float DispAssetMaxStepOnInitialExcessCost[d in isDISP_ASSETS][i in 0..(maxDispInitialStepsOnMax-1)] = dispMinMaxStepOnOffPenaltyCost  * assetStepDurationInHours *  maxDispAssetPower[d]* DispAssetMaxStepOnInitialExcess[d][i];
dexpr float DispAssetMaxStepOnExcessCost[d in isDISP_ASSETS][t in isDECISION_STEPS] = dispMinMaxStepOnOffPenaltyCost * assetStepDurationInHours * maxDispAssetPower[d] * DispAssetMaxStepOnExcess[d][t];
// violation of minimum time allowed to be continuously on by dispatchable asset d over step t
// (d in isDISP_ASSETS, t in DECISION_STEPS)  
dvar float+ DispAssetMinStepOnInitialDeficit[isDISP_ASSETS];
dvar float+ DispAssetMinStepOnDeficit[isDISP_ASSETS][isDECISION_STEPS];
// cost of the violation of minimum time allowed to be continuously on by dispatchable asset d over step t
// (d in isDISP_ASSETS, t in DECISION_STEPS)
dexpr float DispAssetMinStepOnInitialDeficitCost[d in isDISP_ASSETS] = dispMinMaxStepOnOffPenaltyCost * assetStepDurationInHours * maxDispAssetPower[d] * DispAssetMinStepOnInitialDeficit[d];
dexpr float DispAssetMinStepOnDeficitCost[d in isDISP_ASSETS][t in isDECISION_STEPS]  = dispMinMaxStepOnOffPenaltyCost * assetStepDurationInHours * maxDispAssetPower[d] * DispAssetMinStepOnDeficit[d][t];
// violation of minimum time allowed to be off between 2 consecutive uses for dispatchable asset d over step t
// (d in isDISP_ASSETS, t in DECISION_STEPS)  
dvar float+ DispAssetMinStepOffInitialDeficit[isDISP_ASSETS];
dvar float+ DispAssetMinStepOffDeficit[isDISP_ASSETS][isDECISION_STEPS];
// Cost of the violation of minimum time allowed to be off between 2 consecutive uses for dispatchable asset d over step t
// (d in isDISP_ASSETS, t in DECISION_STEPS)
dexpr float DispAssetMinStepOffInitialDeficitCost[d in isDISP_ASSETS] = dispMinMaxStepOnOffPenaltyCost * assetStepDurationInHours * DispAssetMinStepOffInitialDeficit[d] * maxDispAssetPower[d];
dexpr float DispAssetMinStepOffDeficitCost[d in isDISP_ASSETS][t in isDECISION_STEPS] = dispMinMaxStepOnOffPenaltyCost * assetStepDurationInHours * maxDispAssetPower[d] * DispAssetMinStepOffDeficit[d][t];
...
// isSTART_COST_DISP_E_GENS inter isDISP_E_GENS to avoid configuration pb like an asset, since we have no control over the simultaneous entry of the startUpcost and min/max TimeOn/Off fields
dexpr float DispGenTotalStartupCosts = sum(t in isDECISION_STEPS, d in isSTART_COST_DISP_E_GENS inter isDISP_E_GENS) DispAssetStartup[d][t] * dispGenStartupCost[d];
// total starting costs from Hydrogen generators over optimisation window
// isSTART_COST_DISP_E_GENS inter isDISP_E_GENS to avoid configuration pb like an asset, since we have no control over the simultaneous entry of the startUpcost and min/max TimeOn/Off fields
dexpr float DispLoadTotalStartupCosts = sum(t in isDECISION_STEPS, d in isSTART_COST_DISP_E_LOADS inter isDISP_E_LOADS) DispAssetStartup[d][t] * dispLoadStartupCost[d];
// total curtailment costs from intermittent generation over optimisation window
dexpr float InterGenTotalCurtCosts = sum(t in isDECISION_STEPS, i in isCURT_COMP_E_GENS inter isINTER_E_GENS) interGenCurtComp[i] * InterGenCurtEstimation[i][t] * assetStepDurationInHours;
		...
		? sum(t in isDECISION_STEPS, s in isE_STORAGES)
 		...
 		: (
 			microgridName == "MICROGRID TPL Tongatapu"																	// for Tonga, we want to encourage early discharge sor Matatoa BESS only (see storArtificialPenalityCost param)
 				...
 				: (
 					microgridName == "MICROGRID JEPX Komekurayama"															// For Komekurayama, we want to early charge
 					? sum(t in isDECISION_STEPS, s in isSTORAGES : s not in isE_STORAGES)															
 					storArtificialPenalityCost[t] * StorNotElecPowerCharge[s][t] * assetStepDurationInHours
...
// Violation of the Engaged FCR Power. this decision variable is used in cases where the power engaged for the asset fcr d is greater than its certified power to relax the ctFCRMaxActivatedPower constraint.
...
dexpr float maxAvailPowerTargetForNoFCR[fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS][t in isDECISION_STEPS] = maxAvailDispAssetPower[fcr_a][t] - FCRPower[fcr_a][assetStepFCRStep[t]];
//////
// CFD
//////
// Total Cost of energy to be sold on the market Spot, for asset not not subject to the CFD
...
// Revenues of energy to be sold on the market Spot, in the case of a contract for differnce without M0 arbitratio, with a fixed  price.
...
dexpr float GenTotalLinearVarCosts =
    ...
    + sum(t in isDECISION_STEPS, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s] + StorStepFCRCycle[s][t] * cyclingVariableCostFCR[s]) * nomEnergyMax[s] * assetStepDurationInHours;
...
dexpr float OptimisedGenLinearVarCosts =
    ...
    + sum(t in isDECISION_STEPS: isDAStepCleared[assetStepHourlyStep[t]] == 0, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s] + StorStepFCRCycle[s][t] * cyclingVariableCostFCR[s]) * nomEnergyMax[s] * assetStepDurationInHours;
...
// Hydrogen costs (usually a profit, then negative)
dexpr float H2EnergyTotalCosts = -1.0 * sum(c in isE_H2_CONVS, t in isDECISION_STEPS) ConvH2Out[c][t] * h2Price * assetStepDurationInHours;
// H2 costs over steps that are not alredy cleared for day-ahead spot market
dexpr float OptimisedH2EnergyCosts = -1.0 * sum(c in isE_H2_CONVS, t in isDECISION_STEPS : isDAStepCleared[assetStepHourlyStep[t]] == 0) ConvH2Out[c][t] * h2Price * assetStepDurationInHours;
...
constraint ctNLEffSegUpBound[isCONV_NL_EFF_ASSETS][2..maxSegNbr][isDECISION_STEPS];
constraint ctNLEffSegLowBound[isCONV_NL_EFF_ASSETS][2..(maxSegNbr-1)][isDECISION_STEPS];
...
minimize
  ...
  + H2EnergyTotalCosts
  + sum(d in isDISP_E_GENS) (DispAssetInitialPowerUpViolationCost[d] + DispAssetInitialPowerDwnViolationCost[d])
  ...
  + sum(s in isNotE_STORAGES, t in isDECISION_STEPS) (NotElecSOCminDeficitCost[s][t] + NotElecSOCmaxExcessCost[s][t])
  + sum(s in isH2_STORAGES, t in isDECISION_STEPS) (H2SOCminDeficitCost[s][t] + H2SOCmaxExcessCost[s][t])
  ...
  + sum(d in isDISP_ASSETS, i in 0..(dispInitialStepsOnMax[d]-1)) DispAssetMaxStepOnInitialExcessCost[d][i]
  + sum(d in isDISP_ASSETS, t in isDECISION_STEPS) DispAssetMaxStepOnExcessCost[d][t]
  + sum(d in isDISP_ASSETS) DispAssetMinStepOnInitialDeficitCost[d]
  + sum(d in isDISP_ASSETS, t in isDECISION_STEPS) DispAssetMinStepOnDeficitCost[d][t]
  + sum(d in isDISP_ASSETS) DispAssetMinStepOffInitialDeficitCost[d]
  + sum(d in isDISP_ASSETS, t in isDECISION_STEPS) DispAssetMinStepOffDeficitCost[d][t]
  + sum(d in isDISP_E_ASSETS, t in isDECISION_STEPS) DispAssetMinActivePowerDeficitCost[d][t]
  ...
  + sum(d in isDISP_E_LOADS, t in isDECISION_STEPS) DispLoadActivePowerLowerReserveDeficitCost[d][t]
  ...
  + sum(d in isDISP_E_LOADS, t in isDECISION_STEPS) DispLoadReactivePowerLowerReserveDeficitCost[d][t]
 ...
 *********************************************************************/
  subject to {  
	//forall (t in isDECISION_STEPS : 5 <= ord(isDECISION_STEPS, t) <= 20)
		//ctTest1: ConvActivePowerIn["JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"][t] == 0;
	/*forall (t in isDECISION_STEPS)
		ctTest2: StorACActivePower["JEPX_KOMEKURAYAMA_Yamanashi_H2_Load"][t] == 100;*/
	/*forall (t in isDECISION_STEPS: 76 <= ord(isDECISION_STEPS, t) <=79)
		ctTest3: StorACActivePower["VALECO_Baignollais_BESS_bis"][t] == -1000;*/
//	ctTest2: ConvActivePowerIn["JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"]["47"] >= 1000.0;
//	forall (t in isDECISION_STEPS)
//	  ctTest: PowerDeficit[t] <= 0;
//	forall (t in isDECISION_STEPS, d in isDISP_E_GENS: ord(isDECISION_STEPS, t) <= 100)
//	forall (t in isDECISION_STEPS: 43 <= ord(isDECISION_STEPS, t) <= 44)
//		ctTest1: StorACPowerCharge["VALOREM_Limoux_BESS"][t] >= 1000;
//	ctTest2: DispGenActivePower["CHP"]["1"] >= 100;
...
//	  ctTest2: IsDispAssetOn[d][t] <= 0;
//	forall (t in isDECISION_STEPS)
//		ctTest: SOCstrictMinDeficit["VALOREM_Limoux_BESS"][t] <= 0;
//	ctTest: NotInCFDNegativeImbalancePower_imb ["37"] == 200;
...
// and imported from the main grid must balance with the power consumed by dispatchable
	...
	forall (t in isDECISION_STEPS)
	  ctPowerBalance:
	  	...
	  	+ NetElecImportTarget[t] ==
	  	  sum(d in isDISP_E_LOADS) DispLoadActivePower[d][t]
...
// No imbalance when no engagement (step not cleared) for some projects
 	if (microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory") {
 		forall (imb in isIMBALANCE_STEPS_POS : isDAStepCleared[imbStepHourlyStep[imb]] == 0) 	
 	 	ctNoImbalanceWhenBidding: 
 	 		ImbalancePower_imb[imb] == 0;
 	}

// FCR engagement must be equal to the sum of assets' FCRPower
	forall (fcr in isFCR_STEPS_POS)
	  ctFCRPoolEngagement :
	  	fcrReqPower[fcr] == sum(a in isFCR_ASSETS) FCRPower[a][fcr] + FCRPowerEngDeficit[fcr];

// An storage asset's engaged power cannot be higher than its certified power
	forall (a in isFCR_ASSETS inter isE_STORAGES, fcr in isFCR_STEPS_POS)
	  ctFCRMaxActivatedPower:
	  	FCRPower[a][fcr] <= fcrCertfiedPower[a] * IsFCR[a][fcr];

// @Duplicated("to be generalized with the ctFCRMaxActivatedPower constraint ")
// An dispAsset's engaged power cannot be higher than its Availcertified power
	forall (fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS, fcr in isFCR_STEPS_POS)
	  ctDispAssetFCRMaxActivatedPower:
	  	FCRPower[fcr_a][fcr] <= fcrAvailCertfiedPower[fcr_a][fcr] * IsFCR[fcr_a][fcr];
		
// non-fcr bandwidth for assets participating in the fcr market (diff from isE_STORAGES: isE_STORAGES are not involved in this model)
	forall(fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS, t in isDECISION_STEPS)
		ctNoFCRMinActivePower:
			DispAssetActivePower[fcr_a][t] >= minAvailDispAssetPower[fcr_a][t] * IsDispAssetOn[fcr_a][t] + FCRPower[fcr_a][assetStepFCRStep[t]];
	forall(fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS, t in isDECISION_STEPS)
		ctNoFCRMaxActivePower:
			DispAssetActivePower[fcr_a][t] <= maxAvailDispAssetPower[fcr_a][t] - FCRPower[fcr_a][assetStepFCRStep[t]];

// To be generalized to include ctDispGenPowerUp an ctDispGenPowerDown constraint
// Currently for JEPX, for electrolyzers to perform regulation properly, the target must be reached 30 minutes before the fcr commitment period, 
// assetStepDuration of 30 min in Japon.
// first step 
// Only works for elec consumption assets, will have to review the availability part for production ones (taking minAvail)
if (microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory") { 
	forall (fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS: dispAssetInitialState[fcr_a] >= 1 && isFcrPeriod[first(isDECISION_STEPS)]==1 && availGlobalForPmax[fcr_a][first(isDECISION_STEPS)]>= 0.9)
	  ctDispAssetInitFCR:
	  	dispAssetInitialPower[fcr_a] == DispAssetActivePower[fcr_a][first(isDECISION_STEPS)];
// other steps 
	forall (fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS, t in isDECISION_STEPS diff {first(isDECISION_STEPS)})
	  ctFCRPowerTargetBeforeRegulationDownLimit:
	  	DispAssetActivePower[fcr_a][prev(isDECISION_STEPS, t)] >= DispAssetActivePower[fcr_a][t] * isFcrPeriod[t];
	forall (fcr_a in isFCR_ASSETS inter isDISP_E_ASSETS, t in isDECISION_STEPS diff {first(isDECISION_STEPS)})
	  ctFCRPowerTargetBeforeRegulationUpLimit:
	  	DispAssetActivePower[fcr_a][prev(isDECISION_STEPS, t)] <= DispAssetActivePower[fcr_a][t] * isFcrPeriod[t] + maxAvailPowerTargetForNoFCR[fcr_a][prev(isDECISION_STEPS, t)] * (1 - isFcrPeriod[t]);
}
/* POWER IMPORTS */
...
// only applicable to disp gen with non-zero minActivePower (bc if minActivePower(d) = 0 then DispGenActivePower(d) can be = 0 and d considered to be on (IsDispAssetOn(d) = 1))  
	forall (d in isDISP_E_ASSETS: minDispAssetActivePower[d] > 0.0, t in isDECISION_STEPS)
	  ctDispAssetEcoMinPower:
	  	 DispAssetActivePower[d][t] >= minDispAssetActivePower[d] * IsDispAssetOn[d][t] - DispAssetMinActivePowerDeficit[d][t];
	  	 
	forall (d in isDISP_E_GENS: minDispAssetActivePower[d] > 0.0, t in isDECISION_STEPS)
	  ctDispAssetPhysMinPower:
	  	minDispAssetActivePower[d] * IsDispAssetOn[d][t] - DispAssetMinActivePowerDeficit[d][t] >= physMinDispAssetActivePower[d] * IsDispAssetOn[d][t];
	
	forall(d in isDISP_E_ASSETS, t in isDECISION_STEPS)
	  ctDispAssetMaxPower:
	  	DispAssetActivePower[d][t] <= maxDispAssetActivePower[d] * IsDispAssetOn[d][t];
	  	
	forall (d in isDISP_E_ASSETS: minDispAssetActivePower[d] == 0.0, t in isDECISION_STEPS)
	  ctDispAssetMinPower:
	  	DispAssetActivePower[d][t] >= IsDispAssetOn[d][t];
	  	
// dispatchable asset cannot be on if maximum power generation is zero for this asset or if asset is unavailable
	forall (d in isDISP_E_ASSETS, t in isDECISION_STEPS)
	  ctDispAssetZeroMax:
	  	IsDispAssetOn[d][t] <= (maxDispAssetActivePower[d] > 0.0 && scheduledAvailability[d][t] > 0.01 ? 1 : 0);

// if possible, keep first step's average power generation for dispatchable generator d the same as it was initially
// to be included in ctDispASSETPowerUp and ctDispASSETPowerUp constraints (ctDispASSETPowerUp -> ctDispAssetPowerUp && ctDispASSETPowerDown -> ctDispAssetPowerDown)
// (d in DISP_GENS)
	forall (d in isDISP_E_GENS: dispAssetInitialState[d] >= 1 && dispGenAvail[d][first(isDECISION_STEPS)] == 1)
	  ctDispGenPowerUp:
	  	dispAssetInitialPower[d] - DispGenActivePower[d][first(isDECISION_STEPS)] >= -DispAssetInitialPowerUpViolation[d];
	forall (d in isDISP_E_GENS: dispAssetInitialState[d] >= 1 && dispGenAvail[d][first(isDECISION_STEPS)] == 1)
	  ctDispGenPowerDwn:
	  	dispAssetInitialPower[d] - DispGenActivePower[d][first(isDECISION_STEPS)] <= DispAssetInitialPowerDwnViolation[d];
...
// Piecewise Linear Efficiency Model 
// Segment Upper bounds
// first segment: if first segment is used, power on first segment must be lower than first segment's upper limit  
	forall (a in isCONV_NL_EFF_ASSETS, t in isDECISION_STEPS)
	  ctNLEffSegUpBound1:  	
	  	NLEffSegPower[a][1][t] <= NLEffSegFlag[a][1][t] * varCostSegUpLim[assetVarEffModelId[a]][1];

// other segments: if segment s is used, power on s must be lower than the difference between s's upper limit and s-1's upper limit
	forall (a in isCONV_NL_EFF_ASSETS, s in 2..varCostModelSegNumber[assetVarEffModelId[a]], t in isDECISION_STEPS)
	  ctNLEffSegUpBound[a][s][t]:
	  	NLEffSegPower[a][s][t] <= NLEffSegFlag[a][s][t] * (varCostSegUpLim[assetVarEffModelId[a]][s] - varCostSegUpLim[assetVarEffModelId[a]][s-1]);

// Segment Lower bounds
// first segment: if second segment 2 is used, first segment must be completely used and so power on first segment must be equal to first segment's upper bound. 
	forall (a in isCONV_NL_EFF_ASSETS: varCostModelSegNumber[assetVarEffModelId[a]] >= 2, t in isDECISION_STEPS)
	  ctNLEffSegLowBound1:  	
	  	NLEffSegPower[a][1][t] >= NLEffSegFlag[a][2][t] * varCostSegUpLim[assetVarEffModelId[a]][1];
	  	
// other segments:  if segment s+1 is used, segment s must be completely used and so power on s must be equal to the difference between s's upper bound and s-1's upper bound
	forall (a in isCONV_NL_EFF_ASSETS, s in 2..(varCostModelSegNumber[assetVarEffModelId[a]]-1), t in isDECISION_STEPS)
	  ctNLEffSegLowBound[a][s][t]:  	
	  	NLEffSegPower[a][s][t] >= NLEffSegFlag[a][s+1][t] * (varCostSegUpLim[assetVarEffModelId[a]][s] - varCostSegUpLim[assetVarEffModelId[a]][s-1]);


// power consumed by asset a over step t
// (a in isCONV_NL_EFF_ASSETS, t in isDECISION_STEPS)
	forall (a in isCONV_NL_EFF_ASSETS, t in isDECISION_STEPS)
	  ctNLEffSegPowerInSum:
	  	ConvActivePowerIn[a][t] == sum (s in 1..varCostModelSegNumber[assetVarEffModelId[a]]) NLEffSegPower[a][s][t];

// efficient power generated or converted of asset a over step t
// (a in isCONV_NL_EFF_ASSETS, t in isDECISION_STEPS)
	forall (a in isCONV_NL_EFF_ASSETS, t in isDECISION_STEPS)
	  ctNLEffSegPowerOutSum:
	  	ConvPowerOut[a][t] == sum (s in 1..varCostModelSegNumber[assetVarEffModelId[a]]) nlEffSegSlope[assetVarEffModelId[a]][s] * NLEffSegPower[a][s][t];
...
// -1 means d is shut down at t, 0 means d stays on or stays off, and +1 means d is started.
// first steps, the DispAssetEvol on the step 1 must be equal to Zero if the dispGen is not available during this step, for this we must add the availabilityDisp == 1 as a condition in this contraint
// Disagree on this : since minTimeOn constraint on firsts steps is neutralized if there is unavailibility : it is not necessary to put 0.
	forall (a in isDISP_ASSETS : scheduledAvailability[a][first(isDECISION_STEPS)] > 0)
	  ctDispAssetEvol1 :
	  	DispAssetOnEvol[a][first(isDECISION_STEPS)] - IsDispAssetOn[a][first(isDECISION_STEPS)] == -dispAssetInitialState[a];
...
// other steps, we should not add  availabilityDisp == 1 as a condition in this contraint, otherwise we will not taken into consideration the evolution of this asset between the two step   	
  	forall (d in isDISP_ASSETS, t in isDECISION_STEPS diff {first(isDECISION_STEPS)})
  	  ctDispEvol :
	  	DispAssetOnEvol[d][t] == IsDispAssetOn[d][t] - IsDispAssetOn[d][prev(isDECISION_STEPS, t)];

// dispatchable asset d is started up if its status evolution is +1 (1 at t and 0 at t-1)
// DispAssetOnEvol is equal to -1, 0 or 1, but DispAssetStartup 0 or 1. It ensures
	forall (d in isDISP_ASSETS, t in isDECISION_STEPS)
	  ctDispAssetOnEvol1 :
	  	DispAssetOnEvol[d][t] <= DispAssetStartup[d][t];
	  	
// Maximum number of steps each disp asset can be continuously on
// initial steps
	forall (d in isDISP_ASSETS: dispAssetInitialState[d] > 0, i in rgInitialStepOffset: i <= dispInitialStepsOnMax[d])
	  ctMaxStepsOn0:
	  	sum (j in i..(dispInitialStepsOnMax[d]-1)) 1 +
	  	sum (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= dispAssetMaxStepsOn[d] - dispInitialStepsOnMax[d] + i) IsDispAssetOn[d][t]
	  	<= dispAssetMaxStepsOn[d] + DispAssetMaxStepOnInitialExcess[d][i];
	  	
// other steps
	forall (d in isDISP_ASSETS: dispAssetMaxStepsOn[d] > 0, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= card(isDECISION_STEPS) - dispAssetMaxStepsOn[d] - 1)
	  ctMaxStepsOn:
	  	sum (tt in isDECISION_STEPS: ord(isDECISION_STEPS, t) <=  ord(isDECISION_STEPS, tt) <= ord(isDECISION_STEPS, t) + dispAssetMaxStepsOn[d])
	  		IsDispAssetOn[d][tt] <= dispAssetMaxStepsOn[d] + DispAssetMaxStepOnExcess[d][t];
	  	
// Minimum number of steps each disp asset must be continuously on
// we can't penalize the min step on of disp asset, if it is not available during a t
// initial steps
	forall (d in isDISP_ASSETS: dispAssetInitialState[d] > 0 && dispAssetMinStepsOn[d] > 0 && dispInitialStepsOnMin[d] < dispAssetMinStepsOn[d])
	  ctMinStepsOn0:
	  	dispInitialStepsOnMin[d] +
	  	sum (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= dispAssetMinStepsOn[d] - dispInitialStepsOnMin[d] - 1) (IsDispAssetOn[d][t] + dispAssetTotUnavail[d][t])
	  	>= dispAssetMinStepsOn[d] - DispAssetMinStepOnInitialDeficit[d];
// other steps
	forall (d in isDISP_ASSETS: dispAssetMinStepsOn[d] > 0, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= card(isDECISION_STEPS) - dispAssetMinStepsOn[d])
	  ctMinStepsOn:
	  	sum (tt in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= ord(isDECISION_STEPS, tt) <= ord(isDECISION_STEPS, t) + dispAssetMinStepsOn[d] - 1)
	  		IsDispAssetOn[d][tt] >= dispAssetMinStepsOn[d] * DispAssetOnEvol[d][t] - DispAssetMinStepOnDeficit[d][t];

// Minimum number of steps each disp asset must be off between 2 consecutive uses
// initial steps
	forall (d in isDISP_ASSETS: dispAssetInitialState[d] <= 0 && 0 < dispMinRecoverySteps[d])
	  ctMinStepsOff0:
	  	sum (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= dispMinRecoverySteps[d] - dispInitialStepsOff[d] - 1) IsDispAssetOn[d][t]
	  	<= DispAssetMinStepOffInitialDeficit[d];
// other steps
	forall (d in isDISP_ASSETS: dispMinRecoverySteps[d] > 0, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= card(isDECISION_STEPS) - dispMinRecoverySteps[d])
	  ctMinStepsOff:
	  	sum (tt in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= ord(isDECISION_STEPS, tt) <= ord(isDECISION_STEPS, t) + dispMinRecoverySteps[d] - 1)
	  		IsDispAssetOn[d][tt] <= dispMinRecoverySteps[d] * (1 + DispAssetOnEvol[d][t]) + DispAssetMinStepOffDeficit[d][t];
...
//	  	DispGenMaxReactivePower[d][t] == aQmax[d] * DispGenActivePower[d][t] + IsDispAssetOn[d][t] * bQmax[d];
	...
	forall (d in isDISP_E_GENS inter isGRID_FOLL, t in isDECISION_STEPS)
	  ctGFLDispGenMaxSpinRaiseReserve:
	  	DispGenSpinRaiseReserve[d][t] <= maxDispGenActivePower[d] *
	  		( sum (d1 in isDISP_E_GENS inter isGRID_FORM) IsDispAssetOn[d1][t]
	  		+ sum (s in isE_STORAGES inter isGRID_FORM: (storMaxAvailACActivePowerDischarge[s][t] + storMaxAvailACActivePowerCharge[s][t]) > 0.0) 1
	  		+ sum (d in isDISP_E_LOADS inter isGRID_FORM: maxDispLoadPower[d] > 0.0 && dispLoadPartialAvail[d][t] > 0.0) 1
	  		);
...
//	  	(dispGenAvail[d][first(isDECISION_STEPS)] == 1 ? initialAssetPower[d] - assetStepDurationInHours * maxRampRateInkWperH[d] : 0.0) <=
...
//// initial time step
//	forall (d in isDISP_E_GENS inter isRR_E_ASSETS)
//	  ctRRDispGenActivePowerDef0: DispGenActivePower[d][first(isDECISION_STEPS)] == (dispGenAvail[d][first(isDECISION_STEPS)] == 1 ? (DispGenActivePowerEnd[d][first(isDECISION_STEPS)] + initialAssetPower[d]) / 2.0 : 0.0);
//// other time step
//	forall (d in isDISP_E_GENS inter isRR_E_ASSETS, t in isDECISION_STEPS: t != first(isDECISION_STEPS))
...
// HARD-CODED
	...
	// initial step
		/*forall (d in isDISP_E_GENS inter {"CHP_1600kW"})
		  ctCHPEffPowerStartUp_0: DispGenEffActivePower[d][first(isDECISION_STEPS)] == (dispAssetInitialPower[d] + cf[d][first(isDECISION_STEPS)]) / 2.0;
	// other steps
	...
	if (microgridName == "MICROGRID MOPABLOEM") {
	 // CHP comportment for Mopabloem is rule based : the CHP follows the wholesales markets engagements

	 // All steps except last
	 // When must we have the CHP on ?
     forall (d in isDISP_E_GENS inter {"CHP_1600kW"}, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) < card(isDECISION_STEPS) - 1) {
	   ctNoCogeChange10:
	...
	// When must we have the CHP off ?
	// First case
	forall (d in isDISP_E_GENS inter {"CHP_1600kW"}, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) < card(isDECISION_STEPS)) {
	  ctNoCogeChange11:
		  dispGenAvail[d][t] == 1 &&
		  mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0 &&
		  daEngagement[assetStepHourlyStep[t]] + long_term_engagement[assetStepHourlyStep[t]] > -600 &&
		  dispGenAvail[d][nextc(isDECISION_STEPS, t, 1)] != 1 ?
	...
	forall (d in isDISP_E_GENS inter {"CHP_1600kW"}, t in isDECISION_STEPS: ord(isDECISION_STEPS, t) < card(isDECISION_STEPS)) {
     ...
     //idem for the last step
     //When is it on ?
     forall (d in isDISP_E_GENS inter {"CHP_1600kW"}) {
	    ctNoCogeChange20:
	      dispGenAvail[d][last(isDECISION_STEPS)] == 1 && mFRRactivatedPower_mFRR[assetStepmFRRStep[last(isDECISION_STEPS)]] == 0 &&
	      daEngagement[assetStepHourlyStep[last(isDECISION_STEPS)]] + long_term_engagement[assetStepHourlyStep[last(isDECISION_STEPS)]] <= -600 ?

				IsDispAssetOn[d][last(isDECISION_STEPS)] >= 1
	        	: IsDispAssetOn[d][last(isDECISION_STEPS)] <= 1 ;    }
     ...
     forall (d in isDISP_E_GENS inter {"CHP_1600kW"}) {
	    ctNoCogeChange21:
	      ...
	      daEngagement[assetStepHourlyStep[last(isDECISION_STEPS)]] + long_term_engagement[assetStepHourlyStep[last(isDECISION_STEPS)]] > -600 ?
				...
				IsDispAssetOn[d][last(isDECISION_STEPS)] <= 0
	        	: IsDispAssetOn[d][last(isDECISION_STEPS)] <= 1 ;    }
	...
	// Since the value is 0 if it is off, Pmax/2 if it is started up, and Pmax if is on and was on on the step before, we can cod ramps this way
	 forall (d in isDISP_E_GENS inter {"CHP_1600kW"}, t in isDECISION_STEPS)
		ctCHPEffPowerStartUp: DispGenEffActivePower[d][t] == maxDispGenActivePower[d]*(IsDispAssetOn[d][t] - (1/2)*DispAssetStartup[d][t]);
...
// Specific constraints for VALECO
	if (card(isPPAOrOA_ASSETS) > 0) {
		...
		forall (i in isINTER_E_GENS, t in isDECISION_STEPS)
			ctPotentialNetInterGenPPA: InterGensPPANetActivePower[t] >= minl(sum(i in isINTER_E_GENS) interGenActivePowerForecast[i][t] - sum(d in isNF_E_LOADS) NFLoadActivePower[d][t], maxOutput[first(siteIdByAsset[i])][t]);
	}
...
// in the  ctAuthorizedCurt1 constraint, We check the state of charge only for the available batteries; for this we must pass storMaxAvailACActivePowerCharge on argument in the sum.
// curtailment is not penalized if the storage asset is not available, 
// maxl(0, 1 - sum(s in isSTORAGES) storPartialAvail[s][t]) will equal to one if and only if all asset storage are unavailable
...
if (lastMinuteCurtOption == 1 && card(isINTER_E_GENS) > 0 && card(isE_STORAGES) > 0 ) { 
		forall (t in isDECISION_STEPS)
	  		ctAuthorizedCurt1 : sum(s in isE_STORAGES : storMaxAvailACActivePowerCharge[s][t] > 0.0)(nomEnergyMax[s] * storElecMaxSOC[s] / 100 - StorStoredDCEnergy[s][t])
...
// Some intermittent generation assets under contract for difference are eligible for a premium if they stop generation
// when market prices are strictly negative
if (microgridName in {"MICROGRID GDS Leforest A", "MICROGRID GDS Leforest B"}) {
		forall (i in isINTER_E_GENS inter {"GENERALE_DU_SOLAIRE_Leforest_A_PV", "GENERALE_DU_SOLAIRE_Leforest_B_PV", "GENERALE_DU_SOLAIRE_Leforest_B_1MW_PV"}, t in isDECISION_STEPS)
		  ctInterGenContDiffPremium: InterGenActivePower[i][t] <= (daElecPriceRaw[assetStepHourlyStep[t]] >= 0.0 ? -nomPowerMin[i] : 0.0);
 	}
...
//	  	(interGenPartialAvail[i][first(isDECISION_STEPS)] == 1 ? initialAssetPower[i] - assetStepDurationInHours * maxRampRateInkWperH[i] : 0.0) <=
...
//	  ctRRupInterGenActivePowerEnd: InterGenActivePowerEnd[i][t] <= (interGenPartialAvail[i][t] == 1 ? InterGenActivePowerEnd[i][prev(isDECISION_STEPS, t)] + assetStepDurationInHours * maxRampRateInkWperH[i] : 0.0);
//	forall (i in isINTER_E_GENS inter isRR_E_ASSETS, t in isDECISION_STEPS: t != first(isDECISION_STEPS) && interGenPartialAvail[i][t] == 1)
//	  ctRRDwnInterGenActivePowerEnd: InterGenActivePowerEnd[i][t] >= (interGenPartialAvail[i][t] == 1 ? InterGenActivePowerEnd[i][prev(isDECISION_STEPS, t)] - assetStepDurationInHours * maxRampRateInkWperH[i] : 0.0);
//
//// if intermittent generation asset i is subject to ramp rates
//// its average elec power over step t is average between power at end of step t and power at end of step t-1
//// wraning: this is an approximation / simplification and it is not correct when power target at end of t is reached before end of t
//// initial time step
//	forall (i in isINTER_E_GENS inter isRR_E_ASSETS)
//	  ctRRInterGenActivePowerDef0: InterGenActivePower[i][first(isDECISION_STEPS)] == (interGenPartialAvail[i][first(isDECISION_STEPS)] == 1 ? (InterGenActivePowerEnd[i][first(isDECISION_STEPS)] + initialAssetPower[i]) / 2.0: 0.0);
//// other time step
//	forall (i in isINTER_E_GENS inter isRR_E_ASSETS, t in isDECISION_STEPS: t != first(isDECISION_STEPS))
//	  ctRRInterGenActivePowerDef: InterGenActivePower[i][t] == (interGenPartialAvail[i][t] == 1 ? (InterGenActivePowerEnd[i][t] + InterGenActivePowerEnd[i][prev(isDECISION_STEPS, t)]) / 2.0 : 0.0);
//
...
// Power consumption by a storage asset in charge is limited by its maximum charge rate
	forall (s in isE_STORAGES, t in isDECISION_STEPS)
	  ...
	  // Power consumption by a storage asset in charge is limited by its maximum charge rate
	forall (s in isE_STORAGES, t in isDECISION_STEPS)
	  ctStorageChargeMin: StorACPowerCharge[s][t] >= storMinNomDCActivePowerCharge[s] * IsCharging[s][t];

// Definition of active power change on first decision steps
	forall (s in isE_STORAGES)
	  ctStorageACActivePowerChangeFirst: StorACActivePower[s][first(isDECISION_STEPS)] - initialAssetPower[s] == StorACActivePowerInc[s][first(isDECISION_STEPS)] - StorACActivePowerDec[s][first(isDECISION_STEPS)];
...
// BESS cannot inject and absorb in the meantime
	forall (s in isE_STORAGES, t in isDECISION_STEPS)	
	  ctEStorageChargeOrDischarge: 
	  	  IsCharging[s][t] + IsDischarging[s][t] <= 1;
...
// Energy stored at end of step 1
	forall (s in isE_STORAGES)
	  ctStorageStoredEnergy1: StorStoredDCEnergy[s][first(isDECISION_STEPS)] == storInitialElecCharge[s] * scheduledAvailability[s][first(isDECISION_STEPS)] + StorStepDCEnergyIn[s][first(isDECISION_STEPS)];
	  
// Energy stored at end of other steps
	forall (s in isE_STORAGES, t in isDECISION_STEPS: t != first(isDECISION_STEPS))
	  ctStorageStoredEnergy: StorStoredDCEnergy[s][t] == StorStoredDCEnergy[s][prev(isDECISION_STEPS, t)] * storPartialAvail[s][t]/100 + StorStepDCEnergyIn[s][t];
...
// starts at the 1st not-engaged step, such that if we start the optim in FCR and there is a deficit, it is not our problem : it is managed locally.
 	forall (s in isE_STORAGES inter isFCR_ASSETS, t in isDECISION_STEPS: storMaxAvailDCEnergy[s][t] > 0.0 && ord(isDECISION_STEPS ,t) >= firstIsNotEngagedFCRRank)
 	  ctStorageMinSOCFCR: storElecMinSOC[s] + FCRPower[s][assetStepFCRStep[t]] * (storEnergyDwn1kWFCR / storMaxNomDCEnergy[s]) * 100
 	  					- SOCminDeficit[s][t] - SOCstrictMinDeficit[s][t] <= 100 * StorStoredDCEnergy[s][t] / storMaxNomDCEnergy[s];
...
// starts at the 1st not-engaged step, such that if we start the optim in FCR and there is a deficit, it is not our problem : it is managed locally.
 	forall (s in isE_STORAGES inter isFCR_ASSETS, t in isDECISION_STEPS: storMaxAvailDCEnergy[s][t] > 0.0 && ord(isDECISION_STEPS ,t) >= firstIsNotEngagedFCRRank)
 	  ctStorageStrictMinSOCFCR: storStrictElecMinSOC[s] + FCRPower[s][assetStepFCRStep[t]] * (storEnergyDwn1kWFCR / storMaxNomDCEnergy[s]) * 100
 	  						- SOCstrictMinDeficit[s][t] <= 100 * StorStoredDCEnergy[s][t] / storMaxNomDCEnergy[s];
...
// starts at the 1st not-engaged step, such that if we start the optim in FCR and there is a deficit, it is not our problem : it is managed locally.
 	forall (s in isE_STORAGES inter isFCR_ASSETS, t in isDECISION_STEPS: storMaxAvailDCEnergy[s][t] > 0.0 && ord(isDECISION_STEPS ,t) >= firstIsNotEngagedFCRRank)
 	  ctStorageMaxSOCFCR: 100 * StorStoredDCEnergy[s][t] / storMaxNomDCEnergy[s] <=
 	  storElecMaxSOC[s] - FCRPower[s][assetStepFCRStep[t]] * (storEnergyUp1kWFCR / storMaxNomDCEnergy[s])*100+ SOCmaxExcess[s][t];
	...
	forall (s in isE_STORAGES inter isGRID_FOLL, t in isDECISION_STEPS)
	  ctGFLStorageMaxSpinRaiseReserve:
	  	StorSpinRaiseReserve[s][t] <= (storMaxAvailACActivePowerDischarge[s][t] + storMaxAvailACActivePowerCharge[s][t]) *
	  		( sum (d in isDISP_E_GENS inter isGRID_FORM) IsDispAssetOn[d][t]
...
/* DISPATCHABLE LOAD UNITS */
// Maximum and minimum consumption
 	forall (d in isDISP_LOADS, t in isDECISION_STEPS)
 	  ctDispAvailLoadMaxPower: DispLoadPower[d][t] <= maxAvailDispAssetPower[d][t] * IsDispAssetOn[d][t];

	forall (d in isDISP_LOADS, t in isDECISION_STEPS)
 	  ctDispAvailLoadMinPower: minAvailDispLoadPower[d][t] * IsDispAssetOn[d][t] <= DispLoadPower[d][t];

//// Maximum reactive power for dispatchable load unit d over step t is linear function of d's active power target over t
//// modelled as dexpr
//	forall (d in isDISP_E_LOADS, t in isDECISION_STEPS)
//	  ctDispLoadMaxReactivePower:
//	  	DispLoadMaxReactivePower[d][t] == aQmax[d] * DispLoadActivePower[d][t] + bQmax[d];
// Reactive power for dispatchable load unit d over step t is limited by d's max recative power at t
	forall (d in isDISP_E_LOADS, t in isDECISION_STEPS)
	  ctDispLoadReactivePower:
	  	DispLoadReactivePower[d][t] <= DispLoadMaxReactivePower[d][t];

// For dispatchable elec load d operating in grid-forming or grid-following mode, spinning raise reserve over step t
// is limited by the difference between d's AC power target over t and d's min active power
	forall (d in isDISP_E_LOADS inter (isGRID_FORM union isGRID_FOLL), t in isDECISION_STEPS)
	  ctDispLoadMaxSpinRaiseReserve:
	  	DispLoadSpinRaiseReserve[d][t] <= (dispLoadPartialAvail[d][t] / 100.0 == 1 ? DispLoadActivePower[d][t] - minDispLoadPower[d] : 0.0);
// For dispatchable elec load d operating in grid-tied mode, spinning raise reserve over step t is zero
	forall (d in isDISP_E_LOADS inter isGRID_TIED, t in isDECISION_STEPS)
	  ctDispLoadMaxSpinRaiseReserve0:
	  	DispLoadSpinRaiseReserve[d][t] <= 0.0;

/* FLEX LOAD UNITS */	
// Modulation definition, when the load not available we can not penalize the power, for this we must add the availability condition in this constraint
// applied only to Flex Loads.
 	forall (f in isFLEX_LOADS, t in isDECISION_STEPS)
 	  ctFlexLoadModulation: DispLoadPower[f][t] == flexLoadForecast[f][t] + ModulationTarget[f][t];

// Modulation constraints	

if (microgridName == "MICROGRID JEPX Komekurayama") {
	 forall (d in isFLEX_LOADS, t in isDECISION_STEPS)
 	  ctFlexLoadJEPXKomeConstraints: ModulationTarget[d][t] <= 0.0;
 	  
 	 forall (t in isDECISION_STEPS)
 	  ctParametricConsumption:
 	  	DispLoadActivePower["JEPX_KOMEKURAYAMA_Yamanashi_Aux"][t] == auxConvConsoCoeff * ConvActivePowerIn["JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"][t] + auxConvConsoBase*IsDispAssetOn["JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser"][t];
}

else if (microgridName == "MICROGRID JEPX Suntory") {
	forall (d in isFLEX_LOADS, t in isDECISION_STEPS)
 	  ctFlexLoadJEPXSunConstraints: ModulationTarget[d][t] <= 0.0;

	forall (t in isDECISION_STEPS)
	  ctParametricConsumption2:
 	  	DispLoadActivePower["JEPX_SUNTORY_Yamanashi_Aux"][t] == auxConvConsoCoeff * ConvActivePowerIn["JEPX_SUNTORY_Yamanashi_Electrolyser"][t] + auxConvConsoBase*IsDispAssetOn["JEPX_SUNTORY_Yamanashi_Electrolyser"][t];
}
 else {
	// Elec Power modulation constraints
 	forall (d in isFLEX_LOADS, t in isDECISION_STEPS)
 	  ctFlexLoadModConstraints: ModulationTarget[d][t] == 0.0;}
/* RESERVES */
// If dispatchable elec load d is operating in grid-following mode, it can only provide spinning raise reserve if at least one other asset is operating in grid-forming mode
	forall (d in isDISP_E_LOADS inter isGRID_FOLL, t in isDECISION_STEPS)
	  ctGFLDispLoadMaxSpinRaiseReserve:
	  	DispLoadSpinRaiseReserve[d][t] <= (maxDispLoadPower[d] + minDispLoadPower[d]) *
	  		( sum (d in isDISP_E_GENS inter isGRID_FORM) IsDispAssetOn[d][t]
	  		+ sum (s in isE_STORAGES inter isGRID_FORM: storMaxAvailACActivePowerDischarge[s][t] + storMaxAvailACActivePowerCharge[s][t] > 0.0) 1
	  		+ sum (f1 in isDISP_E_LOADS inter isGRID_FORM: maxDispLoadPower[f1] > 0.0 && dispLoadPartialAvail[f1][t] > 0.0) 1
	  		);

// For dispatchable elec load d operating in grid-forming or grid-following mode, spinning lower reserve over step t
// is limited by the difference between d's max active power and d's AC power target over t
	forall (d in isDISP_E_LOADS inter (isGRID_FORM union isGRID_FOLL), t in isDECISION_STEPS)
	  ctDispLoadMaxSpinLowerReserve:
	  	DispLoadSpinLowerReserve[d][t] <= (dispLoadPartialAvail[d][t] / 100.0 == 1 ? maxDispLoadPower[d] - DispLoadActivePower[d][t] : 0.0);

// For dispatchable elec load d operating in grid-tied mode, spinning lower reserve  over step t is zero
	forall (d in isDISP_E_LOADS inter isGRID_TIED, t in isDECISION_STEPS)
	  ctDispLoadMaxSpinLowerReserve0:
	  	DispLoadSpinLowerReserve[d][t] <= 0.0;

// If dispatchable elec load d is operating in grid-following mode, it can only provide spinning lower reserve if at least one other asset is operating in grid-forming mode
	forall (d in isDISP_E_LOADS inter isGRID_FOLL, t in isDECISION_STEPS)
	  ctGFLDispLoadMaxSpinLowerReserve:
	  	DispLoadSpinLowerReserve[d][t] <= (maxDispLoadPower[d] + minDispLoadPower[d]) *
	  		( sum (d in isDISP_E_GENS inter isGRID_FORM) IsDispAssetOn[d][t]
	  		+ sum (s in isE_STORAGES inter isGRID_FORM: storMaxAvailACActivePowerDischarge[s][t] + storMaxAvailACActivePowerCharge[s][t] > 0.0) 1
	  		+ sum (f1 in isDISP_E_LOADS inter isGRID_FORM: maxDispLoadPower[f1] > 0.0 && dispLoadPartialAvail[f1][t] > 0.0) 1
	  		);
...
// Reserve should cover loss of termal generator with largest generation
	forall (d1 in isDISP_E_GENS: activePowerLossPct[d1] > 0, t in isDECISION_STEPS)
	  ...
	  + sum (d in isDISP_E_LOADS) DispLoadActiveRaiseReserve[d][t] >=						// reserve from dispatchable load units
...
// Reserve should cover surge of non-dispatchable load unit with largest consumption
...
// Reserve should cover surge of intermittent generation asset with largest generation
	forall (i1 in isINTER_E_GENS: activePowerSurgePct[i1] > 0, t in isDECISION_STEPS)
	  ...
	  + sum (d in isDISP_E_LOADS) DispLoadActiveLowerReserve[d][t] >=						// reserve from dispatchable load units
...
// Reserve should cover loss of dispatchable load unit with largest consumption
	forall (f1 in isDISP_E_LOADS: activePowerLossPct[f1] > 0, t in isDECISION_STEPS)
	  ctActivePowerLowerReserveDispLoad :
	  	sum (d in isDISP_E_GENS) DispGenActivePower[d][t]							// reserve from dispatchable gens
	  + sum (i in isINTER_E_GENS) InterGenActivePower[i][t]								// reserve from intermittent gen assets
	  + sum (s in isE_STORAGES) StorActiveLowerReserve[s][t]								// reserve from storage assets
	  + (100.0 - activePowerLossPct[f1]) / 100 * DispLoadActiveLowerReserve[f1][t]		// reserve from what is left of f1
	  + sum (d in isDISP_E_LOADS diff({f1})) DispLoadActiveLowerReserve[d][t] >=			// reserve from other dispatchable load units
	  activePowerLossPct[f1] / 100 * DispLoadActivePower[f1][t] - DispLoadActivePowerLowerReserveDeficit[f1][t];

// Reserve should cover loss of non-dispatchable load unit with largest consumption
...
// Reserve should cover loss of termal generator with largest generation
	forall (d1 in isDISP_E_GENS: reactivePowerLossPct[d1] > 0, t in isDECISION_STEPS)
	  ...
	  + sum (d in isDISP_E_LOADS) DispLoadReactiveRaiseReserve[d][t] >=						// reserve from dispatchable load units
...
// Reserve should cover surge of intermittent generation asset with largest generation
	forall (i1 in isINTER_E_GENS: reactivePowerSurgePct[i1] > 0, t in isDECISION_STEPS)
	  ...
	  + sum (d in isDISP_E_LOADS) DispLoadReactiveLowerReserve[d][t] >=					// reserve from dispatchable load units
...
// Reserve should cover loss of dispatchable load unit with largest consumption
	forall (f1 in isDISP_E_LOADS: reactivePowerLossPct[f1] > 0, t in isDECISION_STEPS)
	  ctReactivePowerLowerReserveDispLoad :
	  	sum (d in isDISP_E_GENS) DispGenReactivePower[d][t]							// reserve from dispatchable gens
	  + sum (i in isINTER_E_GENS) InterGenReactivePower[i][t]							// reserve from intermittent gen assets
	  + sum (s in isE_STORAGES) StorReactiveLowerReserve[s][t]							// reserve from storage assets
	  + (100.0 - reactivePowerLossPct[f1]) / 100 * DispLoadReactiveLowerReserve[f1][t]	// reserve from what is left of f1
	  + sum (d in isDISP_E_LOADS diff({f1})) DispLoadReactiveLowerReserve[d][t] >=		// reserve from other dispatchable load units
	  reactivePowerLossPct[f1] / 100 * DispLoadReactivePower[f1][t] - DispLoadReactivePowerLowerReserveDeficit[f1][t];
...
// Raise reserve provided by assets should cover 30% of total non-flexibile load for each decision step
	forall (t in isDECISION_STEPS)
	  ctSpinningRaiseReserveReq:
		  sum (d in isDISP_E_GENS) DispGenSpinRaiseReserve[d][t] + sum (s in isE_STORAGES) StorSpinRaiseReserve[s][t] + sum (d in isDISP_E_LOADS) DispLoadSpinRaiseReserve[d][t]
...
// Lower reserve provided by assets should cover 30% of total non-flexibile load for each decision step
	forall (t in isDECISION_STEPS)
	  ctSpinningLowerReserveReq:
		  sum (d in isDISP_E_GENS) DispGenSpinLowerReserve[d][t] + sum (s in isE_STORAGES) StorSpinLowerReserve[s][t] + sum (d in isDISP_E_LOADS) DispLoadSpinLowerReserve[d][t]
...
// potential of current injection from assets must cover the global requirement for default current
	forall (t in isDECISION_STEPS)
	  ctDefaultCurrentReq :
	    sum (d in isDISP_E_GENS inter isDEFAULT_CURRENT_ASSET) IsDispAssetOn[d][t] * dispGenCurrentInjection[d]
...
// Maximum input and output
 	forall (si in isSITES, t in isDECISION_STEPS)
 	  ...
 	  - sum(d in isDISP_E_LOADS: si == siteID[d]) DispLoadActivePower[d][t]
...
// Generic congestion contraints
 	forall (c in isCONGESTIONS, t in isDECISION_STEPS)
 	  ...
 	  congestionLowerLim[c] <=
 	  	...
 	  	- sum(d in isDISP_E_LOADS) dispElecLoadFactor[c][d] * DispLoadActivePower[d][t]
...
forall (t in isDECISION_STEPS)
	  ctHeatBalance:
	  	...
	  	+ sum(s in isH_STORAGES) StorNotElecOutInPower[s][t]
...
/* HEAT CONGESTIONS */
// HARD-CODED
// heat generation from CHP and e-boilers cannot exceed 1.9MW
if (microgridName == "MICROGRID MOPABLOEM") {
	forall (t in isDECISION_STEPS)
	  ctHeatCongestion:
	  	  sum(d in isDISP_H_GENS) DispGenHeat[d][t] 
...
/*HYDROGEN BALANCE */
// Hydrogen generated elec to hydrogen assets, injected by storage assets
// and converted by converter assets must balance with the hydrogen consumed by
// non-flexible loads on each decision step.
// we don't modelise Hydrogen network assets, only useful to describe storage asset status 
// For Komekurayama : hydrogen first have to go to the storage before beeing consumed

//if (microgridName == "MICROGRID JEPX Komekurayama"){
//	forall (t in isDECISION_STEPS)
//		ctH2BalanceIn:
//		sum(c in isE_H2_CONVS) ConvH2Out[c][t]
//		==
//		sum(s in isH2_STORAGES) StorNotElecPowerCharge[s][t];
//
//	forall (t in isDECISION_STEPS)
//		ctH2BalanceOut:
//		sum(s in isH2_STORAGES) StorNotElecPowerDischarge[s][t]
//		==
//		sum(n in isNF_H2IN_LOADS) NFH2LoadForecast[n][t]
//		+ sum (d in isFLEX_H2IN_LOADS) DispLoadPower[d][t];
//	}
//
//else {
	forall (t in isDECISION_STEPS)
...
/*********************************************
 * Energy Storage related constraints
 *********************************************/

 /*  Not elec storage energy INJECTION */

// Not_elec_energy consumption by a storage asset in charge is limited by its maximum charge rate
	forall (s in isNotE_STORAGES, t in isDECISION_STEPS)
	  ctNotElecStorageChargeMax: StorNotElecPowerCharge[s][t] <= maxStorNotElecPowerCharge[s] * IsCharging[s][t] * storPartialAvail[s][t] / 100.0;

// Not_elec_energy injection by a storage asset in discharge is limited by its maximum discharge rate
	forall (s in isNotE_STORAGES, t in isDECISION_STEPS)
	  ctNotElecStorageDischargeMax: StorNotElecPowerDischarge[s][t] <= maxStorNotElecPowerDischarge[s] * IsDischarging[s][t] * storPartialAvail[s][t] / 100.0;

// Incremental charge / discharge definition
	forall (s in isNotE_STORAGES: storNotElecDischCstEff[s] > 0, t in isDECISION_STEPS)
	  ctNotElecStorageIncrCharge: StorStepNotElecPowerIn[s][t] == (storNotElecChargeCstEff[s] / 100) * StorNotElecPowerCharge[s][t] * assetStepDurationInHours
	  - (100 / storNotElecDischCstEff[s]) * StorNotElecPowerDischarge[s][t] * assetStepDurationInHours;
	forall (s in isNotE_STORAGES: storNotElecDischCstEff[s] <= 0, t in isDECISION_STEPS)
	  ctNotElecStorageIncrCharge0: StorStepNotElecPowerIn[s][t] ==  (storNotElecChargeCstEff[s] / 100) * StorNotElecPowerCharge[s][t] * assetStepDurationInHours;
	  
// Not_elec_energy stored at end of step 1
	forall (s in isNotE_STORAGES)
	  ctNotElecStorageStoredEnergy1: StorStoredEnergy[s][first(isDECISION_STEPS)] == storInitNotElecEnergyCharge[s] * storPartialAvail[s][first(isDECISION_STEPS)] / 100.0 + StorStepNotElecPowerIn[s][first(isDECISION_STEPS)];
	  
// Not_elec_energy stored at end of other steps
// Currently, if you lose stored energy, you can't restore it
	forall (s in isNotE_STORAGES, t in isDECISION_STEPS: t != first(isDECISION_STEPS))
	  ctNotElecStorageStoredEnergy: StorStoredEnergy[s][t] == StorStoredEnergy[s][prev(isDECISION_STEPS, t)] * minl(
			1, (storPartialAvail[s][prev(isDECISION_STEPS, t)] == 0 ? 0 : storPartialAvail[s][t] / storPartialAvail[s][prev(isDECISION_STEPS, t)])
			) + StorStepNotElecPowerIn[s][t]; //+ RestoredNotElecEnergy[s][t];

// Physical capacity constraints for each heat storage units
	 forall (s in isNotE_STORAGES, t in isDECISION_STEPS)
	   ctPhysNotElecCapacity: StorStoredEnergy[s][t] <= availStorNotElecMaxCharge[s][t];
	   
// Minimum SOC
 	forall (s in isNotE_STORAGES, t in isDECISION_STEPS: availStorNotElecMaxCharge[s][t] > 0)
 	  ctNotElecStorageMinSOC: storNotElecMinSOC[s] - NotElecSOCminDeficit[s][t] <= 100 * StorStoredEnergy[s][t] / availStorNotElecMaxCharge[s][t];

// Maximum SOC
 	forall (s in isNotE_STORAGES, t in isDECISION_STEPS: availStorNotElecMaxCharge[s][t] > 0)
 	  ctNotElecStorageMaxSOC: 100 * StorStoredEnergy[s][t] / availStorNotElecMaxCharge[s][t] <= storNotElecMaxSOC[s] + NotElecSOCmaxExcess[s][t];

//// To be cheked with Joshua and Nicolas.
// Unavail Hydrogen stored at the end of the first step
	//forall (s in isH2_STORAGES)
	  //ctH2VolumeUnavailAtStartOfStep1: UnavailStoredH2[s][first(isDECISION_STEPS)] == storInitialH2Charge[s] * (100 - storPartialAvail[s][first(isDECISION_STEPS)]);
// accumulated unavai H2 stored at the end of each step t (t > 1)	
//forall(s in isH2_STORAGES, t in isDECISION_STEPS: t != first(isDECISION_STEPS)) {
//			UnavailStoredH2[s][t] == 
//			sum(tt in isDECISION_STEPS: ord(isDECISION_STEPS, tt) <= ord(isDECISION_STEPS, t)) (
//				StorStoredH2[s][prev(isDECISION_STEPS, tt)] * 
//				(storPartialAvail[s][prev(isDECISION_STEPS, tt)] != 0 ?
//					maxl(1.0 - storPartialAvail[s][tt] / storPartialAvail[s][prev(isDECISION_STEPS, tt)], 0.0) 
//				: 0.0)
//			);
//			- RestoredH2Volume[s][t]
//	};	   
/*********************************************
 * Electricity to other energy sources related constraints
 *********************************************/
...
/* ENERGY CONVERTERS */
// Converters consuming something else than elec are not supported. (To be removed, ConvActivePowerIn is indexed by isEIN_CONVS)
//	forall (c in isCONVS diff isEIN_CONVS, t in isDECISION_STEPS)
//	  ctConvMinMaxNonElecPowerIn:
//	  	ConvActivePowerIn[c][t] <= 0.0;

// Conversion into heat
	forall (c in isE_H_CONVS, t in isDECISION_STEPS)
	  ctToHeatConv : ConvHeatOut[c][t] == convElecToHeatEff[c] * ConvActivePowerIn[c][t];

// Conversion into Hydrogen
	forall (c in isE_H2_CONVS : c not in isCONV_NL_EFF_ASSETS, t in isDECISION_STEPS) // For assets that don't have non linear efficiencies
	  ctToH2Conv : ConvH2Out[c][t] == convElecToH2Eff[c] * ConvActivePowerIn[c][t];
...
{t_empty} ASSET_STEPS_HEADER = {<"asset_id", "step_id", "power_target", "energy_type", "storage_target", "temperature_target", "curtailment_target","target_soc", "", "", "", "", "", "", "", "", "", "", "", ""> | i in 1..1};
...
string strH2EnergyTotalCosts;
string strOptimisedH2EnergyCosts;
{t_operation_output} OPERATION_OUTPUT = {<"operation_id", operationID>	 									// variable holding OPERATION output data
										...
										, <"h2_energy_total_costs", strH2EnergyTotalCosts>					// H2EnergyTotalCosts
										, <"opt_h2_energy_costs", strOptimisedH2EnergyCosts>			// H2EnergyTotalCosts
...
float nonFCRenergyOut[isASSETS diff isDUMMY_ASSETS];
float optimisedNonFCRenergyOut[isASSETS diff isDUMMY_ASSETS];
float FCRenergyOut[isASSETS diff isDUMMY_ASSETS];
float optimisedFCRenergyOut[isASSETS diff isDUMMY_ASSETS];
{t_assets_output} ASSETS_OUTPUT = {<a.asset_id, a.control, nonFCRenergyOut[a.asset_id], optimisedNonFCRenergyOut[a.asset_id], FCRenergyOut[a.asset_id], optimisedFCRenergyOut[a.asset_id]> | a in ASSETS : a.asset_id not in isDUMMY_ASSETS}; // variable holding ASSETS output data
...
float assetPowerTarget[isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS][isDECISION_STEPS];
string assetEnergyType[isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS][isDECISION_STEPS];
float assetStorageTarget[isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS][isDECISION_STEPS];
float assetSOCTarget[isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS][isDECISION_STEPS];
float assetTempTarget[isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS][isDECISION_STEPS];
float assetCurtailmentTarget[isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS][isDECISION_STEPS];
{t_asset_steps_output} ASSET_STEPS_OUTPUT = {<a, t, assetPowerTarget[a][t], assetEnergyType[a][t], assetStorageTarget[a][t], assetTempTarget[a][t], assetCurtailmentTarget[a][t], assetSOCTarget[a][t]> | a in isASSETS union {"MAINGRID"} diff isDUMMY_ASSETS, t in isDECISION_STEPS}; // variable holding ASSET x STEPS output data
string assetStepsExcelRange = "'ASSET_STEPS_OUTPUT'!A2:H";
...
tuple t_market_bids_output {
 	key string step_id;			// day_ahead / fcr market step_id
 	key string type;			// bids  type : day_ahead or fcr
 	key string direction;		// BUY or SELL
 	int step_duration;			// day_ahead / fcr Power step duration
 	float power;				// day_ahead / fcr Power engagement
	float price;				// bids Price (expressed in currency unit/kWh)
 	}

float daThresholdPrice[isDECISION_STEPS];

{t_market_bids_output} MARKET_BIDS_OUTPUT = ((microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory")?
												{<h, "DAY_AHEAD", (DaPosition[h] <= 0 ? "SELL" : "BUY"), daStepDuration, DaPosition[h], daThresholdPrice[h]>| h in isHOURLY_STEPS_POS}
											:{<h, "DAY_AHEAD", (DaPosition[h] <= 0 ? "SELL" : "BUY"), daStepDuration, DaPosition[h], (DaPosition[h] <= 0.0 ? sellOfferPrice : daElecPriceRaw[h])>| h in isHOURLY_STEPS_POS})
											union
											...
											{<fcr, "FCR", "SELL", fcrStepDuration, TotalFCRPower[fcr], 0.0>| fcr in isFCR_STEPS_POS} ; // variable holding ENERGY_BIDS output data
string marketBidsExcelRange = "'MARKET_BIDS_OUTPUT'!A2:F";
int nMarketBids = card(isHOURLY_STEPS_POS)  + (optContext != "OPERATIONS" ? card(isHOURLY_STEPS_POS) : 0) + card(isFCR_STEPS_POS)*(card(isFCR_ASSETS) > 0 ? 1 : 0);
...
float startupCostPaidForAsset[isASSETS diff isSITES diff isDUMMY_ASSETS][isSITES][isDECISION_STEPS];
float assestVarCost[isASSETS diff isSITES diff isDUMMY_ASSETS][isSITES][isDECISION_STEPS];
float assetCurtailmentCost[isASSETS diff isSITES diff isDUMMY_ASSETS][isSITES][isDECISION_STEPS];
string assetStepsCostExcelRange = "'ASSET_STEPS_COST'!A2:F";

{t_asset_steps_cost} ASSET_STEPS_COST = {<a, s, t, startupCostPaidForAsset[a][s][t], assestVarCost[a][s][t], assetCurtailmentCost[a][s][t]>| a in isASSETS diff isSITES diff isDUMMY_ASSETS, s in siteIdByAsset[a], t in isDECISION_STEPS}; // variable holding ASSET_STEPS_COST output data
...
string stepCostsExcelRange = "'STEP_COSTS'!A2:J";
...
execute {
	...
	strH2EnergyTotalCosts = H2EnergyTotalCosts;
	strOptimisedH2EnergyCosts = OptimisedH2EnergyCosts;
	strOptimisedNetRevenues = - OptimisedDayAheadTradeCost - OptimisedTradeNetworkCosts - OptimisedDayAheadFCRCost - OptimisedFCRNetworkCosts - OptimisedGenLinearVarCosts - OptimisedH2EnergyCosts;
 	...
 	for (var t in isDECISION_STEPS) {
		...
		for (var a in Opl.operatorDIFF(isASSETS, isDUMMY_ASSETS))
		{
			if(isEH_ASSETS.contains(a))
			{
				assetEnergyType[a][t] = "ELECTRICITY + HEAT";
			}
			else if (isE_H2_CONVS.contains(a))
			{
				assetEnergyType[a][t] = "ELECTRICITY + H2";
			}
			...
			else if (isH2_ASSETS.contains(a))
			{
				assetEnergyType[a][t] = "H2";
			}
			else
			{
		...
		// site energy type
		for(var s in Opl.operatorDIFF(isSITES, isDUMMY_ASSETS))
		{
			if (Opl.card(Opl.operatorINTER(assetIdsBySite[s], isEH_ASSETS)) > 0)
			{
				assetEnergyType[s][t] = "ELECTRICITY + HEAT";
			}
			else if (Opl.card(Opl.operatorINTER(assetIdsBySite[s], isE_H2_CONVS)) > 0)
			{
				assetEnergyType[s][t] = "ELECTRICITY + H2";
			}
		...
		else
		{
 		...
 		// power generation target for intermittent generation assets at the end of each time step
 		for	(var i in Opl.operatorDIFF(isINTER_E_GENS, isDUMMY_ASSETS)){
// 				assetPowerTarget[i][t] = -InterGenActivePowerEnd[i][t];				// respects Everest's sign convention (-ve values = power generation)
 		...
 		for	(var d in Opl.operatorDIFF(isDISP_E_GENS, isDUMMY_ASSETS))
 		{
		    //assetPowerTarget[d][t] = -DispGenActivePowerEnd[d][t];				// respects Everest's sign convention (-ve values = power generation)
 			assetPowerTarget[d][t] = -DispGenActivePower[d][t];                 // power generation target for dispatchable generation assets
		...
		for(var s in Opl.operatorDIFF(isSITES, isDUMMY_ASSETS))
			{
					for (var a in Opl.operatorINTER(Opl.operatorINTER(isSTART_COST_ASSETS, isDISP_E_ASSETS), assetIdsBySite[s]))
					{
						startupCostPaidForAsset[a][Opl.first(siteIdByAsset[a])][t] = DispAssetStartup[a][t] * assetStartupCost[a];	      //StartupCost for each asset by site and by step
					}
					...
					for(var a in Opl.operatorINTER(Opl.operatorINTER(isCURT_COMP_E_GENS, isINTER_E_GENS), assetIdsBySite[s]))
					{
 		...
 		for	(var n in Opl.operatorDIFF(isNF_E_LOADS, isDUMMY_ASSETS))
 		{
 		...
 		for	(var n in Opl.operatorDIFF(isNF_H_LOADS, isDUMMY_ASSETS))
 		{
 			assetPowerTarget[n][t] = NFLoadHeat[n][t];					// respects Everest's sign convention (+ve values = power consumption)
		}
  		
  		// consumption targets for dispatchable load units 
 		for	(var d in Opl.operatorDIFF(isDISP_E_LOADS, isDUMMY_ASSETS))
 		{
 			assetPowerTarget[d][t] = DispLoadActivePower[d][t];					// respects Everest's sign convention (+ve values = power consumption)
  		} 	
  		
  		for (var f in Opl.operatorDIFF(isFLEX_LOADS, isDUMMY_ASSETS))
  		{
  		 	assetPowerTarget[f][t] = DispLoadPower[f][t];  		
  			  		
  		}		
  		
  		// temperature targets for dispatchable load units that are controled by temperature setpoints
  		for	(var f in Opl.operatorDIFF(isTC_DISP_E_LOADS, isDUMMY_ASSETS)) {
 		...
 		for	(var s in Opl.operatorDIFF(isE_STORAGES, isDUMMY_ASSETS))
 		{
 			...
 			FCRenergyOut[s] = FCRenergyOut[s] + FCRPower[s][assetStepFCRStep[t]] * 1000.0 * FCRUnitarianStepACEnergyOut;
 			if (isDAStepCleared[assetStepHourlyStep[t]] == 0) {
 				...
 				optimisedFCRenergyOut[s] = optimisedFCRenergyOut[s] + FCRPower[s][assetStepFCRStep[t]] * 1000.0 * FCRUnitarianStepACEnergyOut;
   			}
  		}

  		// Not elec power charge / discharge targets and not elec energy storage targets for not elec storage units
 		for	(var s in Opl.operatorDIFF(isNotE_STORAGES, isDUMMY_ASSETS))
 		{
 			assetPowerTarget[s][t] = -StorNotElecOutInPower[s][t];					// respects Everest's sign convention (+ve values = power charge and -ve values = power discharge)
 			assetStorageTarget[s][t] = StorStoredEnergy[s][t];
  		} 			
		...
		// elec soc targets for elec storage units
 		...
 		for	(var h in Opl.operatorDIFF(isNotE_STORAGES, isDUMMY_ASSETS))
 		{
			assetSOCTarget[h][t] = NotElecEnergyStorSocTarget[s][t];
  		}

  		// consumption targets for energy converters consuming elec
 		for	(var c in Opl.operatorDIFF(isEIN_CONVS, isDUMMY_ASSETS))
 		{
...
// 		for	(var c in Opl.operatorDIFF(isEOUT_CONVS, isDUMMY_ASSETS))
// 		{
...
//  		} 			
	...
	for (var s in isE_STORAGES)
		for (var t in isDECISION_STEPS) {
			...
			if (SOCminDeficit[s][t] > epsilon) {
				...
				VIOLATIONS_OUTPUT.add("minimum_SOC_deficit", s, t, SOCminDeficit[s][t], SOCminDeficitCost[s][t], 100);
			}
			...
			if (SOCstrictMinDeficit[s][t] > epsilon) {
				...
				VIOLATIONS_OUTPUT.add("strict_minimum_SOC_deficit", s, t, SOCstrictMinDeficit[s][t], SOCstrictMinDeficitCost[s][t],1);
			}
			// max SOC excess
			if (SOCmaxExcess[s][t] > epsilon) {
				...
				VIOLATIONS_OUTPUT.add("maximum_SOC_excess", s, t, SOCmaxExcess[s][t], SOCmaxExcessCost[s][t], 1);
			}
	...
	// check if any not elec heat storage charge violation
	anyMinSOCdeficit = false;
	anyMaxSOCexcess = false;
	var energyType = "";
	for (var s in isNotE_STORAGES)
		for (var t in isDECISION_STEPS) {
			// min SOC deficit
			if (NotElecSOCminDeficit[s][t] > epsilon) {

				energyType = (isH_STORAGES.contains(s) ? "heat" : "hydrogen");
				if (!anyMinSOCdeficit) {
					strDispatchStatusString += ", some "+energyType+" minimum SOC deficit";
					anyMinSOCdeficit = true;
	 			}

				VIOLATIONS_OUTPUT.add("minimum_"+energyType+"_heat_SOC_deficit", s, t, NotElecSOCminDeficit[s][t], NotElecSOCminDeficitCost[s][t],1);
			}
			// max SOC excess
			if (NotElecSOCmaxExcess[s][t] > epsilon) {
				
				energyType = (isH_STORAGES.contains(s) ? "heat" : "hydrogen");
				if (!anyMaxSOCexcess) {
					strDispatchStatusString += ", some "+energyType+" maximum SOC excess";
					anyMaxSOCexcess = true;
	 			}
	
				VIOLATIONS_OUTPUT.add("maximum_"+energyType+"_SOC_excess", s, t, NotElecSOCmaxExcess[s][t], NotElecSOCmaxExcessCost[s][t], 1);
			}
		}
	...
	//Check if any dispatchable asset minimum active power constraint violation
	anyDeficit = false;
	for (var d in isDISP_E_ASSETS)
		for (var t in isDECISION_STEPS) {
			if(DispAssetMinActivePowerDeficit[d][t] > epsilon){
				if (!anyDeficit) {
					strDispatchStatusString += ", some minimum power deficit";
					anyDeficit = true;
 			}
 			VIOLATIONS_OUTPUT.add("disp_asset_min_active_power_deficit", d, t, DispAssetMinActivePowerDeficit[d][t], DispAssetMinActivePowerDeficitCost[d][t], 1);
			}		
		}
	...
	var dispGenOrLoad = ""
	for (var d in isDISP_ASSETS) {
		// initial min steps on violation
		if (DispAssetMinStepOnInitialDeficit[d] > epsilon) {
			...
			dispGenOrLoad = (isDISP_E_GENS.contains(d) ? "gen" : "load");
			VIOLATIONS_OUTPUT.add("disp_"+dispGenOrLoad+"_initial_min_steps_on_deficit", d, "", DispAssetMinStepOnInitialDeficit[d], DispAssetMinStepOnInitialDeficitCost[d],10);
		}
		// initial max steps on violation
		for (var i = 0 ; i <= dispInitialStepsOnMax[d]-1 ; i++)
			if (DispAssetMaxStepOnInitialExcess[d][i] > epsilon) {
				...
				dispGenOrLoad = (isDISP_E_GENS.contains(d) ? "gen" : "load");
				VIOLATIONS_OUTPUT.add("disp_"+dispGenOrLoad+"_initial_max_steps_on_excess", d, (i-dispInitialStepsOnMax[d]).toString(), DispAssetMaxStepOnInitialExcess[d][i], DispAssetMaxStepOnInitialExcessCost[d][i],10);
			}
		// initial min steps off violation
		if (DispAssetMinStepOffInitialDeficit[d] > epsilon) {
			...
			dispGenOrLoad = (isDISP_E_GENS.contains(d) ? "gen" : "load");
			VIOLATIONS_OUTPUT.add("disp_"+dispGenOrLoad+"_initial_min_steps_off_deficit", d, "", DispAssetMinStepOffInitialDeficit[d], DispAssetMinStepOffInitialDeficitCost[d],10);
		}
	}
	...
	for (var d in isDISP_ASSETS)
		for (var t in isDECISION_STEPS) {
			...
			if (DispAssetMinStepOnDeficit[d][t] > epsilon) {
				if (!anyMinStepsOnViolation) {
				...
				dispGenOrLoad = (isDISP_E_GENS.contains(d) ? "gen" : "load");
				VIOLATIONS_OUTPUT.add("disp_"+dispGenOrLoad+"_min_steps_on_deficit", d, t, DispAssetMinStepOnDeficit[d][t], DispAssetMinStepOnDeficitCost[d][t], 10);
			}
			// max steps on violation
			if (DispAssetMaxStepOnExcess[d][t] > epsilon) {
				if (!anyMaxStepsOnViolation) {
				...
				dispGenOrLoad = (isDISP_E_GENS.contains(d) ? "gen" : "load");
				VIOLATIONS_OUTPUT.add("disp_"+dispGenOrLoad+"_max_steps_on_excess", d, t, DispAssetMaxStepOnExcess[d][t], DispAssetMaxStepOnExcessCost[d][t],10);
			}
			// min steps off violation
			if (DispAssetMinStepOffDeficit[d][t] > epsilon) {
				if (!anyMinStepsOffViolation) {
				...
				dispGenOrLoad = (isDISP_E_GENS.contains(d) ? "gen" : "load");
				VIOLATIONS_OUTPUT.add("disp_"+dispGenOrLoad+"_min_steps_off_deficit", d, t, DispAssetMinStepOffDeficit[d][t], DispAssetMinStepOffDeficitCost[d][t],10);
			}
		}
	...
	// check if any active power reserve constraint violation for dispatchable load
	anyRaiseReserveDeficit = false;
	anyLowerReserveDeficit = false;
	for (var d in isDISP_E_LOADS)
		for (var t in isDECISION_STEPS) {
			// lower reserve violation
			if (DispLoadActivePowerLowerReserveDeficit[d][t] > epsilon) {
				if (!anyLowerReserveDeficit) {
					strDispatchStatusString += ", some deficit of active power lower reserve to cover loss of flexible load";
					anyLowerReserveDeficit = true;
	 			}

				VIOLATIONS_OUTPUT.add("flex_load_active_power_lower_reserve_deficit", d, t, DispLoadActivePowerLowerReserveDeficit[d][t], DispLoadActivePowerLowerReserveDeficitCost[d][t], 10);
			}
 		}		

 	// check if any reactive power reserve constraint violation for dispatchable load
	anyRaiseReserveDeficit = false;
	anyLowerReserveDeficit = false;
	for (var d in isDISP_E_LOADS)
		for (var t in isDECISION_STEPS) {
			// lower reserve violation
			if (DispLoadReactivePowerLowerReserveDeficit[d][t] > epsilon) {
				if (!anyLowerReserveDeficit) {
					strDispatchStatusString += ", some deficit of reactive power lower reserve to cover loss of flexible load";
					anyLowerReserveDeficit = true;
	 			}

				VIOLATIONS_OUTPUT.add("flex_load_reactive_power_lower_reserve_deficit", d, t, DispLoadReactivePowerLowerReserveDeficit[d][t], DispLoadReactivePowerLowerReserveDeficitCost[d][t], 10);
			}
 		}		
	...
	for (d in isFLEX_LOADS)
		for (var t in isDECISION_STEPS) {
			if ( flexLoadChangedForecast[d][t] > epsilon) {
				if (!anyflexLoadChangeForecast) {
					strDispatchStatusString += ", changes in consumption forecast for flex load";
					anyflexLoadChangeForecast = true;
	 			}

				VIOLATIONS_OUTPUT.add("flex_load_active_power_forecast_change_flag", d, t,  flexLoadChangedForecast[d][t], Opl.intValue(""), 100);
			}
 		}
	...
	// JEPX threshold price computation
	function nl_eff_model(power, varCostSegUpLim, varCostModelSegNumber, nlEffSegSlope, assetVarEffModel){
		var eff_power = 0
		for (var s=1; s<=maxSegNbr; s++){
			if (s==1){
				if (power < varCostSegUpLim[assetVarEffModel][s]){
					eff_power = eff_power + power*nlEffSegSlope[assetVarEffModel][s];
				} else {
					eff_power = eff_power +	varCostSegUpLim[assetVarEffModel][s]*nlEffSegSlope[assetVarEffModel][s];
				}
			} else {
				var seg_power;			
				if (power < varCostSegUpLim[assetVarEffModel][s]){
					seg_power = power - varCostSegUpLim[assetVarEffModel][s-1];
				} else {
					seg_power = varCostSegUpLim[assetVarEffModel][s] - varCostSegUpLim[assetVarEffModel][s-1];
				}
				eff_power = eff_power + seg_power*nlEffSegSlope[assetVarEffModel][s];
			}
		}
		return eff_power;
	}
	...
	if (microgridName == "MICROGRID JEPX Komekurayama" || microgridName == "MICROGRID JEPX Suntory"){
		var electrolyser;
		if (microgridName == "MICROGRID JEPX Komekurayama"){
			electrolyser = "JEPX_KOMEKURAYAMA_Yamanashi_Electrolyser";
		} else if (microgridName == "MICROGRID JEPX Suntory"){
			electrolyser = "JEPX_SUNTORY_Yamanashi_Electrolyser";
		}
		
		var nomConvInMaxPower = nomPowerMax[electrolyser];
		var nomConvOutMaxPower;
		if (assetVarEffModelId[electrolyser]=="-1.0" || assetVarEffModelId[electrolyser]=="-1"){
			nomConvOutMaxPower = nomConvInMaxPower * convElecToH2Eff[electrolyser];
		} else {
			nomConvOutMaxPower = nl_eff_model(nomConvInMaxPower, varCostSegUpLim, varCostModelSegNumber, nlEffSegSlope, assetVarEffModelId[electrolyser]);
		}
		var nomAuxMaxPower = auxConvConsoCoeff * nomConvInMaxPower + auxConvConsoBase;
		var daThresholdPriceNom = h2Price * nomConvOutMaxPower/(nomConvInMaxPower+nomAuxMaxPower);
		
		for (var t in isDECISION_STEPS){
			if (DispLoadActivePower[electrolyser][t] > nomPowerMin[electrolyser] && (isFcrPeriod[t]==0) && (t != Opl.last(isDECISION_STEPS) && isFcrPeriod[Opl.next(isDECISION_STEPS,t)]==0)){
				daThresholdPrice[t] = h2Price * ConvH2Out[electrolyser][t] / NetElecImportTarget[t];
			} else {
				daThresholdPrice[t] = daThresholdPriceNom;			
			}	
		}
	} else {
		for (var t in isDECISION_STEPS){
			daThresholdPrice[t] = 0;
		}
	}
}