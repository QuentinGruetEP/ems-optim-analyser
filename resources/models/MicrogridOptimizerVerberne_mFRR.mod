...
float negative_imb_price[imb in isIMBALANCE_STEPS_POS] =
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM" || microgridName == "MICROGRID GP Brielle" || microgridName == "MICROGRID GP Vierpolders" || microgridName == "MICROGRID VERBERNE America" || microgridName =="MICROGRID VAN DEN BERG EST" || microgridName == "MICROGRID VERBERNE Melderslo"// if C&I in NL?
...
execute CPX_PARAM {
	...
	if (operationID == "1663")
		microgridName = "MICROGRID GP Vierpolders";
	if (operationID == "1597" || operationID == "1566" || operationID == "1826") // rajouter ici le nouvel id d'opé ugrid pour America avec procurement spot + mFRR quand créé
		microgridName = "MICROGRID VERBERNE America";
	if (operationID == "1727" )
		microgridName = "MICROGRID VAN DEN BERG EST";
	if (operationID == "1801")
...
// ctTest: DispGenActivePower["Verberne_America_CHP1"]["1"] == 0;
...
//		  		: AFRRCapacityPowerUp[s][assetStepAfrrVoluntaryStep[t]] >= 0);
	...
	forall(d in ismFRR_GEN_UP_ASSETS, t in isDECISION_STEPS: t != first(isDECISION_STEPS) && mFRRUpCapacityEngPower[assetStepmFRRStep[t]] > 0 && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) {
	...
	forall(d in ismFRR_GEN_UP_ASSETS, t in isDECISION_STEPS: mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] > 0) {
