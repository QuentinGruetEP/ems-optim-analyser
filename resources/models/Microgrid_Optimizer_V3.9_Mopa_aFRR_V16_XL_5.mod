...
float storArtificialPenalityCost[t in isDECISION_STEPS] = (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID GEG SYNERGIE MAURIENNE"
	? 0.0
...
float minACEnergyForAFRRPerMWDeficitCost = cat3Pen * penaltyBase;
...
dexpr float aFRRUpInitialAvailEnergyAC[s in isSTORAGES inter isAFRR_UP_ASSETS][t in isDECISION_STEPS] = (initialSOC[s] - (storElecMinSOC[s] / 100) * storMaxDCEnergy[s][t]) * (storefficiencyaFRR/100);
...
dexpr float AFRRUpAvailEnergyAC[s in isSTORAGES inter isAFRR_UP_ASSETS][t in isDECISION_STEPS] = maxl(0, (StorStoredDCEnergy[s][t] - (storElecMinSOC[s] / 100) * storMaxDCEnergy[s][t]) * (storefficiencyaFRR/100));
...
dexpr float AFRRDwnAvailEnergyAC[s in isSTORAGES inter isAFRR_DOWN_ASSETS][t in isDECISION_STEPS] = maxl(0,((storElecMaxSOC[s] / 100) * storMaxDCEnergy[s][t] - StorStoredDCEnergy[s][t]) * (100/storefficiencyaFRR));
...
dvar float+ MinACEnergyForAFRRUpDeficit[isE_STORAGES inter isAFRR_UP_ASSETS][isAFRR_VOLUNTARY_STEPS_POS];
// Energy deficit before entering aFRR Up cost over afrr voluntary steps (afrr_v in isAFRR_VOLUNTARY_STEPS)
dexpr float MinACEnergyForAFRRUpPerMWCost[s_afrr in isE_STORAGES inter isAFRR_UP_ASSETS][afrr_v in isAFRR_VOLUNTARY_STEPS_POS] = MinACEnergyForAFRRUpDeficit[s_afrr][afrr_v] * minACEnergyForAFRRPerMWDeficitCost;
...
dexpr float MinACEnergyForAFRRDwnPerMWCost[s_afrr in isE_STORAGES inter isAFRR_UP_ASSETS][afrr_v in isAFRR_VOLUNTARY_STEPS_POS] = MinACEnergyForAFRRDwnDeficit[s_afrr][afrr_v] * minACEnergyForAFRRPerMWDeficitCost; 
...
minimize
  ...
  + sum (s_afrr in isAFRR_UP_STORAGES, afrr_v in isAFRR_VOLUNTARY_STEPS_POS) MinACEnergyForAFRRUpPerMWCost[s_afrr][afrr_v] 
  + sum (s_afrr in isAFRR_DWN_STORAGES, afrr_v in isAFRR_VOLUNTARY_STEPS_POS) MinACEnergyForAFRRUpPerMWCost[s_afrr][afrr_v]
...
// Within a pool
	if (card(isNotEngagedAFRRUpSteps) != 0){
	...
	if (card(isNotEngagedAFRRDwnSteps) != 0){
...
//  		} 			
	...
	var anyAfrrUpSOCDeficit = false;
	var anyAfrrDwnSOCDeficit = false;
	
	for (var afrr_v in isAFRR_VOLUNTARY_STEPS_POS) {
		if (aFRRUpReqPower[afrr_v] < aFRRUpEngPower[afrr_v] - epsilon) {
			if (!anyAfrrUpEngPowerChange) {
				strDispatchStatusString += ", reductions of aFRR Up engagement";
				anyaFRRUpEngPowerChange = true;
 			}

			VIOLATIONS_OUTPUT.add("afrr_up_engagement_reduction_flag", "", afrr_v,  aFRRUpEngPower[afrr_v] - aFRRUpReqPower[afrr_v], 1);
		}
		
		if (aFRRDwnReqPower[afrr_v] < aFRRDwnEngPower[afrr_v] - epsilon) {
			if (!anyAfrrDwnEngPowerChange) {
				strDispatchStatusString += ", reductions of aFRR Down engagement";
				anyaFRRDwnEngPowerChange = true;
 			}

			VIOLATIONS_OUTPUT.add("afrr_down_engagement_reduction_flag", "", afrr_v,  aFRRDwnEngPower[afrr_v] - aFRRDwnReqPower[afrr_v], 1);
		}

		if (AFRRUpCapacityDeficit[afrr_v] > epsilon) {
			if (!anyAfrrUpEngPowerChange) {
				strDispatchStatusString += ", some AFRR Up requirement violation";
				anyAfrrUpEngPowerChange = true;
 			}
 			VIOLATIONS_OUTPUT.add("Pool_aFRR_Up_Power_requirement_violation", "", afrr_v,  AFRRUpCapacityDeficit[afrr_v], 1);
		}
		
		if (AFRRDwnCapacityDeficit[afrr_v] > epsilon) {
			if (!anyAfrrDwnEngPowerChange) {
				strDispatchStatusString += ", some AFRR Down requirement violation";
				anyAfrrDwnEngPowerChange = true;
		 	}
		 	VIOLATIONS_OUTPUT.add("Pool_aFRR_Down_Power_requirement_violation", "", afrr_v,  AFRRDwnCapacityDeficit[afrr_v], 1);
		}
	}
	
    /*for (s_afrr in isAFRR_UP_STORAGES)
    		for (var afrr_v in isAFRR_VOLUNTARY_STEPS_POS ) {
				writeln("Type de s_afrr: ", typeof(s_afrr), ", valeur: ", s_afrr);
				writeln("Type de afrr_v: ", typeof(afrr_v), ", valeur: ", afrr_v);
				if (MinACEnergyForAFRRUpDeficit[s_afrr][afrr_v] > epsilon) {
					if (!anyAfrrUpSOCDeficit) {
						strDispatchStatusString += ", some AFRR Up Energy soc deficit";
						anyAfrrUpSOCDeficit = true;
		 			}
		 			VIOLATIONS_OUTPUT.add("Pool_energy_soc_for_aFRR_up_deficit", "", afrr_v,  MinACEnergyForAFRRUpDeficit[s_afrr][afrr_v], 10);
				}
 			}	
 	for (s_afrr in isAFRR_DWN_STORAGES)
    		for (var afrr_v in isAFRR_VOLUNTARY_STEPS_POS ) {		
				if (MinACEnergyForAFRRDwnDeficit[s_afrr][afrr_v] > epsilon) {
					if (!anyAfrrDwnSOCDeficit) {
						strDispatchStatusString += ", some AFRR Down Energy soc deficit";
						anyAfrrDwnSOCDeficit = true;
		 			}
		 			VIOLATIONS_OUTPUT.add("Pool_energy_soc_for_aFRR_down_deficit", "", afrr_v,  MinACEnergyForAFRRDwnDeficit[s_afrr][afrr_v], 10);
				}
			}	*/			
