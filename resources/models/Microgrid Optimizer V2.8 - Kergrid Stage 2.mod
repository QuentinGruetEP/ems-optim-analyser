/***************************************************
 * Microgrid Optimisation Model v2.8 w. KERGRID stage 2
 * Author: n.bergevin
...
// hard-coded parameters in this model for Enercal's Ile des Pins and Mar� microgrids
//		- physical min power for thermal generators set to 5 kW for IDP microgrid (see physMinThermalGenActivePower param)
...
// hard-coded parameters in this model for Enercal's Mar� microgrid
...
//		- minimum charging rate for each charging point on charging station 12 set to 9kW (see ctStorageChargeMinStation12 constraint)
//		- minimum charging rate for each charging point on charging station 13 set to 2.5kW (see ctStorageChargeMinStation13 constraint)
//		- maximum charging rate for charging station used by MORB_ENERGIES_Kergrid_V1G_C1 and MORB_ENERGIES_Kergrid_V1G_C2 vehicles set to 18kW (see isCONG_V1GC1C2 set and ctCongV1GC1C2 constraint)
...
// ENERCAL HARD-CODED
float electricityTariff = (microgridName == "Microgrid Srisangtham Microgrid" ? 3.0 : 0.0);
...
float physMinThermalGenActivePower = (microgridName == "MICROGRID ENERCAL Ile des Pins" || microgridName == "MICROGRID ENERCAL Mare" ? 5.0 : 0.0);
...
int inverterNbr[isSTORAGES] = [
	(
		microgridName == "MICROGRID ENERCAL Ile des Pins"
			? 6										// For Ile des Pins, nominal number of inverters = 6
			:
			(
				microgridName == "MICROGRID ENERCAL Mare"
					? 8									// For Mar�, nominal number of inverters = 8
					: 1									// for other microgrids, nominal number of inverters = 1
			)
	)];
float hardCodedPowerMin[s in isSTORAGES] =
	(
		microgridName == "MICROGRID ENERCAL Ile des Pins"
			? 1040.0										// For Ile des Pins, nominal max power = 1040 kW
			:
			(
				microgridName == "MICROGRID ENERCAL Mare"
					? 1047.0									// For Mar�, nominal max power = 800 kW
					: -powerMin[s]							// for other microgrids, we use data from Everest
			)
	);
...
// allowed in storage asset s over decision step t (s in STORAGES, t in DECISION_STEPS)
// HARD-CODED
float strictMinSOC[s in isSTORAGES] = (microgridName == "MICROGRID ENERCAL Ile des Pins" || microgridName == "MICROGRID ENERCAL Mare" ? 5.0 : minSOC[s]);
// asset's site
...
// asset's linear factor of max reactive power approximation
float aQmax[isASSETS];
// asset's constant term of max reactive power approximation
float bQmax[isASSETS];
// asset's linear factor of min reactive power approximation
float aQmin[isASSETS];
// asset's constant term of min reactive power approximation
float bQmin[isASSETS];
...
int availabilityStorage[s in isSTORAGES][t in isDECISION_STEPS] = (availability[s][t] == 1 ? 1 : -1 );
...
float storArtificialPenality[t in isDECISION_STEPS] =
	(
		microgridName == "MICROGRID ENERCAL Ile des Pins" || microgridName == "MICROGRID ENERCAL Mare" 
			? stepIndex[t] / optimisationStepNumber * 10.0	// for IDP and Mar�, we want to encourage early discharge (see StorChargeDischargeCost var)
			: (
...
//float storArtificialPenality[t in isDECISION_STEPS] = (optimisationStepNumber - stepIndex[t]) / optimisationStepNumber / 10.0;
	...
	for (var s in isSTORAGES){
		...
		if (microgridName == "MICROGRID ENERCAL Ile des Pins") {
			if (availInverterNbr[s] >= inverterNbr[s]) {
				aQmax[s] = -0.26241;	// discharge
				bQmax[s] = 1890.0;		// discharge
				aQmin[s] = -0.26241;	// charge
				bQmin[s] = 1890.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-1) {
				aQmax[s] = -0.26241;	// discharge
				bQmax[s] = 1575.0;		// discharge
				aQmin[s] = -0.26241;	// charge
				bQmin[s] = 1575.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-2) {
				aQmax[s] = -0.26241;	// discharge
				bQmax[s] = 1260.0;		// discharge
				aQmin[s] = -0.26241;	// charge
				bQmin[s] = 1260.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-3) {
				aQmax[s] = -0.26241;	// discharge
				bQmax[s] = 945.0;		// discharge
				aQmin[s] = -0.26241;	// charge
				bQmin[s] = 945.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-4) {
				aQmax[s] = -0.26241;	// discharge
				bQmax[s] = 630.0;		// discharge
				aQmin[s] = -0.26241;	// charge
				bQmin[s] = 630.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-5) {
				aQmax[s] = -0.26241;	// discharge
				bQmax[s] = 315.0;		// discharge
				aQmin[s] = -0.26241;	// charge
				bQmin[s] = 315;			// charge
			}		
			if (availInverterNbr[s] <= inverterNbr[s]-6) {
				aQmax[s] = 0.0;		// discharge
				bQmax[s] = 0.0;		// discharge
				aQmin[s] = 0.0;		// charge
				bQmin[s] = 0.0;		// charge
			}
		}			
		...
		if (microgridName == "MICROGRID ENERCAL Mare") {
			if (availInverterNbr[s] >= inverterNbr[s]) {
				aQmax[s] = -0.16294;	// discharge
				bQmax[s] = 2520.0;		// discharge
				aQmin[s] = -0.16294;	// charge
				bQmin[s] = 2520.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-1) {
				aQmax[s] = -0.16294;	// discharge
				bQmax[s] = 2205.0;		// discharge
				aQmin[s] = -0.16294;	// charge
				bQmin[s] = 2205.0;		// charge
			}		
			if (availInverterNbr[s] == inverterNbr[s]-2) {
				aQmax[s] = -0.16294;	// discharge
				bQmax[s] = 1890.0;		// discharge
...
dexpr float InterProdMaxReactivePower[p in isINTER_PRODS][t in isDECISION_STEPS] = (
	maxl(maxInterProdActivePower[p],interProdActivePowerForecast[p][t]) > 0.0
		? aQmax[p] * InterProdActivePower[p][t] + bQmax[p]
		: 0.0
	);
// dvar float+ InterProdMaxReactivePower[isINTER_PRODS][isDECISION_STEPS];
...
// max reactive power discharge (expressed in kVAR) for storage asset s over decision step t 
// (s in STORAGES,t in DECISION_STEPS)
dexpr float StorMaxReactivePowerDischarge[s in isSTORAGES][t in isDECISION_STEPS] = (
	maxStorACActivePowerDischarge[s] > 0.0
		? aQmax[s] * (StorACPowerDischarge[s][t] + StorACPowerCharge[s][t]) + bQmax[s]
		: 0.0
	);
//dvar float+ StorMaxReactivePowerDischarge[isSTORAGES][isDECISION_STEPS];
...
// max reactive power charge (expressed in kVAR) for storage asset s over decision step t 
// (s in STORAGES,t in DECISION_STEPS)
dexpr float StorMaxReactivePowerCharge[s in isSTORAGES][t in isDECISION_STEPS] = (
	maxStorACActivePowerCharge[s] > 0.0
		? aQmin[s] * (StorACPowerDischarge[s][t] + StorACPowerCharge[s][t]) + bQmin[s]
		: 0.0
	);
//dvar float+ StorMaxReactivePowerCharge[isSTORAGES][isDECISION_STEPS];
...
// (s in STORAGES, t in DECISION_STEPS)
dexpr float StorReactiveRaiseReserve[s in isSTORAGES][t in isDECISION_STEPS] = StorMaxReactivePowerDischarge[s][t] - StorReactivePower[s][t];
// active power lower reserve (expressed in kW) from storage asset s over decision step t
// (s in STORAGES, t in DECISION_STEPS)
dexpr float StorReactiveLowerReserve[s in isSTORAGES][t in isDECISION_STEPS] = StorMaxReactivePowerCharge[s][t] + StorReactivePower[s][t];
// Connection to grid
/////////////////////
...
dexpr float NFLoadReactivePower[n in isNF_LOADS][t in isDECISION_STEPS] = (
	maxl(maxNFLoad[n],NFLoadForecast[n][t]) > 0.0
		? aQmax[n] * NFLoadActivePower[n][t] + bQmax[n]
		: 0.0
	);
// Flexible load units
//////////////////////
...
dexpr float FlexLoadMaxReactivePower[f in isFLEX_LOADS][t in isDECISION_STEPS] = (
	maxl(maxFlexLoad[f],flexLoadForecast[f][t]) > 0.0
		? aQmax[f] * FlexLoadActivePower[f][t] + bQmax[f]
		: 0.0
	);
// dvar float+ FlexLoadMaxReactivePower[isFLEX_LOADS][isDECISION_STEPS];
...
dexpr float FlexLoadMinReactivePower[f in isFLEX_LOADS][t in isDECISION_STEPS] = (
	minFlexLoad[f] > 0.0
		? aQmin[f] * FlexLoadActivePower[f][t] + bQmin[f]
		: 0.0
	);
// dvar float+ FlexLoadMaxReactivePower[isFLEX_LOADS][isDECISION_STEPS];
...
// (g in THERMAL_GENS,t in DECISION_STEPS)
dexpr float ThermalGenMaxReactivePower[g in isTHERMAL_GENS][t in isDECISION_STEPS] = aQmax[g] * ThermalGenActivePower[g][t] + IsGenOn[g][t] * bQmax[g];
// dvar float+ ThermalGenMaxReactivePower[isTHERMAL_GENS][isDECISION_STEPS];
...
dexpr float StorChargeDischargeCost =
 	sum(t in isDECISION_STEPS, s in isSTORAGES) storArtificialPenality[t]
 	* (
 		microgridName == "MICROGRID MORBIHAN ENERGIES FlexMobIle" || microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid"	// for FlexMob'Ile and Kergrid, we want to encourage late charge (see storArtificialPenality param)
 			? StorACPowerCharge[s][t] /* modif FlexMob'Ile */+ StorACPowerDischarge[s][t]
 			: StorACPowerDischarge[s][t]												// for IDP and Mar�, we want to encourage early discharge (see storArtificialPenality param) 
 	  )
 	* stepDurationInHours[t];
 ...
 *********************************************************************/
  subject to {
//	forall (t in isDECISION_STEPS, i in isINTER_PRODS)
//	  ctTest: InterProdPowerCurtailment[i][t] <= 0.0; 
//	forall (t in isDECISION_STEPS: ord(isDECISION_STEPS, t) <= 3)
//	  ctTest2: sum (g in isTHERMAL_GENS) IsGenOn[g][t] <= 0.0; 
...
//	forall (g in isTHERMAL_GENS diff {"ENERCAL_IDP_GE_1_NC"}, t in isDECISION_STEPS)
...
// Power generation from an thermal production asset is limited by the maximum and the minimum power generation possible for that asset. 
	...
	forall (g in isTHERMAL_GENS, t in isDECISION_STEPS)
	  ctThermalGenPhysMinPower:
	  	minThermalGenActivePower[g] * IsGenOn[g][t] - ThermalGenMinActivePowerDeficit[g][t] >= physMinThermalGenActivePower * IsGenOn[g][t];
	forall(g in isTHERMAL_GENS, t in isDECISION_STEPS)
...
if (lastMinuteCurtOption == 1 && card(isINTER_PRODS) > 0 && card(isSTORAGES) > 0) { 
		forall (t in isDECISION_STEPS)
	  		ctAuthorizedCurt1 : sum(s in isSTORAGES : maxStorACActivePowerCharge[s] > 0)(maxCharge[s] * maxSOC[s] / 100 - StorStoredDCEnergy[s][t]) 
	  			<= card(isSTORAGES) * max(s in isSTORAGES) maxCharge[s] * (1 - AreAllStoragesFull[t]);
		forall (t in isDECISION_STEPS)
...
// HARD-CODED
// For Kergrid microgrid's charging station #12, power consumption by a storage asset in charge is limited by its minimum charge rate
	if (microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid") {
		forall (s in isSTORAGES inter {"MORB_ENERGIES_Kergrid_V1G_C1", "MORB_ENERGIES_Kergrid_V1G_C2"}, t in isDECISION_STEPS)
		  ctStorageChargeMinStation12: StorACPowerCharge[s][t] >= 9 * IsCharging[s][t];
  }		  

// For Kergrid microgrid's charging station #13, power consumption by a storage asset in charge is limited by its minimum charge rate
	if (microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid") {
		forall (s in isSTORAGES inter {"MORB_ENERGIES_Kergrid_V1G_C13b"}, t in isDECISION_STEPS)
		  ctStorageChargeMinStation13: StorACPowerCharge[s][t] >= 2.5 * IsCharging[s][t];
  }		  

// Power injection by a storage asset in charge is limited by its maximum discharge rate
	forall (s in isSTORAGES, t in isDECISION_STEPS)
...
// Target SOC
    if (card(isSTORAGES)> 0)
    {
    	...
    	{
    		if (socTargetStorage[s][t] !=-1.0 && availabilityStorage[s][t] == 1) 
    		{
    		ctStorageTargetSoc: 100 * StorStoredDCEnergy[s][t] / maxCharge[s] >= socTargetStorage[s][t]- SocTargetStorageDeficit[s][t];
    		}
    	}
    }
//// Maximum reactive power discharge for storage asset s over step t is linear function of s's active power discharge over t
//// modelled as dexpr
//	forall (s in isSTORAGES, t in isDECISION_STEPS)
//	  ctStorMaxReactiveDischarge:
//	  	StorMaxReactivePowerDischarge[s][t] == aQmax[s] * StorACPowerDischarge[s][t] + (1 - IsCharging[s][t]) * bQmax[s];
// Reactive power discharge for storage asset s over step t is limited by s's max recative discharge at t
	forall (s in isSTORAGES, t in isDECISION_STEPS)
	  ctStorageReactiveDischarge:
	  	StorReactivePowerDischarge[s][t] <= StorMaxReactivePowerDischarge[s][t]; 

//// Maximum reactive charge power for storage asset s over step t is linear function of s's active power discharge over t
//// modelled as dexpr
//	forall (s in isSTORAGES, t in isDECISION_STEPS)
//	  ctStorMaxReactiveCharge:
//	  	StorMaxReactivePowerCharge[s][t] == aQmin[s] * StorACPowerCharge[s][t] + IsCharging[s][t] * bQmin[s];
// Reactive power charge for storage asset s over step t is limited by s's max recative charge at t
	forall (s in isSTORAGES, t in isDECISION_STEPS)
	  ctStorageReactiveCharge:
	  	StorReactivePowerCharge[s][t] <= StorMaxReactivePowerCharge[s][t]; 
...
// Specific congestion contraints
	if (microgridName == "MICROGRID MORBIHAN ENERGIES Kergrid") {
	 	forall (t in isDECISION_STEPS)
	 	  ctCongV1GC1C2 : sum (s in isSTORAGES inter isCONG_V1GC1C2) StorACActivePower[s][t] >= -18.0;
	 forall (t in isDECISION_STEPS)
	 	  ctCongV1GC6aToC19b : sum (s in isSTORAGES inter isCONG_V1GC6aToC19b) StorACActivePower[s][t] >= -76.0;
  } 	  
 }
