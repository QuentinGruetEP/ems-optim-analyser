...
// hard-coded parameters in this model for MOPABLOEM and Globe Plant Brielle and Vierpoldres microgrids
...
{string} isNF_LOADS = {a.asset_id | a in ASSETS: a.type == "LOAD" && a.asset_id != "E_boiler_1000kW" && a.asset_id != "E_boiler_1200kW" && a.asset_id != "GP_BRIELLE_EBOILER" && a.asset_id != "GP_Vierpolders_EBOILER"};
...
float negative_imb_price[imb in isIMBALANCE_STEPS_POS] =
			(microgridName == "MICROGRID MOPABLOEM" || microgridName == "MICROGRID GP Brielle" || microgridName == "MICROGRID GP Vierpolders"
				// Upward activation ?
