...
{string} isFCR_ASSETS = (microgridName == "MICROGRID VALOREM LIMOUX") ? isE_ASSETS inter {"VALOREM_Limoux_BESS"} :
						...
						{};
{string} isAFRR_ASSETS = (microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B"} ) ? isE_STORAGES : {};
...
{string} isAFRR_UP_ASSETS = (microgridName == "MICROGRID MOPABLOEM" ) ? isE_ASSETS inter {"BESS_MOPABLOEM","CHP_1600kW"}:
						    (microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B"}) ? isE_STORAGES :
 						    {};
		...
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM"				// if MOPABLOEM
				...
				: (daElecPrice[imbStepHourlyStep[imb]] >= 0		// if not MOPABLOEM
				    	? (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_AFRR" ? daElecPrice[imbStepHourlyStep[imb]] * 2 + 50/1000: daElecPrice[imbStepHourlyStep[imb]] * 1.2)
				    	: (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_AFRR" ? daElecPrice[imbStepHourlyStep[imb]] / 2 + 50/1000: daElecPrice[imbStepHourlyStep[imb]] / 1.2))
			)
		);
				    	...
				    	: (daElecPrice[imbStepHourlyStep[imb]] >= 0	// if no downward mFFR activation
				    		? (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_AFRR"
				    			? (daElecPriceRaw[imbStepHourlyStep[imb]] >= sellOfferPrice	// are DA prices >= offer prices?
				    				? daElecPrice[imbStepHourlyStep[imb]] / 2 - 50/1000
				    				: daElecPrice[imbStepHourlyStep[imb]] * 2 - 50/1000	// we penalise positive imbalances when DA prices are below sell offer prices
				    			)
				    			: (daElecPriceRaw[imbStepHourlyStep[imb]] >= sellOfferPrice	// are DA prices >= offer prices?
				    				? daElecPrice[imbStepHourlyStep[imb]] / 1.2
				    				: daElecPrice[imbStepHourlyStep[imb]] * 2 - 50/1000	// we penalise positive imbalances when DA prices are below sell offer prices, because it is pretty dangerous
				    			)
				    		)
				  	   		: (optContext == "MARKET_STRAT_FCR" || optContext == "MARKET_STRAT_DA" || optContext == "MARKET_STRAT_AFRR"
				  	   			? (daElecPriceRaw[imbStepHourlyStep[imb]] >= sellOfferPrice	// are DA prices >= offer prices?
...
int isP0Submitted = (microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B"}  && card ({m.step_index | m in MARKET_ENGAGEMENTS : m.type == "P0"}) > 0 ?
							1
							:0);
...
float poolaFRRCertifiedPower = 
							(microgridName == "MICROGRID ZEE EDR1") ? 3000 
...
//	forall (t in isDECISION_STEPS : ord(isDECISION_STEPS,t) >= 60, i in isINTER_E_GENS)
//	ctTEST : InterGenPowerCurtailment[i][t] == 0;
...
// For ZEE, during aFRR bidding over non-cleared steps, we prevent BESS to do spot arbitration, as long as we have current bidding strat (forcing step by step aFRR engagement)
// Then for spot bidding, BESS can be valorised on spot if there is the opportunity
	forall (s in isAFRR_STORAGES, t in isDECISION_STEPS : isDAStepCleared[assetStepHourlyStep[t]] == 0 && microgridName in {"MICROGRID ZEE EDR1", "MICROGRID GDS Leforest B"}  && optContext == "MARKET_STRAT_AFRR")
	  ctZEEBESSNoSpotArbitration:
...
// Constraints for HPP in CFD or spot : for DA bid, we don't want to curtail PV based on price forecast, beacuse price is not certain, then we force to be at least equal to forecast or avail congestion - flex engagements
	if (microgridName in {"MICROGRID GDS Leforest B", "MICROGRID VALOREM LIMOUX"} && optContext == "MARKET_STRAT_DA") {
		forall (si in isSITES, t in isDECISION_STEPS : isDAStepCleared[assetStepHourlyStep[t]] == 0)  
			ctNoCurtailHPPDAbid: 
				sum(i in isINTER_E_GENS : siteID[i] == si) InterGenActivePower[i][t] >= 
										maxl(0, 
											minl(sum(i in isINTER_E_GENS : siteID[i] == si) interGenActivePowerForecast[i][t] - sum(nf in isNF_E_LOADS : siteID[nf] == si) NFLoadActivePower[nf][t], 
												maxOutput[si][t] - sum(fcr_a in isFCR_ASSETS: si == siteID[fcr_a]) FCRPower[fcr_a][assetStepFCRStep[t]] - sum(afrr_a in isAFRR_ASSETS: si == siteID[afrr_a]) afrrMaxAssetBandwith[afrr_a] * IsAFRRUp[afrr_a][assetStepAfrrVoluntaryStep[t]])
											);
	}
//Specific constraint pour ZEE EDR 1 for now
// When submitted to P0, max PV possible is the max output of the site minus BESS max bandwih during aFRR
// Rest of the time, 
	if (microgridName == "MICROGRID ZEE EDR1") {
		forall (si  in isSITES, i in isINTER_E_GENS : first(siteIdByAsset[i]) == si , t in isDECISION_STEPS : isSubmitedP0[assetStepHourlyStep[t]] == 1)
			ctPotentialNetInterGenPV4: InterGenActivePower[i][t] <= minl(interGenActivePowerForecast[i][t], maxl(0,maxOutput[si][t] - sum(s in isE_STORAGES :  first(siteIdByAsset[s]) == si) afrrMaxAssetBandwith[s] * IsAFRRUp[s][assetStepAfrrVoluntaryStep[t]]));
...
if (microgridName in {"MICROGRID GDS Leforest A", "MICROGRID GDS Leforest B"}) {
		forall (i in isINTER_E_GENS inter {"GENERALE_DU_SOLAIRE_Leforest_A_PV", "GENERALE_DU_SOLAIRE_Leforest_B_PV", "GENERALE_DU_SOLAIRE_Leforest_B_1MW_PV"}, t in isDECISION_STEPS : isDAStepCleared[assetStepHourlyStep[t]]==1)
...
if (microgridName in {"MICROGRID NEXHOS Montbert", "MICROGRID NEXHOS Colisee"}) {
		forall (i in isINTER_E_GENS inter {"NEXHOS_MONTBERT_PV", "NEXHOS_COLISEE_PV"}, t in isDECISION_STEPS : isDAStepCleared[assetStepHourlyStep[t]]==1)
...
if (card(isNotSubmitedP0Steps) >0 ){
...
//													-minl(sum(si in isSITES : si != "ZEE_Saint_Sauveur")(min(t in isDECISION_STEPS : assetStepHourlyStep[t] == h) maxOutput[si][t]), 
//													sum(i in isINTER_E_GENS, t in isDECISION_STEPS : assetStepHourlyStep[t] == h) minl(interGenActivePowerForecast[i][t], maxl(0,maxOutput[si][t] - sum(s in isE_STORAGES :  first(siteIdByAsset[s]) == si) afrrMaxAssetBandwith[s]))/4)
													-sum(si in isSITES : si != "ZEE_Saint_Sauveur") (
													    sum(i in isINTER_E_GENS : first(siteIdByAsset[i]) == si, t in isDECISION_STEPS : assetStepHourlyStep[t] == h)
													        minl(
													            interGenActivePowerForecast[i][t],
...
// 				assetPowerTarget[i][t] = -InterGenActivePowerEnd[i][t];				// respects Everest's sign convention (-ve values = power generation)
 				...
 				if ((microgridName == "MICROGRID GDS Leforest A" || microgridName == "MICROGRID GDS Leforest B")
 					&& daElecPriceRaw[assetStepHourlyStep[t]] < 0.0 || Math.abs(availGlobalForPmin[i][t])<epsilon) 
