...
// indexing set of mfrr capacity steps with only positive values
...
{string} isNF_LOADS = {a.asset_id | a in ASSETS: a.type == "LOAD" && a.asset_id != "E_boiler_1000kW" && a.asset_id != "E_boiler_1200kW" && a.asset_id != "Van_Den_Berg_Est_Eboiler1"};
...
// For joshua => i added e-boiler for Brielle amory and nico c think we need it to fully compaensate CHP's Pmin...
{string} isAFRR_UP_ASSETS = (
	(microgridName == "MICROGRID MOPABLOEM") ? isE_ASSETS inter {"BESS_MOPABLOEM","CHP_1600kW"} :
	(microgridName == "MICROGRID GP Brielle") ? isE_ASSETS inter {"GP_BRIELLE_BESS","GP_BRIELLE_CHP", "GP_BRIELLE_EBOILER"} :
	{}
	);
...
int isaFRRCapacityStepNumberSpecified = card({o.param_val | o in OPERATION: o.param_id == "afrr_capacitiy_step_number"});
// Number of aFRR capacity steps (NOT USED, to be deleted?)
int aFRRCapacityStepNumber = (isaFRRCapacityStepNumberSpecified > 0 ? intValue(first({o.param_val | o in OPERATION: o.param_id == "afrr_capacitiy_step_number"})) : 0);
// flag indicating if the afrrCapacityStepDuration is present in the input
int isaFRRCapacityStepDurationSpecified = card({o.param_val | o in OPERATION: o.param_id == "afrr_capacitiy_step_duration"});
...
int afrrCapacityStepDuration = (isaFRRCapacityStepDurationSpecified > 0 ? intValue(first({o.param_val | o in OPERATION: o.param_id == "afrr_capacitiy_step_duration"})): 0);
...
float storElecDailyMaxCycles [s in isE_STORAGES] =  (dailyMaxCycles[s] < 0.0 ? -1 : dailyMaxCycles[s]/2.0);
...
// HARD-CODED
// maximum activation duration (expressed in hours) for CHP for MICROGRID VAN DEN BERG EST microgrid on mFRR
float maxActivationDurationOnmFRRInHour = (microgridName == "MICROGRID VAN DEN BERG EST" ? 1.5 : 0.0);
// HARD-CODED
...
float sellOfferPrice = (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID VALECO Baignollais") ? 1.0 / 1000.0	// Valorem pays EP 0.4 �/MWh so we decided that it was not profitable for them to generate for less than 1 �/MWh
						: (microgridName in {"MICROGRID GDS Leforest A", "MICROGRID GDS Leforest B"}) ? -0.10 / 1000.0	// GDS is on a contract for difference agreement so it is profitable for them to generate as long as prices are >= 0 �/MWh
						: (microgridName in {"MICROGRID NEXHOS Montbert", "MICROGRID NEXHOS Colisee"}) ? -0.10 / 1000.0	// NEXHOS is on a contract for difference agreement so it is profitable for them to generate as long as prices are >= 0 �/MWh
						:  0.0;
		...
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			// if C&I in NL
				? (mFRRactivatedPower_imb[imb] < 0				// if upward mFRR activation
					? 1 // we need to hcnge this with the forced power code
		...
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			// if C&I in NL
			    ...
			    : (mFRRactivatedPower_imb[imb] > 0					// if not MOPABLOEM but downward mFFR activation
				    	? -1 // we need to hcnge this with the forced power code
						// We put an arbitrary price, less favourable than day-ahead elec spot price
...
int afrrUpCertfiedPower[a in isASSETS] =
	...
	(microgridName ==  "MICROGRID GP Brielle") ? (a == "GP_BRIELLE_BESS" ? 1500 : a == "GP_BRIELLE_CHP" ? 1700 : a == "GP_BRIELLE_EBOILER" ? 1200 : 0) :
	 0;						  
...
int mfrrUpCertfiedPower[a in isASSETS] = (microgridName ==  "MICROGRID VAN DEN BERG EST") ? (a == "Van_Den_Berg_Est_CHP1" ? 1000 : 0) : 0;
...
int mfrrDwnCertfiedPower[a in isASSETS] =(microgridName ==  "MICROGRID VAN DEN BERG EST") ? (a == "Van_Den_Berg_Est_CHP1" ? 0 : 0) : 0;
...
float mFRRUpAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] = 
	(a in ismFRR_UP_ASSETS ?
...
// availability can be included into max/min power.
float mFRRDwnAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] = 
	(a in ismFRR_DOWN_ASSETS ?
...
execute CPX_PARAM {
	...
	if (operationID == "1335" || operationID == "1530")
...
//		}
	...
	for (var d in isDISP_EH_GENS){
		...
		// Values for MICROGRID Van den Berg Est	
...
// possible heat that can be generated on mFRR (expressed in kwh) of all disp generator d over the maxActivationDurationOnmFRRInHour / assetStepDurationInHours mFRR steps coming up
// (t in DECISION_STEPS)
...
dvar float+ MFRRCapacityPowerUp[isASSETS][ismFRR_STEPS];
// idem, in MW
dexpr float MFRRCapacityPowerUp_MW[a in isASSETS][mfrr in ismFRR_STEPS] = MFRRCapacityPowerUp[a][mfrr] / 1000;
// Maximum power of service, in kW, which an asset participates to mFRR down, on mfrr step, to reach mFRR down commitment.
dvar float+ MFRRCapacityPowerDwn[isASSETS][ismFRR_STEPS];
// idem, in MW
dexpr float MFRRCapacityPowerDwn_MW[a in isASSETS][mfrr in ismFRR_STEPS] = MFRRCapacityPowerDwn[a][mfrr] / 1000;
...
dvar boolean IsMFRRUp[isASSETS][ismFRR_STEPS];
...
dvar boolean IsMFRRDwn[isASSETS][ismFRR_STEPS];
...
//  forall (t in isDECISION_STEPS : ord(isDECISION_STEPS, t)==0)
// 	  ctTest:
// 	  	DispGenActivePower["Van_Den_Berg_Est_CHP1"][t] == 0;
...
forall (mfrr in ismFRR_STEPS_POS)
	  ctMFRRUpPoolEngagement:
	  	mFRRUpReqPower[mfrr] == sum(a in isASSETS) MFRRCapacityPowerUp[a][mfrr] + MFRRUpPowerEngDeficit[mfrr];
// mFRR down engagement
forall (mfrr in ismFRR_STEPS_POS)
	  ctMFRRDwnPoolEngagement:
	  	mFRRDwnReqPower[mfrr] == sum(a in isASSETS) MFRRCapacityPowerDwn[a][mfrr] + MFRRDownPowerEngDeficit[mfrr];	  	  
...
// HARD-CODED
	...
	forall(d in isDISP_H_GENS inter {"Van_Den_Berg_Est_CHP1"}, t in isDECISION_STEPS: t != first(isDECISION_STEPS) && mFRRUpCapacityEngPower[assetStepmFRRStep[t]] > 0 && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) {
	  ctForceCHPToBeOff_current:
	  	IsGenOn[d][t] <= 0;
	  ctForceCHPToBeOff_previous:
        IsGenOn[d][prev(isDECISION_STEPS, t)] <= 0;
	}
	...
	forall(d in isDISP_H_GENS inter {"Van_Den_Berg_Est_CHP1"}, mfrr in ismFRR_STEPS_POS: mFRRUpCapacityEngPower[mfrr] > 0 && mFRRactivatedPower_mFRR[mfrr] == 0) {
	  ctCHPHeatProduction:
	  	PossibleDispGenHeatGenOnMfrrUp[d][mfrr] == MFRRCapacityPowerUp[d][mfrr] * disGenHeatElecRatio[d];
	} 
	}
...
// Maximum input and output
 	forall (si in isSITES, t in isDECISION_STEPS)
	  ...
	  - sum(mfrr_dwn in ismFRR_DOWN_ASSETS: si == siteID[mfrr_dwn]) MFRRCapacityPowerDwn[mfrr_dwn][assetStepmFRRStep[t]]   
 	...
 	forall (si in isSITES, t in isDECISION_STEPS)
	  ...
	  + sum(mfrr_up in ismFRR_UP_ASSETS: si == siteID[mfrr_up]) MFRRCapacityPowerUp[mfrr_up][assetStepmFRRStep[t]]
 	  + SiteMaxInputViolation[si][t] - SiteMaxOutputViolation[si][t]
 	  <= maxOutput[si][t];
...
// Generic congestion contraints
 	forall (c in isCONGESTIONS, t in isDECISION_STEPS)
 	  ...
 	  congestionLowerLim[c] <=
		...
		- sum(mfrr_dwn in ismFRR_DOWN_ASSETS) mfrrDwnAssetFactor[c][mfrr_dwn] * MFRRCapacityPowerDwn[mfrr_dwn][assetStepmFRRStep[t]]
 	 ...
 	 forall (c in isCONGESTIONS, t in isDECISION_STEPS)
 	  ctCongestionsOut :
		...
		+ sum(mfrr_up in ismFRR_UP_ASSETS) mfrrUpAssetFactor[c][mfrr_up] * MFRRCapacityPowerUp[mfrr_up][assetStepmFRRStep[t]]
 	  	+ CongestionLowerLimViolation[c][t] - CongestionUpperLimViolation[c][t]
...
// we should not charge and discharge in the same step
	forall (s in isH_STORAGES, t in isDECISION_STEPS)
	  ctChargeOrDischarge: IsCharging[s][t] + IsDischarging[s][t] <= 1;
...
// Incremental charge / discharge definition
	forall (s in isH_STORAGES: storHeatDischargeEfficiency[s] > 0, t in isDECISION_STEPS)
	  ...
	  - (100 / storHeatDischargeEfficiency[s]) * StorHeatDischarge[s][t];
	forall (s in isH_STORAGES: storHeatDischargeEfficiency[s] <= 0, t in isDECISION_STEPS)
	  ctHeatStorageIncrCharge0: StorStepHeatIn[s][t] ==  (storHeatChargeEfficiency[s] / 100) * StorHeatCharge[s][t];
...
// Maximum Avail heat energy
 	forall (s in isH_STORAGES, t in isDECISION_STEPS)
 	  ctHeatStorageMaxEnergy: StorStoredHeatAvail[s][t] + PossibleDispGenHeatGenOnMfrrUpAll[t] + maxActivationDurationOnmFRRInHour / mFRRStepDurationInHours * mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] <= storMaxUsefullAvailHeat[s][t] + HeatStorEnerMinExcess[s][t];
...
// 				assetPowerTarget[i][t] = -InterGenActivePowerEnd[i][t];				// respects Everest's sign convention (-ve values = power generation)
 				...
 				if (microgridName == "MICROGRID NEXHOS Montbert" || microgridName == "MICROGRID NEXHOS Colisee") {
 					...
 					if (daElecPriceRaw[assetStepHourlyStep[t]] < 0.0)
