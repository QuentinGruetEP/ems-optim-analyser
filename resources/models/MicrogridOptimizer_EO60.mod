...
//		- ratio giving heat enery generated per unit of elec energy generated set to 1.9 / 1.6 (see dispGenHeatElecRatio param)
...
//		- energy conversion efficiency expressed as a ratio for e-boilers set to 0.93 (see assetEnergyConvEfficiency param)
//		- heat congestion for combined output from CHP and 2 e-boilers set to 50.0MW (see ctHeatCongestion constraint)
...
//		- heat/elec ratio giving the heat energy produced by CHPs per unit of elec energy set to 35.9 / 55.9 and 20.67 / 44.04 (see assetHeatElecRatio param)
//		- energy conversion efficiency expressed as a ratio for heat pumps set to 3.0 (see assetEnergyConvEfficiency param)

// hard-coded parameters in this model for Verberne Melderslo microgrid
//		- heat/elec ratio giving the heat energy produced by CHP per unit of elec energy set to 1.175059952 (see assetHeatElecRatio param), same as Van Den Berg for now (waiting for Sten's update)
...
tuple t_assets {
	...
	float initial_gas_energy;				// in kWh: initial gas energy 	
...
tuple t_asset_steps {
	...
	float forced_power;                          // asset active power for each optimisation step during the optimisation period, entered on the portal side by the customer
}
...
tuple t_market_engagements {
 	key string type;				// Name of the market (DAY_AHEAD / ELECTRICITY_LONG_TERM_AGREGATED_BIDS  / MFRR_R3_ACTIVATION / FCR)
...
{string} isDECISION_STEPS_WITHOUT_FIRST = { t | t in isDECISION_STEPS : t != first(isDECISION_STEPS) };
...
// indexing set of step engaged on mfrr Up
{string} isENGAGED_MFRR_UP_STEPS = {mfrr.step_index | mfrr in MARKET_ENGAGEMENTS : mfrr.type == "MFRR_R3_CAPACITY_UP" && mfrr.engagement > 0};
...
{t_assets} E_ASSETS = (
	...
	? ASSETS inter {a | a in ASSETS:
		...
		a.asset_id == "GP_VIERPOLDERS_TRANSFO_CHP" ||
		a.asset_id == "GP_VIERPOLDERS_TRANSFO_EBOILER" ||
		a.asset_id == "dummy_congestion_asset"
		}
	...
	: microgridName == "MICROGRID VAN DEN BERG EST"
	? ASSETS inter {a | a in ASSETS: 
		a.asset_id == "Van_Den_Berg_Est_CHP1" ||
		a.asset_id == "dummy_congestion_asset" ||
		a.asset_id == "Van_Den_Berg_Est_PV1" ||
		a.asset_id == "Van_Den_Berg_Est_Eboiler1" ||
		a.asset_id == "Van_Den_Berg_Est_Light_2A_Afd2" ||
		a.asset_id == "Van_Den_Berg_Est_Light_2B_Afd2" ||
		a.asset_id == "Van_Den_Berg_Est_Light_Afd1" ||
		a.asset_id == "Van_Den_Berg_Est_Light_Afd3" ||
		a.asset_id == "Van_Den_Berg_Est_Light_Afd4" ||
		a.asset_id == "Van_Den_Berg_Est_Load1"
		}
	: microgridName == "MICROGRID VERBERNE Melderslo"
	? ASSETS inter {a | a in ASSETS:
		a.asset_id == "Verberne_Melderslo_CHP" ||
		a.asset_id == "dummy_congestion_asset" ||
		a.asset_id == "Verberne_Melderslo_LOAD" ||
		a.asset_id == "Verberne_Melderslo_PV"
		}
	: ASSETS
	);
...
{t_assets} H_ASSETS = (
	...
	? ASSETS inter {a | a in ASSETS:
		...
		a.asset_id == "Verberne_America_Heatpump_3" ||
		a.asset_id == "Verberne_America_Heatpump_4" ||
		a.asset_id == "Verberne_America_Gboiler1"
		}
	: microgridName == "MICROGRID VAN DEN BERG EST"
	? ASSETS inter { a | a in ASSETS: 
	a.asset_id == "Van_Den_Berg_Est_CHP1" ||
	a.asset_id == "Van_Den_Berg_Est_HeatBuffer1" ||
	a.asset_id == "Van_Den_Berg_Est_Eboiler1" ||
	a.asset_id == "Van_Den_Berg_Est_HeatNeed" ||
	a.asset_id == "Van_Den_Berg_Est_Gboiler1"
		}
	: microgridName == "MICROGRID VERBERNE Melderslo"
	? ASSETS inter {a | a in ASSETS:
		a.asset_id == "Verberne_Melderslo_CHP" ||
		a.asset_id == "Verberne_Melderslo_Heat_Buffer" ||
		a.asset_id == "Verberne_Melderslo_Heat_Need" ||
		a.asset_id == "Verberne_Melderslo_Gboiler1"
		}
	: {}
	);
...
// indexing set of dispatchable heat-only generation assets
{string} isDISP_H_ONLY_GENS = {h | h in isDISP_GENS : !(h in isE_ASSETS)};
...
{string} isNF_LOADS = {a.asset_id | a in ASSETS: a.type == "LOAD" && a.asset_id not in {"E_boiler_1000kW", "E_boiler_1200kW", "GP_BRIELLE_EBOILER", "GP_VIERPOLDERS_EBOILER", "Verberne_America_Heatpump_1", 
						 "Verberne_America_Heatpump_2", "Verberne_America_Heatpump_3", "Verberne_America_Heatpump_4", "Van_Den_Berg_Est_Eboiler1"}};
...
// indexing set of loads
{string} isLOADS = isNF_LOADS union isFLEX_LOADS;
...
{string} isCONVS = microgridName == "MICROGRID MOPABLOEM" ? 
					...
					: microgridName == "MICROGRID VAN DEN BERG EST" ?
					isASSETS inter {"Van_Den_Berg_Est_Eboiler1"}
					: {};
...
// indexin of consumption assets that convert elec into heat
{string} isE_H_LOADS = isE_H_CONVS inter {a.asset_id | a in ASSETS: a.type in {"LOAD", "FLEX_LOAD"}} diff isLOADS;
...
// indexing set of assets in Vierpolders' 1st transformer congestion
{string} isCONG_GP_VIER_CHP = {"GP_VIERPOLDERS_CHP", "GP_VIERPOLDERS_LIGHTS_1200"};
// indexing set of assets in Vierpolders' 2nd transformer congestion
{string} isCONG_GP_VIER_EB = {"GP_VIERPOLDERS_EBOILER", "GP_VIERPOLDERS_LIGHTS_860", "GP_VIERPOLDERS_BUILDINGS"};
...
// indexing set of heat generation assets with linear variable costs
{string} isLIN_COST_H_GENS = {a.asset_id | a in H_ASSETS : (a.variable_cost > 0) && (a not in E_ASSETS)};
...
{string} isAFRR_UP_ASSETS = (
	(microgridName == "MICROGRID MOPABLOEM") ? isE_ASSETS inter {"BESS_MOPABLOEM","CHP_1600kW"} :
	(microgridName == "MICROGRID GP Brielle") ? isE_ASSETS inter {"GP_BRIELLE_BESS","GP_BRIELLE_CHP"} :
	{}
	);
...
{string} isAFRR_DOWN_ASSETS = (
	(microgridName == "MICROGRID MOPABLOEM") ? isE_ASSETS inter {"E_boiler_1200kW","E_boiler_1000kW"} :
	(microgridName == "MICROGRID GP Brielle") ? isE_ASSETS inter {"GP_BRIELLE_EBOILER"} :
	(microgridName == "MICROGRID GP Vierpolders") ? isE_ASSETS inter {"GP_VIERPOLDERS_EBOILER"} :
	{}
	);
...
// indexing set of microgrid assets participating in mFRR Up
// {string} ismFRR_UP_ASSETS = {a.asset_id | a in E_ASSETS: a.mfrr_up_certifiate_power > 0};
{string} ismFRR_UP_ASSETS = (
	(microgridName == "MICROGRID VAN DEN BERG EST") ? isE_ASSETS inter {"Van_Den_Berg_Est_CHP1"} :
	(microgridName == "MICROGRID VERBERNE America") ? isE_ASSETS inter {"Verberne_America_CHP2"} :
	(microgridName == "MICROGRID VERBERNE Melderslo") ? isE_ASSETS inter {"Verberne_Melderslo_CHP"} :
	{}
	);
// {string} set of microgrid elec generation assets participating in mFRR Up
{string} ismFRR_GEN_UP_ASSETS = ismFRR_UP_ASSETS inter isDISP_E_GENS;
// indexing set of microgrid assets participating in mFRR down
// {string} ismFRR_DOWN_ASSETS = {a.asset_id | a in E_ASSETS: a.mfrr_dwn_certifiate_power > 0};
//{string} ismFRR_DOWN_ASSETS = (
//	(microgridName == "MICROGRID VAN DEN BERG EST") ? isE_ASSETS inter {"Van_Den_Berg_Est_Eboiler1"} :
//	{}
//	);;
{string} ismFRR_DOWN_ASSETS = {};
// {string} set of microgrid elec consumtion assets participating in mFRR down
{string} ismFRR_LOAD_DOWN_ASSETS = ismFRR_DOWN_ASSETS inter isE_H_LOADS;
// indexing set of microgrid storage assets participating in mFRR
{string} ismFRR_ASSETS = ismFRR_UP_ASSETS union ismFRR_DOWN_ASSETS;
// indexing set of microgrid storage assets participating in mFRR up
{string} ismFRR_UP_STORAGES = isAFRR_UP_ASSETS inter isH_STORAGES;
// indexing set of microgrid storage assets participating in mFRR down
{string} ismFRR_DOWN_STORAGES = ismFRR_DOWN_ASSETS inter isH_STORAGES;
// indexing set of microgrid storage assets participating in mFRR down
// site id to wich the asset a is connected
// For joshua => this doesn't work at run time on local windows machine. why use a collection of sets of strings? i think it should be a simple collection of strings
{string} siteIdByAsset[isASSETS] = [a.asset_id:{a.site}| a in ASSETS];
// {string} siteIdByAsset[isASSETS] = [{"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}, {"Verberne_America"}];
...
string assetStepmFRRStep[isDECISION_STEPS] = [t.asset_step : t.mfrr_step | t in OPERATION_STEPS_LINK: t.mfrr_step != "-1" && t.mfrr_step != "-1.0"];
...
// variable costs for elec and heat generation asset with linear costs g (expressed in currency unit) (g in isLIN_COST_E_GENS union isLIN_COST_H_GENS)
float genVariableCost[g in isLIN_COST_E_GENS union isLIN_COST_H_GENS] = assetVariableCost[g];
...
// 1.175059952 is only for the microgrid VAN DEN BERG EST
float assetHeatElecRatio[isASSETS];
// [a.asset_id : a.elec_heat_ratio | a in ASSETS];
float dispGenHeatElecRatio[d in isDISP_EH_GENS] = assetHeatElecRatio[d];
// HARD-CODED
// maximum activation duration (expressed in hours) for CHPs for MICROGRID VAN DEN BERG EST, MICROGRID VERBERNE America, MICROGRID VERBERNE Melderslo microgrids on mFRR
float maxActivationDurationOnmFRRInHour = (microgridName == "MICROGRID VAN DEN BERG EST" || microgridName == "MICROGRID VERBERNE America" || microgridName == "MICROGRID VERBERNE Melderslo" ? 1.5 : 0.0);
...
// left hand side coefficient of mfrr asset unit mfrr_a particpating to mfr in network congestion constraint c
// (mfrr_a in isMFRR_UP_ASSETS, c in CONGESTION)
float mfrrUpAssetFactor[c in isCONGESTIONS][mfrr_a in ismFRR_UP_ASSETS] = assetCongestionFactor[c][mfrr_a];
...
// left hand side coefficient of mfr down asset unit mfr_dwn_a particpating to mfr down in network congestion constraint c
// (mfr_dwn_a in isMFRR_DOWN_ASSETS, c in CONGESTION)
float mfrrDwnAssetFactor[c in isCONGESTIONS][mfr_dwn_a in ismFRR_DOWN_ASSETS] = assetCongestionFactor[c][mfr_dwn_a];
/* Energy Markets */
...
float previsousDAEngagement = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "DAY_AHEAD" && m.step_index == "0"});
...
float sellOfferPrice = (microgridName == "MICROGRID VALOREM LIMOUX" || microgridName == "MICROGRID VALECO Baignollais") ? 1.0 / 1000.0	// Valorem pays EP 0.4 ï¿½/MWh so we decided that it was not profitable for them to generate for less than 1 ï¿½/MWh
						: (microgridName in {"MICROGRID GDS Leforest A", "MICROGRID GDS Leforest B"}) ? -0.01 / 1000.0	// GDS is on a contract for difference agreement so it is profitable for them to generate as long as prices are >= 0 ï¿½/MWh
						: (microgridName in {"MICROGRID NEXHOS Montbert", "MICROGRID NEXHOS Colisee"}) ? -0.01 / 1000.0	// NEXHOS is on a contract for difference agreement so it is profitable for them to generate as long as prices are >= 0 ï¿½/MWh
						:  0.0;
...
float mFRRactivatedPower_mFRR[ismFRR_STEPS] = [m.step_index : m.engagement | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3_ACTIVATION" && m.step_index != "0"];
// activated power for mFRR on the mFRR step before 1st mfrr step of the optimization window.
float previsousMFRRactivatedPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3_ACTIVATION" && m.step_index == "0"});
// Capacity engagement in kW, in mFRR up over mFRRUp step mfrr_up_capacity, (mfrr_up_capacity in isMFRR_CAPACITY_STEPS)
float mFRRUpCapacityEngPower[ismFRR_STEPS] = [m.step_index : (m.engagement >= 0 ? m.engagement : 0) | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3_CAPACITY_UP" && m.step_index != "0"];
// Capacity engagement in kW, in mFRR down over mFRRDown step mfrr_down_capacity, (mfrr_down_capacity in isMFRR_CAPACITY_STEPS)
float mFRRDownCapacityEngPower[ismFRR_STEPS] = [m.step_index : (m.engagement >= 0 ? m.engagement : 0) | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3_CAPACITY_DOWN" && m.step_index != "0"];
// mFRR Up electricity market engagement status (1 = cleared / 0 = not cleared),
// (mfrr in ismFRR_STEPS_POS)
float ismFRRCapacityUpStepCleared[ismFRR_STEPS_POS] = [m.step_index : m.is_step_cleared | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3_CAPACITY_UP" && m.step_index != "0"];
// mFRR Down electricity market engagement status (1 = cleared / 0 = not cleared),
// (mfrr in ismFRR_STEPS_POS)
float ismFRRCapacityDownStepCleared[ismFRR_STEPS_POS] = [m.step_index : m.is_step_cleared | m in MARKET_ENGAGEMENTS : m.type == "MFRR_R3_CAPACITY_DOWN" && m.step_index != "0"];
...
float negative_imb_price[imb in isIMBALANCE_STEPS_POS] =
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName in {"MICROGRID MOPABLOEM", "MICROGRID GP Brielle", "MICROGRID GP Vierpolders", "MICROGRID VERBERNE America", "MICROGRID VAN DEN BERG EST", "MICROGRID VERBERNE Melderslo", "MICROGRID Weyers"}// if C&I in NL?
...
// penalty cost (expressed in currency unit per kwh) applied if aFRR up required engagement  constraint cannot be satisfied
...
// penalty cost (expressed in currency unit per kWh) applied if mfrr required up engagement constraint cannot be satisfied
float mFRRUpPowerEngDeficitPenaltyCost = cat2_1Pen * penaltyBase;
// penalty cost (expressed in currency unit per kWh) applied if mfrr required down engagement constraint cannot be satisfied
float mFRRDwnPowerEngDeficitPenaltyCost = cat2_1Pen * penaltyBase;
...
float previsousFcrEngPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "FCR" && m.step_index == "0"});
...
int isAFRRBackedUp = (microgridName in {"MICROGRID MOPABLOEM", "MICROGRID GP Brielle", "MICROGRID GP Vierpolders"} ?
							1 : 0);
...
// For joshua => i'm not sure about values for Brielle and Vierpolders. can you please check?
int afrrUpCertfiedPower[a in isASSETS] =
	(microgridName ==  "MICROGRID MOPABLOEM") ? (a == "BESS_MOPABLOEM" ? 1000 : a == "CHP_1600kW" ? 1600 : 0) :
	(microgridName ==  "MICROGRID GP Brielle") ? (a == "GP_BRIELLE_BESS" ? 1500 : a == "GP_BRIELLE_CHP" ? 1700 : 0) :
	 0;						  
...
int afrrDwnCertfiedPower[a in isASSETS] =
	(microgridName ==  "MICROGRID MOPABLOEM") ? (a == "E_boiler_1200kW" ? 1200 : a == "E_boiler_1000kW" ? 1000 : 0) :
	(microgridName ==  "MICROGRID GP Brielle") ? (a == "GP_BRIELLE_EBOILER" ? 1200 : 0) :
	(microgridName ==  "MICROGRID GP Vierpolders") ? (a == "GP_VIERPOLDERS_EBOILER" ? 1200 : 0) :
	0;
...
float afrrUpAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] = 
	(a in (isINTER_E_GENS union isDISP_E_GENS union isE_STORAGES) ?
		(scheduledAvailability[a][t] > 0.99 && - nomPowerMin[a]>= afrrUpCertfiedPower[a] ? afrrUpCertfiedPower[a] : 0)
			: (scheduledAvailability[a][t] > 0.99 && nomPowerMax[a]>= afrrUpCertfiedPower[a] ? afrrUpCertfiedPower[a] : 0) // Load
	);
...
float afrrDwnAssetAvailPower[a in isASSETS][t in isDECISION_STEPS] = 
	(a in (isINTER_E_GENS union isDISP_E_GENS union isE_STORAGES) ?
		(scheduledAvailability[a][t] > 0.99 && - nomPowerMin[a]>= afrrDwnCertfiedPower[a] ? afrrDwnCertfiedPower[a] : 0)
		: (scheduledAvailability[a][t] > 0.99 && nomPowerMax[a]>= afrrDwnCertfiedPower[a] ? afrrDwnCertfiedPower[a] : 0) // Load
	);
...
float previsousAFRRUpCapacityEngPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "AFRR_R2_CAPACITY_UP" && m.step_index == "0"});
...
float previsousAFRRDwnCapacityEngPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "AFRR_R2_CAPACITY_DOWN" && m.step_index == "0"});
...
float previsousAFRRUpVoluntaryEngPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "AFRR_R2_VOLUNTARY_UP" && m.step_index == "0"});
...
float previsousAFRRDwnVoluntaryEngPower = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "AFRR_R2_VOLUNTARY_UP" && m.step_index == "0"});
...
float p0Engagement[isAFRR_CAPACITY_STEPS] = [m.step_index : (m.engagement >= 0 ? m.engagement : 0) | m in MARKET_ENGAGEMENTS : m.type == "P0"  && m.step_index != "0"];
// P0 engagement on afrr capacity step before first capacity step of the optimization window
float previousP0 = first({m.engagement | m in MARKET_ENGAGEMENTS : m.type == "P0" && m.step_index == "0"});
...
//// MFRR
//////////
// mFRR power that can be engaged into mFRR UP for asset a , expressed in kW. (a in ASSETS)
//int mfrrUpCertfiedPower[mfrr_up_a in isMFRR_UP_ASSETS] = [mfrr_up_a.asset_id : a.mfrr_up_certfied_power | a in ASSETS];
int mfrrUpCertfiedPower[a in isASSETS] = (microgridName ==  "MICROGRID VAN DEN BERG EST") ? (a == "Van_Den_Berg_Est_CHP1" ? 1000 : 0)
									     : (microgridName ==  "MICROGRID VERBERNE America") ? (a == "Verberne_America_CHP2" ? 540 : 0)
									     : (microgridName ==  "MICROGRID VERBERNE Melderslo") ? (a == "Verberne_Melderslo_CHP" ? 1700 : 0)
										 : 0;
// mFRRDownPower that can be engaged into mFRR Down for asset a , expressed in kW. (a in ASSETS)
//int mfrrDwnCertfiedPower[mfrr_dwn_a in isMFRR_DOWN_ASSETS] = [mfrr_dwn_a.asset_id : a.mfrr_dwn_certfied_power | a in ASSETS];
int mfrrDwnCertfiedPower[a in isASSETS] =(microgridName ==  "MICROGRID VAN DEN BERG EST") ? (a == "Van_Den_Berg_Est_Eboiler1" ? 1000 : 0) : 0;
// Define max available power to do mFRR up, per asset, taking into consideration the global availability
// availability can be included into max/min power. 
// the mFRRUpAssetAvailPower and mFRRDwnAssetAvailPower calculations need to be revised to cover all cases, up and down, with either cosno or prod.
float mFRRUpAssetAvailPower[a in ismFRR_UP_ASSETS][t in isDECISION_STEPS] = 
	(a in ismFRR_GEN_UP_ASSETS ?
		(availGlobalForPmin[a][t] > 0.99 && - nomPowerMin[a]>= mfrrUpCertfiedPower[a] ? mfrrUpCertfiedPower[a] : 0) // Generator
			: (availGlobalForPmax[a][t] > 0.99 && nomPowerMax[a]>= mfrrUpCertfiedPower[a] ? mfrrUpCertfiedPower[a] : 0) // Load
	);
float mFRRUpAvailPower[t in isDECISION_STEPS] = sum(a in ismFRR_UP_ASSETS) mFRRUpAssetAvailPower[a][t];
// Define max available power to do mFRR down, per asset, taking into consideration the global availability
// availability can be included into max/min power.
float mFRRDwnAssetAvailPower[a in ismFRR_DOWN_ASSETS][t in isDECISION_STEPS] = 
	(a in ismFRR_LOAD_DOWN_ASSETS ?
	(availGlobalForPmax[a][t] > 0.99 && nomPowerMax[a]>= mfrrDwnCertfiedPower[a] ? mfrrDwnCertfiedPower[a] : 0): //Load 
		(availGlobalForPmin[a][t] > 0.99 && - nomPowerMin[a]>= mfrrDwnCertfiedPower[a] ? mfrrDwnCertfiedPower[a] : 0) // Generator
	);
float mFRRDwnAvailPower[t in isDECISION_STEPS] = sum(a in ismFRR_DOWN_ASSETS) mFRRDwnAssetAvailPower[a][t];
// Required power, in kW, in mFRR Up over mFRR capacity step mfrr_c, (mfrr_c in ismFRR_STEPS_POS)
// accounts for partial availability of mFRR assets and grid connection
// Not totally accurate, we could do mFRR up without injecting to the grid, or at least not as much
float mFRRUpReqPower[mfrr_c in ismFRR_STEPS_POS] = maxl(0.0, minl(
		mFRRUpCapacityEngPower[mfrr_c],
		min(t in isDECISION_STEPS: assetStepmFRRStep[t] == mfrr_c) (mFRRUpAvailPower[t]),
		maxl(0,min(a in isASSETS inter {"site"}, t in isDECISION_STEPS: assetStepmFRRStep[t] == mfrr_c) nfElecLoadMaxAvailPower[a][t])
		));
// Required power, in kW, in mFRR Down over mFRR capacity step mfrr_c, (mfrr_c in ismFRR_STEPS_POS)
// accounts for partial availability of mFRR assets and grid connection
float mFRRDwnReqPower[mfrr_c in ismFRR_STEPS_POS] = maxl(0.0, minl(
		mFRRDownCapacityEngPower[mfrr_c],
		min(t in isDECISION_STEPS: assetStepmFRRStep[t] == mfrr_c) (mFRRDwnAvailPower[t]),
		maxl(0,min(a in isASSETS inter {"site"}, t in isDECISION_STEPS: assetStepmFRRStep[t] == mfrr_c) nfElecLoadMaxAvailPower[a][t])
		));		
// HVAC hardcoded data
//For Mopabloem
...
execute CPX_PARAM {
	...
	if (operationID == "1529" || operationID == "1628" || operationID == "1661")
		microgridName = "MICROGRID GP Brielle";
	if (operationID == "1663")
		microgridName = "MICROGRID GP Vierpolders";
	if (operationID == "1597" || operationID == "1566" || operationID == "1826") // rajouter ici le nouvel id d'opÃ© ugrid pour America avec procurement spot + mFRR quand crÃ©Ã©
		microgridName = "MICROGRID VERBERNE America";
	if (operationID == "1727" )
		microgridName = "MICROGRID VAN DEN BERG EST";
	if (operationID == "1801")
		microgridName = "MICROGRID VERBERNE Melderslo";
	if (operationID == "182692")
		microgridName = "MICROGRID Weyers";
	if (operationID == "1802" || operationID == "1802_FAT")
		microgridName == "MICROGRID Topjoynt";
...
//		}
	...
	for (var d in isDISP_EH_GENS){
		...
		if (microgridName == "MICROGRID VAN DEN BERG EST")
			assetHeatElecRatio[d] = 1.175059952;
		if (microgridName == "MICROGRID VERBERNE Melderslo")
			assetHeatElecRatio[d] = 1.175059952;
	...
	for (var c in isCONVS){
		...
		// Values for MICROGRID Globe Plant Brielle
		if (microgridName == "MICROGRID GP Brielle")
			assetEnergyConvEfficiency[c] = 0.93;
		// Values for MICROGRID Globe Plant Vierpolders
		if (microgridName == "MICROGRID GP Vierpolders")
			assetEnergyConvEfficiency[c] = 0.93;
		// Values for MICROGRID Verberne America
		// TO BE CHECKED WITH CLIENT
	...
	if (optContext == "MARKET_STRAT_FCR"){
...
// (i in isINTER_E_GENS, t in DECISION_STEPS)
dvar float+ InterGenForcedPowerViolationExcess[isINTER_E_GENS][isDECISION_STEPS];
...
// (i in isINTER_E_GENS, t in DECISION_STEPS)
dexpr float InterGenForcedPowerViolationCost[i in isINTER_E_GENS][t in isDECISION_STEPS] = (InterGenForcedPowerViolationExcess[i][t] + InterGenForcedPowerViolationDeficit[i][t]) * forcedPowerPenaltyCost * assetStepDurationInHours;
// Storage assets
/////////////////
...
// average heat generation on mFRR (expressed in kw) for dispatchable generator d over decision step t
// (isDISP_H_GENS, ismFRR_STEPS_POS)
dvar float+ PossibleDispGenHeatGenOnMfrrUp[isDISP_H_GENS][ismFRR_STEPS];
// possible heat that can be generated on mFRR  Up(expressed in kwh) of all disp generator d over the maxActivationDurationOnmFRRInHour / assetStepDurationInHours mFRR steps coming up
// (t in DECISION_STEPS)
dexpr float PossibleDispGenHeatGenOnMfrrUpAll[t in isDECISION_STEPS] = sum(d in isDISP_H_GENS) PossibleDispGenHeatGenOnMfrrUp[d][assetStepmFRRStep[t]] * maxActivationDurationOnmFRRInHour / mFRRStepDurationInHours;
// average heat generation on mFRR down (expressed in kw) from elec to heat converter asset c over decision step t (boiler)
// (c in HOUT_CONVS, t in DECISION_STEPS)
dvar float+ PossibleConvHeatGenOnMfrrDown[ismFRR_LOAD_DOWN_ASSETS][ismFRR_STEPS];
// possible heat that can be generated on mFRR Down (expressed in kwh)
// of all elec to heat converter asset c over the maxActivationDurationOnmFRRInHour / assetStepDurationInHours mFRR steps coming up
// (t in DECISION_STEPS)
dexpr float PossibleConvHeatGenOnMfrrDownAll[t in isDECISION_STEPS] = sum(c in ismFRR_LOAD_DOWN_ASSETS) PossibleConvHeatGenOnMfrrDown[c][assetStepmFRRStep[t]] * maxActivationDurationOnmFRRInHour / mFRRStepDurationInHours;
// possible heat that can be produced on mFRR (expressed in kwh) by all generator heat assets participant on mFRR over the maxActivationDurationOnmFRRInHour / assetStepDurationInHours mFRR steps coming up
// (t in DECISION_STEPS)
dexpr float PossibleHeatGenOnMfrrAll[t in isDECISION_STEPS] = PossibleDispGenHeatGenOnMfrrUpAll[t] + PossibleConvHeatGenOnMfrrDownAll[t];
// average heat power consumption (expressed in kW) for
// Heat storage assets
//////////////////////
...
/*mFRR*/
// Maximum power of service, in kW, which an asset participates to mFRR up, on mfrr step, to reach mFRR up commitment.
dvar float+ MFRRCapacityPowerUp[ismFRR_UP_ASSETS][ismFRR_STEPS];
// idem, in MW
dexpr float MFRRCapacityPowerUp_MW[a in ismFRR_UP_ASSETS][mfrr in ismFRR_STEPS] = MFRRCapacityPowerUp[a][mfrr] / 1000;
// Maximum power of service, in kW, which an asset participates to mFRR down, on mfrr step, to reach mFRR down commitment.
dvar float+ MFRRCapacityPowerDwn[ismFRR_DOWN_ASSETS][ismFRR_STEPS];
// idem, in MW
dexpr float MFRRCapacityPowerDwn_MW[a in ismFRR_DOWN_ASSETS][mfrr in ismFRR_STEPS] = MFRRCapacityPowerDwn[a][mfrr] / 1000;
// Violation variable of the engaged mFRR up Power at pool level. this decision variable is used in cases where the power engaged in mFRR up is greater than the sum of availabe power mfrR up within the pool to relax the ctMFRRUpPoolEngagement constraint. 
dvar float+ MFRRUpPowerEngDeficit[ismFRR_STEPS];
// Violation variable of the engaged mFRR Down Power at pool level. this decision variable is used in cases where the power engaged in mFRR down is greater than the sum of availabe power mfrR downup within the pool to relax the ctMFRRDownPoolEngagement constraint. 
dvar float+ MFRRDownPowerEngDeficit[ismFRR_STEPS];
// mFRR Up eng deficit cost over mfrr in ismFRR_STEPS_POS
dexpr float MFRRUpPowerEngDeficitCost[mfrr in ismFRR_STEPS_POS] = mFRRUpPowerEngDeficitPenaltyCost * mFRRStepDurationInHours * MFRRUpPowerEngDeficit[mfrr];
// mFRR down eng deficit cost over mfrr in ismFRR_STEPS_POS
dexpr float MFRRDownPowerEngDeficitCost[mfrr in ismFRR_STEPS_POS] = mFRRDwnPowerEngDeficitPenaltyCost * mFRRStepDurationInHours * MFRRDownPowerEngDeficit[mfrr];
// Flag indicating whether an asset is doing mFRR up or not. Usefull to respect the max activation duration of the dispGen on mFRR.
dvar boolean IsMFRRUp[ismFRR_UP_ASSETS][ismFRR_STEPS];
// Flag indicating whether an asset is doing mFRR down or not. Usefull to respect the max activation duration of the dispGen on mFRR.
dvar boolean IsMFRRDwn[ismFRR_DOWN_ASSETS][ismFRR_STEPS];
...
dexpr float GenTotalLinearVarCosts =
    ...
    + sum(t in isDECISION_STEPS, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s]) * nomEnergyMax[s]
    + sum(fcr in isFCR_STEPS_POS, s in isE_STORAGES inter isLIN_COST_E_GENS) (cyclingVariableCostFCR[s] * FCRPower[s][fcr])
    + sum(t in isDECISION_STEPS, d in isDISP_H_ONLY_GENS inter isLIN_COST_H_GENS) DispGenHeat[d][t]*genVariableCost[d]*assetStepDurationInHours;
...
dexpr float OptimisedGenLinearVarCosts =
    ...
    + sum(t in isDECISION_STEPS: isDAStepCleared[assetStepHourlyStep[t]] == 0, s in isE_STORAGES inter isLIN_COST_E_GENS) (CyclingStepContribution[s][t] * cyclingVariableCostEnergyArbitration[s]) * nomEnergyMax[s]
    ...
    + sum(t in isDECISION_STEPS: isDAStepCleared[assetStepHourlyStep[t]] == 0, d in isDISP_H_ONLY_GENS inter isLIN_COST_H_GENS) DispGenHeat[d][t]*genVariableCost[d]*assetStepDurationInHours;
...
minimize
  ...
  + sum (mfrr in ismFRR_STEPS_POS) MFRRDownPowerEngDeficitCost[mfrr]
  + sum(mfrr in ismFRR_STEPS_POS) MFRRUpPowerEngDeficitCost[mfrr]
...
// for other asset types (not CFDf assets) da ahead offers are made thanks to other elec Power sources
	forall (imb in isIMBALANCE_STEPS_POS)
		ctNotInCFDStepImbPower:
			long_term_engagement[imbStepHourlyStep[imb]] + NotInCFDDaPosition[imbStepHourlyStep[imb]] - NotInCFDImbalancePower[imb] + mFRRactivatedPower_imb[imb] == NetElecImportTarget_imb[imb]
...
// mFRR
////////////////
// mFRR Pool engagement
// mFRR up engagement
forall (mfrr in ismFRR_STEPS_POS)
	  ctMFRRUpPoolEngagement:
	  	mFRRUpReqPower[mfrr] == sum(a in ismFRR_UP_ASSETS) MFRRCapacityPowerUp[a][mfrr] + MFRRUpPowerEngDeficit[mfrr];
// mFRR down engagement
forall (mfrr in ismFRR_STEPS_POS)
	  ctMFRRDwnPoolEngagement:
	  	mFRRDwnReqPower[mfrr] == sum(a in ismFRR_DOWN_ASSETS) MFRRCapacityPowerDwn[a][mfrr] + MFRRDownPowerEngDeficit[mfrr];	  	  

// An asset participation to mFRR is limited by its certified power/its capacity max to provide the service
...
// We flag mFRR participation, that will be then use to block simultaneaous multi-markets
	// up
	if (card(ismFRR_STEPS_POS)>0){
		forall (a in ismFRR_UP_ASSETS, t in isDECISION_STEPS)
		  ctMFRRUpMaxActivePower:
		      MFRRCapacityPowerUp[a][assetStepmFRRStep[t]] <= IsMFRRUp[a][assetStepmFRRStep[t]] * mFRRUpAssetAvailPower[a][t];
	// down
		forall (a in ismFRR_DOWN_ASSETS, t in isDECISION_STEPS)
		  ctMFRRDwnMaxActivePower:
		      MFRRCapacityPowerDwn[a][assetStepmFRRStep[t]] <= IsMFRRDwn[a][assetStepmFRRStep[t]] * mFRRDwnAssetAvailPower[a][t];
	}	
...
// Within a pool
	if (card(isNotEngagedAFRRUpSteps) != 0 && card(isAFRR_VOLUNTARY_STEPS_POS)>0){
	...
	if (card(isNotEngagedAFRRDwnSteps) != 0 && card(isAFRR_VOLUNTARY_STEPS_POS)>0){
 ...
 if (microgridName == "MICROGRID MOPABLOEM" || microgridName == "MICROGRID GP Brielle"){
 // We force down assets (boilers) to do up, to compensate CHP over production (over engagement) 
		forall (a in isAFRR_DOWN_ASSETS, t in isDECISION_STEPS)
		  ctForceAllAssetsAFRRDwn2:
...
//Same on the step before, to avoid overshoot at the end of the step. Just for back-up pools. For HPP P0 programmation can lead to BESS not beein goff on step before
		if (isAFRRBackedUp == 1){
			forall (s in isE_STORAGES, t in isDECISION_STEPS : t!= first(isDECISION_STEPS))
			ctBatNoPowerOutaFRRUp11:
				StorACPowerDischarge[s][prev(isDECISION_STEPS,t)] <= storMaxAvailACActivePowerDischarge[s][t] * (1 - IsAFRRUp[s][assetStepAfrrVoluntaryStep[t]]);

			forall (s in isE_STORAGES, t in isDECISION_STEPS : t!= first(isDECISION_STEPS))
			ctBatNoPowerOutaFRRUp21:
				StorACPowerCharge[s][prev(isDECISION_STEPS,t)] <= storMaxAvailACActivePowerCharge[s][t] * (1 - IsAFRRUp[s][assetStepAfrrVoluntaryStep[t]]);

			forall (s in isE_STORAGES, t in isDECISION_STEPS : t!= first(isDECISION_STEPS))
			ctBatNoPowerOutaFRRDwn11:
				StorACPowerDischarge[s][prev(isDECISION_STEPS,t)] <= storMaxAvailACActivePowerDischarge[s][t] * (1 - IsAFRRDwn[s][assetStepAfrrVoluntaryStep[t]]);

			forall (s in isE_STORAGES, t in isDECISION_STEPS : t!= first(isDECISION_STEPS))
			ctBatNoPowerOutaFRRDwn21:
				StorACPowerCharge[s][prev(isDECISION_STEPS,t)] <= storMaxAvailACActivePowerCharge[s][t] * (1 - IsAFRRDwn[s][assetStepAfrrVoluntaryStep[t]]);
		}		
	}
// Disp gen
...
//		  ctForceBESSaFRRUp:
//		  	(aFRRUpReqPower[assetStepAfrrVoluntaryStep[t]] > 0 ?
//		  		AFRRCapacityPowerUp[s][assetStepAfrrVoluntaryStep[t]] >= aFRRUpReqPower[assetStepAfrrVoluntaryStep[t]] * (availGlobalForEmax[s][t] >= 0.85 ? 1 : 0)
//		  		: AFRRCapacityPowerUp[s][assetStepAfrrVoluntaryStep[t]] >= 0);
	...
	// Constaints applied to Van Den Berg Est CHP and VERBERNE America and Melderslo CHPs that participate in mFRR Up
	//// constraint to force the CHP to be off when there is engagement
	//// the engagement is only considered when there is no mFRR Up activation and mFRRUpCapacityEngPower is greater than 0
	forall(d in ismFRR_GEN_UP_ASSETS, t in isDECISION_STEPS: mFRRUpCapacityEngPower[assetStepmFRRStep[t]] > 0 && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) {
		ctForceCHPToBeOffOnEngagement:
			DispGenActivePower[d][t] <= 0.0;
	}

	forall(d in isDISP_E_GENS diff ismFRR_GEN_UP_ASSETS, t in isDECISION_STEPS: mFRRUpCapacityEngPower[assetStepmFRRStep[t]] > 0) {
			ctForceCHPToBeOffOnEngagement2:
				DispGenActivePower[d][t] <= 0.0;
		}


	forall(d in ismFRR_GEN_UP_ASSETS, t in isDECISION_STEPS_WITHOUT_FIRST: mFRRUpCapacityEngPower[assetStepmFRRStep[t]] > 0 && mFRRUpCapacityEngPower[assetStepmFRRStep[prev(isDECISION_STEPS, t)]] <= 0 && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) {
	  ctForceCHPToBeOff_previous:
  	      IsGenOn[d][prev(isDECISION_STEPS, t)] <= 0.0;
	}

	// CHP forced to be on when there is an activation of mFRR Up
	forall(d in ismFRR_GEN_UP_ASSETS, t in isDECISION_STEPS: -mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] > 0 && availGlobalForPmin[d][t] >= 0.99) {
		ctForceCHPToBeOnOnActivation:
			DispGenActivePower[d][t] >= -mFRRactivatedPower_mFRR[assetStepmFRRStep[t]];
	}

	//// Possible heat produced by the CHP when engaged in mFRR Up
	//// that only works with a single CHP that represents the pool engagement 
	forall(d in ismFRR_GEN_UP_ASSETS, mfrr in ismFRR_STEPS_POS: mFRRactivatedPower_mFRR[mfrr] == 0) {
	  ctCHPHeatProduction:
	  	PossibleDispGenHeatGenOnMfrrUp[d][mfrr] == MFRRCapacityPowerUp[d][mfrr] * dispGenHeatElecRatio[d];
	} 
	

	// Constraints applied to Van Den Berg Est Eboiler that participate in mFRR Down
	// we suppose that if the eboiler is engaged in mFRR, it cannot do anything else
	//// the engagement is only considered when there is no mFRR Down activation and mFRRDownCapacityEngPower is greater than 0
	forall(c in ismFRR_LOAD_DOWN_ASSETS, t in isDECISION_STEPS: mFRRDownCapacityEngPower[assetStepmFRRStep[t]] > 0 && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) {
		ctForceEboilerToBeOffOnEngagement:
			ConvActivePowerIn[c][t] <= 0.0;
	}

	forall(c in ismFRR_LOAD_DOWN_ASSETS, t in isDECISION_STEPS: t != first(isDECISION_STEPS) && mFRRDownCapacityEngPower[assetStepmFRRStep[t]] > 0 && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) {
		ctForceEboilerToBeOffOnEngagement_previous:
			ConvActivePowerIn[c][prev(isDECISION_STEPS, t)] <= 0.0;
	}

	// We force the eboiler to be on when there is an activation of mFRR Down
	forall(c in ismFRR_LOAD_DOWN_ASSETS, t in isDECISION_STEPS: mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] > 0) {
		ctForceEboilerToBeOnOnActivation:
			ConvActivePowerIn[c][t] >= mFRRactivatedPower_mFRR[assetStepmFRRStep[t]];
	}

	// possible heat produced by the eboiler when engaged in mFRR Down
	// that only works with a single eboiler that represents the pool engagement
	forall(c in ismFRR_LOAD_DOWN_ASSETS, mfrr in ismFRR_STEPS_POS: mFRRactivatedPower_mFRR[mfrr] == 0) {
		ctEboilerHeatProduction:
			PossibleConvHeatGenOnMfrrDown[c][mfrr] == MFRRCapacityPowerDwn[c][mfrr] * convElecToHeatEff[c];
	}
...
// Electricity generation from a interGen  asset should respect the power input by the portal customer.
	forall(i in isINTER_E_GENS, t in isDECISION_STEPS: interGenForcedPower[i][t] != 1)
	  ctInterGenForcedPower:
	  	InterGenActivePower[i][t] == interGenForcedPower[i][t] + InterGenForcedPowerViolationExcess[i][t] - InterGenForcedPowerViolationDeficit[i][t];
...
// when forcedPower different from -1, StorACPower should respect the value of forcedPower
	forall(s in isE_STORAGES, t in isDECISION_STEPS: storageForcedPower[s][t] != 1)
		ctStorageForcePower: StorACActivePower[s][t] == storageForcedPower[s][t] + StorageAssetChargeForcedPowerViolationExcess[s][t] - StorageAssetChargeForcedPowerViolationDeficit[s][t];
...
// For Topjoynt, during DA bidding over non-cleared steps, we prevent BESS to be used
forall (s in isE_STORAGES, t in isDECISION_STEPS : isDAStepCleared[assetStepHourlyStep[t]] == 0 && microgridName in {"MICROGRID Topjoynt"}  && optContext == "MARKET_STRAT_DA")
      ctTopjoyntBESSNotUsed:
        IsCharging[s][t] + IsDischarging[s][t] == 0;	  	  
...
// Initial Charge is the available charge
	forall (s in isE_STORAGES)
	  ctStorageStoredEnergyAvail1: StorStoredDCEnergyAvail[s][first(isDECISION_STEPS)] == storInitialElecEnergy[s] * availGlobalForEmax[s][first(isDECISION_STEPS)] + StorStepDCEnergyIn[s][first(isDECISION_STEPS)];
...
// Maximum input and output
 	forall (si in isSITES, t in isDECISION_STEPS)
	  ...
	  - sum(conv in isEIN_CONVS : si == siteID[conv]) ConvActivePowerIn[conv][t]
 	  - sum(fcr_a in isFCR_ASSETS: si == siteID[fcr_a]) FCRPower[fcr_a][assetStepFCRStep[t]]
 	  - (isAFRRBackedUp == 0 ?
 	  		// It is just the sum if no back up 
 	  		sum(afrr_dwn_a in isAFRR_DOWN_ASSETS: si == siteID[afrr_dwn_a]) AFRRCapacityPowerDwn[afrr_dwn_a][assetStepAfrrVoluntaryStep[t]] 
 	  		// If back up, we limit to one minimum asset partiticipation. Will need to be changed if more assets and/or more thant 1MW aFRR
 	  		: (card({afrr_dwn_a | afrr_dwn_a in isAFRR_DOWN_ASSETS : si == siteID[afrr_dwn_a]}) > 0 ?
 	  			max(afrr_dwn_a in isAFRR_DOWN_ASSETS: si == siteID[afrr_dwn_a]) AFRRCapacityPowerDwn[afrr_dwn_a][assetStepAfrrVoluntaryStep[t]]
 	  			:0))
		- sum(mfrr_dwn in ismFRR_DOWN_ASSETS: si == siteID[mfrr_dwn] && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) MFRRCapacityPowerDwn[mfrr_dwn][assetStepmFRRStep[t]] 
 	...
 	forall (si in isSITES, t in isDECISION_STEPS)
	  ...
	  - sum(conv in isEIN_CONVS : si == siteID[conv]) ConvActivePowerIn[conv][t]
 	  ...
 	  + (isAFRRBackedUp == 0 ?
 	  		...
 	  		// If back up, we limit to one minimum asset partiticipation. Will need to be changed if more assets and/or more thant 1MW aFRR
 	  		: (card({afrr_up_a | afrr_up_a in isAFRR_UP_ASSETS : si == siteID[afrr_up_a]}) > 0 ?
 	  			max(afrr_up_a in isAFRR_UP_ASSETS: si == siteID[afrr_up_a]) AFRRCapacityPowerUp[afrr_up_a][assetStepAfrrVoluntaryStep[t]]
 	  			:0))
	  + sum(mfrr_up in ismFRR_UP_ASSETS: si == siteID[mfrr_up] && mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) MFRRCapacityPowerUp[mfrr_up][assetStepmFRRStep[t]] 
 	  + SiteMaxInputViolation[si][t] - SiteMaxOutputViolation[si][t]
 	  <= maxOutput[si][t];
...
// Generic congestion contraints
 	forall (c in isCONGESTIONS, t in isDECISION_STEPS)
 	  ...
 	  congestionLowerLim[c] <=
		...
		- sum(conv in isEIN_CONVS) assetCongestionFactor[c][conv] * ConvActivePowerIn[conv][t]
 	  	...
 	  	- (isAFRRBackedUp == 0 ?
 	  		...
 	  		// If back up, we limit to one minimum asset partiticipation. Will need to be changed if more assets and/or more thant 1MW aFRR
 	  		: (card(isAFRR_DOWN_ASSETS) > 0 ? 
 	  			max(afrr_dwn_a in isAFRR_DOWN_ASSETS) afrrDwnAssetFactor[c][afrr_dwn_a] * AFRRCapacityPowerDwn[afrr_dwn_a][assetStepAfrrVoluntaryStep[t]]
 	  			:0))
		- sum(mfrr_dwn in ismFRR_DOWN_ASSETS: mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) mfrrDwnAssetFactor[c][mfrr_dwn] * MFRRCapacityPowerDwn[mfrr_dwn][assetStepmFRRStep[t]]
 	  ...
 	  ctCongestionsOut :
		...
		- sum(conv in isEIN_CONVS) assetCongestionFactor[c][conv] * ConvActivePowerIn[conv][t]
 	  	...
 	  	+ (isAFRRBackedUp == 0 ?
 	  		...
 	  		// If back up, we limit to one minimum asset partiticipation. Will need to be changed if more assets and/or more thant 1MW aFRR
 	  		:(card(isAFRR_UP_ASSETS) > 0 ? 
 	  			max(afrr_up_a in isAFRR_UP_ASSETS) afrrUpAssetFactor[c][afrr_up_a] * AFRRCapacityPowerUp[afrr_up_a][assetStepAfrrVoluntaryStep[t]]
 	  			:0))
		+ sum(mfrr_up in ismFRR_UP_ASSETS: mFRRactivatedPower_mFRR[assetStepmFRRStep[t]] == 0) mfrrUpAssetFactor[c][mfrr_up] * MFRRCapacityPowerUp[mfrr_up][assetStepmFRRStep[t]]
 	  	+ CongestionLowerLimViolation[c][t] - CongestionUpperLimViolation[c][t]
...
// maximum charging rate for charging station used by V1G_C1 and V1G_C2 vehicles set to 18kW
	 	forall (t in isDECISION_STEPS)
	 	  	ctCongV1GC1C2 : sum (s in isE_STORAGES inter isCONG_V1GC1C2) StorACActivePower[s][t] >= -18.0;
// maximum charging rate for charging station used by V1G_C6a to V1G_C19b (19 vehicles) set to 76kW
		forall (t in isDECISION_STEPS)
		 	ctCongV1GC6aToC19b : sum (s in isE_STORAGES inter isCONG_V1GC6aToC19b) StorACActivePower[s][t] >= -76.0;
  }
	if (microgridName == "MICROGRID GP Vierpolders") {
// maximum power offtake through CHP's transformer set to 1st transformer's max capacity
	 	forall (a in isNF_E_LOADS inter {"GP_VIERPOLDERS_TRANSFO_CHP"}, t in isDECISION_STEPS)
	 	  	ctOfftCongGPVierCHP:
		 	  	-nfElecLoadMaxAvailPower[a][t] <=
	 	  			  sum(d in isDISP_E_GENS inter isCONG_GP_VIER_CHP) DispGenActivePower[d][t]
			 	  	+ sum(i in isINTER_E_GENS inter isCONG_GP_VIER_CHP) InterGenActivePower[i][t]
			 	  	+ sum(s in isE_STORAGES inter isCONG_GP_VIER_CHP) StorACActivePower[s][t]
			 	  	- sum(f in isFLEX_E_LOADS inter isCONG_GP_VIER_CHP) FlexLoadActivePower[f][t]
			 	  	- sum(n in isNF_E_LOADS inter isCONG_GP_VIER_CHP) NFLoadActivePower[n][t]
			 	  	- sum(fcr_a in isFCR_ASSETS inter isCONG_GP_VIER_CHP) FCRPower[fcr_a][assetStepFCRStep[t]]
			 	  	- sum(afrr_dwn_a in isAFRR_DOWN_ASSETS inter isCONG_GP_VIER_CHP) AFRRCapacityPowerDwn[afrr_dwn_a][assetStepAfrrVoluntaryStep[t]];
// maximum power injection through CHP's transformer set to 1st transformer's max capacity
	 	forall (a in isNF_E_LOADS inter {"GP_VIERPOLDERS_TRANSFO_CHP"}, t in isDECISION_STEPS)
	 	  	ctInjCongGPVierCHP:
	 	  			  sum(d in isDISP_E_GENS inter isCONG_GP_VIER_CHP) DispGenActivePower[d][t]
			 	  	+ sum(i in isINTER_E_GENS inter isCONG_GP_VIER_CHP) InterGenActivePower[i][t]
			 	  	...
			 	  	- sum(n in isNF_E_LOADS inter isCONG_GP_VIER_CHP) NFLoadActivePower[n][t]
			 	  	+ sum(fcr_a in isFCR_ASSETS inter isCONG_GP_VIER_CHP) FCRPower[fcr_a][assetStepFCRStep[t]]
			 	  	+ sum(afrr_up_a in isAFRR_UP_ASSETS inter isCONG_GP_VIER_CHP) AFRRCapacityPowerUp[afrr_up_a][assetStepAfrrVoluntaryStep[t]]
		 	  	<= nfElecLoadMaxAvailPower[a][t];
// maximum power offtake through e-boilers's transformer set to 1st transformer's max capacity
	 	forall (a in isNF_E_LOADS inter {"GP_VIERPOLDERS_TRANSFO_EBOILER"}, t in isDECISION_STEPS)
	 	  	ctOfftCongGPVierEB:
		 	  	-nfElecLoadMaxAvailPower[a][t] <=
	 	  			  sum(d in isDISP_E_GENS inter isCONG_GP_VIER_EB) DispGenActivePower[d][t]
			 	  	+ sum(i in isINTER_E_GENS inter isCONG_GP_VIER_EB) InterGenActivePower[i][t]
			 	  	+ sum(s in isE_STORAGES inter isCONG_GP_VIER_EB) StorACActivePower[s][t]
			 	  	- sum(f in isFLEX_E_LOADS inter isCONG_GP_VIER_EB) FlexLoadActivePower[f][t]
			 	  	- sum(n in isNF_E_LOADS inter isCONG_GP_VIER_EB) NFLoadActivePower[n][t]
			 	  	- sum(fcr_a in isFCR_ASSETS inter isCONG_GP_VIER_EB) FCRPower[fcr_a][assetStepFCRStep[t]]
			 	  	- sum(afrr_dwn_a in isAFRR_DOWN_ASSETS inter isCONG_GP_VIER_EB) AFRRCapacityPowerDwn[afrr_dwn_a][assetStepAfrrVoluntaryStep[t]];
// maximum power injection through e-boiler's transformer set to 1st transformer's max capacity
	 	forall (a in isNF_E_LOADS inter {"GP_VIERPOLDERS_TRANSFO_EBOILER"}, t in isDECISION_STEPS)
	 	  	ctInjCongGPVierEB:
	 	  			  sum(d in isDISP_E_GENS inter isCONG_GP_VIER_EB) DispGenActivePower[d][t]
			 	  	+ sum(i in isINTER_E_GENS inter isCONG_GP_VIER_EB) InterGenActivePower[i][t]
			 	  	...
			 	  	- sum(n in isNF_E_LOADS inter isCONG_GP_VIER_EB) NFLoadActivePower[n][t]
			 	  	+ sum(fcr_a in isFCR_ASSETS inter isCONG_GP_VIER_EB) FCRPower[fcr_a][assetStepFCRStep[t]]
			 	  	+ sum(afrr_up_a in isAFRR_UP_ASSETS inter isCONG_GP_VIER_EB) AFRRCapacityPowerUp[afrr_up_a][assetStepAfrrVoluntaryStep[t]]
		 	  	<= nfElecLoadMaxAvailPower[a][t];
  }
...
// Initial charge is the available charge
	forall (s in isH_STORAGES)
	  ctHeatStorageStoredAvailEnergy1: StorStoredHeatAvail[s][first(isDECISION_STEPS)] == storInitialHeatEnergy[s]*availGlobalForEmax[s][first(isDECISION_STEPS)] + StorStepHeatIn[s][first(isDECISION_STEPS)];
...
// Minimum Avail heat energy
 	forall (s in isH_STORAGES, t in isDECISION_STEPS)
 	  ctHeatStorageMinEnergy: storMinUsefullAvaiHeat[s][t] - HeatStorEnerMinDeficit[s][t] <=  StorStoredHeatAvail[s][t];

// Maximum Avail heat energy
 	forall (s in isH_STORAGES, t in isDECISION_STEPS)
 	  ctHeatStorageMaxEnergy: StorStoredHeatAvail[s][t] + PossibleHeatGenOnMfrrAll[t] <= storMaxUsefullAvailHeat[s][t] + HeatStorEnerMinExcess[s][t];
...
// Relation between elec generation and heat generation for dispatchable elec and heat generation assets
	forall (d in isDISP_EH_GENS, t in isDECISION_STEPS)
	  ctDispGenElecHeatRel : DispGenHeat[d][t] == dispGenHeatElecRatio[d] * DispGenEffActivePower[d][t];
...
//{t_empty} MARKET_BIDS_OUTPUT_HEADER = {<"step_id", "type", "direction", "step_duration", "power", "price", "", "", "", "", "", "", "", "", "", "", "", "", "", ""> | i in 1..1};//{t_empty} ASSET_STEPS_COST_HEADER = {<"asset_id", "site_id", "step_id", "start_up_cost", "var_cost_model", "curtailment_cost", "", "", "", "", "", "", "", "", "", "", "", "", "", ""> | i in 1..1};
...
{t_market_bids_output} MARKET_BIDS_OUTPUT = {<h, "DAY_AHEAD", (DaPosition[h] <= 0 ? "SELL" : "BUY"), daStepDuration, DaPosition[h], (DaPosition[h] <= 0.0 ? sellOfferPrice : 
											...
											union {<afrr_c, "P0", (P0Position[afrr_c] >= 0 ? "SELL" : "BUY"), afrrCapacityStepDuration, P0Position[afrr_c], 0.0> | afrr_c in isAFRR_CAPACITY_STEPS_POS }
											union {<mfrr, "MFRR_POWER", "UP", mFRRStepDuration, mFRRUpReqPower[mfrr], 0.0> | mfrr in ismFRR_STEPS_POS}
											union {<mfrr, "MFRR_POWER", "DOWN",	mFRRStepDuration, mFRRDwnReqPower[mfrr], 0.0> | mfrr in ismFRR_STEPS_POS};
...
int nMarketBids = card(isHOURLY_STEPS_POS) 
                ...
                + card(isAFRR_CAPACITY_STEPS_POS)
				+ 2 * card(ismFRR_STEPS_POS);
/*ASSET_STEPS_COST*/
...
// 				assetPowerTarget[i][t] = -InterGenActivePowerEnd[i][t];				// respects Everest's sign convention (-ve values = power generation)
 				...
 				if ((microgridName == "MICROGRID GDS Leforest A" || microgridName == "MICROGRID GDS Leforest B")
 					&& (daElecPriceRaw[assetStepHourlyStep[t]] < 0.0 || interGenForcedPower[i][t]==0))
 				...
 				if (microgridName == "MICROGRID NEXHOS Montbert" || microgridName == "MICROGRID NEXHOS Colisee") {
 					...
 					if (daElecPriceRaw[assetStepHourlyStep[t]] < 0.0 || interGenForcedPower[i][t]==0)
 				...
 				if (microgridName == "Equans BIC microgrid"
 					&& (daElecPriceRaw[assetStepHourlyStep[t]] < aggregation_fee || interGenForcedPower[i][t]==0)
 					&& i == "BIC_PV1")
				...
				if (microgridName == "Equans BIC microgrid"
  		        ...
  		        if (microgridName == "MICROGRID VALOREM LIMOUX") {
 					...
 					if (daElecPriceRaw[assetStepHourlyStep[t]] < -networkDrawingTax[t] + sellOfferPrice
 						&& (InterGenActivePower[i][t] <= 0.0 || interGenForcedPower[i][t]==0))
 						assetCurtailmentTarget[i][t] = 0.0;							// this is to indicate to the PMS that PV generation must be curtailed to 0
 					if (daElecPriceRaw[assetStepHourlyStep[t]] >= -networkDrawingTax[t] + sellOfferPrice
...
// 			assetPowerTarget[d][t] = -DispGenActivePowerEnd[d][t];				// respects Everest's sign convention (-ve values = power generation)
		...
		for(var s in isSITES)
 		...
 		for	(var s in isH_STORAGES)
 		{
  		...
  		// heat power generation target for heat only generation assets
  		for (var d in isDISP_H_ONLY_GENS)	
  		{
  			assetPowerTarget[d][t] =  -DispGenHeat[d][t];					  // respects Everest's sign convention (-ve values = power generation)
  		}
...
//  		} 			
	...
	if(InterGenForcedPowerViolationExcess[i][t] > epsilon){
		if (!anyForcedPowerViolation) {
			strDispatchStatusString += ", some minimum power ";
			anyForcedPowerViolation = true;
		}
	VIOLATIONS_OUTPUT.add("inter_gen_forced_active_power_excess", i, t, InterGenForcedPowerViolationExcess[i][t], InterGenForcedPowerViolationCost[i][t], 100);
	}	

	if(InterGenForcedPowerViolationDeficit[i][t] > epsilon){
		if (!anyForcedPowerViolation) {
			strDispatchStatusString += ", some minimum power ";
			anyForcedPowerViolation = true;
		}
	VIOLATIONS_OUTPUT.add("inter_gen_forced_active_power_deficit", i, t, InterGenForcedPowerViolationDeficit[i][t], InterGenForcedPowerViolationCost[i][t], 100);
	}	
	}
	...
	for (var t in isAFRR_VOLUNTARY_STEPS_POS) {
		...
		if (isAFRRBackedUp == 1){
			...
			for (var a in isAFRR_DOWN_ASSETS){
	...
	// check if there is any mFRR up or down Power requirement violation
	var anyMfrrUpEngPowerChange = false;
	var anyMfrrDwnEngPowerChange = false;
	var anyMfrrUpReqPowerDeficit = false;
	var anyMfrrDwnReqPowerDeficit = false;
	for (var mfrr in ismFRR_STEPS_POS) {
		if (mFRRUpReqPower[mfrr] < mFRRUpCapacityEngPower[mfrr] - epsilon) {
			if (!anyMfrrUpEngPowerChange) {
				strDispatchStatusString += ", reductions of mFRR Up engagement";
				anyMfrrUpEngPowerChange = true;
 			}

			VIOLATIONS_OUTPUT.add("mFRR_up_engagement_reduction_flag", "", mfrr,  mFRRUpCapacityEngPower[mfrr] - mFRRUpReqPower[mfrr], Opl.intValue(""), 1);
		}
		
		if (mFRRDwnReqPower[mfrr] < mFRRDownCapacityEngPower[mfrr] - epsilon) {
			if (!anyMfrrDwnEngPowerChange) {
				strDispatchStatusString += ", reductions of mFRR Down engagement";
				anyMfrrDwnEngPowerChange = true;
 			}

			VIOLATIONS_OUTPUT.add("mFRR_down_engagement_reduction_flag", "", mfrr,  mFRRDownCapacityEngPower[mfrr] - mFRRDwnReqPower[mfrr], Opl.intValue(""), 1);
		}

		if (MFRRUpPowerEngDeficit[mfrr] > epsilon) {
			if (!anyMfrrUpReqPowerDeficit) {
				strDispatchStatusString += ", some mFRR Up requirement violation";
				anyMfrrUpReqPowerDeficit = true;
			}
			VIOLATIONS_OUTPUT.add("Pool_mFRR_Up_Power_requirement_violation", a, mfrr,  MFRRUpPowerEngDeficit[mfrr], MFRRUpPowerEngDeficitCost[mfrr], 1);
		}			

		if (MFRRDownPowerEngDeficit[mfrr] > epsilon) {				
			if (!anyMfrrDwnReqPowerDeficit) {
				strDispatchStatusString += ", some mFRR Down requirement violation";
				anyMfrrDwnReqPowerDeficit = true;
			}
			VIOLATIONS_OUTPUT.add("Pool_mFRR_Down_Power_requirement_violation", a, mfrr,  MFRRDownPowerEngDeficit[mfrr], MFRRDownPowerEngDeficitCost[mfrr], 1);
		}
	}
