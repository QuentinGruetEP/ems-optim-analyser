...
//int daStepRealHourStep[isDA_STEPS] = [t.day_ahead_step : t.real_hour | t in OPERATION_STEPS_LINK];
int daStepRealHourStep[isDA_STEPS];
//real minute according to da step
string daStepRealMinuteStep[isDA_STEPS] = [da.day_ahead_step : da.asset_step_minute_local_time | da in OPERATION_STEPS_LINK];;
...
{string} is1am = {da | da in isDA_STEPS : daStepRealHourStep[da] == 1};
...
{string} is4am = {da | da in isDA_STEPS : daStepRealHourStep[da] == 4};
...
{string} is5am = {da | da in isDA_STEPS : daStepRealHourStep[da] == 5};
...
{string} is10am = {da | da in isDA_STEPS : daStepRealHourStep[da] == 10};
...
{string} is10pm = {da | da in isDA_STEPS : daStepRealHourStep[da] == 22};
...
{string} isMidnight = {da | da in isDA_STEPS : daStepRealHourStep[da] == 0};
		...
		(microgridName == "Equans BIC microgrid" ? 				// If Equans : we are not exposed to imabalance.
			...
			:(microgridName == "MICROGRID MOPABLOEM" ?				// if MOPABLOEM
				...
				:(microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2" ? 				// If Qair /!\ electricityPrice is not indexed on imb step but rather on decision step (but imb=t for Qair)
					 (daStepRealHourStep[imb] < 9 ? // We penalize more imbalance during night because it is not suppose to go to imbalance unless there are unavailabilities or other unexpected things
			    			1.2 * ppaTariff
			    			: 0.8 * ppaTariff)
...
//		}
 ...
 // hourString to get the hour from input
    ...
    for (var da in isDA_STEPS) {
      for (var t in isDECISION_STEPS) {
		    if (da == assetStepDAStep[t]) {
		       daStepRealHourStep[da] = assetStepRealHour[t];
          }	
		}
	}
	...
	for (var da in isDA_STEPS){
		var h = daStepRealHourStep[da]
		
		if (((2<=h && h<=3) || (6<=h && h<= 8) || (11<=h && h<=20) || (23<= h) ) ){
			isPPAEngagementStepSameAsBefore.add(parseFloat(da))		
		}
		else if(((h == 1 || h == 5 || h == 10 || h == 22) && daStepRealMinuteStep[da]!= "0")){
			isPPAEngagementStepSameAsBefore.add(parseFloat(da))			
		}
	}	
...
//dvar int DAOffer100kW[isDA_STEPS];
...
dexpr float DaPosition[da in isDA_STEPS] = 
				(isDAStepCleared[da] == 1 ?
				  (microgridName in {"MICROGRID Qair STORSUN1", "MICROGRID Qair STORSUN2"} ?
	                  daEngagement[da] *
	                    (daStepRealHourStep[da] in {0,9} ? //Ramp up ?
	                      (daStepRealMinuteStep[da] == "0" ? (1/8)
	                      	:(daStepRealMinuteStep[da] == "15" ? (1/4 + 1/8)
	                      		:(daStepRealMinuteStep[da] == "30" ? (2/4 + 1/8)
	                      			: (3/4 + 1/8)
	                      			)
	                      		)
	                      	)
		                  :(daStepRealHourStep[da] in {4,21} ? //Ramp down ?
		                      (daStepRealMinuteStep[da] == "0" ? (3/4 + 1/8)
		                      	:(daStepRealMinuteStep[da] == "15" ? (2/4 + 1/8)
		                      		:(daStepRealMinuteStep[da] == "30" ? (1/4 + 1/8)
		                      			: (1/8)
		                      			)
		                      		)
		                      	) 
...
dexpr float StepRampLimitation[t in isDECISION_STEPS] = 
      ...
      (assetStepRealHour[t] < 3 ? 
        (ord(isDA_STEPS, t) + 4 < card(isDA_STEPS) ?
          DaPosition [item(isDA_STEPS,ord(isDA_STEPS,assetStepDAStep[t])+4)] / 4
...
if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2") {
	...
	// first step always cleared, no need to deal with
 	forall (da in isDA_STEPS : ( da != first(isDA_STEPS) && isDAStepCleared[da] != 1 && da in isPPAEngagementStepSameAsBefore))
		ctPPAEngagementConstant:
		-DaPosition[da] == -DaPosition[prev(isDA_STEPS, da)];

	//Uncleared steps : ramps
	// Ramp up from 0:00am to 1:00am and 9:00am to 10:00am, must be linear
	forall (da in isDA_STEPS : daStepRealHourStep[da] in {0,9} && isDAStepCleared[da] != 1)
	  ...
	  ctPPAEngagementRampUp0amdAnd9am:
	  	 -DaPosition[da] == -DaPosition[item(isDA_STEPS, (ord(isDA_STEPS, da) + 4 < card(isDA_STEPS) ?// If 09:00am->09:45am is in the optimization window
	  														ord(isDA_STEPS, da) + 4							// 09:00am->09:45am the same day
											  				: ord(isDA_STEPS, da) - 20 * 4 ))					// 01:00pm->01:45pm the day before
										] * 
                                            (daStepRealMinuteStep[da] == "0" ? (1/8)
                                           	:(daStepRealMinuteStep[da] == "15" ? (1/4 + 1/8)
                                           		:(daStepRealMinuteStep[da] == "30" ? (2/4 + 1/8)
    ...
    forall (da in isDA_STEPS : daStepRealHourStep[da] in {4,21} && isDAStepCleared[da] != 1)
      ctPPARampDownEngagement5amAnd9pm:
       -DaPosition[da] == -DaPosition[item(isDA_STEPS, (ord(isDA_STEPS, da) - 4 >= 0 ?
                                                         ord(isDA_STEPS, da) -4
	  													: ord(isDA_STEPS, da) + 20 * 4 ))					// 01:00pm->01:45pm the day before
										] * 
                                            (daStepRealMinuteStep[da] == "0" ? (3/4 + 1/8)
	...
	// Position must be larger or equal to minimum plateau value. If already cleared, not relevant since it is an input parameter.
		forall (da in isDA_STEPS : daStepRealHourStep[da] >= 10 && daStepRealHourStep[da] <= 20 && isDAStepCleared[da] != 1) 	
		  ctPPAEngagementMin:	
		  	-DaPosition[da] >= minPlateauValue;
		  	
	// Position cannot be larger than max grid capacity
		forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 10 && assetStepRealHour[t] <= 20 && isDAStepCleared[assetStepDAStep[t]] != 1) 	
		  ctPPAEngagementMax:	
		  	-DaPosition[assetStepDAStep[t]] <= siteNomMaxExport[first(isSITES)] ;
	
	//PPA position must be in between max import from aux and zero outside from the previous hours
		forall (da in isDA_STEPS : (daStepRealHourStep[da] >= 5 && daStepRealHourStep[da] <= 8) || (daStepRealHourStep[da] >= 22))	
		  ctPPAEngagement0:	
		  	-maxImportFromAux <= -DaPosition[da] <= 0 ;
	...
	forall (t in isDECISION_STEPS : assetStepRealHour[t]<=9 || assetStepRealHour[t]>=21)
	  ctPPAImbalanceNoTolerance:  	 	  	
	  	-DaPosition[assetStepDAStep[t]] + ImbalancePower_imb[assetStepImbalanceStep[t]] == -AvgNetElecImportTarget[t];
	...
	forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 10 && assetStepRealHour[t] <= 20)
	  ctPPAImbalanceToleranceDwn:  	 	  	
	  	(1-toleranceDwnPercentImb/100)*(-DaPosition[assetStepDAStep[t]]) - NegativeImbalancePower_imb[assetStepImbalanceStep[t]] <= -AvgNetElecImportTarget[t];
	
	forall (t in isDECISION_STEPS : assetStepRealHour[t] >= 10 && assetStepRealHour[t] <= 20)	
	  ctPPAImbalanceToleranceUp:	
	  	-AvgNetElecImportTarget[t] 
	  	<= (1+toleranceUpPercentImb/100)*-DaPosition[assetStepDAStep[t]] + PositiveImbalancePower_imb[assetStepImbalanceStep[t]];
...
//													    )
//													:DaPosition[h])
													daNomination[h]  , (DaPosition[h] <= 0.0 ? sellOfferPrice : buyOfferPrice[h] )>| h in isDA_STEPS_POS} union
...
//  		} 			
	...
	// Qair hard-coded post treatment, to correct DAPosition and set it 'end of target'
	if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2"){
		for (var da in isDA_STEPS){	
	
                    if (daStepRealHourStep[da] == 0 || daStepRealHourStep[da] == 9){ //Ramp up ?
                        daNomination[da] = DaPosition[Opl.item(isDA_STEPS, (Opl.ord(isDA_STEPS, da) + 4 < Opl.card(isDA_STEPS) ?// If 09:00am->09:45am is in the optimization window
	  														Opl.ord(isDA_STEPS, da) + 4							// 09:00am->09:45am the same day
											  				: Opl.ord(isDA_STEPS, da) - 20 * 4 ))					// 01:00pm->01:45pm the day before
										] * 
                          (daStepRealMinuteStep[da] == "0" ? (1/4)
                          	:(daStepRealMinuteStep[da] == "15" ? (2/4)
                          		:(daStepRealMinuteStep[da] == "30" ? (3/4)
                          :(4/4))))
                          }
                   else if (daStepRealHourStep[da] == 4 || daStepRealHourStep[da] == 21){ //Ramp down ?
                      daNomination[da] = DaPosition[Opl.item(isDA_STEPS, (Opl.ord(isDA_STEPS, da) - 4 >= 0 ?
                                                             Opl.ord(isDA_STEPS, da) -4
    	  													: Opl.ord(isDA_STEPS, da) + 20 * 4 ))					// 01:00pm->01:45pm the day before
    										] *
                          (daStepRealMinuteStep[da] == "0" ? (3/4)
                          	:(daStepRealMinuteStep[da] == "15" ? (2/4)
                          		:(daStepRealMinuteStep[da] == "30" ? (1/4)
                          			:(0/4))));
                        }
                   else{
                   	daNomination[da] =  DaPosition[da]; //Other hours, we directly take the engagement                  
                   }
	...
	writeln(InCFDDaStepTradRevenues);
	writeln(NotInCFDImbalanceStepCost);
