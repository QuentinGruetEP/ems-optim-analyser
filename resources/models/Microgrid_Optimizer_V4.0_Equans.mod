...
dexpr float aFRRUpInitialAvailEnergyAC[s in isSTORAGES inter isaFRRUp_ASSETS][t in isDECISION_STEPS] = (initialSOC[s] - (storElecMinSOC[s] / 100) * storMaxNomDCEnergy[s]) * (storefficiencyaFRR/100);
...
dexpr float AFRRUpAvailEnergyAC[s in isSTORAGES inter isaFRRUp_ASSETS][t in isDECISION_STEPS] = maxl(0, (StorStoredDCEnergy[s][t] - (storElecMinSOC[s] / 100) * storMaxNomDCEnergy[s]) * (storefficiencyaFRR/100));
...
dexpr float AFRRDwnAvailEnergyAC[s in isSTORAGES inter isaFRRDwn_ASSETS][t in isDECISION_STEPS] = maxl(0,((storElecMaxSOC[s] / 100) * storMaxNomDCEnergy[s] - StorStoredDCEnergy[s][t]) * (100/storefficiencyaFRR));
