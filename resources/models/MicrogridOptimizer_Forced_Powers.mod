...
// penalty cost for non-respect of forced active power for storage assets
float storageAssetForcedPowerPenaltyCost = cat5_2Pen * penaltyBase;
// penalty cost for non-respect of forced active power for disp Gen assets
float dispGenForcedPowerPenaltyCost = cat5_2Pen * penaltyBase;
////////////////////////////
// FCR
////////////////////////////
...
// forced power violation for storage asset s over decision step t during the charging phase
// (s in isSTORAGES, t in DECISION_STEPS)
dvar float StorageAssetChargeForcedPowerViolation[isSTORAGES][isDECISION_STEPS];
...
// (s in isSTORAGES, t in DECISION_STEPS)
dexpr float StorageAssetForcedPowerViolationCost[s in isSTORAGES][t in isDECISION_STEPS] = (abs(StorageAssetChargeForcedPowerViolation[s][t]) + abs(StorageAssetDischargeForcedPowerViolation[s][t])) * storageAssetForcedPowerPenaltyCost * assetStepDurationInHours;
// Connection to grid
/////////////////////
...
// forced power violation for generator d over step t
// (d in DISP_E_GENS, t in DECISION_STEPS)
dvar float DispGenForcedPowerViolation[isDISP_E_GENS][isDECISION_STEPS];
// cost of the non-respect of the forced power for dispGen d over step t
// (d in DISP_E_GENS, t in DECISION_STEPS)
dexpr float DispGenForcedPowerViolationCost[d in isDISP_E_GENS][t in isDECISION_STEPS] = abs(dispGenForcedPowerPenaltyCost * assetStepDurationInHours * DispGenForcedPowerViolation[d][t]);
...
// Electricity generation from a disp generation asset should respect the power input by the portal customer.
	forall(d in isDISP_E_GENS, t in isDECISION_STEPS: dispGenForcedPower[d][t] > 0)
	  ctDispGenForcedPower:
	  	DispGenActivePower[d][t] == dispGenForcedPower[d][t] - DispGenForcedPowerViolation[d][t];
...
// when forcedPower different from zero, StorACPowerCharge should respect the value of forcedPower
	forall(s in isE_STORAGES, t in isDECISION_STEPS: storageForcedPower[s][t] != 0)
		ctStorageChargeForcePower: StorACPowerCharge[s][t] == (storageForcedPower[s][t] > 0 ? storageForcedPower[s][t] : 0) - StorageAssetChargeForcedPowerViolation[s][t];
...
// when forcedPower different from zero, StorACPowerDischarge should respect the value of forcedPower
	forall(s in isE_STORAGES, t in isDECISION_STEPS: storageForcedPower[s][t] != 0)
		ctStorageDischargeForcePower: StorACPowerDischarge[s][t] == (storageForcedPower[s][t] < 0 ? - storageForcedPower[s][t] : 0) - StorageAssetDischargeForcedPowerViolation[s][t];
...
//  		} 			
	...
	for (var s in isE_STORAGES)
		for (var t in isDECISION_STEPS) {
			...
			if(storageForcedPower[s][t] > 0) {   // charge ForcedPower
			  deficitOrExcess =  StorageAssetChargeForcedPowerViolation[s][t] > 0 ? "deficit" : "excess";
			  if(Opl.abs(StorageAssetChargeForcedPowerViolation[s][t]) > epsilon) {
			    if(!anyForcedPowerViolation) {
			      	strDispatchStatusString += ", some forced power violation ";
			    	anyForcedPowerExcess = true;
			    }
				VIOLATIONS_OUTPUT.add("elec_storage_forced_active_power_" + deficitOrExcess , s, t, Opl.abs(StorageAssetChargeForcedPowerViolation[s][t]) + Opl.abs(StorageAssetDischargeForcedPowerViolation[s][t]), StorageAssetForcedPowerViolationCost[s][t], 100);
			   }

			}
			else if(storageForcedPower[s][t] < 0){ // discharge ForcedPower
			  deficitOrExcess =  StorageAssetDischargeForcedPowerViolation[s][t] > 0 ? "deficit" : "excess";
			   if(Opl.abs(StorageAssetDischargeForcedPowerViolation[s][t]) > epsilon){
			      if(!anyForcedPowerViolation) {
			      	strDispatchStatusString += ", some forced power violation ";
			    	anyForcedPowerExcess = true;
			    }
	...
	for (var d in isDISP_E_GENS)
		for (var t in isDECISION_STEPS) {
		deficitOrExcess = DispGenForcedPowerViolation[d][t] > 0 ? "deficit" : "excess";
		if(Opl.abs(DispGenForcedPowerViolation[d][t]) > epsilon){
				if (!anyForcedPowerViolation) {
					strDispatchStatusString += ", some minimum power ";
					anyForcedPowerViolation = true;
 			}
 			VIOLATIONS_OUTPUT.add("disp_gen_forced_active_power_" + deficitOrExcess , d, t, Opl.abs(DispGenForcedPowerViolation[d][t]), DispGenForcedPowerViolationCost[d][t], 100);
			}
		}
