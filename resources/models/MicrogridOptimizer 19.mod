...
dexpr float PPATotalTradeCost = sum(t in isDECISION_STEPS) AvgNetElecImportTarget[t] * ppaTariff * assetStepDurationInHours;
...
if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2") {
	...
	// The linearity will be hard-coded in the PMS, here we just need to represent the energy exchanged
    ...
    forall (da in isDA_STEPS : daStepRealHourStep[da] in {5,21} && isDAStepCleared[da] != 1)
...
//  		} 			
	...
	if (microgridName == "MICROGRID Qair STORSUN1" || microgridName == "MICROGRID Qair STORSUN2"){
		for (var da in isDA_STEPS){	
                    ...
                    if (daStepRealHourStep[da] == 0 || daStepRealHourStep[da] == 9){ //Ramp up ?
                   ...
                   else if (daStepRealHourStep[da] == 5 || daStepRealHourStep[da] == 21){ //Ramp down ?
                      daNomination[da] = DaPosition[Opl.item(isDA_STEPS, (Opl.ord(isDA_STEPS, da) - 4 >= 0 ?
                   	...
                   	daNomination[da] =  DaPosition[da];                  
                   }
                         //Other hours, we directly take the engagement	
   		}    	
    }
