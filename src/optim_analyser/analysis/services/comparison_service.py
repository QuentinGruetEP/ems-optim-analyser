...
class ComparisonService:
    ...
    def compare_jobs_from_folder(
        self, jobs_folder: str, output_folder: str, color_blind: bool = False
    ) -> ComparisonResult:
        ...
        try:
            ...
            if not json_list:
                raise DataError(
                    "No JSON files found in folder", error_code="NO_JOBS_FOUND", context={"folder": jobs_folder}
                )

            (html_path, plot_param) = path.get_compare_paths_and_param_folder(jobs_folder, output_folder)

            compare.compare_jobs_from_folder(
                jobs_folder=jobs_folder, html_path=html_path, client_param=plot_param, color_blind=color_blind
            )

            # Create ComparisonResult (simplified - would need more info in real impl)
            jobs = [
                OptimizationJob(job_id=json_file, input_data=None, status=JobStatus.COMPLETED)  # Would need to load
                for json_file in json_list
            ]

            return ComparisonResult(jobs=jobs, comparison_name=Path(jobs_folder).name, html_path=Path(html_path))

        except Exception as e:
            raise VisualizationError(
                f"Comparison failed: {str(e)}", error_code="COMPARISON_FAILED", context={"jobs_folder": jobs_folder}
            ) from e

    def compare_specific_jobs(
        self, json_paths: List[str], output_folder: str, comparison_name: str = "comparison", color_blind: bool = False
    ) -> ComparisonResult:
        ...
        try:
            if not json_paths:
                raise DataError("No jobs provided for comparison", error_code="EMPTY_COMPARISON")
            ...
            compare.compare_jobs_from_list(
                json_paths=json_paths, html_path=html_path, client_param=plot_param, color_blind=color_blind
            )

            jobs = [
                OptimizationJob(job_id=Path(jp).stem, input_data=None, status=JobStatus.COMPLETED) for jp in json_paths
            ]

            return ComparisonResult(jobs=jobs, comparison_name=comparison_name, html_path=Path(html_path))
