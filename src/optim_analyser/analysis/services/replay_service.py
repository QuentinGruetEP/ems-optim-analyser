...
class ReplayService:
    ...
    def replay_local(
        ...
        try:
            ...
            optimization.prepare_optimization(
                data, model_path, run_model_path,
            ...
            raise OptimizationFail(
                f"Local replay failed: {str(e)}",
                error_code="REPLAY_LOCAL_FAILED",
    ...
    def replay_remote(
        ...
        try:
            ...
            (excel_init_path, excel_input_path, excel_output_path,
             model_path, run_dat_path, run_model_path, json_output_path,
             dat_costs_extension_path, mod_costs_extension_path,
             html_path,
             plot_param) = path.get_run_paths_and_param_json(json_path, output_folder)
            ...
            raise IBMJobError(
                f"Remote replay failed: {str(e)}",
                error_code="REPLAY_REMOTE_FAILED",
                context={"json_path": json_path}
            ) from e
