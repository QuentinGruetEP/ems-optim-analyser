...
def get_df(input_data:dict[str,pd.DataFrame],
           output_data:dict[str,pd.DataFrame],
           subplots_param:pd.DataFrame) -> tuple[pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,pd.DataFrame,
                                                 pd.DataFrame,pd.Series,pd.DataFrame,pd.DataFrame,pd.DataFrame,
    ...
    if 'ASSET_STEPS_OUTPUT' not in output_data:
        raise OptimizationFail(f"Output data is missing 'ASSET_STEPS_OUTPUT' sheet. Available sheets: {list(output_data.keys())}")
    
    asset_steps_df = output_data['ASSET_STEPS_OUTPUT'].filter(items=["asset_id", "step_id", "power_target","target_soc", "storage_target", "energy_target"])
    maingrid_serie = asset_steps_df[asset_steps_df['asset_id'] == 'MAINGRID'].set_index('step_id', drop=True)["power_target"].astype(float)
    ...
    asset_steps_power_df = asset_steps_df.pivot_table(index='step_id', columns='asset_id', values='power_target', fill_value=0, aggfunc=lambda x:float(x.iloc[0]))

    # Intermittent potential
    intermittent_steps_df = input_data['ASSET_STEPS'].filter(items=["asset_id", "step_id", "power_prediction"])
    intermittent_steps_df = intermittent_steps_df.loc[intermittent_steps_df['asset_id'].isin(intermittent_assets_df.index)]
    intermittent_steps_df.loc[:,'step_id'] = pd.to_numeric(intermittent_steps_df['step_id'])
    intermittent_steps_df = intermittent_steps_df.sort_values(by='step_id').pivot_table(index='step_id', columns='asset_id', values='power_prediction', fill_value=0, aggfunc=lambda x:x)

    # ENERGY MARKET PRICES & ENGAGEMENTS ------------------------------------------------------
    operation_steps_link = input_data['OPERATION_STEPS_LINK'].astype(float).astype(int).set_index('asset_step',drop=True) # All steps are converted into int (str->float->int), they are used as indexes later

    # PRICES
    prices_df = pd.DataFrame()

    # Spot price
    day_ahead_init_df = input_data['MARKET_PRICE_STEPS'][input_data['MARKET_PRICE_STEPS']['type'] == 'DAY_AHEAD'].astype({'step_index':int}).filter(items=['step_index','price']).set_index('step_index', drop=True,)
    ...
    ppa_init_df = ppa_init_df[ppa_init_df['type'] == 'PPA'].filter(items=['step_index','price']).set_index('step_index', drop=True)
    ...
    # TURPE price
    prices_df['transport'] = input_data['MARKET_PRICE_STEPS'][input_data['MARKET_PRICE_STEPS']['type'] == 'TRANSPORT'].astype({'step_index':int}).filter(items=['step_index', 'price']).set_index('step_index', drop=True)

    # Threshold on day-ahead price
    if subplots_param['spot_threshold'] :
        threshold_init_df = output_data['MARKET_BIDS_OUTPUT'][output_data['MARKET_BIDS_OUTPUT']['type'] == 'DAY_AHEAD'].astype({'step_id':int}).filter(items=['step_id','price']).set_index('step_id', drop=True,)
    ...
    long_term_engagement_init_df = input_data['MARKET_ENGAGEMENTS'].astype({'step_index':int}).set_index('step_index', drop=True)
    long_term_engagement_init_df = long_term_engagement_init_df[long_term_engagement_init_df['type'] == 'ELECTRICITY_LONG_TERM_AGREGATED_BIDS'].filter(items=['engagement']).astype(float)
    if not(long_term_engagement_init_df.empty):
        for index, steps in operation_steps_link.iterrows():
            engagement_df.at[index, 'long_term'] =  long_term_engagement_init_df.loc[steps['day_ahead_step'],'engagement']

    if subplots_param['engagement'] :
        # Day ahead engagement and clearing
        day_ahead_engagement_init_df = output_data['MARKET_BIDS_OUTPUT'].astype({'step_id':int}).set_index('step_id', drop=True)
        day_ahead_engagement_init_df = day_ahead_engagement_init_df[day_ahead_engagement_init_df['type'] == 'DAY_AHEAD'].filter(items=['power']).astype(float)
        day_ahead_clearing_init_df = input_data['MARKET_ENGAGEMENTS'].astype({'step_index':int}).set_index('step_index', drop=True)
        day_ahead_clearing_init_df = day_ahead_clearing_init_df[day_ahead_clearing_init_df['type'] == 'DAY_AHEAD'].filter(items=['is_step_cleared']).astype(int)
        if not(day_ahead_engagement_init_df.empty):
            for index, steps in operation_steps_link.iterrows():
                engagement_df.at[index, 'day_ahead'] = day_ahead_engagement_init_df.loc[steps['day_ahead_step'],'power']
                engagement_df.at[index, 'is_step_cleared'] = day_ahead_clearing_init_df.loc[steps['day_ahead_step'],'is_step_cleared']

        # PPA engagement
        ppa_engagement_init_df = output_data['MARKET_BIDS_OUTPUT'].set_index('step_id', drop=True)
        ppa_engagement_init_df = ppa_engagement_init_df[ppa_engagement_init_df['type'] == 'PPA'].filter(items=['power']).astype(float)
        if not(ppa_engagement_init_df.empty):
            for index, steps in operation_steps_link.iterrows():
                engagement_df.at[index, 'ppa'] =  -ppa_engagement_init_df.loc[int(steps['ppa_step']),'power'] #PPA engagement sign seems to be the opposite of usual signs for engagements
        
        # FCR engagement
        fcr_engagement_init_df = output_data['MARKET_BIDS_OUTPUT'].set_index('step_id', drop=True)
        fcr_engagement_init_df = fcr_engagement_init_df[fcr_engagement_init_df['type'] == 'FCR'].filter(items=['power']).astype(float)
        ...
        afrr_capacity_up_engagement_init_df = input_data['MARKET_ENGAGEMENTS'].astype({'step_index':int}).set_index('step_index', drop=True)
        afrr_capacity_up_engagement_init_df = afrr_capacity_up_engagement_init_df[afrr_capacity_up_engagement_init_df['type'] == 'AFRR_R2_CAPACITY_UP'].filter(items=['engagement']).astype(float)
        if not(afrr_capacity_up_engagement_init_df.empty):
            for index, steps in operation_steps_link.iterrows():
                engagement_df.at[index, 'afrr_capacity_up'] = - afrr_capacity_up_engagement_init_df.loc[steps['afrr_capacity_step'],'engagement']

        # aFRR capacity down engagement (input)
        afrr_capacity_down_engagement_init_df = input_data['MARKET_ENGAGEMENTS'].astype({'step_index':int}).set_index('step_index', drop=True)
        afrr_capacity_down_engagement_init_df = afrr_capacity_down_engagement_init_df[afrr_capacity_down_engagement_init_df['type'] == 'AFRR_R2_CAPACITY_DOWN'].filter(items=['engagement']).astype(float)
        if not(afrr_capacity_down_engagement_init_df.empty):
            for index, steps in operation_steps_link.iterrows():
                engagement_df.at[index, 'afrr_capacity_down'] = + afrr_capacity_down_engagement_init_df.loc[steps['afrr_capacity_step'],'engagement']

        # aFRR voluntary up engagement (input)
        afrr_voluntary_up_engagement_init_df = input_data['MARKET_ENGAGEMENTS'].astype({'step_index':int}).set_index('step_index', drop=True)
        afrr_voluntary_up_engagement_init_df = afrr_voluntary_up_engagement_init_df[afrr_voluntary_up_engagement_init_df['type'] == 'AFRR_R2_VOLUNTARY_UP'].filter(items=['engagement']).astype(float)
        if not(afrr_voluntary_up_engagement_init_df.empty):
            for index, steps in operation_steps_link.iterrows():
                engagement_df.at[index, 'afrr_voluntary_up'] = - afrr_voluntary_up_engagement_init_df.loc[steps['afrr_voluntary_step'],'engagement']
        
        # aFRR voluntary down engagement (input)
        afrr_voluntary_down_engagement_init_df = input_data['MARKET_ENGAGEMENTS'].astype({'step_index':int}).set_index('step_index', drop=True)
        afrr_voluntary_down_engagement_init_df = afrr_voluntary_down_engagement_init_df[afrr_voluntary_down_engagement_init_df['type'] == 'AFRR_R2_VOLUNTARY_DOWN'].filter(items=['engagement']).astype(float)
        if not(afrr_voluntary_down_engagement_init_df.empty):
            for index, steps in operation_steps_link.iterrows():
                engagement_df.at[index, 'afrr_voluntary_down'] = + afrr_voluntary_down_engagement_init_df.loc[steps['afrr_voluntary_step'],'engagement']
    ...
    if 'target_soc' in list(asset_steps_df.columns):
        asset_steps_soc_df = asset_steps_df.pivot_table(index='step_id', columns='asset_id', values='target_soc', fill_value=0, aggfunc=lambda x:x)
        ...
        if 'storage_target' in list(asset_steps_df.columns):
            asset_steps_soc_df = asset_steps_df.pivot_table(index='step_id', columns='asset_id', values='storage_target', fill_value=0, aggfunc=lambda x:x)
        elif 'energy_target' in list(asset_steps_df.columns): 
            asset_steps_soc_df = asset_steps_df.pivot_table(index='step_id', columns='asset_id', values='energy_target', fill_value=0, aggfunc=lambda x:x)
    ...
    asset_steps_availability_df = asset_steps_availability_df.pivot_table(index='step_id', columns='asset_id', values='availability', fill_value=0, aggfunc=lambda x:x)
    ...
    return (operation_df, operation_steps_df, assets_df, storage_assets_df, intermittent_assets_df, site_assets_df,
            maingrid_serie, asset_steps_power_df, intermittent_steps_df,
            prices_df,
            engagement_df,
            asset_steps_soc_df, asset_steps_availability_df,
            operation_steps_output_df)

def fig_from_input_output_data(input_data:dict[str,pd.DataFrame],
                                output_data:dict[str,pd.DataFrame],
                                sc_name:str,
                                subplots_param:pd.DataFrame,
                                add_costs:bool=True,
                                color_blind:bool=False) -> go.Figure :
    """
    ...
    :rtype: go.Figure
    """
    (operation_df, operation_steps_df, assets_df, storage_assets_df, intermittent_assets_df, site_assets_df,
    maingrid_serie, asset_steps_power_df, intermittent_steps_df,
    prices_df,
    engagement_df,
    asset_steps_soc_df, asset_steps_availability_df,
    operation_steps_output_df) = get_df(input_data, output_data, subplots_param)
    ...
    dates = pd.Series([optimisation_interval_start + timedelta(minutes=asset_step_duration)*i for i in range(optimisation_step_number)])
    dates.index += 1
    ...
    if any(["HEAT" in asset_id or asset_id in ["Buffer_850m3", "Verberne_America_HeatBuffer", "Verberne_America_Heat_Need"] for asset_id in assets_df.index]) :
        energy_vectors_number += 1
    ...
    all_subplot_titles_flat = flatten_list([power_target_titles,
                                            'Energy market prices & Market engagements',
                                            'Imbalances',
                                            congestions_titles,
                                            'Target SOC & Energy market prices',
    ...
    subplot_titles=[title for index,title in enumerate(all_subplot_titles_flat)
                                            if subplots_flat[index]]

    all_specs_flat = [[{"secondary_y": True}]]*(len(subplots_flat)-1)
    all_specs_flat.append([{"type": "table"}])
    specs = [title for index,title in enumerate(all_specs_flat)
                                    if subplots_flat[index]]
    ...
    for index, title in enumerate(subplot_titles):
        ...
        if title.startswith('Power Target') and not power_target_plot_done:
            ...
            subplot.plot_power_target_by_energy_vector([row+i for i in range(energy_vectors_number)],
                                                            fig, color_map, convention,
                                                            dates,
                                                            assets_df, asset_steps_power_df, intermittent_assets_df, intermittent_steps_df,
                                                            maingrid_serie, subplots_param['maingrid'],
                                                            energy_vectors)
        elif title == 'Energy market prices & Market engagements':
            subplot.plot_energy_market_prices_engagements(row, fig, color_map, currency_unit, convention,
                                                        dates, prices_df, engagement_df,
                                                        operation_steps_output_df)    
        elif title == 'Imbalances':
            subplot.plot_imbalances(row, fig, color_map, convention,
                                    dates,
                                    engagement_df, maingrid_serie,
                                    operation_steps_output_df, storage_assets_df, asset_steps_power_df)
            ...
            subplot.plot_congestions_by_site([row+i for i in range(subplots_congestions_number)], 
                                                      ...
                                                      operation_steps_df,
                                                      site_assets_df,
                                                      asset_steps_power_df, assets_df,
                                                      engagement_df)
        elif title == 'Target SOC & Energy market prices':
            subplot.plot_soc(row, fig, color_map, currency_unit,
                     dates,
                     storage_assets_df, asset_steps_soc_df, prices_df, operation_steps_output_df)
        elif title == 'Assets availability':
            subplot.plot_asset_availability(row, fig, color_map,
                                    dates,
                                    assets_df, asset_steps_availability_df)
        elif title == 'Costs':
            subplot.plot_costs(row, fig, color_map_costs, currency_unit,
                                dates,
                                input_data, output_data)
        elif title == 'Total costs':
            subplot.plot_total_costs(row, fig, color_map_costs, currency_unit,
                                     input_data, output_data)
        elif title == 'Violations':
            subplot.plot_violations(row, fig,
...
def plot_from_input_output_data(input_data:dict[str,pd.DataFrame],
                                output_data:dict[str,pd.DataFrame],
                                sc_name:str,
                                html_path:str,
                                subplots_param:pd.DataFrame,
...
def plot_from_data(all_data:dict[str,pd.DataFrame],
                   sc_name:str,
                   html_path:str,
                   subplots_param:pd.DataFrame,
                   color_blind:bool=False) -> None :
    ...
    input_fields = ['assets', 'asset_steps', 'congestions', 'congestion_assets', 'market_engagements',
                    'market_price_steps', 'operation', 'operation_steps', 'operation_steps_link',
                    'variable_cost_models']
    input_fields_upper = [field.upper() for field in input_fields]

    output_fields = ['operation_output','operation_steps_output', 'assets_output', 'asset_steps_output',
                     'violations_output', 'market_bids_output', 'costs']
...
def plot_from_excel(excel_input_path:str, excel_output_path:str,
                    sc_name:str,
                    html_path:str,
