...
def display_from_json(json_path:str, output_folder:str,
    ...
    display.plot_from_data(all_data=data,
                            sc_name=None,
                            html_path=html_path,
                            subplots_param=plot_param,
                            color_blind=color_blind)

def display_from_excel(excel_input_path:str, excel_output_path:str,
                       output_folder:str,
                       sc_name:str=None,
                       color_blind:bool=False) -> None:
    """
    ...
    # Display and save visuals in the .html
    display.plot_from_excel(excel_input_path,
                            excel_output_path,
                            sc_name=sc_name,
                            html_path=html_path,
...
def replay_from_json_and_display_local(json_path:str, output_folder:str,
                                 add_costs:bool=True,
                                 color_blind:bool=False) -> None:
    """
    ...
    (excel_init_path, excel_input_path, excel_output_path,
    model_path, run_dat_path, run_model_path, _,
    dat_costs_extension_path, mod_costs_extension_path,
    html_path,
    plot_param) = path.get_run_paths_and_param_json(json_path, output_folder)
    ...
    optimization.prepare_optimization(data, model_path, run_model_path,
    ...
    display.plot_from_data(all_data=(data | data_recomputed), # This should keep the data from the .json and replace the recomputed sheets
                           sc_name=None,
                           html_path=html_path,
                           subplots_param=plot_param,
                           color_blind=color_blind)

def run_from_excel_and_display_local(excel_input_path:str, output_folder:str,
    ...
    plot_param) = path.get_run_paths_and_param_excel(excel_input_path, output_folder)
    ...
    data_recomputed = dataframes.excel_to_dataframe(run_excel_input_path) | dataframes.excel_to_dataframe(run_excel_output_path)
    
    shutil.move(run_excel_input_path, excel_input_final_path)
    shutil.move(run_excel_output_path, excel_output_final_path)

    # Display results and save the graphs in a html file
    display.plot_from_data(all_data=(data | data_recomputed),
                           sc_name=None,
                           html_path=html_path,
                           subplots_param=plot_param,
                           color_blind=color_blind)

def run_scenarios_local(run_model_path:str, run_dat_path:str, run_data_folder:str,
                  excel_folder_path:str, sc_list:list[str], in_place:bool) -> None:
    """
...
def run_scenarios_from_folder_local(excel_folder_path:str, output_folder:str, sc_list:list[str]=None, add_costs:bool=False, in_place:bool=True) -> None :
    """ 
    ...
    # Create all files paths
    (model_path, run_dat_path, run_model_path, run_data_folder,
    dat_costs_extension_path, mod_costs_extension_path, _, _, _) = path.get_run_paths_and_param_excel(excel_input_path, output_optim_folder)
    ...
    run_scenarios_local(run_model_path, run_dat_path, run_data_folder,
                        excel_folder_path, sc_list=sc_list, in_place=in_place)
     
def display_scenarios(excel_folder_path:str, sc_list:list[str]=None, add_costs:bool=False, color_blind:bool=True) -> None:
    """
    ...
    for sc_name in sc_list :
        ...
        # Get all paths and plotting parameters 
        (html_path, plot_param) = path.get_display_paths_and_param_scenario(excel_input_path, excel_folder_path, sc_name)
...
def display_optimization_series(excel_folder_path:str, output_folder:str, add_costs:bool=False, color_blind:bool=True) -> None:
    """
    ...
    for iteration in optim_list:
        ...
        excel_input_path = os.path.join(iteration_folder_path, [f.name for f in os.scandir(iteration_folder_path) if f.name.endswith("input.xlsx")][0])
        excel_output_path = os.path.join(iteration_folder_path, [f.name for f in os.scandir(iteration_folder_path) if f.name.endswith("output.xlsx")][0])
        ...
        html_path_iteration = os.path.join(os.path.dirname(html_path_basic), html_basic_filename + "_"+iteration+".html")
...
def compare_from_excel(excel_input_init_path:str, excel_output_init_path:str,
                       excel_input_forced_path:str, excel_output_forced_path:str,
                       output_folder:str=None, color_blind:bool=False):
    """
    ...
    compare.compare_from_input_output_data(data_in_i, data_out_i, data_in_f, data_out_f, html_path=html_path, subplots_param=plot_param, color_blind=color_blind)

def compare_from_json_excel(json_init_path:str,
                            excel_input_forced_path:str, excel_output_forced_path:str,
                            output_folder:str=None, color_blind:bool=False):
    """
...
def run_from_excel_and_display_distant(excel_input_path:str, output_folder:str,
                                       add_costs:bool=True,
                                       color_blind:bool=False) -> None:
    """
    ...
    # Replay the optimisation
    model_id, deployment_id = optimizationIBM.create_model_and_deployment_distant(ibm_watson_ml_properties=ibm_watson_ml_properties, modelPath=run_model_path, modelName="OptimAnalyser")
    optimizationIBM.run_optimization_distant(in_data=input_data, output_path=output_path, ibm_watson_ml_properties=ibm_watson_ml_properties, modelId=model_id, deploymentId=deployment_id)
    
    # DELETE DEPLOYMENT AND MODEL
    optimizationIBM.delete_deployment_and_model_distant(ibm_watson_ml_properties=ibm_watson_ml_properties, model_id=model_id, deployment_id=deployment_id)
    ...
    # Display results and save the graphs in a html file
    display.plot_from_data(all_data=data_recomputed, # This should keep the data from the .json and replace the recomputed sheets
                           sc_name=None,
                           html_path=html_path,
                           subplots_param=plot_param,
                           color_blind=color_blind)
    
def replay_from_json_and_display_distant(json_path:str, output_folder:str,
                                         add_costs:bool=True,
                                         color_blind:bool=False) -> None:
    """
    ...
    # Display results and save the graphs in a html file
    display.plot_from_data(all_data=data_recomputed,
                           sc_name=None,
                           html_path=html_path,
                           subplots_param=plot_param,
                           color_blind=color_blind)
    
def run_scenarios_distant(model_id:str, deployment_id:str,
                          excel_folder_path:str, sc_list:list[str]) -> None:
    """
...
def run_scenarios_from_folder_distant(excel_folder_path:str, output_folder:str, sc_list:list[str]=None, add_costs:bool=False) -> None :
    """ 
    ...
    # Run scenarios that appear in the scenario list
    run_scenarios_distant(model_id, deployment_id,
                          excel_folder_path, sc_list=sc_list)
