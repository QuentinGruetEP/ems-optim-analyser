...
def plot_power_target(row:int, fig:go.Figure, color_map:dict[str,str], convention:int,
                      dates:pd.Series,
                      assets_df:pd.DataFrame, asset_steps_power_df:pd.DataFrame, intermittent_assets_df:pd.DataFrame, intermittent_steps_df:pd.DataFrame,
                      maingrid_serie:pd.Series, maingrid_bool:bool=True,
    ...
    exec("fig.update_layout("+
            "yaxis%d" % (row*2-1) + "=dict(ticksuffix=' kW', tickformat='digits'))")

def plot_power_target_by_energy_vector(rows:list[int], fig:go.Figure, color_map:dict[str,str], convention:int,
                                       dates:pd.Series,
                                       assets_df:pd.DataFrame, asset_steps_power_df:pd.DataFrame, intermittent_assets_df:pd.DataFrame, intermittent_steps_df:pd.DataFrame,
                                       maingrid_serie:pd.Series, maingrid_bool:bool=True,
    ...
    if not elec_assets_steps_power_df.empty :
        plot_power_target(rows[energy_vectors.index('ELECTRICITY')], fig, color_map, convention, dates,
                          elec_assets_df, elec_assets_steps_power_df,
                          intermittent_assets_df, intermittent_steps_df, maingrid_serie,
                          maingrid_bool, diff)
    if not h2_assets_steps_power_df.empty :
        plot_power_target(rows[energy_vectors.index('H2')], fig, color_map, convention, dates,
                          h2_assets_df, h2_assets_steps_power_df,
                          pd.DataFrame(), pd.DataFrame(), pd.DataFrame(),
                          False, diff)
    if not gas_assets_steps_power_df.empty :
        plot_power_target(rows[energy_vectors.index('GAS')], fig, color_map, convention, dates,
                          gas_assets_df, gas_assets_steps_power_df,
                          pd.DataFrame(), pd.DataFrame(), pd.DataFrame(),
                          False, diff)
    if not heat_assets_steps_power_df.empty :
        plot_power_target(rows[energy_vectors.index('HEAT')], fig, color_map, convention, dates,
                          heat_assets_df, heat_assets_steps_power_df,
                          pd.DataFrame(), pd.DataFrame(), pd.DataFrame(),
                          False, diff)

def plot_energy_market_prices_engagements(row:int, fig:go.Figure, color_map:dict[str,str], currency_unit:str, convention:int,
                                          dates:pd.Series,
                                          prices_df:pd.DataFrame, engagement_df:pd.DataFrame,
                                          operation_steps_output_df:pd.DataFrame) -> None:
    """
    ...
    if engagements :
        exec("fig.update_layout("+
            "yaxis%d" % (row*2-1) + "=dict(ticksuffix=' kW', tickformat='digits'), " +
            "yaxis%d" % (row*2) +   "=dict(ticksuffix=' '+currency_unit+'/MWh', tickformat='digits'))") #tickmode='sync' to have only one y grid
    else :
        exec("fig.update_layout("+"yaxis%d" % (row*2) + "=dict(ticksuffix=' '+currency_unit+'/MWh', tickformat='digits'))")
...
def plot_congestions(row:int, fig:go.Figure, color_map:dict[str,str], convention:int,
                     dates:pd.Series,
                     min_power:float, max_power:float,
                     asset_steps_power_df:pd.DataFrame, assets_df:pd.DataFrame, maingrid_serie:pd.Series,
                     engagement_df:pd.DataFrame, diff:bool=False, legend_prefix:str="") -> None:
    """
...
def plot_congestions_by_site(rows:list[int], fig:go.Figure, color_map:dict[str,str], convention:int,
                             dates:pd.Series,
                             operation_steps_df:dict[str,pd.DataFrame],
                             site_assets_df:pd.DataFrame,
                             asset_steps_power_df:pd.DataFrame, assets_df:pd.DataFrame,
                             engagement_df:pd.DataFrame,
                             diff:bool=False, legend_prefix:str="") -> None:
    """
    ...
    for site_index,site_name in enumerate(site_assets_df['asset_id'].values) :
        row = rows[site_index]
        min_power = max(site_assets_df['min_power'][site_assets_df['asset_id']==site_name].values[0],
                    max(- operation_steps_df['max_export_to_main_grid'])) #max bc <0 values
        max_power = min(site_assets_df['max_power'][site_assets_df['asset_id']==site_name].values[0],
        ...
        for asset_id in assets_df.index:
            ...
            site_e_asset_bool.append(not(asset_id.startswith("Buffer"))
            ...
            for asset_id in assets_df.index:
                if asset_id == "CHP_1600kW" :
                    for index_power in site_assets_steps_power_df["CHP_1600kW"].index[1:] :
                        if site_assets_steps_power_df.loc[index_power, "CHP_1600kW"] < 0.0 and site_assets_steps_power_df.loc[index_power - 1, "CHP_1600kW"] == 0.0 :
                            site_assets_steps_power_df.loc[index_power, "CHP_1600kW"] = site_e_assets_df.loc[asset_id,'max_power'] / 2
                    if site_assets_steps_power_df.loc[1, "CHP_1600kW"] < 0 and assets_df.loc[asset_id,'initial_power'] >= 0 :
                            site_assets_steps_power_df.loc[1, "CHP_1600kW"] = site_e_assets_df.loc[asset_id,'max_power'] / 2
        ...
        plot_congestions(row, fig, color_map, convention, dates,
                        min_power, max_power,
                        site_assets_steps_power_df, site_e_assets_df, site_net_power_df,
                        engagement_df, diff, legend_prefix)

def plot_imbalances(row:int, fig:go.Figure, color_map:dict[str,str], convention:int,
                    dates:pd.Series,
                    engagement_df:pd.DataFrame, maingrid_serie:pd.Series,
                    operation_steps_output_df:dict[str,pd.DataFrame], storage_assets_df:pd.DataFrame, asset_steps_power_df:pd.DataFrame) -> None:
    """
...
def plot_soc(row:int, fig:go.Figure, color_map:dict[str,str], currency_unit:str,
                dates:pd.Series,
                storage_assets_df:pd.DataFrame, asset_steps_soc_df:pd.DataFrame, prices_df:pd.DataFrame, operation_steps_output_df:pd.DataFrame,
                diff:bool=False, legend_prefix:str="") -> None:
    """
    ...
    exec("fig.update_layout("+
            "yaxis%d" % (row*2-1) + "=dict(ticksuffix=' %', range=[0,100]), " +
            "yaxis%d" % (row*2) +   "=dict(ticksuffix=' '+currency_unit+'/MWh', tickformat='digits'))")

def plot_asset_availability(row:int, fig:go.Figure, color_map:dict[str,str],
                            dates:pd.Series,
                            assets_df:pd.DataFrame, asset_steps_availability_df:pd.DataFrame) -> None:
    """
    ...
    exec("fig.update_layout("+
            "yaxis%d" % (row*2-1) + "=dict(ticksuffix=' %'))")

def plot_costs(row:int, fig:go.Figure, color_map_costs:dict[str,str], currency_unit:str,
                dates:pd.Series,
                input_data:dict[str,pd.DataFrame], output_data:dict[str,pd.DataFrame]) -> None:
    """
    ...
    try :
        violations_df = output_data['VIOLATIONS_OUTPUT'].filter(items=['violation_type', 'step_id', 'violation_cost']).set_index('step_id')
        violations_df = violations_df.pivot_table(index='step_id', columns='violation_type', values='violation_cost', fill_value=0)
    ...
    long_term_costs_init_df = long_term_costs_init_df[long_term_costs_init_df['type'] == 'ELECTRICITY_LONG_TERM_AGREGATED_BIDS'].filter(items=['step_index','engagement', 'price']).set_index('step_index',drop=True)
    if not long_term_costs_init_df.empty :
        ...
        for index, steps in input_data['OPERATION_STEPS_LINK'].iterrows():
            long_term_costs_series.at[index] =  (long_term_costs_init_df.loc[steps['day_ahead_step'],'engagement']
                                                * long_term_costs_init_df.loc[steps['day_ahead_step'],'price']
                                                * asset_step_duration/60 )
    ...
    exec("fig.update_layout("+
            "yaxis%d" % (row*2-1) + "=dict(ticksuffix=' '+currency_unit))") 

def plot_total_costs(row:int, fig:go.Figure, color_map_costs:dict[str,str], currency_unit:str,
                     input_data:dict[str,pd.DataFrame], output_data:dict[str,pd.DataFrame]) -> None:
    """_summary_
    ...
    exec("fig.update_layout("+
            "xaxis%d" % (row) + "=dict(type='category'))")
    exec("fig.update_layout("+
            "yaxis%d" % (row*2-1) + "=dict(ticksuffix=' "+currency_unit+"'))")
    ...
    long_term_costs = long_term_costs[long_term_costs['type'] == 'ELECTRICITY_LONG_TERM_AGREGATED_BIDS'].filter(items=['step_index','engagement', 'price']).set_index('step_index',drop=True)
    if not long_term_costs.empty :
        costs['long_term_costs (not in objective function)'] = (long_term_costs['engagement'] * long_term_costs['price']).sum()
...
def plot_violations(row:int, fig:go.Figure,
    ...
    fig.add_trace(go.Table(
        ...
        cells=dict(
            values=[[str(violation).replace("_", " ") for violation in violations_df[k].tolist()] for k in violations_df.columns],
            align = "left")
        ),
        row=row, col=1
    )
