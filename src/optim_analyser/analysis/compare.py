...
def input_comparison(data_in_init: dict[str, pd.DataFrame], data_in_forced: dict[str, pd.DataFrame]) -> str:
    """
    ...
    for sheet_name in data_in_init.keys():
        data_in_changed[sheet_name] = data_in_init[sheet_name].compare(
            data_in_forced[sheet_name], result_names=("init", "forced")
        )
...
def obj_func_comparison(optim_objective_value_init: float, optim_objective_value_forced: float, epgap: float = 1e-6):
    ...
    if optim_objective_value_init == optim_objective_value_forced:
        ...
        optim_objective_value_init * (1 - epgap)
        <= optim_objective_value_forced
        <= optim_objective_value_init * (1 + epgap)
    ):
...
def compare_from_input_output_data(
    data_in_init: dict[str, pd.DataFrame],
    ...
    optim_objective_value_init = (
        data_init["OPERATION_OUTPUT"].set_index("param_id").transpose()["optimiser_objective_value"]["param_val"]
    )
    optim_objective_value_forced = (
    ...
    obj_func_df = pd.DataFrame(
        {
            "optimiser_objective_value": [
                data_init["OPERATION_OUTPUT"]
                .set_index("param_id")
                .transpose()["optimiser_objective_value"]["param_val"],
                data_forced["OPERATION_OUTPUT"]
                .set_index("param_id")
                .transpose()["optimiser_objective_value"]["param_val"],
                data_forced["OPERATION_OUTPUT"]
                .set_index("param_id")
            ...
            "tot_costs": [
                tot_costs_df["tot_costs_init"].sum(),
    ...
    if "violation_cost" in data_init["VIOLATIONS_OUTPUT"].columns:
        ...
        for violation_type in np.unique(
            list(data_forced["VIOLATIONS_OUTPUT"]["violation_type"].values)
            + list(data_init["VIOLATIONS_OUTPUT"]["violation_type"].values)
        ):
    ...
    tot_costs_df.loc["violation_costs"] = [
        obj_func_df.loc["initial", "violation_costs"],
    ...
    (
        ...
        intermittent_assets_df_init,
        site_assets_df_init,
        maingrid_serie_init,
        asset_steps_power_df_init,
        intermittent_steps_df_init,
        prices_df_init,
        engagement_df_init,
        asset_steps_soc_df_init,
        asset_steps_availability_df_init,
        operation_steps_output_df_init,
    ) = get_df(data_in_init, data_out_init, subplots_param)
    ...
    (
        ...
        intermittent_steps_df_diff,
        _,
        engagement_df_forced,
        asset_steps_soc_df_diff,
        asset_steps_availability_df_diff,
        operation_steps_output_df_diff,
    ) = get_df(data_in_forced, data_out_forced, subplots_param)
    ...
    if any(
        [
            ...
            for asset_id in assets_df_init.index
        ]
    ):
        energy_vectors_number += 1
    ...
    # AVAILABILITY --------------------------------------------------------------------------
    all_available = (asset_steps_availability_df_init - 1).sum(axis=1).sum(
        axis=0
    ) == 0.0 and asset_steps_availability_df_diff.sum(axis=1).sum(axis=0) == 0.0
    ...
    all_subplot_classic_titles_flat = flatten_list(
        [
            power_target_titles,
            "Energy market prices & Market engagements",
            "Imbalances",
    ...
    all_subplot_comp_titles_flat = [
        "Optimization objective values and violation costs",
        "Optimization costs repartition",
        "Initial optimization violations",
        "Forced optimization violations",
    ]
    subplot_classic_titles = [
        title for index, title in enumerate(all_subplot_classic_titles_flat) if subplots_classic_flat[index]
    ]
    subplot_comp_titles = [title for index, title in enumerate(all_subplot_comp_titles_flat) if subplots_comp[index]]
    ...
    specs_classic = [title for index, title in enumerate(all_specs_classic_flat) if subplots_classic_flat[index]]
    specs_comp = [title for index, title in enumerate(all_specs_comp_flat) if subplots_comp[index]]
    ...
    fig_comp = make_subplots(
        rows=subplots_comp_number,
        cols=1,
        row_heights=[0.2]
        + [(1 - 0.2 - 0.15 / subplots_comp_number) / (subplots_comp_number - 1)] * (subplots_comp_number - 1),
        shared_xaxes=True,
    ...
    for index, title in enumerate(subplot_comp_titles):
        ...
        if title == "Optimization objective values and violation costs":
            ...
            fig_comp.add_trace(
                go.Table(
                    header=dict(values=[k for k in obj_func_df.columns if k != "tot_costs"]),
                    cells=dict(
                        values=[obj_func_df[k].tolist() for k in obj_func_df.columns if k != "tot_costs"],
    ...
    fig_comp.update_layout(
        title_text="Optimization comparison" "<br><br><sup>" + input_comparison_text + "</sup>",
        height=height_comp,
        margin=dict(t=150),
        barmode="group",
    )
    ...
    combine_plotly_figs_to_html(
        [fig_comp, fig_classic], html_path, auto_open=True
    )  # separator="<p>Test afficher du texte entre les deux figures</p>",
