...
def json_to_dataframe(json_file:str) -> dict[str,pd.DataFrame]:
    ...
    with open(json_file, 'r') as f:
        ...
        for sheet_data in json_data["decision_optimization"]["output_data"]:
            ...
            if sheet_name.endswith('.csv'):
                sheet_name = sheet_name[:-4]
                if sheet_name in ["OPERATION_OUTPUT", "OPERATION_STEPS_OUTPUT", "ASSETS_OUTPUT", "ASSET_STEPS_OUTPUT",
                                "VIOLATIONS_OUTPUT", "MARKET_BIDS_OUTPUT", "ASSET_STEPS_COST", "STEP_COSTS", "COSTS"]:
                    columns = sheet_data["fields"]
                    values = sheet_data["values"]
                    sheet_df = pd.DataFrame(data=values, columns=columns)
                    data[sheet_name] =  sheet_df
...
def get_cloud_input_from_dataframe(input_data:dict[pd.DataFrame]) -> list[dict]:
    ...
    for sheet_name in input_data.keys():
        ...
        values = input_data[sheet_name].to_json(orient='split', index=False).replace('"columns":', '"fields":', 1).replace('"data":', '"values":', 1)
