...
def find_in_out_fields(model_file: str) -> tuple[list[str]]:
    ...
    try:
        with open(model_file, "r") as f:
            ...
            while l < len(mod_lines) and not (
                mod_lines[l].startswith("// Converting this reference to")
            ):  # Could probably replace 2nd condition with tuple t_empty limit
                    ...
                    if line.startswith("tuple t_" + i + " {"):
                        ...
                        while not ("}" in line):
                            ...
                            if "//" not in words[0]:  # Check if the line is not commented
                                words = words[0].split(
                                    " "
                                )  # Separate the words of the first part of the line (before the ';')
                                words = list(filter(None, words))  # Remove empty characters resulting from the splits
                                sheet_data.append(
                                    words[-1]
                                )  # Keep only the name of the column (last word before the ';')
                            l += 1
                            line = mod_lines[l]
...
def reorder_columns(data: dict[str, pd.DataFrame], used_columns: dict[str, list[str]]) -> dict[str, pd.DataFrame]:
    """
    ...
    for sheet_name in used_columns.keys():
        sheet_data = data[sheet_name].filter(
            items=used_columns[sheet_name]
        )  # Reorder and keep only the columns of used_columns
...
def prepare_input_data(
    data: dict[str, pd.DataFrame], mod_file: str, input_fields: list[str]
...
def create_cost_extraction_opl_model(model_file: str, opl_cost_extraction_path: str, dat_extension_path: str) -> None:
    ...
    with open(model_file, "r") as f:
        ...
        def find_obj_function_term_definition(mod_lines: list[str], costs_variables_names: list[str]) -> list[str]:
            """
            ...
            for l, line in enumerate(mod_lines):
                ...
                if line.startswith("dexpr float "):
                    for costs_variable in costs_variables_names:
                        if line.startswith("dexpr float " + costs_variable + "=") or line.startswith(
                            "dexpr float " + costs_variable + " ="
                        ):
        ...
        costs_variables_definitions, totalFCRNetworkCostsByStep_defined = find_obj_function_term_definition(
            mod_lines=mod_lines, costs_variables_names=costs_variables_names
        )
        ...
        if not (cost_definition.startswith("dexpr float TotalFCRNetworkCosts") and totalFCRNetworkCostsByStep_defined):
            ...
            if "[imb]" in new_cost:
                ...
                step_recalibration = (
                    "\nfloat "
                    + raw_cost_name
                    + "ByStep[t in isDECISION_STEPS];\n"
                    + "execute{\n"
                    + "\tfor (var osl in OPERATION_STEPS_LINK){\n"
                    + "\t\tosl.imbalance_step > 0 ?\n"
                    + "\t\t"
                    + raw_cost_name
                    + "ByStep[osl.asset_step] = "
                    + raw_cost_name
                    + "ByStepImb[osl.imbalance_step] / StepMultiplicationImb[osl.asset_step]\n"
                    + "\t\t:"
                    + raw_cost_name
                    + "ByStep[osl.asset_step] = 0;\n"
                    + "\t}\n"
                    + "}"
                )
                ...
                step_recalibration = (
                    ...
                    + "ByStep[osl.asset_step] = "
                    + raw_cost_name
                    + "ByStepH[osl.day_ahead_step] / StepMultiplicationH[osl.asset_step]\n"
                    + "\t\t:"
                    + raw_cost_name
                    + "ByStep[osl.asset_step] = 0;\n"
                    + "\t}\n"
                ...
                step_recalibration = (
                    ...
                    + "ByStep[osl.asset_step] = "
                    + raw_cost_name
                    + "ByStepFCR[osl.fcr_step] / StepMultiplicationFCR[osl.asset_step]\n"
                    + "\t\t:"
                    + raw_cost_name
                    + "ByStep[osl.asset_step] = 0;\n"
                    + "\t}\n"
    ...
    if conversion_imb:
        step_conversion += (
            "int assetStepImbalanceStep_int[isDECISION_STEPS] = [osl.asset_step : intValue(osl.imbalance_step) | osl in OPERATION_STEPS_LINK];\n"
            + "int StepMultiplicationImb[t in isDECISION_STEPS] = count(assetStepImbalanceStep_int, assetStepImbalanceStep_int[t]);\n"
        )
    if conversion_h:
        step_conversion += (
            "int assetStepHourlyStep_int[isDECISION_STEPS] = [osl.asset_step : intValue(osl.day_ahead_step) | osl in OPERATION_STEPS_LINK];\n"
            + "int StepMultiplicationH[t in isDECISION_STEPS] = count(assetStepHourlyStep_int, assetStepHourlyStep_int[t]);\n"
        )
    if conversion_fcr:
        step_conversion += (
            "int assetStepFCRStep_int[isDECISION_STEPS] = [osl.asset_step : intValue(osl.fcr_step) | osl in OPERATION_STEPS_LINK];\n"
    ...
    print(
        f"cost_extension.mod file created and .dat updated.\nPlease check the file at '{opl_cost_extraction_path}'.\n"
    )


def create_dat_file(
    data_dat: dict[str, pd.DataFrame],
    dat_path: str,
    output_fields_dat: str,
    ...
    excel_input_relative_path = os.path.relpath(
        os.path.join(model_folder, "Data", os.path.basename(excel_input_path)), model_folder
    )  # os.path.join(model_folder, "Data", os.path.basename(excel_input_path))
    new_dat.append('SheetConnection inputSheet("' + ("/").join(excel_input_relative_path.split("\\")) + '");\n')
    new_dat.append("\n")
    ...
    for sheet_name, sheet_data in data_dat.items():
        ...
        new_dat.append(sheet_name + ' from SheetRead(inputSheet, "' + "'" + sheet_name + "'!A2:" + last_cell + '");\n')
    new_dat.append("\n")

    # Output sheet declaration
    excel_output_relative_path = os.path.relpath(
        os.path.join(model_folder, "Data", os.path.basename(excel_output_path)), model_folder
    )  # os.path.join(model_folder, "Data", os.path.basename(excel_output_path))
    new_dat.append('SheetConnection outputSheet("' + ("/").join(excel_output_relative_path.split("\\")) + '");\n')
    new_dat.append("\n")

    # Output writing instructions
    excel_range = dict(
        {
            "operation_output": '"' + "'OPERATION_OUTPUT'" + '!A2:B10001"',
    ...
    output_header = dict(
        {
            "operation_output": "OPERATION_HEADER",
    ...
    for output_field in output_fields_dat:
        ...
        new_dat.append(
            output_header[output_field] + ' to SheetWrite(outputSheet,"' + output_field.upper() + '!A1:T1");\n'
        )
        new_dat.append(
            output_data[output_field]
            + " to SheetWrite(outputSheet,"
            + excel_range[output_field]
            + "); // Check the excel range\n"
        )
