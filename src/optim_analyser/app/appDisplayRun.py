...
class DisplayRun(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.checkbutton_run_local = ttk.Checkbutton(self.frame_options,
        ...
        self.checkbutton_add_costs = ttk.Checkbutton(self.frame_options,
        ...
        self.button_json = ttk.Button(self.frame_json_select, text="Select json/txt",
                                command=(lambda:[appOptimJob.click_choose_json(self.job, self.label_txt_json_selected),
        ...
        self.button_excel_in = ttk.Button(self.frame_excel_select, text="Select Excel input file",
                                    command=(lambda:[appOptimJob.click_choose_excel_in(self.job, self.label_txt_excel_input_selected),
                                                     self.check_files(self.job)]))
        self.button_excel_out = ttk.Button(self.frame_excel_select, text="Select Excel output file",
                                     command=(lambda:[appOptimJob.click_choose_excel_out(self.job, self.label_txt_excel_output_selected),
                                                      self.check_files(self.job)]))
        ...
        self.button_sc = ttk.Button(self.frame_sc_select, text="Select folder",
                              command=(lambda:[appOptimJob.click_choose_sc_folder(self.job, self.label_txt_sc_selected),
                                               self.check_files(self.job)]))
        ...
        self.button_display_json = ttk.Button(self.frame_json_button, text="Display", command=lambda:self.display_json(self.job.json_path, self.color_blind))
        self.button_display_excel = ttk.Button(self.frame_excel_button, text="Display", command=lambda:self.display_excel(self.job.excel_input_path, self.job.excel_output_path, self.color_blind))
        self.button_display_sc = ttk.Button(self.frame_sc_button,  text="Display", command=lambda:self.display_sc(self.job.sc_folder, self.add_costs, self.color_blind))

        self.button_run_json = ttk.Button(self.frame_json_button, text="Replay", command=lambda:self.run_json(self.job.json_path, self.add_costs, self.color_blind, self.run_local))
        self.button_run_excel = ttk.Button(self.frame_excel_button, text="Run", command=lambda:self.run_excel(self.job.excel_input_path, self.add_costs, self.color_blind, self.run_local))
        self.button_run_sc = ttk.Button(self.frame_sc_button, text="Run", command=lambda:self.run_sc(self.job.sc_folder, self.add_costs, self.color_blind, self.run_local))

        # Console redirected in frame
        self.console_text = tk.Text(self.frame_console, state='disabled',)
    ...
    def display_excel(self, excel_input_path:str, excel_output_path:str, color_blind:bool):
        analyse.display_from_excel(excel_input_path, excel_output_path, self.output_folder, sc_name=None, color_blind=color_blind)
    ...
    def run_json(self, json_path:str, add_costs:bool, color_blind:bool, run_local:bool):
        if run_local:
            t = threading.Thread(target=analyse.replay_from_json_and_display_local(json_path, self.output_folder, add_costs, color_blind))
    ...
    def run_excel(self, excel_input_path:str, add_costs:bool, color_blind:bool, run_local:bool):
        if run_local:
            t = threading.Thread(target=analyse.run_from_excel_and_display_local(excel_input_path, self.output_folder, self.console_text, add_costs, color_blind))
            t.daemon = True # close pipe if GUI process exits
            t.start()
        else :
    ...
    def run_sc(self, sc_folder:str, add_costs:bool, color_blind:bool, run_local:bool):
        ...
        # All scenarios require the same model and .dat file (same number of step_id)
        if run_local:
            t = threading.Thread(target=analyse.run_scenarios_from_folder_local(sc_folder, output_folder=self.output_folder, sc_list=None, add_costs=add_costs, in_place=True))
            t.daemon = True # close pipe if GUI process exits
            t.start()
