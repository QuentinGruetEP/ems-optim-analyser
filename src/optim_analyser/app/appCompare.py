...
class Comparison(ttk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.label_txt_optim_init_input_excel_in_selected = ttk.Label(
            self.frame_optim_init_excel_json, text=self.job_init.excel_input_path
        )
        self.label_txt_optim_init_input_excel_out_selected = ttk.Label(
            self.frame_optim_init_excel_json, text=self.job_init.excel_output_path
        )

        self.button_optim_init_excel_in = ttk.Button(
            self.frame_optim_init_excel_json,
            text="Select Excel input file",
            command=(
                lambda: [
                    appOptimJob.click_choose_excel_in(self.job_init, self.label_txt_optim_init_input_excel_in_selected),
                    self.check_files(self.job_init, self.job_forced),
                ]
            ),
        )
        self.button_optim_init_excel_out = ttk.Button(
            self.frame_optim_init_excel_json,
            text="Select Excel output file",
            command=(
                lambda: [
                    appOptimJob.click_choose_excel_out(
                        self.job_init, self.label_txt_optim_init_input_excel_out_selected
                    ),
                    self.check_files(self.job_init, self.job_forced),
                ]
            ),
        ...
        self.label_txt_optim_init_json_selected = ttk.Label(
            self.frame_optim_init_excel_json, text=self.job_init.json_path
        )
        self.button_optim_init_json = ttk.Button(
            self.frame_optim_init_excel_json,
            text="Select json file",
            command=(
                lambda: [
                    appOptimJob.click_choose_json(self.job_init, self.label_txt_optim_init_json_selected),
                    self.check_files(self.job_init, self.job_forced),
                ]
            ),
        )
        ...
        self.label_txt_optim_forced_input_excel_in_selected = ttk.Label(
            self.frame_optim_forced_excel, text=self.job_forced.excel_input_path
        )
        self.label_txt_optim_forced_input_excel_out_selected = ttk.Label(
            self.frame_optim_forced_excel, text=self.job_forced.excel_output_path
        )

        self.button_optim_forced_excel_in = ttk.Button(
            self.frame_optim_forced_excel,
            text="Select Excel input file",
            command=(
                lambda: [
                    appOptimJob.click_choose_excel_in(
                        self.job_forced, self.label_txt_optim_forced_input_excel_in_selected
                    ),
                    self.check_files(self.job_init, self.job_forced),
                ]
            ),
        )
        self.button_optim_forced_excel_out = ttk.Button(
            self.frame_optim_forced_excel,
            text="Select Excel output file",
            command=(
                lambda: [
                    appOptimJob.click_choose_excel_out(
                        self.job_forced, self.label_txt_optim_forced_input_excel_out_selected
                    ),
                    self.check_files(self.job_init, self.job_forced),
                ]
            ),
        ...
        self.button_compare_excels = ttk.Button(
            self,
            text="Compare with initial Excel files",
            command=lambda: self.compare_excels(
                self.job_init.excel_input_path,
                self.job_init.excel_output_path,
                self.job_forced.excel_input_path,
                self.job_forced.excel_output_path,
                color_blind=self.color_blind,
            ),
        )
        self.button_compare_json_excels = ttk.Button(
            self,
            text="Compare with initial json",
            command=lambda: self.compare_json_excel(
                self.job_init.json_path,
                self.job_forced.excel_input_path,
                self.job_forced.excel_output_path,
                color_blind=self.color_blind,
            ),
    ...
    def check_files(self, job_i: OptimJob, job_f: OptimJob):
        json_init_ready = job_i.json_path != None and (
            job_i.json_path.endswith(".json") or job_i.json_path.endswith(".txt")
        )
        excel_init_ready = (
            job_i.excel_input_path != None
            and job_i.excel_input_path.endswith(".xlsx")
            and job_i.excel_output_path != None
            and job_i.excel_output_path.endswith(".xlsx")
        )
        excel_forced_ready = (
            job_f.excel_input_path != None
            and job_f.excel_input_path.endswith(".xlsx")
            and job_f.excel_output_path != None
            and job_f.excel_output_path.endswith(".xlsx")
        )
        row = 4
    ...
    def compare_excels(
        ...
        output_folder: str = output_path("./output/"),
        color_blind: bool = False,
    ):
        analyse.display_from_excel(
            excel_input_init_path, excel_output_init_path, output_folder, "Initial behavior", color_blind
        )
        analyse.display_from_excel(
            excel_input_forced_path, excel_output_forced_path, output_folder, "Forced behavior", color_blind
        )
        analyse.compare_from_excel(
            excel_input_init_path,
            excel_output_init_path,
            excel_input_forced_path,
            excel_output_forced_path,
            output_folder,
            color_blind,
        )

    def compare_json_excel(
        self,
        json_init_path: str,
        excel_input_forced_path: str,
        excel_output_forced_path: str,
        html_path: str = output_path("./output/comparison.html"),
        color_blind: bool = False,
    ):
        analyse.display_from_json(json_init_path, output_path("./output/"), color_blind=True)
        analyse.display_from_excel(
            excel_input_forced_path, excel_output_forced_path, output_path("./output/"), "Forced behavior", color_blind
        )
        analyse.compare_from_json_excel(
            json_init_path, excel_input_forced_path, excel_output_forced_path, html_path, color_blind
        )
